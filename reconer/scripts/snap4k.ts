...
// 只在中帧测)。已测负 (2026-07-12): 21-30 链触发但 |θ|p50 仅 1.5° (噪声级,
// 0.02 惩罚拦不住), 记分板 304/9/40/1/1416 全面劣于基线 35/4/18/1/5 —
// 与 --multikey 同一死因: 贴合目标分辨率 ~0.3-0.5 格 / ~2°, 够整格级救援
// 不够亚格/小角精修。两轴齐死 → 基线 (中帧平移常量) 即当前目标函数的最优点,
// 再动几何须先升级目标函数本身 (更长基线的格线采样 / 亚像素梯度)。
