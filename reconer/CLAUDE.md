...
- **下一步方向 (prior art 调研见 `prior_art.md`, 无竞品; 2026-07-09 负结果 ⑤⑥⑦ 后重排)**: **单格颜色证据通道已在本素材上穷尽** — 任何静态分类/标定/软概率/每帧光照建模都过不了墙 (LOO k-NN 天花板 ~70-77%, 混淆是每格级镜面反光/模糊)。剩余可行路: ① **拍摄侧直接消根因** (零代码, 最高性价比): 偏振镜消贴纸镜面反光 + 机位稍高俯拍 + 柔光 — 每格噪声是实测头号杀手, 新素材直接从源头解决; ② **手部关键点序列作第二证据通道** (MediaPipe hands 级; felikemath 原型手部 LSTM 82%/融合 94% 认 R/U/F) — 本机无 torch, 需环境; ③ 模型基 6-DoF 跟踪的价值重估: 颜色残差通道已证封顶 (负⑦), 只剩几何/时间平滑/亚格结构收益, 优先级降; ④ 运动模糊按 Seibold 2017 "blur 即曝光内运动信息"思路, 不盲目去模糊。
