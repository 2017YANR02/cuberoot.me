...
- **负结果 (勿重试)**: ① **pin (每路径固定指派隐变量, `rawPinWindows`)** 在噪声下反伤 (0.62 满覆盖 10%→2.5%): 固定隐变量把状态空间 ×16, "错态+碰巧对齐 pin"组合爆炸快于它锐化 SNR, beam 被稀释; drift 变体同理。② **junk 混合鲁棒似然 (`rawJunkProb`, robust logaddexp 地板)** 救不了 60% 精度: 60% 观测不是均匀垃圾 (地板抓不到), 而是系统性偏向似是而非的错误态。③ **perframe 密集逐帧喂搜索 (`--perframe`, visualWeight 0.35)** → 2.5%, 比链式 12.5% 更差: 密集但噪声, 邻帧误分类高度相关不能靠平均消 (印证"弱观测是负资产")。④ **forward 正向搜索 (从打乱锚起, `--forward`)** GT t=0 即死 (未根因, 疑实现或早段证据比设想更弱)。基础设施保留在 `anchored-search.ts` (pin/drift/junk/forward) + `real-eval.ts` (oracle 三模), 供未来精度过悬崖后复用。
...
- **下一步方向 (prior art 调研见 `prior_art.md`, 无竞品)**: ① **模型基 6-DoF 跟踪** — M3T/DeepAC 式 region + 波束搜索状态假设动态渲染贴纸纹理做 render & compare (残差=观测似然, 与搜索天然耦合), 100fps 帧间位移小是利好; ② **手部关键点序列作第二证据通道** (MediaPipe hands 级; felikemath 原型手部 LSTM 82%/融合 94% 认 R/U/F); ③ 运动模糊按 Seibold 2017 "blur 即曝光内运动信息"思路, 不盲目去模糊; ④ 新标注视频拍摄建议: 机位稍高俯拍 (正对顶面), 避免顶面反光。
