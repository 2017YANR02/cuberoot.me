...
- **下一步方向 (prior art 调研见 `prior_art.md`, 无竞品; 2026-07-09 夜负结果 ⑧⑨ 后再排)**: **单格颜色分类已穷尽 (⑤⑥⑦), 手部通道与带光流已证伪 (⑧⑨)**。剩余可行路: ① **拍摄侧直接消根因** (零代码, 最高性价比): 偏振镜消贴纸镜面反光 + 机位稍高俯拍 + 柔光 — 每格噪声是实测头号杀手, 新素材直接从源头解决; ② **贴纸外观守恒动作打分** (`scripts/conserve-eval.ts` v1 已建): 一步动作 = 可见格已知置换, 段后格外观 ≈ 段前来源格外观, **比原始 RGB 相似度、零颜色分类** — 负⑤⑥⑦杀的是外观→标签映射, 没杀同贴纸跨一步自相似; 评分含"预测不动却大变要付分" (防 F 假设白捡) + 藏面流入给随机对中性分 (y 靠无罪胜出)。v1 单帧冷提取: 覆盖仅 29%, face top1 31% (换位后镜面外观漂移 + 覆盖薄) — **升级点: 静止区间共识 RGB (复用 real-eval runs 机制, 覆盖应回 ~62%+), 色度-only 相似度 (镜面主要打 v 轴), B+U 双视图联合指派, 跨段全局指派一致性**; 判据 face 精度须显著超 probs 57.6% 才接搜索; ③ 模型基 6-DoF 跟踪: 颜色残差封顶 (负⑦), 只剩几何/时间平滑收益, 低优; ④ 运动模糊按 Seibold 2017 思路作信息, 不盲目去模糊。
