...
(function () {
     ...
     * NOTE: 用 transform:scale() 缩放 twisty-player（Shadow DOM 不响应外部 height）
     */
    function setupResizeHandle(container) {
        ...
        // NOTE: 水平居中缩放，垂直从顶部开始（配合容器高度同步调整）
        player.style.transformOrigin = 'center top';
        var baseH = 0;       // 初始渲染高度（缓存）
        var startY = 0;
        var currentScale = 1;
        var MIN_SCALE = 0.6;
        var MAX_SCALE = 2.0;
