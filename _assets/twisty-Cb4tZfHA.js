...
`);var gt=!1,V=!1;B?.addEventListener(`mousedown`,e=>{e.which&&(V=!0)},!0),B?.addEventListener(`mouseup`,e=>{e.which&&(V=!1)},!0);var H=0,U=0;B?.addEventListener(`mousedown`,()=>{U++},!1),B?.addEventListener(`mousemove`,_t,!1),B?.addEventListener(`mouseenter`,_t,!1);function _t(e){H=e.pageY}var vt=0,yt=0,bt=!1,xt=0,St=class extends v{constructor(e,t){super(),this.model=e,this.controller=t}async onDetailedTimelineInfo(e){let t=await this.inputElem();t.min=e.timeRange.start.toString(),t.max=e.timeRange.end.toString(),t.disabled=t.min===t.max,t.value=e.timestamp.toString()}async connectedCallback(){this.addCSS(ht),this.addElement(await this.inputElem()),this.model?.twistySceneModel.colorScheme.addFreshListener(this.updateColorScheme.bind(this))}updateColorScheme(e){this.contentWrapper.classList.toggle(`dark-mode`,e===`dark`)}#e=null;async inputElem(){return this.#e??=(async()=>{let e=document.createElement(`input`);return e.type=`range`,e.disabled=!0,this.model?.detailedTimelineInfo.addFreshListener(this.onDetailedTimelineInfo.bind(this)),e.addEventListener(`input`,this.onInput.bind(this)),e.addEventListener(`keydown`,this.onKeypress.bind(this)),e})()}async onInput(e){if(bt)return;let t=await this.inputElem();await this.slowDown(e,t);let n=parseInt(t.value,10);this.model?.playingInfo.set({playing:!1}),this.model?.timestampRequest.set(n)}onKeypress(e){switch(e.key){case`ArrowLeft`:case`ArrowRight`:this.controller?.animationController.play({direction:e.key===`ArrowLeft`?-1:1,untilBoundary:`move`}),e.preventDefault();break;case` `:this.controller?.togglePlay(),e.preventDefault();break}}async slowDown(e,t){if(gt&&V){let n=t.getBoundingClientRect(),r=n.top+n.height/2;console.log(r,e,H,V);let i=Math.abs(r-H),a=1;i>64&&(a=Math.max(2**(-(i-64)/64),1/32));let o=parseInt(t.value,10);if(console.log(`cl`,xt,U,o),xt===U){let e=(o-yt)*a;console.log(`delta`,e,i),bt=!0;let n=o;n=vt+e*a+(o-vt)*Math.min(1,(1/2)**(i*i/64)),t.value=n.toString(),console.log(a),bt=!1,this.contentWrapper.style.opacity=a.toString()}else xt=U;yt=o}}};x.define(`twisty-scrubber`,St);var Ct=null;async function wt(e,t){let[{ThreePerspectiveCamera:n,ThreeScene:r},i,a,o,s,c,l]=await Promise.all([(async()=>{let{ThreePerspectiveCamera:e,ThreeScene:t}=await C();return{ThreePerspectiveCamera:e,ThreeScene:t}})(),await e.puzzleLoader.get(),await e.visualizationStrategy.get(),await e.twistySceneModel.stickeringRequest.get(),await e.twistySceneModel.stickeringMaskRequest.get(),await e.legacyPosition.get(),await e.twistySceneModel.orbitCoordinates.get()]),u=t?.width??2048,d=t?.height??2048,f=u/d,p=Ct??=await(async()=>new n(20,f,.1,20))(),m=new r,h=new rt(e,{scheduleRender:()=>{}},i,a);m.add(await h.twisty3DPuzzle()),await se(p,l);let g=(await me(u,d,m,p)).toDataURL(),ee=await Tt(e);return{dataURL:g,download:async e=>{Et(g,e??ee)}}}async function Tt(e){let[t,n]=await Promise.all([e.puzzleID.get(),e.alg.get()]);return`[${t}]${n.alg.experimentalNumChildAlgNodes()===0?``:` ${n.alg.toString()}`}`}function Et(e,t,n=`png`){let r=document.createElement(`a`);r.href=e,r.download=`${t}.${n}`,r.click()}var Dt=new S;Dt.replaceSync(`
:host {
  width: 384px;
  height: 256px;
  display: grid;
...
`,` `].includes(d.at(-1))&&![`
`,` `].includes(n[i]);function m(e,t){let n=e+d+t,r=!!hr(a+n+o);return r&&(d=n),r}f&&p&&m(` `,` `)||f&&m(` `,``)||p&&m(``,` `),B?.execCommand(`insertText`,!1,d)||e.setRangeText(d,r,i,`end`)}var yr=new S;yr.replaceSync(`
:host {
  width: 384px;
  display: grid;
}
