...
(function () {
    ...
    function setupSelector(allPanelSections, insertBeforeNode, onEventChanged = null) {
        ...
        allPanelSections.forEach(sections => {
            sections.forEach(s => {
                if (!allIds.includes(s.eventId))
                    allIds.push(s.eventId);
            });
        });
        if (allIds.length < 2)
            return;
        ...
        window.addEventListener('popstate', () => {
            ...
            if (h.event) {
                _allSelectors.forEach(s => {
                    if (s.uniqueIds.includes(h.event))
                        s.show(h.event);
                });
            }
            // 恢复 metric
    ...
    function handleMetricPage(metricPanels) {
        ...
        const metricSupportedEvents = new Map();
        ...
        const onEventChanged = (eventId) => {
            ...
            metricSupportedEvents.forEach((supportedSet, mp) => {
                ...
                if (ddItem) {
                    if (hasData)
                        ddItem.classList.remove('disabled');
                    else
                        ddItem.classList.add('disabled');
                }
                // 找药丸按钮
