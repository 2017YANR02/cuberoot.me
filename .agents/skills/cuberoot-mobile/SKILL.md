...
- 打乱逐步提示、匹配判定、走偏修正和异步重试统一使用 `@cuberoot/shared/smart-cube/scramble-hint`；Kociemba cubie 运算只从 `@cuberoot/puzzle-solvers/kociemba/cube` 取。Web 与五端必须共用 `TimerScrambleStrip` 和 `@cuberoot/shared/timer` 的 Solo/Local/Net capability helpers；宿主只可提供 solver Worker/原生 transport，不得复制提示算法或模式名单。当前 Web `SoloView` 与 App `App.tsx` 仍各有一层 React lifecycle coordinator，这是明确待收敛债务；不得增加第三份，后续触及任一份时优先提取 runtime-neutral controller。pending solver 必须以当前 facelets 重验，同 target 请求 coalesce，新 target 排在旧请求完成后重试；切题、运行、断线或协议错误时 fail closed。
