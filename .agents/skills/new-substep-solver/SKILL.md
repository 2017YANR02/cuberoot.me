...
- **WCA 项目也可能纯 TS 可证最优(别默认「WCA = 上 Rust」)**:群结构能**因子分解成几张小表**时纯 TS 就够。范本魔表 `lib/clock-solver.ts` —— 群 ≅ (Z₁₂)¹⁴ 阿贝尔,前后各一张 Z₁₂⁴ 表(双侧枚举 + 迭代加深),~17 ms/次、零下载表、零 worker,却算穿 12¹⁴ = 1.28e15 态。判据 = 有没有正交坐标块,**不是**态数大小。上 `/dev/solvers` 别塞 `NONWCA_TS`(event 集被 CI 锁死 == `CSTIMER_SOLVABLE_IDS`,且 A/B/C/D 四档讲的是非 WCA 状态空间打法)→ 单开一节。
