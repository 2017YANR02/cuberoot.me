...
description: "Use when regenerating `stats/scramble/*.json` or touching `core/jobs/scramble-stats-build/` / `core/packages/client/app/[lang]/scramble/stats/`. Covers CSV 列名、pair rotation 记号、WCA 配色、UI 朝向→底色。Triggers: \"scramble-stats-build\", \"distribution.json\", \"打乱分布\", \"scramble_stats\"."
---
...
- 聚合：`core/jobs/scramble-stats-build/src/build.ts`（`VARIANTS` + `angleToColor` 映射；`SUBSET_KEYS` = sizes 1/2/4/6）
