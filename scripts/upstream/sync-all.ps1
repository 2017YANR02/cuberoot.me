...
.DESCRIPTION
    ...
    子脚本（可单独跑，但日常一律走根目录 sync_upstream.ps1）：
      sync-cstimer.ps1             csTimer 全量构建 → tools/cstimer/ + client public/scramble_module.js
      sync-cstimer-scramble.ps1    csTimer 打乱源码 → tools/cstimer-scramble/（纯拷贝）
      sync-rubiks-solver-demo.ps1  Solver → tools/{src,solver,2x2x2,...}/
      sync-alg-trainers.ps1        Alg-Trainers → tools/alg_trainers/
      sync-blddb.ps1               BLDDB → tools/blddb/(next build 静态导出)
      sync-recordranks.ps1         RecordRanks → fork main + ops/contests/recordranks-ref.txt
.PARAMETER Only
...
.PARAMETER RepoRoot
    cuberoot.me 仓库根目录；默认为本脚本向上两级的仓库根。
...
$syncBootstrapRoot = if ($RepoRoot) { [IO.Path]::GetFullPath($RepoRoot) } else { $defaultRepoRoot }
. (Join-Path $syncBootstrapRoot '.sync\sync_utils.ps1')
$root = Resolve-CubeRootRepoRoot -RepoRoot $RepoRoot -ScriptRoot $defaultRepoRoot
...
foreach ($u in $upstreams)
{
    ...
    $createdStash = Push-SyncWorkingTreeStash -WorkingDirectory $u.Dir -Message 'sync-all: local patches'
...
if ($targets -contains 'cstimer')
{
    ...
    else
    {
        ...
        & (Join-Path $root 'scripts\upstream\sync-cstimer.ps1') -RepoRoot $root -SkipPull
        Write-Host "`n--- csTimer 打乱源码 ---" -ForegroundColor Cyan
        & (Join-Path $root 'scripts\upstream\sync-cstimer-scramble.ps1') -RepoRoot $root -SkipPull
    }
}
...
if ($targets -contains 'solver')
{
    ...
    & (Join-Path $root 'scripts\upstream\sync-rubiks-solver-demo.ps1') -RepoRoot $root @dry
}

if ($targets -contains 'algtrainers')
{
    Write-Host "`n--- Alg-Trainers ---" -ForegroundColor Cyan
    & (Join-Path $root 'scripts\upstream\sync-alg-trainers.ps1') -RepoRoot $root @dry
}
...
if ($targets -contains 'recordranks')
{
    ...
    & (Join-Path $root 'scripts\upstream\sync-recordranks.ps1') @recordRanksArgs
