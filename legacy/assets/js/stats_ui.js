...
function syncTabSuffix(scope, tabSuffix) {
    ...
    scope.querySelectorAll('.stat-tab').forEach(function (t) {
        ...
        if (tm && tm[1].split('-').pop() === tabSuffix) {
            ...
            if (!tp) {
                const metricPanel = scope.closest('.metric-panel');
...
function initStatsUI() {
    ...
    if (loosePanels.length >= 2) {
        ...
        let currentGroup = [loosePanels[0]];
    ...
    if (topStatPanels.length >= 2) {
        ...
        for (let j = 1; j < topStatPanels.length; j++) {
            if (tg[tg.length - 1].nextElementSibling === topStatPanels[j]) {
                ...
                if (tg.length >= 2)
                    createTabButtons(tg[0].parentElement, tg, false);
                tg = [topStatPanels[j]];
            }
        }
        if (tg.length >= 2)
...
document.addEventListener('toggle', function (e) {
    const d = e.target;
    if (!d.classList || !d.classList.contains('solve-details'))
        return;
    if (!d.open || d.querySelector('.solve-list'))
        return;
...
function initHideDays0() {
    ...
    if (!/\/stats\/average_of\b/.test(location.pathname))
        return;
...
function syncDaysFilterVisibility(isHistory) {
    document.querySelectorAll('.days-filter').forEach(function (f) {
        f.style.display = isHistory ? 'inline-flex' : 'none';
    });
}
...
function restoreTabFromHash() {
    ...
    if (metric) {
        const metricBtn = document.querySelector('.metric-btn[onclick*="\'' + metric + '\'"]');
        if (metricBtn)
            metricBtn.click();
    }
