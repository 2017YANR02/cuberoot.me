...
三维实况重新定锚期间静默保留上一次已验证的画面，不显示同步文案、不切换为平面预览；首次定锚完成前保留三维视图尺寸的占位。智能魔方起表后的短暂断连也不切回普通打乱预览。Web 智能魔方连接期间及由魔方开始的当前计时禁止键盘和触摸手动起停，保留 Esc 取消；自动打乱预备使用独立状态机动作，未启用观察时直接进入绿色 READY，启用观察时进入观察，均由首个真实转动起表。页面不再显示“拧回原打乱”“打乱已就绪”和“校准朝向”，陀螺仪校准保留在连接窗口；左下统计标签为 `best`。
