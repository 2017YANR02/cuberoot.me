...
智能魔方三维实况的初始视角参考 DCTimer-BLE：无有效姿态数据时采用 `(4.8, 4.1, 7.2)` 朝向原点的三面斜视角，收到有效陀螺仪四元数后采用 `(0, 4.1, 7.2)` 的抬高正前视角。在当前引擎中通过等效场景旋转实现，保留原有距离和缩放；短暂未收到姿态不会切回斜视角。视角调整不改变物理动作、贴纸状态或复盘相机。

计时复盘不显示“这把”标题、摘要中的魔方型号、“首动延迟”卡片和“顶层拆开”标签；设备元数据与分析计算仍保留。分步分析表的表头和首列均使用透明背景，与数据单元格一致；QTM 与最长停顿两张卡片采用紧凑间距，在移动端保持两列。

根据 V10 真机反馈，MY32 首次连接直接要求用户输入 MAC，不尝试广播解析或名称推导。成功解析动作或有效状态后，沿用现有按设备名称保存 MAC 的机制，后续连接复用缓存；取消输入则不启动驱动。其他品牌的 MAC 获取、MY32 协议解析与握手流程保持原有实现。下文为历史规划，涉及 MY32 自动获取地址的描述不作为当前连接行为。

Web 单人计时页点击设备连接按钮直接触发蓝牙设备选择，同时打开紧凑的设备状态窗口显示“连接中”；重置状态和重置陀螺仪在连接完成前禁用。型号列表仅在连接失败后按 GAN、魔域、奇艺和其他设备分组展示。不在页面加载时自动发起首次连接。MAC 输入页引导用户在 Chrome/Edge 的 `bluetooth-internals/#devices` 中按 Name 查找设备并复制 Address。已连接窗口保留设备、电量、转动和状态操作，移除恢复提示及耗时诊断，增加“重置陀螺仪”，复用实况魔方的朝向校准，不改变贴纸状态或记录。

Web 单人计时读数居中，打乱独立置于上方，实时魔方位于读数下方。打乱换行、纠错和状态文案不参与读数及魔方的布局计算；超出上方可用空间时只在打乱区内部滚动。共享 TimingSurface 使用可选展示属性启用该布局，原生宿主保留原布局。

三维 NxN 魔方销毁时，必须先取消该实例的转层和暂停动画、清空待播动作，再释放渲染资源及转层引用。销毁不能调用全局动画收尾，也不能触发成绩或动作回调；其他视图的魔方动画继续运行。此约束同时适用于实时魔方、复盘和模拟器。

三维实况重新定锚期间静默保留上一次已验证的画面，不显示同步文案、不切换为平面预览；首次定锚完成前保留三维视图尺寸的占位。智能魔方起表后的短暂断连也不切回普通打乱预览。Web 智能魔方连接期间及由魔方开始的当前计时禁止键盘和触摸手动起停，保留 Esc 取消；自动打乱预备使用独立状态机动作，未启用观察时直接进入绿色 READY，启用观察时进入观察，均由首个真实转动起表。打乱条统一不显示“与打乱不符”“拧回原打乱”“打乱已就绪”等状态文案及纠正说明，仅保留动作进度高亮和用户主动复制后的反馈。页面不再显示“校准朝向”，陀螺仪校准保留在连接窗口；左下统计标签为 `best`。

## GAN v4 多转动通知

共享 GAN v4 解码器参考 DCTimer-BLE 的 `handleV4Move`，按 72 bit（9 字节）逐条解析同包 MOVE 记录，保留每步独立的设备时间戳和计数器。完整通知入队后才检查序号缺口；正常同包动作不依赖 history 补回，真实丢步仍走现有恢复逻辑。重复包及与已消费计数器重叠的前缀不重复应用，填充和不完整尾部不解释成动作；遇到非法转动停止解析后续记录，已验证的前缀仍可输出。

此实现由 Web、App 和小程序共同消费，不改变 GAN v2/v3 的解析路径。独立合成帧测试覆盖多步、重复、回绕、真实缺口和 Web 加密通知的复原末步；多转动真机通知仍需实测验收。

## QiYi / Tornado V4 姿态

Web QiYi 驱动参考 DCTimer-BLE `QiyiCubeProtocol`，在普通 `FE` 消息之前处理解密后的 `CC 10` 姿态帧：前 14 字节计算 CRC16-Modbus，与末尾小端 CRC 比较；偏移 6/8/10/12 按大端有符号 int16 读取 ax/ay/az/aw，除以 1000 后归一化。坏帧与零四元数丢弃，姿态帧不发送 ACK，也不产生动作、状态快照或设备时间。

QiYi 保留参考实现的 x/y/z 轴，使用 identity 传感器基，不沿用 GAN 的 Z-up 映射。支持能力以当前连接收到合法姿态为准；QYSC 没有发送姿态时不标记陀螺仪，Tornado V4 收到姿态后接入现有实时显示、姿态录制和重置。初始化期间收到的姿态也计入能力，重连重新识别，旧连接回调不能更新新连接。

测试采用 DCTimer-BLE 中的真实解密样本 `CC100004F663FDBCFE59FEA0FDACDEA1`，并覆盖加密通知、CRC、普通消息共存及能力生命周期。参考样本验证不等于主项目的真机验收，XMD-TornadoV4-i 的握手、整体 x/y/z 转动、重置后方向及记录回放仍待真机确认。

## QiYi 历史、GAN 恢复时间与设备校准

- QiYi 读取从偏移 36 开始的 11 个历史槽，跳过空槽和非法转动，按时间排序并去重。普通动作先应用，再同步不落后于已消费动作的快照，最后应用 future history。事件的 `futureHistory` 标记让 Web 单人、对战的打乱预备跳过这些提前带出的动作；动作记录和实况仍接收它们，旧快照不会回覆盖最新状态。
- GAN v3/v4 历史动作有前后设备时间锚时继续插值。尾部缺少后一个时间锚时，参考 DCTimer-BLE，使用发起 history 请求时的本地时刻估算终点，间隔最多 65535ms，不计入等待回复的额外时延。估算动作带 `estimatedTime` 事件元数据；无可用锚时仍保留未知时间，不编造固定 1ms。该标记目前用于实时事件，不新增成绩持久化字段。
- Web GAN v2/v3/v4、MoYu32 和 QiYi 的“重置状态”会先要求确认实物已复原，再回写设备。仅在普通计时页 idle/stopped 时可操作；有效的已复原回报和成功写入都到达后才确认成功。写入失败、4 秒内未确认或断开均报错，不乐观地重置本地状态；期间的回报不触发成绩起停。
- MoYu32 参考 [协议 PR #4](https://github.com/lukeburong/weilong-v10-ai-protocol/pull/4) 和 DCTimer-BLE `d3fa686`：发送固定 20 字节 `A20000002492494924926DB6DB924924B6DB6D00`，等待设备自动返回 `A3`，不额外请求状态。QiYi 按 DCTimer-BLE 的 `04 17 88 8B 31` 前缀、27 字节 LRDUFB 面片和两字节零尾构建 CRC/ECB 消息，等待 `04` 回报。
- 自动回报可能早于 GATT 写入 Promise 完成。等待态从写队列头实际发起请求时开始，早到的回报必须等写入成功后才生效；旧 MOVE 不取消确认，期间数据暂存并在成功时按新基线过滤重放，失败时沿用旧基线。设备确认之后发生的真实转动不能被本地强制 solved 覆盖。
- MoYu32 解码器已经完成 `(w,x,y,z) → (w,x,z,-y)` 换轴，品牌传感器基必须是 identity，避免再应用默认 rotX270。该结果与 DCTimer-BLE 的一次换轴流程一致。
- 不支持设备回写的旧协议仍只能本地重置；“重置陀螺仪”只校准视角。设备回写不执行物理转动，不是 GAN Robot 的复原功能；原生端 UI 尚未扩展回写入口。

自动化覆盖历史排序/稀疏槽/重复/future 回跳、尾部估算与连接重置，以及 GAN 三代的真实命令字节、加密写入和状态确认。真机仍需核对 QYSC 高速转动与最终停表、GAN 丢步恢复、各型号校准及失败后的可恢复性。

## 后续值得对照的智能魔方能力

1. 专项训练：对照 CLL、EOCP 等完成条件、AUF 容许范围和阶段结束后保留物理状态的连续训练流程。
2. 原生端品牌覆盖：在现有 transport 边界内逐项接入 GAN v2/v3、QiYi、MoYu32；Web 已接入不表示原生端已具备同样能力。
