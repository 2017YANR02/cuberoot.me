...
- Web GAN v2/v3/v4 支持独立的“校准设备状态”入口。仅在普通计时页 idle/stopped 时可操作，用户确认实物已复原后发送参考项目的重置命令，再读取设备状态；仅有效的已复原回报可以确认成功。写入失败、未复原回报、4 秒内未确认或断开均报错，不乐观地重置本地状态。校准产生的回报不触发成绩起停。
- 本地“重置状态”、设备状态校准和“重置陀螺仪”是不同操作。上述校准不执行物理转动，不是 GAN Robot 的复原功能；暂不扩展到 QiYi/MoYu32 或原生端 UI。
