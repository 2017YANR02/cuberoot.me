...
- 会员可在“我的上传”中确认后永久删除自己的上传，包括已发布、待审核和未通过曲目；`DELETE /v1/music/tracks/:id` 通过当前登录账号与曲目 ID 原子校验归属，删除音频与封面并刷新曲库。管理员继续通过原管理接口删除任意上传，静态曲库仍只有管理员可下架。
...
- 现有 486 首静态曲目继续由静态 manifest 管理，不复制音频进数据库；管理员页面读取完整清单，并把改名、分类和可恢复下架状态写入 `music_static_overrides`，播放器载入时统一套用。
- 静态曲目下载通过 `/v1/music/static-tracks/:id/download` 复用会员或管理员鉴权；API 从 `MUSIC_LIBRARY_ROOT`（默认 `/www/wwwroot/toolkit/music/library`，与静态 nginx 根目录一致）读取现有生成清单和哈希音频，不复制文件。按清单映射源曲目 ID 到音频哈希，拒绝已下架曲目；静态和上传曲目的下载响应均不缓存。
- 音乐页复用全站外观菜单与文档级背景；启用场景时页面底色透明，选择无背景时恢复主题底色。
...
- 2026-09-03：将会员上传记录与管理员审核表单从播放器弹窗迁至独立 `/music/manage` 页面；播放器入口与单曲编辑均改为真链接，桌面及 390px 窄屏无横向溢出，页面不再渲染管理 `dialog`。代码仅本地提交，未 push。
- 2026-09-03：修复管理页只能读取数据库投稿、无法管理 486 首静态曲库的范围缺口；管理员现在可修改静态曲目信息、从公开曲库下架及恢复，覆盖状态持久化到 PostgreSQL，源音频保留以防误删；同时压缩管理页页头，让列表进入首屏。代码仅本地，未 push。
