...
- 当前仓库没有完整的专业音乐播放器状态模型、曲库 schema 或媒体转码流水线。
...
- 预计最终曲库约 5.04 GiB、最坏峰值约 6.04 GiB；盘点时 `Z:` 可用约 34.4 GiB，完成后预计仍有约 28.4 GiB。源清单位于 `Z:\cuberoot-music-staging\inventory\source-manifest.jsonl`。
- 只有 6 首带嵌入封面；11 张外部图片和 15 份 LRC 无同名媒体，必须进入人工绑定队列，不能猜配。
...
公开清单不暴露 `E:\Music` 原始绝对路径、Windows 用户名或其他本机信息。当前试跑 ID 是规范化相对路径的 SHA-256 截断，只用于幂等续跑；全量转码前必须改为内容哈希并完成重复内容检测，显示 metadata 与源路径分离。
...
现有文件夹只作为弱提示，不直接成为公开分类。分类优先级为：人工覆盖表 → 嵌入 metadata → 文件名/目录推断 → `未分类`。公开分类至少包含：艺术家、专辑、流派、语言、年份和来源类型；允许一首歌属于多个流派。

- 统一大小写、空白、全半角标点和多艺术家分隔，不擅自翻译专有名词。
- 同内容哈希只产一个音频文件；不同版本、现场版、混音版必须有可解释的版本字段，不能只按标题去重。
- 保留人工覆盖表作为分类事实源，重新扫描不覆盖人工结论。
- 源绝对路径只存在于本机私有盘点表；公开清单仅保留内容 ID 和发布 URL。

### 转码输出

- 每个源只生成一份浏览器主播放文件，第一阶段不同时常驻 MP3、AAC、Opus 和无损副本。
- 在兼容性实测后固定一种有损规范；候选为 AAC-LC/M4A 192 kbps。若目标浏览器矩阵不能稳定播放，则统一改为 MP3，而不是为所有曲目保留双格式。
- 视频直接从 `E:` 读取音轨并写最终编码文件；禁止先把原视频复制到 `Z:`，禁止产生中间 WAV。
- 在最终编码步骤完成响度 metadata、封面复用和标签写入，避免为补标签再次复制整首音频。
- 转码保留原采样率上限并禁止升采样；无法解码、无音轨、时长异常或输出校验失败的文件进入失败清单，不发布半成品。

### staging 与磁盘硬门槛

- staging 根目录固定为 `Z:\cuberoot-music-staging\`；一个批次一个目录，同时只运行一个批次。
...
- 每首完成后校验时长、可解码性、文件大小和内容哈希；批次全部通过后才进入发布目录。
- staging 清理必须在发布回读和清单校验后单独执行，遵守仓库的回收站规则；不得以清理 staging 为由触碰 `E:\Music`。

## 封面与歌词

封面来源优先级：嵌入封面 → 同目录明确匹配的封面 → 人工覆盖 → CubeRoot 统一占位。封面按内容哈希去重，纠正方向和色彩空间，限制到合理像素尺寸后生成一次 Web 版本；禁止为每个列表尺寸保存一套图片。

歌词来源优先级：同名 LRC/VTT/SRT → 嵌入同步歌词 → 普通文本歌词 → 无歌词。视频内嵌字幕只能在确认与歌曲对应后转换；字幕流语言和时间轴必须记录，不能默认第一条字幕就是歌词。
...
| D | 冻结曲库 schema、转码规范、封面/歌词规则和 `Z:` 批次工具 | `PENDING` |
...
| I | 完成曲库转码、分类、封面和歌词清单，分批发布并回读验证 | `PENDING` |
...
- 2026-09-02：完成 8 个开源候选调查与 `E:\Music` 只读盘点；实现 `/music`、首页卡片、唯一浏览器 transport、同步 LRC、Media Session 及音乐/节拍器悬浮音频中心。媒体脚本以 9 个跨格式样本试跑成功并验证幂等，输出约 54.53 MiB；共享构建、客户端 typecheck、29 个聚焦测试及 1440px/390px Chromium 视觉验收通过。内容哈希去重、人工分类、全量转码和静态发布仍待完成。
