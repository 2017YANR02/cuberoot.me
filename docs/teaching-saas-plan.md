...
当前已完成课后反馈的教师端第一闭环:PostgreSQL `0154` 保存按课次和学员递增的不可变 revision,Core 只允许机构 owner/admin 或该课次授课老师/助教在已完课课堂中发布,主站课次详情可创建并分页查看历史。公开反馈与内部备注分列,作者账号删除时只匿名化 live 外键并保留快照。学员/监护人读取、周报快照、消息和已读回执仍属后续阶段。
...
- `/reports`:周报、反馈汇总与发布状态,属于 Stage 4 后续切片。
- `/messages`:机构会话。
...
- 已完成:教师在已完课课次中按学员发布不可变课后反馈 revision,区分对外内容与内部备注,并保留审计和作者快照。
- 待完成:周报快照、学员/监护人读取、会话、通知与已读回执。

验收:一次课堂结束后可完成反馈、周报发布、学员/监护人阅读与后续沟通,全过程可审计。当前只完成教师课后反馈子验收,不把它表述为整个 Stage 4 已完成。
