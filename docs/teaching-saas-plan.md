...
当前本地已完成课后反馈与教师周报两个切片:PostgreSQL `0154` 保存按课次和学员递增的不可变反馈 revision,`0155` 保存按机构时区周一到周日聚合的周报草稿和不可变发布快照。Core 只允许机构 owner/admin 或当前负责范围内的老师/助教生成、查看和发布,主站课次详情与 `/org/[orgSlug]/reports` 提供对应工作流。周报只聚合权威课次、课时流水、训练证据、任务目标和对外反馈,不泄露内部备注;作者账号删除时只匿名化 live 外键并保留名字与角色快照。学员/监护人读取、消息、通知和已读回执仍属后续阶段。
...
- 已完成:教师在已完课课次中按学员发布不可变课后反馈 revision;按机构时区生成周报草稿,聚合出勤、课时、训练、任务和对外反馈,填写总结与下周计划后发布不可变版本;两者均保留审计和作者快照。
- 待完成:学员/监护人读取、会话、通知与已读回执。

验收:一次课堂结束后可完成反馈、周报发布、学员/监护人阅读与后续沟通,全过程可审计。当前只完成教师端反馈与周报子验收,不把它表述为整个 Stage 4 已完成。
