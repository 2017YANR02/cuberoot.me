...
前端架构已经调整:最终不再扩展独立 `@cuberoot/platform` 前端。教职员工工作台统一进入主站 `/org/*`,学员与监护人入口统一进入主站 `/learn/*`;本文的领域模型、Core API、PostgreSQL 权限与审计设计继续有效。迁移顺序见[教学平台前端统一计划](./platform-unification-plan.md)。
...
带任务上下文的主站 `/timer`、`/predict`、`alg-trainer` 生产者接入和本地可靠重试已在 Stage 3B 完成。普通访问不生成训练证据,作业访问仍由 Core 重验账号绑定、任务目标与时间窗。禁止 iframe 抓页面或复制主站训练引擎。
...
- `teaching_conversations`、`teaching_conversation_participants` 与 `teaching_messages`:机构内沟通、参与关系和单调已读游标。
...
当前已完成课后反馈、教师周报、学员/监护人读取与家校沟通切片:PostgreSQL `0154` 保存按课次和学员递增的不可变反馈 revision,`0155` 保存按机构时区周一到周日聚合的周报草稿和不可变发布快照,`0156` 提供监护人账号绑定与 learner 投影,`0158` 保存会话参与关系、不可变消息、单调已读游标和去重通知。Core 每次读取或写入都实时重验机构、学员与负责范围;主站 `/org/*` 和 `/learn/*` 复用同一 shared/Core 契约。周报与学员投影不泄露内部备注,作者账号删除时只匿名化 live 外键并保留名字与角色快照。
...
- `/schedule`、`/sessions/[id]`:排课、签到、完课、课堂记录与教师课后反馈。
...
- `/reports`:周报草稿生成、权威数据汇总、发布状态和不可变历史。
- `/students/[studentId]/messages`:按学员收敛的家校会话。
- `/settings`:校区、角色、规则、审计和集成设置。

学员/监护人端统一放在 `/learn`:按服务端派生的 learning context 查看训练入口、课后反馈、周报和消息。学员端训练归属必须先完成一次性账号绑定,监护人也必须通过一次性邀请绑定既有关系;不能因老师建档而自动获得主站训练记录。现有 `/instructor` 继续表示内容课程讲师,在数据迁移完成前不要把它直接改名或混入机构 teacher 权限。
...
- 已完成:教师在已完课课次中按学员发布不可变课后反馈 revision;按机构时区生成周报草稿并发布不可变版本;学员/监护人按实时关系读取允许公开的反馈和周报;双方可在按学员隔离的会话中发送消息,并使用去重通知和单调已读游标。
- 待完成:邮件提醒、附件、会话归档/导出与更完整的监护人管理工作台。

验收:一次课堂结束后可完成反馈、周报发布、学员/监护人阅读与后续沟通,全过程可审计。源码、契约和 PostgreSQL 并发夹具已闭环,真实登录态浏览器与线上部署仍属于发布验收。
...
阶段 0 后再落课包账本。Stage 0 的最小证据类型只是历史占位契约,现已由去客户端身份字段、严格 registry、JSON 和时间边界的正式证据契约取代。

## 当前实施状态与验证边界

截至 2026-08-20,教学系统已经统一到主站前端与 Core 教学 API:

- Stage 0 至 Stage 2 已覆盖多租户权限、校区 / 班级 / 学员关系、课包账本、排课、出勤、正常消课、课时退款与精确冲正;主站同时提供调整流水入口,不从分页流水伪算历史余额。
- Stage 3 已覆盖训练模板、任务发布、学员账号绑定、自助任务、主站工具证据上报、证据查看与批改;浏览器证据继续明确标记为 `self_reported`。
- Stage 4 已通过 PostgreSQL `0154` 至 `0158` 覆盖教师课后反馈、周报、学员 / 监护人绑定与只读门户、站内通知和家校沟通。会话消息只追加,已读游标单调递增,账号删除只清空 live actor 引用并保留行为快照。
- `packages/client` 的 `/org/*` 是教职员工工作台,`/learn/*` 是学员 / 监护人入口;两者复用 shared/Core 契约与中性教学组件。`packages/platform` 仅保留迁移来源和历史兼容,不再建设独立教学前端。
- shared、server 与 client 类型检查、定向契约测试和 PostgreSQL 并发夹具已覆盖当前源码边界;生产验收仍要求 migration、真实多角色登录、通知深链、跨租户拒绝审计、备份恢复与观察窗口逐项通过。

仍未完成的产品面包括支付接单与实际资金退款结算、请假补课规则、批量导入、远程搜索选择器和完整经营 / 财务对账报表。当前“退款”只减少课包课时,现金退款仍需在外部系统处理。源码完成不能替代生产部署与登录态验收。

## 历史实施记录（截至 2026-08-17）
