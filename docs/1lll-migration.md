...
`y` 和 `U` 在**顶层上的作用完全相同**(都绕 U 轴转一格)。唯一差别:`y` 顺手把底层也转了。
所以把 `y` 换成 `U` 之后,顶层解开了但底层没跟着转 —— **差的正好是一个 U 转,补在末尾即可**。

对每条公式数值搜索 `U^a + 原样 body + U^b`(a, b ∈ Z4),实测 4146 条起手带 y 的公式:

| | |
|---|---|
| **可改写(body 完全不变)** | **4129 / 4146 = 99.6%** |
| 无解 | **17** |
...
其余 ~800 条需要别的 (a,b) 组合 —— 它们本来就需要 AUF(§6' 的 1279 条),这一遍顺手一起修好。

**但收尾 AUF 不进公式** —— 站长规格:「末尾的 U 对魔友没有任何帮助」。
它是**派生数据**,播放时算出来补上:

| 层面 | 行为 |
|---|---|
| DB / 前端显示 / 复制 | `U R U R' U' R' F R F'` —— **无末尾 U** |
| `AlgPlayer` 播放 | 运行时算出收尾 AUF 并补上,动画跑完**精确还原** |
| 校验器 | 判据 = 「setup + alg **允许一个收尾 AUF** 后还原」 |

⟹ **规范存储形式:剥掉全部末尾 U-family move。** 这与站上原有的「禁末尾 AUF」规则一致,
只是把校验判据从"必须精确还原"放宽成"允许一个收尾 AUF"。
新增 `finalAufFor(setup, alg)`(放 `lib/cube3.ts`),播放器和校验器共用。
...
可行(§4.3'),做法是同一遍改写把三件事一起解决:

**改写算法**(幂等):

```
给定 case 的 setup S 和存库公式 X:
  1. X 若以 y^k 起手,剥掉它 → body(其余情况 body = X)
  2. 搜 (a, b) ∈ Z4²,使 S + U^a + body + U^b 还原
  3. 新存库公式 = simplify(U^a + body)      ← 收尾 U^b 不存
  4. 搜不到 (17 条) → X 原样保留
```

步骤 3 的 `simplify` 只合并**起手**的相邻 AUF(`U + U2` → `U'`),body 一个字节不动。

1. **跑上面的改写**,9 个顶层集合全量。
   - 起手 `y` 的 4146 条:4129 条改成 `U` 起手(99.6%),17 条 body 含 x/z 的原样保留。
   - 顺带修好那 1279 条「播放跑完不还原」的公式(它们缺的就是收尾 AUF)。
2. **改 `lib/alg_validation.ts:88-93`**(先决条件):判据从「精确还原」放宽成
   「**允许一个收尾 AUF** 后还原」;存库公式一律剥掉末尾 U-family move。
3. **`AlgPlayer` 播放时补收尾 AUF**(新增 `finalAufFor(setup, alg)`)。
4. **修 `lib/cube3.ts:69` 的 `mirrorAlg`**(见 §4.1)+ 补测试。
5. `OLL` / `COLL` / `CMLL` / `OLLCP` 不是整解集合(公式不还原魔方),要各自的判据才能校验 —— 单独一轮。
