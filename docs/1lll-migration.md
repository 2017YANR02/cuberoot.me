...
>
> **🐛 表侧的 `MIRRORLR` 带着第一轮的那个 bug。** Phase 0 实证:`1/PLL-U+` 的 alg[0]
...
### 4.5 计步(表里的 `SH` / `SQ`)—— **假说已被 3915 行逐行证实**

```
HTM(表里叫 SH,其实是 STM)= 删掉 [ ()'xyz234·↑↓./] 后剩余字符数
                              → 转体 0 步、slice 1 步、**宽块 1 步**
QTM(表里叫 SQ)             = HTM + ("2" 的个数) + 2×("3" 的个数) + 3×("4" 的个数)
GEN                         = alg 里 [A-Wa-w] 的去重排序(用 W 截断把 x/y/z 排除)
DELETE_AUF                  = TRIM(REGEXREPLACE(alg, "^U2'?|^U'|^U", ""))
```

计步前先 `DELETE_AUF` 再 `EXPANDALG`(展开 `(...)N`),标签 / 换握记号 / 署名全部剥掉。

**2026-07-13 全量验证,9 个指标全部 3915/3915 逐行吻合**:

| SH | SQ | GEN | DELETE_AUF | 镜像 SH | 逆 SH | QTM(Z) | rotation? | has move? |
|---|---|---|---|---|---|---|---|---|
| ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |

外加一条独立交叉验证:表里 `Move Count Tool` sheet 用的是**原生** `=HTM()` / `=QTM()`,
它缓存的 8 行结果也全部对上。

⚠ 站上**没有**正确的 3x3 计步器,而且**有 4 个各自为政的**:`lib/cube3.ts:43` `countMoves`、
`lib/recon-stats.ts:30` `htm`(字符法,把宽块 `Rw` 数成 2 步 —— 错的)、
`lib/recon-alg-utils.ts:165` `countMovesExpanded`、`math/god` 里还有个内联的。Phase 1 收成一个。
...
- **`PLL-U+` 的 alg[0] 是个坏镜像**。`L2 U' S U2' S' U' L2` vs `PLL-U-` 的 `R2 U' S' U2' S U' R2`:
  ...
  这与 §4.1 记的站上 `mirrorAlg` 是**同一类 bug**。表里的 `MIRRORLR` 很可能也漏翻了 U/F/B/D。
  该行另有 **9 条**公式都正确指向 `Ub`,靠这个 9:1 的多数派定的案。
- `3419/O-U8`(坏)与 `3347/O+U9` 互为 LR 镜像,而 3347 也在坏名单里 —— **坏公式经镜像传染**,
  佐证上面那条。Phase 1 修 MIRROR 时要把这批一起重算。
