...
| `pll` | 21 | **21** | 只换公式 |
| `zbll` | 472 | **472** | 只换公式 |
| `ell` | — | **25** | **新建 set**;20 个(ELL-A 12 + ELL-B 8)从 1lll 拆出,5 个(ELL-X)是新增 |
| `1lll` | **3400** | **3397** | −20(ELL-A/B 移入 ell)+17(OLL 20 组里非 ELL 的) |
| 合计 | 3893 | **3915** | 净 +22 |

> `+22 −25` 是净额,别当成"整组进整组出"。真实拆法:站上缺的**整个 OLL 20 组 = 22 个**
> (17 个 `Subset=1LLL` + 5 个 `ELL-X`),而站上 1lll 里现有的 **20 个 ELL**(`ELL-A` 12 + `ELL-B` 8)
> 要移进新的 `ell` set。3400 − 20 + 17 = **3397** ✓;20 + 5 = **25** ✓
...
**结论:表的 3915 行与站上 3893 个 case 完全对上,零残留。**

```
对上 3891 行  |  站上没有的新 case 24 个  |  定不了的 0 个
站上 3893 个 case 全部被认领(3893 / 3893)
```
...
| | 判据 | 结果 |
|---|---|---|
| ① | 状态轨道:表的公式解的 case ↔ 站的 setup 的 case,16 折 key 相等 | 3891 / 3915 |
...
`{X: 22, DS-: 1, O+: 1}`,Subset 是 `1LLL` 19 + `ELL-X` 5。

- **22 个 = 整个 OLL 20 组**(字母 `X`)—— 站上 `1lll` 压根没有第 20 组。
- **2 个 = 被站上那两对重复 case 挤掉的位置**。`DS-` = OLL 4、`O+` = OLL 5,
  正是 `1LLL 4 63==4 64`、`1LLL 5 21==5 22` 所在的组。**这是跑之前就写下的预言,逐行命中。**

对 GT 收口:`3400 + 17(X 组非 ELL)− 20(ELL-A 12 + ELL-B 8 移入 ell)= 3397` ✓;`20 + 5 = 25` ✓
...
| 期 | 内容 | 验收 |
|---|---|---|
| ~~**0**~~ | ~~状态轨道 join:表 3915 ↔ 站 3893~~ | ✅ **已完成**(§6''):3891 对上 / 24 个新 case / 0 个定不了;站上 3893 个全部认领;22 个新 case 由**纯群论枚举**独立证实;`Speedcubedb no.`(**472** 行,不是 474)465/472 交叉验证通过,7 条是表侧脏数据 |
...
- **2026-07-13(四)** — **Phase 0 完成:状态轨道 join 零残留**(§6'')。
  ...
  - 结果:**3891 行对上 / 24 个新 case / 0 个定不了;站上 3893 个 case 全部被认领**。
  - 24 个新 case = **22(整个 OLL 20 / 字母 X 组)+ 2(被站上那两对重复 case 挤掉的,落在 OLL 4 和 5)**。
