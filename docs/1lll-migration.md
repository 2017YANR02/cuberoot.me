...
实测 612 条站上以 y 开头且确实能解的公式,**100% 覆盖**:
...
> **🐛 "起手 y2 换成 U2、y 换成 U" 的规则是错的。** 实测:站上 3305 条以 y 开头且原本能解的公式,
> 字面替换后 **100% 全部解不开**。`y` 转的是整个坐标系(中心块跟着转),后面的招式写在转过之后的框里;
> 换成 `U` 只转了顶层、没转坐标系。
...
| # | 决定 |
|---|---|
| A1 | 起手转体的处理见 §4.3(用户原提的 y→U 已被实证推翻,去重语义**待重新确认**) |
...
2. **`setup` 用谁的**:保留站上现有 setup(魔方图不变,但站长的公式可能需要末尾 AUF —— 与站上
   "禁末尾 AUF"的校验冲突),还是改成 `inverse(站长首条公式)`(图会变成另一个 AUF 呈现)。

3. **OLL 字母 ↔ 数字映射**:从表的 `OLL` sheet 提取(该表有 `OLL` 字母列 + `No.` 数字列)。

---

## 9'. 进度日志

- **2026-07-13** — 分析完成。表已导出、公式层+值层齐全;case 轨道理论坐实;镜像/旋转/消转体三条规则实测通过;
