...
**代价:末尾多一个 AUF。** 它不是装饰,是 `y` 原本对底层做的那件事的显式化。
换来的是公式跑完**精确还原**(现状:1279 条跑完不还原)。
...
1. **对每条公式搜索 `U^a + 原样 body + U^b` 并写回**,让它精确解开 `setup`。
   - 起手 `y` 的 4146 条:4129 条可改写成 `U` 起手(99.6%),17 条 body 含 x/z 的保留原样。
   - 顺带修好那 1279 条「播放跑完不还原」的公式(它们缺的就是 AUF)。
2. **修 `lib/alg_validation.ts:88-93` 的"禁末尾 AUF"规则**(先决条件):它无条件拒绝末尾的
   U-family move,但改写后的末尾 AUF 是**承重的**(去掉就解不开)。
   正确规则:末尾 U 只有在**去掉后仍能解**时才算冗余。
3. **修 `lib/cube3.ts:69` 的 `mirrorAlg`**(见 §4.1)+ 补测试。
