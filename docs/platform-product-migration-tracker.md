...
状态：`P0-P8 技术迁移与发布验收已完成；P9 本地实现与三路复审已完成，待发布和线上角色态复验；旧资产 30 天观察中，删除决策未到期`
...
- `/platform` 首页已改为角色与任务驱动入口：公共导航收敛为发现、课程、社区、讲师、机构，登录后按真实课程、进度、讲师、机构与管理员权限显示一个对应工作台入口；旧编号目录、完整路由表和后台 CRUD 清单已移除。P9 尚未发布，线上状态仍以 P8 为准。
...
### P9 产品入口与角色体验重做 — `本地实现与复审完成，待发布验收`
