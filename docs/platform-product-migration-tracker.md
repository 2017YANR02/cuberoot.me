...
状态：`P0-P8 技术迁移与发布验收已完成；P9 产品入口与角色体验重做待实施；旧资产 30 天观察中，删除决策未到期`
...
- 当前 `/platform` 首页只完成了路由聚合，不是最终产品体验；2026-08-23 截图评审确认其重复导航、角色混排、超长管理目录与首屏无主要行动，P9 完成前不得再称 UI/UX 已完成。
...
- [ ] 盘点 `PlatformShell`、首页、导航、账户菜单和各角色工作台的现有组件，先提取共享能力再改版。
- [ ] 删除首页三重信息架构：顶部领域导航、右侧编号目录和底部完整路由表不得重复表达同一层级。
- [ ] 将公共主导航收敛为发现、课程、社区、讲师、机构；登录态与角色入口按权限显示。
- [ ] 为游客、学员、讲师、机构成员、管理员分别定义首屏主要任务、真实状态、空状态和下一步行动。
- [ ] 学员首页接入继续学习、下一课、进度与消息；无课程时引导浏览课程，不制造演示数据。
- [ ] 讲师、机构和管理员使用各自工作台；公共首页只给一个条件入口，不平铺后台 CRUD 路由。
- [ ] 重做视觉层级、文案、间距、焦点态和窄屏布局；避免通用报刊目录模板，体现魔方学习路径和教学服务。
- [ ] 保留 AppLink、中键/Ctrl 打开、键盘导航、reduced motion、双语、主题、metadata 与权限契约。
- [ ] 增加角色化渲染、权限可见性、关键链接、空/错/载入状态、桌面和窄屏的回归测试。
- [ ] 完成 1280px、1024px、430px、390px 中英文截图矩阵；覆盖游客、学员、讲师、机构成员、管理员代表状态。
- [ ] 由产品信息架构、无障碍/响应式、代码复用三路 reviewer 独立审核，修完全部 blocker/major 后再发布。
