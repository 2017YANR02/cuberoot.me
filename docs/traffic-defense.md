...
## 09-26 Vercel 名单写入失败后的运行预算

11:40 UTC 复查：23 条规则中有 13 条精确名单，实际下发 24,000 个 IP。扩大第 13 条到 1,700 或 1,875 个 IP，以及另增 1,500 个 IP 的规则，均返回 `FIREWALL_INTERNAL_ERROR`（HTTP 500）；原内容更新和第 13 条 1,500 个 IP 写入成功。尚未确定平台内部限制的具体原因，不能把此前按槽位计算的 52,500 当作可用容量。

同步器默认使用已验证的 24,000 个 IP 运行预算，超出记录保留排队；它不是平台公布的统一 IP 上限。过期记录移除后按原顺序补入。root-only 配置 `maxManagedIps` 可在平台写入恢复并验证后调高；无验证不扩容，不再因持续尝试扩容而阻止正常同步。指纹/扫描拒绝规则与验证码独立生效，但排队记录不代表已经完成 30 天精确 IP 封禁。没有新增付费服务。

## 09-26 验证通行期延长至 7 天；名单按容量排队

用户要求一次验证后通行一周。浏览器凭证和 Cookie 的最大期限改为 604,800 秒，固定到期、不按访问续期；服务间精确路径凭证仍为 60 秒。旧的 30 分钟签名仍按原到期时间处理，不自动延长。中国大陆豁免、全站验证码覆盖及封禁规则保留。账号文档同步说明访问凭证不代表账号登录。

滚动发布时 API 的 `COMPETITION_ACCESS_ISSUE_TTL_SECONDS=1800` 暂时维持旧签发期限；等 Vercel、Next、API 都支持 7 天后，将该值改为 604800 并 reload API。Cookie 的 Max-Age、响应 expiresIn 和签名到期秒使用同一值。源码默认最大期限为 7 天，该环境项只用于缩短签发期。

容量核实（2026-09-26 10:20 UTC）：active 共有 23 条规则，13 条管理名单实际包含 23,830 个去重 IP，账本已有 67,847 个 IP。5 万是此前同步器自行设的保护线，不是平台统一的 IP 数上限；直接超限抛错造成整批同步和到期清理停住。Vercel Pro 的 [自定义规则上限是 40 条](https://vercel.com/docs/vercel-firewall/vercel-waf#limits)，项目级独立 IP Blocking 为 100 条，不能代替大规模名单。此前 API 已确认每个条件 75 个值、每条 25 组，因此每条名单规则最多放 1,875 个 IP。

同步器改为按实际剩余规则分批写入，预留两条规则给后续防护调整；现有其他规则为 10 条时，28 条名单规则可容纳 52,500 个 IP。已分配的名单槽位保留，过期记录先移除，空位按账本顺序补入；超出的完整记录排队，不能算作已获得 30 天持久封禁。同步输出分别报告已下发、排队、容量及观察总数，容量告警每 6 小时最多一次。容量不再阻止已下发名单的到期解除；平台 API 写入失败仍保留账本并告警。现有指纹拒绝、原生封禁和全站验证码独立运行。不扩大到未命中的 IP 或网段，不启用新的收费服务。

发布后发现积压事件单分钟超过 1,000 条时，旧读取器仍会停住。读取改为自动缩小时间窗口，并只推进已完整读取的区间；每轮最多 30 次请求、45 秒，后续继续补读。读取失败保留进度，已观察记录仍落盘，现有名单同步和到期解除继续执行。日志另外报告采集延迟，不能将名单同步成功当作事件已经追平。

7 天版本 `96857d6817` 的 API、Next、运维和测试均通过，Vercel 生产部署 READY 后已将签发期限切为 604800 并 reload API。阿里云页面实测无凭证 307、7 天签名凭证 200；API 健康与数据库连接正常。

## 09-26 全站页面统一手动验证

本次将保护范围从比赛页扩展至主站所有内容页面，包括首页、计时器、选手、公式、论坛、开发文档及工具 HTML。中国大陆继续豁免；其他来源在无有效签名凭证时先跳到六位字符输入页，通过一次后通行 30 分钟。沿用原有 cookie 与验证码服务，已签发且未过期的凭证继续有效。验证码不是账号登录，登录回调与独立 API 的认证规则不变。

验证码页、验证码接口、两个 OAuth 回调、站点声明和实际静态资源保留通路，防止验证循环。Next 在语言改写和缓存内容之前校验，RSC/预取和带伪造 cookie 的请求仍须验证；任意页面路径不会仅因带图片扩展名而放行。直接由 nginx 提供的 tools/blog HTML 在访问阶段校验，含 static 工具源；Vercel 工具代理使用 60 秒、限定上游路径的服务签名回源，不把服务密钥发给浏览器。独立 blog.cuberoot.me 属于另一仓库，本次未改动。

未验证反复请求的 10 次/分钟、封禁 1 小时策略同步扩展至所有受保护页面与原有比赛 API，验证页停留、关闭和资源加载不计数。Vercel WAF 仍只能判断 cookie 是否存在，应用负责签名；伪造 cookie 不能读页面，但不能声称这条 WAF 计数规则能识别伪造签名。大陆豁免仍适用于自动访问。现有扫描封禁、指纹封禁、费用保护及临时计算器拒绝保留。

本地验证：客户端 12 项专项及类型检查通过；隔离 nginx 验证 HTML 拒绝/放行、静态资源、查询串回跳、CN 豁免、服务签名传递、API 403 与 static 回跳均通过。发布：`0a4dc67dd5` 的 [Next 部署](https://github.com/2017YANR02/cuberoot.me/actions/runs/36233829272)成功，Vercel `dpl_81dv3JWihsZBsfY9pKtp5joaRWcu` READY。阿里云生产进程实测首页、计时器、选手、开发页无凭证 307、有签名凭证 200；真实 Chrome 访问 next 与主域的计时器均进入六位字符页，空输入禁止继续。static 工具 HTML 无凭证返回 307 + private,no-store。Vercel active v274 的全站 1 小时规则已读回 valid，CN bypass 仍第一。

首轮 nginx 发布被既有 30 天名单同步器的 50,000 IP 容量保护挡住，未应用新配置；`c7358516d4` 将两个独立部署步骤解耦，保留 timer 和明确告警，[nginx 发布](https://github.com/2017YANR02/cuberoot.me/actions/runs/36234081493)成功。nginx 语法检查通过、Next 与 timer active。该成功不等于 Vercel 30 天名单同步恢复：09:53 左右账本已有 58,377 个不同 IP，新增记录仍保存，但超过本地 5 万上限，既有规则保留，新增封禁和到期解除同步可能延迟。对账本做无损连续地址合并仍需 57,615 条条件值，不能靠合并解决容量，不扩大封禁到未命中的地址。全站手动验证、原生扫描/指纹规则与 1 小时规则继续独立运行。此容量问题待单独处理，不声称全部新 IP 已获得 30 天持久封禁。

首轮客户端 CI 的唯一失败是稀疏检出未包含新读取的 WAF JSON；`c2701360da` 补齐配置文件及触发范围，随后同步 workflow 路径契约的精确预期，相关 21 项本地检查通过；继续观察对应 CI，未放宽或跳过校验。
...
补足 Vercel 30 天的实现是 `ops/vercel-ban-relay/relay.ts`：既有阿里云服务器每分钟读取上述两个规则的封禁事件，按首次事件保存 30 天的绝对到期时间，再维护独立精确 IP 拒绝规则；到期移除，重复事件不延长，CN 仍排除。状态在 root-only `/var/lib/cuberoot-vercel-bans/state.json`，专用令牌在 `/etc/cuberoot-vercel-bans.json`。只修改自己的规则，更新后读取 active 核对，不启用付费监控。Vercel 每个条件最多 75 个值、每条规则最多 25 组，名单按 1,875 个 IP 分规则；不扩大为未命中的网段。最多保留 50,000 个 IP，另检查项目规则总数不超过 40。更新前先落盘事件，写入失败不丢证据；容量、事件截断风险或超过 24 小时的采集缺口会保留现有规则并通过原有 Bark 提醒。提醒每 6 小时最多一次，不启用新服务。令牌到期前 7 天开始提醒。同步故障期间，新增与到期解除可能延迟。

2026-09-26 07:53 UTC 已经用户确认，在 Vercel 创建仅限 `cuberoot-me` 的专用令牌，60 天有效，控制台显示 2026-11-25 到期；只存阿里云 root-only 配置。首次实际同步暴露了单条件 75 个值的限制，已分组修正。07:59 UTC，timer 为 active，补录 17,695 个不同 IP。08:01 UTC active v72 读回 10 条有效规则、18,024 个去重 IP，所有条件组保留 CN 排除、CN bypass 仍第一。原生事件 07:56:32–08:01:32 UTC 的抓取特征规则记录 842 个不同 IP、842 次拒绝；这不是全站请求数。
...
隔离 nginx 验证已通过 162 项：26 种探测路径、普通页面、查询串误报、CN/loopback 豁免、多个域名、IPv6、伪造转发头、reload 及完整重启后封禁保留、过期记录不再封禁。测试只访问独立 loopback mock，不扫描生产文件。发布 workflow 在应用完整配置之前执行相同检查；生产部署状态需结合对应 workflow 结果确认。

### 分布式比赛抓取的额外规则

2026-09-26 05:47–06:47 UTC 的 Vercel 查询中，同一 IP 出现 Edge 120 与 Firefox 121 的轮换声明、同一 JA4 `t13d1516h2_8daaf6152771_d8a2da3f94cd`、多个比赛结果页和验证码页；多个国家来源复用该组合。UA 与指纹本身不是身份认证，但这些组合为本次临时防护提供依据。抓取规则同时匹配该 JA4 和已观察到的精确桌面 UA（首轮八种，08:08–08:09 UTC 补齐 Mac Chrome 119 与 Mac Firefox 121，共十种），排除 CN，命中拒绝并触发原生 IP 封禁。active v15 已确认规则位于普通比赛 challenge 之前。06:51:56–06:53:06 UTC，原生事件记录到 192 个不同 IP、194 次拒绝，实际到期为次日，30 天需上述同步器补足。它不因为 IP 在 Top 列表就封禁，也不单凭 JA4 封禁所有浏览器。仍有误伤使用相同组合的真实访问者的可能；可在 Firewall 规则中停用并检查持久封禁记录。匹配不到的新指纹不受这条规则保护，仍保留验证码与既有限流。

## 09-26 未验证反复请求：封禁 1 小时
