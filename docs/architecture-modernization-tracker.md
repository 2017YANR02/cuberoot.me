...
> Batch 8 决策更新：Batch 7 的“不执行物理目录整理”保留为当时的历史裁决。仓库所有者现基于四端长期演进和 AI 可发现性明确重开并授权目录迁移，目标采用 `core/apps/* + core/packages/* + core/jobs/*`；只允许按 [Core 工作区目录迁移方案](./core-workspace-layout-migration-plan.md) 渐进实施。LAY2-00 至 LAY2-07 已完成：workspace、边界守卫与 Knip 接受双布局，Web/API/stats 发布合同按 package identity 解析并覆盖旧新触发路径，四个离线 job 均已移至 `core/jobs`，小程序已移至 `core/apps/miniprogram`，Web 私有 solver 与跨 app 品牌资产依赖均已收口到中性入口。
>
