...
状态：独立复审完成，审核意见已合并。除文档外尚未授权实施架构改造。
...
详细现状、证据与锐评见 [architecture-audit-2026-08.md](./architecture-audit-2026-08.md)。历史迁移文档只保存历史，不得用其中未勾选事项覆盖本文的当前状态。
...
| Platform | 仓库所有者确认产品迁移已经完成 | 不列为未来 app，不再建立产品迁移阶段，不建 `apps/platform-web`；旧运行责任是否完全退役须独立核验 |
...
| DOC-04 | 建立文档状态约定并标记 Platform 遗留目录 | `待授权` | 活跃计划、已完成记录、历史文档和退役说明可被明确区分；Platform 文档不再把旧目录写成新功能归属 |
...
### G. Platform 遗留责任核验：不是产品迁移

| ID | 任务 | 状态 | 验收 |
| --- | --- | --- | --- |
| RET-01 | 只读建立旧运行责任清册 | `待授权` | 核对流量、写入、登录、API、回调、上传、域名、身份桥、workflow、service 与恢复路径；不以目录仍存在推断迁移未完成 |
| RET-02 | 建立数据保管与恢复清册 | `待授权` | 登记 SQLite、上传、日志、凭据、备份周期和恢复演练；代码回滚不冒充数据回滚 |
| RET-03 | 如仍有责任，制定切换与观察方案 | `待授权` | 冻结写入、外部 URL、回调和流量分别验证；只写方案，不执行停服 |
| RET-04 | 经单独授权执行退役 | `待授权` | 先停自动部署，再停服务；保留可恢复代码包和数据，最后才移除配置、workflow 与源码 |

G 阶段是对已完成迁移后的遗留责任做证据核验，不重新打开 Platform 产品迁移，也不默认认定必须退役。任何停用或删除都必须单独授权并走可恢复路径。

## 9. 推荐实施批次

只有本轮审核完成并得到用户授权后，才从以下批次开始：
...
Platform 的 RET 工作包不进入上述流水线，只能另行授权；RET-01/02 的盘点也不授权 RET-03/04 的切换或退役。
...
| --- | --- | --- | --- |
| 架构边界与 package 判据 | `architecture_boundary_review` | `复验通过` | 放宽单消费者特殊构建边界；按边类型守卫；PKG-01 前置；纳入非 workspace 依赖 |
| 多端 API、兼容与生成契约 | `multiclient_contract_review` | `复验通过` | 校正 iOS 事实；先做消费者矩阵与 `auth/web-session` 试点；暂不建 contracts package 或 codegen |
| 迁移、CI、部署与退役风险 | `migration_risk_review` | `复验通过` | 冻结 PS1 CLI 与无副作用检查；强化部署产物 smoke；Platform 遗留责任独立核验 |

审核要求：

1. Reviewer 必须用仓库当前证据反驳或确认计划，不能只评价文字。
2. 必须区分“建议优化”和“实施前阻断项”。
3. 不得把 Platform 重新列为待迁移产品。
4. 不得因追求目录标准化而忽略现有 workflow、构建产物和部署路径。
5. 审核只读，不编辑文件；由主 Agent 统一合并结论。

本轮三名 Reviewer 均确认总体路线成立，并在规则冲突和验收缺口修正后完成第二轮复验，结果均为 `PASS`。这代表计划可以进入“等待授权”，不代表任何源码、目录、部署或退役操作已获批准。

## 13. 变更记录

| 日期 | 变更 | 证据 |
| --- | --- | --- |
| 2026-08-21 | 建立跟踪文档；确认 Platform 不属于未来迁移或 `apps/*` 计划 | 仓库所有者最新确认；架构审计已同步修正 |
