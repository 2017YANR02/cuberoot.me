...
状态：独立复审完成，审核意见已合并。Platform 产品与数据迁移重新打开，架构源码改造在其完成前暂停；当前只允许继续只读调查。

> 阻塞说明：此前“Platform 产品迁移已经完成”的前提已被新的数据库与路由盘点推翻。当前边界、实施顺序和恢复门槛以 [Platform 产品能力与数据迁移跟踪](./platform-product-migration-tracker.md) 为准；本文中依赖旧前提的 Platform 决定暂不执行。
...
| Platform | 源码接入、新机构教学 SaaS 和独立前端退役已完成；旧公开产品能力与业务数据迁移未完成 | 不建 `apps/platform-web`；先按独立产品与数据跟踪表补齐主站迁移，再刷新本架构计划的真实边界 |
...
4. 不恢复独立 Platform app；旧产品能力与数据先迁入主站，在完成前不实施依赖其边界的架构改造。
...
G 阶段暂缓。Platform 产品能力与数据迁移已经重新打开，先按独立跟踪表完成；任何停用或删除仍必须单独授权并走可恢复路径。

## 9. 推荐实施批次

以下批次全部暂停。只有 Platform 产品迁移跟踪表 P7 完成、依赖基线重新刷新、架构方案再次复审通过并得到用户重新授权后，才可开始任何批次：
...
2026-08-21 的三名 Reviewer 曾确认当时的总体架构路线成立，但该结论建立在已被推翻的 Platform 完成前提上，只保留为历史审查证据。当前不得据此恢复任何源码、目录、部署或退役操作。
