# 2026-09-22—25 流量事件复盘

公开阅读页：`/zh/dev/traffic-incident-2026-09`（英文 `/dev/traffic-incident-2026-09`）。本文是事件档案，不是实时状态页。数据与处置记录截至 2026-09-25；新截图取于 09:37–09:38 UTC。未新增收费监控，未重新开启 Analytics。

## 给普通读者的说明

9 月 22 日，计算器页面出现流量突增。抽样请求快速更换比赛、选手和轮次参数，部分声明使用无头浏览器，支持“自动化遍历链接”的判断。9 月 24—25 日我们临时限制计算器、暂停和恢复两条生产线路，并加入限流、缓存保护和服务器自动停站。

访问者在处置期间可能遇到验证、403 拒绝或 503 维护页。这是防护措施造成的可用性影响。记录不能证明每一条请求恶意，也未建立攻击者身份、动机或数据泄露结论。后续密集日志中有一部分实际上来自网页预取；这一判断纠正必须与处置结果一起公开。
...
- 2026-09-25 09:37:04 UTC 重新打开同一历史窗口，界面仍显示以上数值；仅截取报表区域，未更改数据。新截图不是事发时保存的截图。
- 图片：[Vercel 历史曲线](../core/packages/client/public/assets/incidents/2026-09/vercel-analytics-sep22-v1.png)。
...
- 判断先于标签：路径、预取字段、UA 可信度、来源集合、回源压力与错误率需一起看。日志密集不是单独证据。
...
- 安全措施有用户成本：全站挑战和停站会打扰正常访问，计算器拒绝会损失功能可用性；应在证据支持后逐步收紧范围。
...
截图保留原始像素，不用 AI 重绘；仅通过浏览器截图区域排除无关和敏感内容。页面的时间线、请求放大示意、线路覆盖图和流程图是手绘矢量/HTML 图，示意图不含伪造流量序列。报表图按已记录的离散快照绘制，不插值为连续时间曲线。

截图 SHA-256（PNG 原始字节）：

```text
vercel-analytics-sep22-v1.png
