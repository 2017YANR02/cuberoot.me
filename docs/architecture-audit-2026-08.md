...
状态说明（2026-08-25）：本文是 2026-08-21 起形成的审计快照，架构证据仍可用于后续现代化调查。Platform P0-P8 技术迁移与发布验收现已完成；P9 主站产品体验改版的代码与发布已验收，Test、Deploy Next、Deploy Core 全绿，线上真实角色态仍待复验。仓库所有者已明确豁免 RET-04 原定观察等待；非仓库资产处置完成，仓库删除由所有者自行执行。Platform 当前边界以[架构现代化跟踪](./architecture-modernization-tracker.md)和[主站完整迁移跟踪](./platform-product-migration-tracker.md)为准；迁移后的源码实施仍须刷新依赖基线、重新复审并取得用户授权。
...
RET-04 已于 2026-08-25 按所有者明确决定完成：临时明文数据库副本进入回收站，加密权威归档保留；遗留仓库保持原位并由所有者自行删除。不要为了目录观感搬进长期 `apps/*`。架构图应同时标明当前主站运行单元和 Platform 历史归档，不能把归档误画成活动前端。
...
- P9 产品入口改版继续走独立跟踪；RET-04 非仓库资产处置已完成，仓库删除由所有者执行，不恢复独立前端，也不进入 `apps/*`。
- 迁移后的依赖图已经变化；任何架构源码实施仍先重建基线并重新授权，与 Platform 角色态复验重叠的改动需单独协调，不得把无登录 smoke 冒充角色态验收。

验收：P0-P8 发布验收和三路复审已有记录；RET-04 已按 2026-08-25 的所有者明确决定完成并记录可恢复处置证据，AI 不执行仓库删除。
