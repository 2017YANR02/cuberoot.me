...
The hourly nginx monitor covers the separate server line and therefore cannot classify this Vercel-only spike on its own. Use the filtered Vercel views above to review this incident while the logs are retained. The monitor's source attribution for future nginx spikes is documented in [traffic-monitor.md](traffic-monitor.md).
