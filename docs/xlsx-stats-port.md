...
| B13 | 一轮五把里 ≥n 把跳步(二项) | partial(§1.9)。`atLeastKInRound(p,k,rounds)` 已落地并锁死(k=3 对上表格的 1/38,114),速查表用了 k=1 那一列 | 补一个独立小节把 k 也放开 |
...
4. ~~B1 速查表(收敛 `method_dna`)~~ ✅ → B12 + B13 独立小节 ← **在这**
...
- [ ] §3 第 4 步:B12 Roux skip + B13 二项独立小节
