...
只能靠「组内消去 + CP 约束 + 多数派投票」落位 —— 下面这 8 张落错了行:

| case | 原本挂着 | 应当挂 |
|---|---|---|
| `1LLL 7 7` / `7 12` | 3496 / 3491 | 3491 / 3496 |
| `1LLL 17 26` / `17 38` / `17 41` | 698 / 701 / 686 | 686 / 698 / 701 |
| `1LLL 19 50` / `19 62` / `19 64` | 578 / 580 / 566 | 566 / 578 / 580 |

判据是表**自己**给的,三路独立:①`689` `701` `568` `580` 四行的六个打乱列全票一致,其中 `701` `580`
落的位置与库里不符 —— 这两条不靠投票就定案;②其余四行由全票行占位后排除到唯一解,且各自的
`SQ*`+`Q*` 两列独立指向同一答案;③四张的 `CP` 字母与它该落的角置换类一致。`7 7`/`7 12` 那对
另由六条来自全票行的关系式判定(`3419.Inv` `3280.Inv` `3563.Mirror` `3568.Mirror` `3347.IM` `3208.IM`)。

改完(migration `0102_fix_1lll_meta_assignment.sql`)之后,三个关系在状态判据下**残差为零**,
`CP` 只剩 `X` 组那 12 条 —— 那不是错,是 `ident()` 的角置换类在高对称 OLL 下天然更粗(见 `ll_ident.mjs`)。

> 站上那个「校验」按钮发现不了这类问题:它只跑 `setup + alg → 目标态`,一条条验公式,从不读 meta。
> 这 8 张的公式确实都解得开自己,所以一直显示「全部通过」。**这一层要靠上面那个脚本。**
