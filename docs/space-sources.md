...
核对日期：2026-09-07。魔方空间页面底部“本页来源与建模说明”默认展开，直接展示各项目链接、用途及上海轮廓拉伸和高度估算说明，也可收起；“查看全站致谢”进入 `/about`。公开致谢的唯一数据源是 [credits_data.json](../core/packages/client/app/[lang]/about/credits_data.json)，`space: true` 标记本页使用的条目。本文件记录技术版本、实际用途和未接入的边界，不维护第二份产品致谢列表。
