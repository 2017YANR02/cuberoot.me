...
状态:Platform 迁移已经全部完成,独立前端进入退役状态。主站是唯一教学前端,旧源码与运行数据仅作为可恢复档案保留。
...
- [x] 停止 Platform 运行时写入,保留 SQLite、uploads 与源码归档,不迁移旧计时历史。
- [ ] 备份和观察窗口结束后,由仓库所有者删除旧目录、旧 GitHub 仓库和其余退役资源。
...
- 不为旧计时历史建立迁移项目。
- 本次退役不删除 Platform 源码、SQLite 或上传文件;旧仓库由仓库所有者在确认备份后删除。
