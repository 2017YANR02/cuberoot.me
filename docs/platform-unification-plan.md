...
状态:机构教学前端统一已经完成,独立前端已退役;旧 Platform 的公开产品能力和业务数据尚未完成迁移。后续执行以[Platform 产品能力与数据迁移跟踪](./platform-product-migration-tracker.md)为准。
...
- [ ] 仅在产品迁移跟踪表 P7 全部完成且仓库所有者逐项批准后,才能删除旧目录、旧 GitHub 仓库和其余退役资源。
...
- 不为旧计时历史建立迁移项目。
- 本次退役不删除 Platform 源码、SQLite 或上传文件;旧仓库只有在产品迁移跟踪表 P7 全部完成且仓库所有者逐项批准后才能删除。
