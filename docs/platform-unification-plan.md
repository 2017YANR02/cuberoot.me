...
- 在 `packages/client` 建 `/org` 路由组、后台布局、机构切换和按权限生成的真链接导航。
- 直接使用主站登录态调用 Core,不再经过 Platform HMAC bridge。
- 复用主站 AppLink、i18n、表单、分页、空状态和响应式组件;缺少的通用组件先提取再复用。
