...
状态:机构教学前端统一与旧 Platform 产品能力已经实施并通过 P8 发布验收,独立前端已退役;仓库所有者于 2026-08-25 明确豁免原定观察等待并完成 RET-04 决策,旧 seed/demo 和计时历史按决策不迁移。仓库删除由所有者自行执行。后续执行以[Platform 产品能力与数据迁移跟踪](./platform-product-migration-tracker.md)为准。
...
- [x] 2026-08-25 仓库所有者明确豁免原定 30 天观察等待并逐项决定退役资源:临时明文数据库副本移入回收站,加密权威归档保留,旧目录与 GitHub 仓库由所有者自行删除,AI 不执行仓库删除。
...
- 不为旧计时历史建立迁移项目。
- 本次退役不由 AI 删除 Platform 源码、仓库内 SQLite 或上传文件;旧本地目录、远端仓库和主仓历史归档均交由仓库所有者自行处置。
...
- 仓库内旧 SQLite、uploads 和源码不属于 AI 删除范围;加密权威数据归档继续保留,仓库最终物理删除由所有者执行。
