...
- [~] recon 双播放器合并。**①引擎对 `Sq1ReconPlayer`↔`CuberReconPlayer`(~85% 逐字复制的 WebGL 播放器)已合**:抽 `components/recon/ReconPlayerBase.tsx` + `ReconPlayerAdapter<M>`(parseMoves/setupPuzzle/applyPrefix/pushMove/backView/deps/kind),两播放器塌成 ~70 行 adapter 薄壳,两份 CSS 并成 `recon-player.css`。同名同路径同 props 零消费者改动;849→598 行。commit `<recon>`;验:typecheck 绿 + 双引擎 `/recon/[id]` 实机跑 move-parse(49 空格 token / 25 sq1 token)/step/back/scrub/play/reset + backview 有无(cuber 有 / sq1 详情无),WebGL 渲染循环与原版逐字 diff 一致(headless 挂不出 GL canvas,原版同样,故渲染像素按构造不变)。**②派发器对 `ReconPlayerPane`↔`ReconPlayerCanvas` 待做**:同分支逻辑(sq1/NxN cuber|cubing/其余),差在状态归属(内部 debounce vs 提升到父)、toggle 皮肤(radiogroup vs PillToggle);至少 Pane 里内联重抄的 `loadReconEngine/saveReconEngine` 该 import Canvas 已导出的那份。
