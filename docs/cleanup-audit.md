...
- [~] 其它 god component：**`WcaStatView.tsx`（1290）已拆**（commit `3a08db3be3`：types 63 + cells 230 + views 710 + 333 行编排壳,同路径同具名导出零消费者改动;i18n 三元→`tr()`、metric-pill 假按钮补 `role=button`+键盘,static-onclick BASELINE 45→44;EN/ZH × rows/sections/panels/metricPanels 四态实机验)。**剩 3 个暂缓**(逐文件核过,见 §C):`wca/results/page.tsx`（1030，4 视图共享 ~20 locals→拆需重度 prop-drill）、`FrameCountPage.tsx`（2820，129 hooks 深耦合,只叶子可搬）、`LandingSearch.tsx`（744，一下拉套 10 条互相牵连匹配列表,无干净接缝）——都是「一个复杂页写成大文件」非复制粘贴,拆纯美观、回归面大,非特别嫌先不动。
...
- [~] recon 双播放器合并。**①引擎对 `Sq1ReconPlayer`↔`CuberReconPlayer`(~85% 逐字复制的 WebGL 播放器)已合**:抽 `components/recon/ReconPlayerBase.tsx` + `ReconPlayerAdapter<M>`(parseMoves/setupPuzzle/applyPrefix/pushMove/backView/deps/kind),两播放器塌成 ~70 行 adapter 薄壳,两份 CSS 并成 `recon-player.css`。同名同路径同 props 零消费者改动;849→598 行。commit `<recon>`;验:typecheck 绿 + 双引擎 `/recon/[id]` 实机跑 move-parse(49 空格 token / 25 sq1 token)/step/back/scrub/play/reset + backview 有无(cuber 有 / sq1 详情无),WebGL 渲染循环与原版逐字 diff 一致(headless 挂不出 GL canvas,原版同样,故渲染像素按构造不变)。**②派发器对 `ReconPlayerPane`↔`ReconPlayerCanvas` 已合**(commit `b82cad6064`):抽共享 `components/recon/ReconEnginePlayer.tsx`(sq1→Sq1ReconPlayer,NxN→CuberReconPlayer,其余→cubing.js TwistySection),两派发器只留各自框架(Pane 内部 debounce + 强制 backView;Canvas 提升态 + hideControls)委托它。**顺带按用户要求砍掉 cuberoot/cubing.js 引擎切换开关**——recon 只留自有 cuber 动画:NxN/sq1 永远走 cuber,`loadReconEngine/saveReconEngine/ReconEngine/engine` 全删(ReconDetailClient/AttemptPopover 同步去 state),两份 toggle CSS + catalog 描述清掉。净 -83 行/8 文件;验:typecheck 绿 + 3x3 `/recon/[id]` SSR 无 `detail-engine-toggle`/无 `cubing.js`/渲 cuber 基座 + 守卫(static-onclick/i18n/catalog-sync)绿。**Phase 2 进行中**:cubing.js/TwistySection 现只剩 pyraminx/megaminx/skewb/clock 的兜底;金字塔/五魔方/斜转/minx 迁到 cuber 引擎(sim 引擎已支持,`/sim?puzzle=pyraminx` 已上线),clock 暂留 cubing.js。
- [~] 无共享 `ModalShell`:Donate/Feedback/Login/AlgCaseMeta 各搓 Escape+overlay。**否掉「大 ModalShell」,改抽窄 hook `useModalDismiss(onClose, disabled?)`**(只管 Escape→onClose + `body` 滚动锁,overlay-click/关闭按钮各 modal 差异大不并)。已接:DonateModal、FeedbackModal、ReconReuseModal、ScramblePicker、**PayModal**(其 setTimeout 查单清理拆成独立 unmount effect 后即净替)。**留手写不动**:LoginModal、`alg/_roux` `Modal`——都是 `open` prop 常挂型(`if(!open)return`),hook 挂载即锁滚动会在关闭态误锁;要接需么父层改条件渲染、么给 hook 加「按 active 门控滚动锁」参(会动现有 6 消费者语义,`FeedbackModal` 靠 `disabled` 只压 Escape 保留锁滚),收益不抵风险,保留。
- [ ] `/dev` intro 三套并存范式（数据驱动壳 vs 28 个 bespoke language/*/page vs algorithms 手写）统一

## P1 跨区收口

- [x] **`getIp(c)` 复制进 21 个后端 route**，都保留可伪造 `X-Forwarded-For` 回退 → 已加共享 `getIp(c)`（`analytics_helpers.ts`，委托权威 `getClientIp`、去 XFF 回退），21 route 删本地 def 改 import，111 调用点不变。server typecheck 绿。**★安全硬化，commit 未 push（需上线才生效）**。**分层防回归**（「立约束要分层」）：写入态 hook `.codex/hooks/block-server-forwarded-for.ps1`（→ `hook-detect-server-forwarded-for.mjs`，实活验证真拦）+ CI 兜底 `tests/server-no-forwarded-for.test.ts`，已登记 `/dev/guards`（`_guards.ts` PAIRED_GUARDS）；新 route 再抄回 XFF 回退会被写入即拦 + CI 红。行内 `allow-forwarded-for` 豁免正当用途
...
- [x] 内联 `isZh ? '中' : 'EN'` 文案三元 → `tr({en,zh})` / `<T>`。**实为早已迁完**:`tests/i18n-no-isz-text-ternary.test.ts` 的 BASELINE 已锁 **0**(注释:2026-06-14 `codemod-isz-burndown.mjs` 419→0),CI 棘轮守着——任何新增内联 UI 语言文案三元直接红。当前 `grep isZh?` 的 544 处全是**合法**用法(util 参数 `displayCuberName(name,isZh)` / `'/zh'` 前缀 / `isZh?16:12` 数值三元),非文案三元。审查当时「80+ 处」把这些合法用法误计入了。
- [x] 本地 `t=(zh,en)=>isZh?zh:en` shim → `useT()`。**逐文件核对后大幅缩小**:58 文件里 **46 个 `isZh` 是 prop**(CLAUDE.md 明许「`isZh` 仅可作 util/param」,shim 是合法局部表达,**不迁**)、3 个特例(`AdminSubmissionsPanel` 故意渲染在 `I18nProvider` 外靠 `lang` prop、`_shared.tsx` 只在 JSDoc 里出现假 shim、`PersonHero` 非 client 组件),只有 **9 个** `isZh` 由全局 i18n 派生。其中又只有 **2 个**是「shim 为 isZh 唯一消费者」的净收敛(`sim/GroupTheoryPanel`、`_PaintToolbar` 的 `PaintPalette`)——`useTranslation`+`isZh`+`t` 三行塌成一个 `useT()`,已迁;其余 7 个(math/god/page、PlayerControls、feedback/admin、AppearanceToggle、FeedbackConversation、PaintActions、两个 InteractiveCube)`isZh` 还要喂 `friendlyValidErr(raw,isZh)`/`displayCuberName(...,isZh)`/子组件 prop,迁 shim 是净零(isZh 派生仍留),局部 shim 反而与同一处 isZh 读取一致,**保留**。
...
- [x] `CountrySelect` / `RegionCountrySelect` 半截迁移收尾。`results` 早已换成 `RegionCountrySelect`(RegionPicker 适配器,带大洲分组,接口与旧 `CountrySelect` 逐字相同),但 `cohort-ranks`/`all-events-done`/`success-rate` 仍用旧的扁平 `CountrySelect`。后端 `wca_stats_extra.ts` 的 `resolveCountry()` 早支持 `_`前缀大洲 id,三页零功能缺口。收尾:把共用的 `useCountries()`/`CountryOption` 抽到中性 `components/wca-stats/useCountries.ts`,3 页 import+tag 换 `RegionCountrySelect`,`results`/`fun-stats` 的 `useCountries` import 改指新模块,删旧 `CountrySelect.tsx`(80 行 bespoke 下拉,与 RegionPicker 重复)+ `/dev` 目录条 + `_wca_stats_extra.css` 死 `wse-country-*` 内类(保留 `.wse-country` 外壳,RegionCountrySelect 仍挂)。typecheck + dev-catalog-sync 5/5 绿,safe-chrome 验 success-rate 渲染出 RegionPicker。
...
- [x] Vite 残渣（代码分支）:`viteDev` 死分支（`shared/src/api/stats-base.ts`、`shared/src/alg.ts`）→ **已删**。Vite 全站移除后 `import.meta.env.DEV` 永不被替换(Next/Turbopack 不注入 `import.meta.env`),`.env?.DEV` 恒 `undefined`→`false`,是死分支。两处 `isDev()` 只留 `process.env.NODE_ENV === 'development'`(Turbopack build 时替换字面量),注释同步去 Vite 引用。shared rebuild + client typecheck 绿。(CLAUDE.md「dev/prod 用 `import.meta.env.DEV`」是 Vite 期措辞,本意=用 build-time flag 而非 hostname,现由 NODE_ENV 承接,意图不变。)
...
- [x] `components/wca-stats/ShowToggle.tsx` 死组件（仅 /dev 画廊引用；`ShowMode` type 被 wca/results 用）→ 已把 `ShowMode` 内联进唯一消费者 wca/results，删组件 + 去画廊 import/demo/registry/metadata 四处。`dev-catalog-sync` 测试 5/5 绿，typecheck 无新错
- [x] `/dev` `cuberootDesc` 字段（49 文件填充但 0 渲染器读）→ **已删**。核实真相：它是被取代的短草稿——每个工具另有一个**已渲染**的长版 `cuberoot: {zh,en}`（页面「它在 cuberoot.me 上做什么」那节），短版 `cuberootDesc` 内容长版都有,删掉不丢信息。sed 逐行删 50 文件 / −100 行(98 内容 + 2 类型,全单行),grep 归零 + client typecheck 绿
