...
China mainland exemption (CN, 2026-09-26): nginx derives `cn_exempt=1` from the connection IP using DB-IP country ranges, including IPv4 and IPv6. The marker is appended after the complete combined-log fields; headers and User-Agent text cannot set it. CN requests remain in reports and error counts but do not contribute to automatic shutdown decisions. When maintenance is enabled for other regions, CN requests continue to the application. This exemption covers incident traffic controls, not application authentication or write-operation abuse controls. A proxy's connection address is not proof of the original visitor's country.

Country ranges are generated by `ops/geoip/update-cn-ranges.ts` and installed with the nginx workflow at `/etc/nginx/cuberoot-cn-ranges.conf`. A daily timer checks for the monthly release; if already current it performs no download. Failed downloads leave the previous database in use; nginx validation failure restores its backup. Missing/unknown country entries receive the existing limits. Data attribution: [IP Geolocation by DB-IP](https://db-ip.com), country Lite, CC BY 4.0. Geolocation is approximate, not an identity check.

The server-local `cuberoot-traffic-guard.timer` checks the three nginx access logs once per minute, 15 seconds after the minute boundary. It evaluates the last two complete minutes. A very large one-minute spike or two sustained high minutes in non-exempt successful page requests, non-blocked web/API requests, or 5xx responses trips the root-owned `/etc/nginx/cuberoot-maintenance-state.conf` switch and reloads nginx. Non-CN traffic sees the existing maintenance notice on the main site and 503 on `next.cuberoot.me` and `api.cuberoot.me`; CN and ACME validation stay reachable. The guard sends a Bark alert using the existing key stored root-only on the server. It never auto-reopens restricted traffic; inspect the incident and explicitly set `default 0;` in the state file, then run `nginx -t && nginx -s reload`.
