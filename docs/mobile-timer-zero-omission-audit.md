...
| GAP-008 | Web 设置有 8 类、64 个字段（其中 1 个仅开发环境） | 8 类与 64 个稳定 ID、copy/value/visibility/disabled/effect 已迁 shared；“计时”8 字段及“外观”的点击打乱动作已由 Web/Mobile 共用真实 UI/effect，App effect ledger 为 9 项。OPPO 已读到 canonical 8/8 计时 ID、20～340px 内容边界与滚动底部；其余 55 字段、点击动作真机效果、iOS/其他三端和全视口仍缺 | P0，9 项接线完成不等于 64 项设置 parity |
...
| `scramble.click-action` | 无操作/下一条/复制，复制勾和智能魔方修正路径特例 | 枚举/default/normalizer/persistence 与选择控件已由 Web/五端共用；App 三动作、错误态重试优先级及复制反馈已接自动化，旧数据缺字段默认 `copy`、显式坏值拒绝。仍待 OPPO/iOS/Harmony/Windows/macOS 三动作、智能魔方修正路径与视觉矩阵 |
...
| 外观 | 计时器字体/字号、打乱字体/字号、紧凑打乱、打乱图、3D 魔方、点击打乱动作、运行隐藏全 UI、排名徽章、排名国家 | “点击打乱动作”已用同一 shared 控件、三选项、默认/迁移和 effect；其余 10 项仍缺，且该项五端真机/实体电脑视觉与交互矩阵未验 |
