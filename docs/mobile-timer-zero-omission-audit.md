...
2026-09-12 所有者明确调整计时输入：五端 App 的当前打乱不允许点击或手势复制；旧 `scrambleClickAction=copy` 在产品层按 `none` 执行，设置只保留无操作与显式换题。数字区按住及拖移只走起停，不打开轮盘；共享 `TimingSurface` 屏蔽数字和打乱正文的原生选择/上下文菜单，输入框仍可正常编辑。该决定覆盖早期三动作复制验收记录，历史记录/房间邀请的显式复制不属于当前计时面。Web 同样消费数字区保护，网站打乱的显式点击偏好保持原契约。
