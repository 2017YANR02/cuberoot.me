...
- SQLite 的 33 个 migration 与 journal 原样保留,不与 Hono/PostgreSQL migrations 混合。
...
迁入代码已经有用户登录、课程 / 章节、学习进度、订单、会员、讲师入驻与后台、支付、内容、社群、上传和运营日志。它是完整平台的可复用基线,但还不是所讨论的多机构教学 SaaS。

后续应在同一 `@cuberoot/platform` 产品边界内补齐:
...
- [x] `platform` typecheck 通过;Vitest 2 个文件、4 个测试全部通过;平台范围 Knip 通过。
...
- [x] 两个 workflow 通过 YAML 1.2 解析;`docker compose config --quiet` 通过。
- [x] Docker ignore 已显式重新包含 `patches/`、`packages/` 及 `packages/platform/` 父目录,避免白名单子路径被父目录排除。
...
4. 服务器 `/etc/cube-platform.env` 已配置新的强随机 `ADMIN_PASSWORD`、`SESSION_SECRET`,旧默认凭据已失效。
5. 对生产 SQLite 做一致性备份,单独备份 uploads,并实际验证备份可读。workflow 失败时会自动回滚代码包;迁移器用单个 SQLite transaction 防止半迁移,但已成功提交的 schema 不会随代码包反向回滚,数据库恢复必须使用已验证备份并评估期间新增写入。
6. 逐项处理旧本地目录中的非 Git 状态:`data.db`、`data.db-wal`、`data.db-shm`、`qr-layout.png`、`.tmp` 内商业文档 / 调试产物与 `.password.md`。每项必须先外部备份或明确确认放弃;凭据要迁入安全存储并轮换,不能复制进主仓。
7. 首次推送产生的平台、主站和后端 workflow 都成功;平台首页、登录、管理员、讲师、课程、订单、上传和数据库 migration 完成 smoke test。
8. 已确认代码包与数据库各自的失败恢复步骤,上一版本包和数据库备份都可用。
9. 在主仓 tracked 文件中搜索旧本地路径、旧 workflow / unit 路径和旧仓唯一依赖,结果为 0。
10. 再保留一个观察窗口;确认无回滚需要后,才删除旧本地目录和旧 GitHub 仓库。
