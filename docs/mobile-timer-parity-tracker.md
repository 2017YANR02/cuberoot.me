...
| PAR-015 | 多人智能魔方与本地 Battle | 第一批 runtime-neutral 规则已进 `@cuberoot/shared/timer` 并由 Web store 真实消费：2～4 人归一化、active slots、按键映射/冲突交换、同项目分组、共享打乱显隐、`ok/+2/DNF` 与并列胜者。下一步必须建立原子 round DTO + 纯 reducer，再提同一 `LocalBattleView`；不能新写 Mobile battle store | 2～4 人、共享/独立魔方、pointer cancel、旋转不改业务态、混合项目原子回合 fixture；当前定向状态机已覆盖 pointer cancel 与纯布局变更，原子历史/reducer 仍缺 | 触摸与真实 BLE 组合 | `进行中` |
