...
| BASE-07 | session CRUD/项目关联已迁到 shared 纯操作与共用 UI；7 个 quick action 规则与完整成绩行、单一快捷菜单/底部操作表、注释失焦保存和 5 秒 Undo toast 分别由 `@cuberoot/shared/timer` 与 `@cuberoot/timer-ui` 单源维护。Mobile 已接通 OK/+2/DNF/DNS、备注/复制/删除、详情移组及搜索/日期/时间/罚时/OLL/PLL/标签筛选。8 类派生标签的 ID、双语文案、顺序、toggle、PB/ao/MBLD 计算与响应式徽标/筛选器也已进入同一 shared/timer-ui 实现，Web 和五端产品层使用同一派生 map；标签不写入 DB/备份，罚时、删除、导入和移组后会自动重算。但 App 新产生的智能魔方成绩尚未持久化 `moves/stageSegments`，因此跳O/跳P 目前只对已有分段数据的导入/旧记录真实可用；该 producer 必须后续从 Web 迁到 shared，不得在 App 另写识别器。批量/对比/完整 SolveModal/复盘/统计图表也仍缺 | shared history action/filter/tag/move 合同；共用 History 行/菜单/editor/toast/tag/overlay DOM 与交互回归；Client 标签定向 21/21、App UI 全集 222/222、四包 typecheck 与 Mobile/Desktop/Harmony Web build 通过；新 APK 已覆盖安装并字节核对。手机锁屏期间未完成新标签点击/TalkBack/全视口真机验收 | `进行中` |
