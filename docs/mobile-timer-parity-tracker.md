...
| PAR-008 | 打乱文本与魔方图 | `@cuberoot/timer-ui/TimerCubePreview` 为 Web/五端唯一 dispatcher；cubing.js 和 `puzzle-render-core` 的 SQ1/Megaminx renderer 单源，宿主只控制响应式外框 | renderer identity、333/pyra 分派、SQ1/Mega SVG、invalid fail-closed/recovery 与生产 chunk 测试 | OPPO 横竖屏、动态字号、全部支持项目与 2D/3D/显示设置仍待完整矩阵 | `进行中` |
