...
| PAR-012 | 更多菜单和设置 | 菜单模型与共用设置控件单源；宿主只注入真实平台动作，未绑定项不得显示 | More 12 项与 Settings 8 类/64 字段 exact registry/条件/effect、Web 真实 consumer、App implemented/missing ledger；App 已接 More 10 项、“计时”8 字段、“训练”2 个分段字段及“外观”的打乱图、2D/3D、点击打乱动作共用 UI/effect，专项/replay 和其余 51 个设置字段仍待迁移 | OPPO 已显示 8/8 计时 setting ID并实证点击打乱三动作；新增两开关、新分段设置/触摸、iOS 与其他三端待验 | `进行中` |
...
| 日期 | 证据 | 结论 |
| --- | --- | --- |
| 2026-09-02 | App 复用 `TimerWcaDifficultyConfig.topControlsSlot`，把共享的“合并/难度”控件 portal 到“最优打乱”所在行，没有复制开关或状态。App 34 files / 233 tests、App/Mobile typecheck、Mobile production build、Capacitor Android sync 与 OPPO `PFDM00` `installDebug` 通过。真机 360×749 下三组边界均为 y=106～128px；CDP 模拟 320px 时仍同为 y=106～128px、按 x=22.49～119.49、127.49～218.50、226.50～297.51 排列，互不相交，文档宽 320/320。恢复 360px 后 `0.000` 为 y=271.67～358.07px，完整位于 Surface y=259.67～654.30px | 关闭 Android 中文窄屏把“最优打乱/合并/难度”拆成两行的回归，同时确认没有重新引入横向溢出或计时数字裁切。修复位于五端共用 `@cuberoot/app-ui` 产品层并消费既有 timer-ui 插槽；英文长标签允许按既有响应式规则换行，其他语言/字号、横屏、TalkBack 及 iOS/Harmony/Windows/macOS 仍待矩阵验收，整体继续 `ACTIVE — NOT COMPLETE` |
