...
| `GAP-010` / `PAR-014/015/016` 设备 | Web 有智能魔方、智能计时器、Stackmat 及完整设备弹层 | GAN v2/v3/v4、MoYu32、QiYi 协议、MoveClock/状态模型、跨端 SessionController 与复盘 producer 已共享；其他 Web driver 仍需逐项核对 | 实况魔方、姿态、自动预备设置已共享；设备中心弹层未共享 | `否` | Android 已能过滤已知魔方名称、仅显示名称并完成当前真机扫描/连接/使用；型号级证据仍只登记 GAN v4。App 尚无智能计时器/Stackmat，其他四端 BLE 矩阵未齐 | `进行中（P0）`；提取 `TimerDeviceCenter`，平台只注入 transport/permission |
...
2026-09-22 增量：Web 与 Installed App 的标准化 move/state 会话已共用 `SmartCubeSessionController`，连接 lease、MoveClock、状态推进与 solved 边沿不再各自实现；发现、权限、GATT、MAC、重连策略和训练投影仍由平台宿主持有。2026-09-11 的完整复盘和实时魔方同源结论继续有效。以下表项区分源码/自动化与真机验收；早期证据日志保留其历史含义，不得由共享组件存在推断智能魔方所有型号和模式已完成。
...
| PAR-014 | 智能魔方 | GAN v2/v3/v4、MoYu32、QiYi 协议与复盘 producer 进入 shared/app-ui；Web/App 共用 `SmartCubeSessionController` 管理 MoveClock、权威状态、solved 边沿和连接 lease，宿主只保留 discovery/permission/GATT/MAC/重连及训练投影；LiveCubeState/SimCubeView 共用 | 共享 controller 覆盖 move 先推进后通知、设备时间戳、state resync 单次完成、reset 不制造完成事件和旧 lease 失效；Web 重连/首次握手/小程序桥接及 App native bridge 均接入同一控制器。Android picker 前缀过滤已有自动化，2026-09-20 起仅显示设备名，地址只用于内部去重/连接 | Android 当前真机扫描、连接和使用已由所有者确认；本轮未构建 APK、未新增型号证据，所以型号级完成仍只认 GAN v4。iOS 新版停表/姿态、多品牌、诊断/重置、自动重连和五平台权限/后台矩阵仍未完成 | `进行中` |
