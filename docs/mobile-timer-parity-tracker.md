...
| BASE-05 | 三项来源入口、手动队列、比赛/日期、比赛搜索、国旗、轮次/组别、2×2 配置、WCA 难度/合并/最优、非二阶按步数及完整出处已改为 Web/Mobile 共用 UI/纯逻辑；随机难度、来源进度/标记、错误文案及其他项目相关配置仍未关闭 | shared source/editor/WCA difficulty/by-steps/provenance 对比 `ScrambleSourceBar` / `WcaSourceConfig`，Web/Mobile 定向回归与 360px 成对实测 | `进行中` |
