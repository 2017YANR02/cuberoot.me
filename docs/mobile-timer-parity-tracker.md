...
2026-09-12 所有者明确调整计时输入：五端 App 的当前打乱不允许点击或手势复制；旧 `scrambleClickAction=copy` 在产品层按 `none` 执行，设置只保留无操作与显式换题。数字区按住及拖移只走起停，不打开轮盘；共享 `TimingSurface` 屏蔽数字和打乱正文的原生选择/上下文菜单，输入框仍可正常编辑。该决定覆盖早期三动作复制验收记录，历史记录/房间邀请的显式复制不属于当前计时面。后续所有者明确将禁止复制当前打乱扩展到网站：Web 单人、本地多人、联机同步取消点击复制及提示，单人轮盘禁用复制，旧复制偏好按无操作处理。

同日进一步明确：单人计时面上的打乱正文与时间数字使用完全相同的按压起停流程（holding → ready → 松开起表，运行中按下停表），拖移也不打开轮盘。Web/App 移除打乱点击动作设置，旧 copy/next 偏好不再影响正文；显式来源、重试和换题按钮继续独立操作。
...
| `GAP-010` / `PAR-014/015/016` 设备 | Web 有智能魔方、智能计时器、Stackmat 及各自设备流程；智能魔方状态/恢复区已改为共享弹层，Web 仅包环境检测、失败详情与 MAC 输入 | GAN v2/v3/v4、MoYu32、QiYi 协议、MoveClock/状态模型、跨端 `SmartCubeSessionController`、Solo 起表/录制/引导/停表 controller 与复盘 producer 已共享；其他 Web driver 仍需逐项核对 | 智能魔方状态/恢复弹层、实况魔方、姿态、自动预备设置已共享；统一设备中心、智能计时器和 Stackmat 未共享 | `智能魔方局部同视图` | Android 已能过滤已知魔方名称、仅显示名称并完成当前真机扫描/连接/使用；App 现可显示设备名、电量、协议、连接/还原状态，并共用重置、状态回读、陀螺仪校准、断开与关闭行为。型号级证据仍只登记 GAN v4，智能计时器/Stackmat 与其他四端 BLE 矩阵未齐 | `进行中（P0）`；继续提取完整 `TimerDeviceCenter`，补齐智能计时器、Stackmat 和权限状态，平台只注入 transport/permission |
...
| BASE-07 | session CRUD/项目关联、7 个 quick action、完整成绩行、快捷菜单/底部操作表、备注、Undo、8 类派生标签及筛选均由 `@cuberoot/shared/timer` 与 `@cuberoot/timer-ui` 单源维护。App 智能魔方成绩通过共用 recorder 落盘 `moves/device/stageSegments`。成绩对比与基础成绩详情也已迁入同一 shared model + timer-ui modal；详情覆盖原始/生效成绩、日期、4 罚时、打乱、CFOP/BLD/MBLD、备注、移组、删除、焦点/Backdrop/Escape/Android Back、窄屏布局及 QTM/QTPS、首动、停顿基础复盘指标，Web 和五端只传持久化与可选重型复盘 slot，旧 Web/App 私有详情已删除。网站当前的 NxN/Clock/Pyraminx/Skewb/SQ1/Megaminx/FTO 预览也已共用同一 dispatcher/renderer；完整动作谱/时间线/三维回放/反馈已于 2026-09-11 收敛为共享 ReconstructReport/SolveRecap；完整统计图表及真机全矩阵仍缺；网站主路径当前没有传入 `onBulkDelete`，不可达的批量删除代码不冒充既有 parity 要求 | shared history action/filter/tag/move/compare/detail/preview/reconstruction metrics 合同；基础指标边界与旧 Web identity、compact/full 单实例、中英文回归通过；App 34 files / 230 tests 与相关 typecheck 通过；Mobile/Desktop/Harmony production build、Capacitor sync、304-task Android assembleDebug 和 OPPO 覆盖安装/字节一致通过，屏幕仍在系统通知层，详情交互待解锁 | `进行中` |
...
2026-09-22 增量：Web 与 Installed App 的标准化 move/state 会话已共用 `SmartCubeSessionController`，连接 lease、MoveClock、状态推进与 solved 边沿不再各自实现；Solo 模式继续共用 `SmartCubeSoloTimerController`，统一 armed 首转起表、首转及恢复动作录制、打乱完成预备、权威状态同步和 solved 停表顺序。同日智能魔方设备名、电量、协议、连接/还原状态及重置、状态回读、陀螺仪校准、断开交互收敛到 `TimerSmartCubeDeviceModal`，Web 与 Installed App 均为真实 consumer；Web 仍保留浏览器环境检测、失败详情与 MAC 输入，平台仍分别持有发现、权限、GATT、重连策略、比赛断连和多人路由。智能计时器、Stackmat、完整权限矩阵和更多型号真机证据仍未完成。2026-09-11 的完整复盘和实时魔方同源结论继续有效。以下表项区分源码/自动化与真机验收；早期证据日志保留其历史含义，不得由共享组件存在推断智能魔方所有型号和模式已完成。
...
7. 对照方式：必须成对运行当前 Web 与当前 App，并固定相同的 CSS 视口/DPR、缩放/系统字号、语言、主题、项目、人数、来源、持久化设置、数据 fixture、计时阶段、弹层和键盘状态；每对同时保存截图、关键 DOM 边界、computed style 与点击结果。只检查 App 自身“不溢出/同一行/有颜色”只能算 App 回归证据，不能据此判断与 Web 一致；单张首页截图也不能代表菜单、加载、错误、空态、计时中或键盘态。
8. 打乱公式的等宽字体与加宽 `word-spacing` 只能作用于公式节点；比赛出处、国旗、项目、轮组、题号和状态标签必须使用共享界面字体并显式恢复正常字距。验收须读取子节点 computed style 和逐词 Range 边界，不能只看整行未溢出。
...
| 日期 | 证据 | 结论 |
| --- | --- | --- |
| 2026-09-22 | 新增 `@cuberoot/timer-ui/TimerSmartCubeDeviceModal`，Web `BluetoothModal` 收敛为浏览器环境/失败详情/MAC 输入 adapter，Installed App 接入同一设备名、电量、协议、连接/还原状态及重置、状态回读、陀螺仪校准、断开交互；共享层统一 portal、焦点陷阱/恢复、Escape/遮罩关闭、拖出保护和忙碌关闭门禁，Android Back 进入统一 overlay 优先级。`@cuberoot/timer-ui`、Client、App UI typecheck 通过；Client 2 files / 12 tests、App 3 files / 15 tests 通过，Web consumer guard 锁定不得复制回私有状态面板 | 关闭 Web/App 智能魔方状态与恢复弹层双实现；Web 特有环境建议和 MAC 流程继续留在 adapter。未构建 APK、未新增实体魔方或型号证据，智能计时器、Stackmat、完整诊断、权限矩阵及其余平台仍未完成，PAR-014 与整体继续 `ACTIVE — NOT COMPLETE` |
| 2026-09-22 | 新增 runtime-neutral `@cuberoot/shared/smart-cube/solo-timer`，Web `SoloView` 与 Installed App `App` 共用 armed 首转起表、首转及 `futureHistory` 录制、打乱完成自动预备、权威 facelets 同步、solved 停表和运行项目冻结顺序；Web BLE move 回调统一携带 move/timestamp/facelets/metadata，Solo、联机 Battle 与本地 Battle 消费者已同步。`@cuberoot/shared` build、App UI 与 Client typecheck 通过；Client 14 files / 200 passed / 7 skipped，App 8 个定向文件共 45 条相关测试通过（含断连用例隔离回归），架构边界与两端 Battle 路由均通过。App 全量智能魔方集成文件仍有并行复盘 UI 提交留下的旧 `Move stream (3)` 文案断言失败，本颗粒未修改复盘区 | 关闭 Web/App 各自编排 Solo 智能魔方起表、录制、引导和停表的重复实现；恢复历史不再触发起表/引导/广播，断连迟到动作被丢弃，运行中门禁关闭仍可停表。发现/GATT/MAC/重连、比赛断连和多人路由继续由宿主持有；本轮未构建 APK、未做新增实体魔方复测，PAR-014 与整体继续 `ACTIVE — NOT COMPLETE` |
| 2026-09-22 | 新增 runtime-neutral `@cuberoot/shared/smart-cube/session`，Web `useBluetoothCube` 与 Installed App `useInstalledSmartCube` 共用状态推进、设备时钟、solved 上升沿及 connection lease；Web 的首次握手、自动重连、小程序 bridge、校准、训练 hijack/reset 和卸载均改为消费共享会话，Bluetooth picker、GATT、MAC 与协议 driver 继续留在平台层。`@cuberoot/shared` build、App UI 与 Client typecheck 通过；App 3 files / 16 tests、Client 7 files / 76 passed / 7 skipped 定向回归通过，跳过项为本机未提供的 csTimer 参考仓 oracle | 关闭 Web/App 各维护 tracker、MoveClock、solved-edge 和迟到连接防护的重复实现；旧连接回调在新 lease、断连或卸载后失效，state 补帧与 move 走同一完成边沿。未构建新 APK、未做本轮实体魔方复测，也未迁移完整 `TimerDeviceCenter`、Web-only driver 或五平台权限/后台策略，PAR-014 与整体继续 `ACTIVE — NOT COMPLETE` |
| 2026-09-03 | 随机来源的完整 3×3-family 难度链已从 Web 私有实现收敛：生成引擎位于 `@cuberoot/puzzle-solvers/cross-trainer`，队列/状态/最优组合位于 `@cuberoot/shared/timer`，底色选择、方法/阶段/槽位/范围与题目答案行位于 `@cuberoot/timer-ui`；Web 原路径仅保留 compatibility wrapper，五端 `@cuberoot/app-ui` 只注入同一 Worker 模块和宿主 transport。题目 metadata 随不可变 history occurrence 保存，不再按打乱文本查 Map；切题/切语言的迟到答案被 identity 拒绝，智能魔方与 trainer 分离 Worker owner，empty/rare/transport error 在“难度+最优”下保持 trainer 语义并能正确重试。迁移后发现并修复了 Next/Turbopack 无法解析旧 `.js` 相对导入的 500 根因，统一走 puzzle-solvers 公开子路径。Client 定向 7 files / 53 tests、App 37 files / 270 tests、puzzle-solvers/client/timer-ui/app-ui typecheck/build、Mobile production build、Capacitor Android sync 与 304-task `assembleDebug` 全绿；三名独立 agent 最终无 P0/P1。当前 Web 360px 与 OPPO WebView 360×749 均显示同一绿色难度开关、可视底色选择器、方法/阶段/范围、真实题目与答案且文档 360/360，无 `BGORWY` 泄漏；OPPO 答案实得 `橙 U2 F L' D' L'`。8,789,365-byte Debug APK SHA-256 `86c9da0984574d29fd407c6d9443082ea441a88dfb39134b84c250a704a90c00` 已覆盖安装，设备 `base.apk` 回读一致，进程 14783 无 crash/ANR | 关闭随机 3×3-family 难度的 Web/App 双 UI、双池、文本 metadata、迟到答案和当前 Next 500 反例；本轮未登录，因此“难度+云端最优”的真实认证/断网/401 仍未冒充真机通过。320/390/412、横屏、大字、TalkBack、iOS/Harmony/Windows/macOS 实机矩阵及其他 Timer 功能继续 `ACTIVE — NOT COMPLETE` |
| 2026-09-03 | 随机难度收尾复审把 Web/App 重复的 Worker 预热、预算和批量抽取循环下沉为 `@cuberoot/puzzle-solvers/cross-trainer/batch` 单一实现；共用 RPC 在预取消时不再创建 Worker，合法 ID 的非法回包立即失败。颜色选择器在持久化失败回滚时重新服从 settings；离开计时页只暂停后台任务而不改变题源身份，返回时续跑原 loading 题；打乱文字单独承担点击语义，不再包住来源/答案按钮；计时中共用 chrome 隐藏且答案按钮禁用；颜色菜单与说明弹层为 OPPO Chrome WebView 103 补 token fallback。Puzzle 9 files / 48 tests、App 37 files / 274 tests、Client 定向 5 files / 34 tests及 puzzle/shared/timer-ui/app-ui/client typecheck 全绿；Mobile production build、Capacitor Android sync 与 304-task `assembleDebug` 通过。8,789,365-byte Debug APK SHA-256 `e435741c02ecdd55eb7b774139d0d5797e6276092ab225124971a0ea801013e9` 已覆盖安装到 OPPO `PFDM00`，设备 `base.apk` 回读一致 | 关闭重复抽取循环、回滚后 UI/行为分裂、离页后台生成/丢题、嵌套交互、计时中答案剧透和 WebView 103 无边框/阴影反例。手机停在系统通知层，故本次最终包只记安装与字节一致，不冒充解锁后的新交互证据；完整跨端矩阵继续 `ACTIVE — NOT COMPLETE` |
| 2026-09-02 | 普通随机 333 与 57 OLL + 21 PLL 专项的最优打乱已收敛到 `@cuberoot/shared/timer`：Web 只保留 URL/auth adapter，五端 App 产品层共用同一 pool、SSE 协议、等价纯 HTM 转换、loading/error/retry 与 source identity。完整 78-case fixture 锁定纯 HTM 和仅绕 y 轴等价；永久挂起的 stale base/optimizer、切账号后的迟到 401、认证恢复期间普通题泄漏、退出未清 Account iframe 等反例均已关闭。App 37 files / 270 tests、Client 4 files / 39 tests、Mobile CSS 2 tests及 shared/timer-ui/app-ui/client/mobile typecheck 全绿；production build、Capacitor Android sync、Gradle `installDebug` 通过。8,789,365-byte APK SHA-256 `6eb801cb43c3414fc27eb8b83bc8fb0907d963d1a5ff7c5ddc7675d761e9e5e8` 与 OPPO `PFDM00` 内 `base.apk` 回读一致。真实 Chrome WebView `103.0.5060.129` 发现同规则内备用 border 会被不支持的 `color-mix()` 整条置无效，现改为 `@supports` 增强；真机 computed spinner 为 2px solid、accent 顶边，来源菜单含真题/随机/手动，随机题可生成，未登录最优显示禁用提示，设置行均在 x=20～340、文档 360/360，`0.000` 完整可见 | 关闭当前普通随机 333、OLL/PLL 专项与云端最优的重复核心实现及 WebView 103 spinner/窄屏设置行源码反例。App 当前未登录，故不把已登录云请求、401 与断网恢复记作真机通过；同步种子、随机难度及其与最优组合仍缺，iOS/Harmony/Windows/macOS 设备矩阵也未关闭，整体继续 `ACTIVE — NOT COMPLETE` |
| 2026-09-02 | 专项练习的 57 OLL + 21 PLL 数据、严格逆公式 + 四 AUF 生成器、搜索/分组/OLL-PLL 切换、退出与弹层已收敛到 `@cuberoot/shared/timer` + `@cuberoot/timer-ui`；Web/五端只保留状态、持久化和宿主 adapter，旧 Web 文件缩成 compatibility re-export。三名独立 agent 分别做架构、功能反例与 UI/可访问性审查，修正了假别名、搜索范围、活动态、body scroll lock、短屏滚动和 catalog/文档遗漏。真机进一步定位 OPPO `PFDM00` 使用 Chrome WebView `103.0.5060.129`：不支持 `color-mix()`/`dvh`，且 Vite 默认压缩把 `max-width/max-height` 改成它不识别的 range syntax；根因已在唯一 Mobile Vite 宿主固定 `target/cssTarget: chrome103`，共享 CSS 保留直接色值 fallback。App UI 全集 37 files / 270 tests、Client 专项 4 files / 25 tests、shared/timer-ui/app-ui/client/mobile typecheck、Mobile production build、Android sync 与 206-task `installDebug` 全绿；最终 APK 8,789,280 bytes / SHA-256 `f961b295e86f9c4452ffcb8c471eb5e6fd00ff921648f8f2743b5e398f3796ba` 已覆盖安装。OPPO 360×749 文档 360/360，绿色 toggle、70% 遮罩、OLL 21 专项题、当前 case、退出与 Back 均实证；CDP 568×320 时 `matchMedia('(max-height: 420px)')=true`，普通及根字号 32px 两次均可滚到底，列表固定为 134px 滚动区，且“关闭”按钮完整位于 y=243～287px | 关闭专项练习的 Web/App 双实现和当前 Android 短屏媒体查询失效根因；More 已真实接通 11/12，仍缺 replay。Web/App 当前可见题源仍显示“真题”是两端既有同一结构，专项活动态由 More 入口标记；未改用户成绩。随机 3×3 完整难度/最优管线、iOS/Harmony/Windows/macOS 实机矩阵仍未关闭，整体继续 `ACTIVE — NOT COMPLETE` |
| 2026-09-02 | 当前提交的 APK 覆盖安装并冷启动后，在 OPPO `PFDM00` 的真实 App WebView 中切到手动来源：输入两行得到 `MANUAL ONE → MANUAL TWO → MANUAL ONE` 精确顺序与循环；清空后显示 shared 的“在上方「打乱来源」粘贴打乱,每行一条”，计时 Surface 仍保持可交互 `role=button`，文档始终 `scrollWidth=clientWidth=360`。测试后恢复原手动文本 `RACE A` 和真题来源，未新增或修改成绩 | 关闭当前 Android 设备上的手动队列编辑、顺序、循环、空提示、可起表语义与窄屏横向溢出反例；本轮通过设备 WebView 调试协议触发 DOM/键盘事件，不冒充手指触摸、进程重启持久化、横屏/大字/TalkBack 或其余四平台证据，整体继续 `ACTIVE — NOT COMPLETE` |
| 2026-09-02 | 打乱的 17 类 loading/empty/error/unsupported 原因、中英文文案与重试策略收敛到 `@cuberoot/shared/timer/scramble-status`，手动空队列提示也收回 shared；Web 与五端 App 共用 `TimerScrambleStrip` 的 spinner、alert/live region 和 44px 重试按钮。WCA 难度多 bin 任一请求失败时三路均保持 transient。Web 所有异步生成按启动时的 history entry ID 精确回填，切题后旧结果不再覆盖当前题；optimal+trainer 只由 optimal pool 单独消费。App 重试先原子进入 loading、清 failure，并拒绝旧错误按钮二次启动。App 全量 36 files / 265 tests、Client 定向 11 files / 70 tests及来源补充回归通过；shared/timer-ui/app-ui/client/mobile typecheck、Mobile production build、Capacitor Android sync 与 206-task `installDebug` 通过。最终 APK SHA-256 `45c8bf9f21f47cb0740811e786dd2e8db53cf62d04a582be971f685c9e4e663e` 与 OPPO `PFDM00` 设备回读一致，冷启动成功、进程 25304 且检查窗口内无 crash/ANR；同一 UI 变更前的真实前台 360dp ready 截图中 `0.000` 完整、真题控件/绿色开关/出处国旗/打乱/预览/三栏均可见，文档无明显横向溢出。三名独立 agent 终审均 GO、P0/P1=0 | 关闭状态文案与手动提示双事实源、不可恢复错误、partial-bin 误判空集、异步回填错题、optimal/trainer 竞争、App 重复重试和失败态残留。最新重建只移动同文案事实源，未重新取视觉截图；完整 Client 全集本轮在并行重型测试资源压力下出现未改模块的既有/不稳定失败，未冒充全绿；真实断网→重试→恢复、320/340、横屏、大字、TalkBack、iOS/Harmony/Windows/macOS 成对矩阵仍未完成，整体继续 `ACTIVE — NOT COMPLETE` |
| 2026-09-02 | 稀有 WCA 题池进度与公开打卡已收敛为 shared `TimerWcaFinitePoolProgressTracker`、严格 slot/marks codec、marks HTTP client 和 `@cuberoot/timer-ui/TimerWcaScrambleProgress`；Web/App 只保留题池、认证、持久化和导航 adapter。日期/比赛/难度/precomputed/cache 全部保留官方 competition/event/round/group/extra/number occurrence；同文本不同官方题号分别计数，任一外部脏行只跳过自身，selected-comp 网络 `null` 保持 transient 并允许重试，只有权威 `[]` 才记 empty。自动打卡只在 durable `repository.addSolve` 成功后执行；首次 owner 随 pending 保存贯穿 retry，登出、换号、认证 busy、DNF/DNS、未登录均 fail closed，同 owner token 刷新使用实时 token。关闭自动打卡时共用认证 `PATCH /v1/scramble-marks` 只更新本人已有记录、绝不新增；POST/PATCH 空 country 不抹旧国旗；API 与客户端共用 strict key decoder，非法 `x` 等直接 400。弹层使用 body portal、visualViewport 四边钳制和实测三栏高度，顶部外/可见区/导航区三种锚点均断言不遮挡；synthetic owner 不生成伪 WCA 链接。API 2 files / 12 tests、App 36 files / 263 tests、Client 7 files / 58 tests、server/shared/timer-ui/app-ui/client/mobile typecheck、Mobile production build/Android sync 与 OPPO 206-task `installDebug` 全绿；六轮以上独立对抗审查最终 P0=0、P1=0。此前 OPPO 360×749 证据仍为文档 360/360、`0.000` 完整、出处链接 44px；当前零足迹题不画假按钮，未向生产写测试打卡 | 关闭来源进度/全练过/公开打卡双实现、文本吞 occurrence、脏 slot 中断、网络暂态永久空、公开 top-100 推断所有权、旧账号/新账号竞态、国家丢失、非法自然键和底栏遮挡。当前非零足迹弹层尚无真机视觉样本；iOS/Harmony/Windows/macOS、逐类 loading/empty/error、随机难度与全配置矩阵仍未关闭，整体继续 `ACTIVE — NOT COMPLETE` |
| 2026-09-02 | WCA 出处的国旗、本地化比赛名、项目图标、轮/组/题号/加赛与比赛深链收敛为 `@cuberoot/timer-ui/TimerWcaScrambleSource`，Web/App 删除各自 DOM/CSS，可见双语文案收敛为 shared copy。首轮独立审查发现 Web 全局国旗样式覆盖、来源行空白冒泡、触控目标仅 15.6px 和图标读出原始代码；根因已在共享组件/CSS 修复。同一 360×749 视口成对实测中，两端关键 class 及 computed style 一致：12px/15.6px、`word-spacing: 0px`、`overflow-wrap: anywhere`、来源链接 44px 高、国旗约 16×12px 且无额外 margin，均为 `scrollWidth=clientWidth=360`；明确 aria-label 使用本地化项目名，国旗/图标作为装饰不再读出国家代码。OPPO 真机显示一枚国旗、一枚项目图标和 `TCG Spring Open 2015 R2,C,5`；实点来源进入 Tools 的 `https://cuberoot.me/zh/scramble/gen?comp=TCGSpringOpen2015`，点行空白或返回后打乱原文完全不变且无复制反馈。App 34 files / 233 tests、Client shared strip 7/7、守卫 41/41、四包 typecheck、Mobile production build/Android sync 与 206-task `installDebug` 通过 | 关闭了出处行重复实现、App 原始比赛名、加赛/深链缺失、点来源误复制/换题、国旗跨端尺寸/间距偏差、可访问性与非最优标志漏传。来源罕见池进度/打卡人数、逐类加载/空/错误文案、iOS/Harmony/Windows/macOS 设备矩阵仍未关闭，整体继续 `ACTIVE — NOT COMPLETE` |
| 2026-09-02 | OPPO 当前比赛名 `Hangzhou Autumn 2019` 修前继承打乱公式的 `word-spacing: 6.5px`，两个可见词间距均约 13px；根因已在共享 `@cuberoot/timer-ui/scramble-strip.css` 修复，只让 `.scramble-moves` 保留 `0.25em`，Web/App 出处共同使用 `timer-scramble-source-meta` 的界面字体与正常字距。重装后比赛名 computed `word-spacing: 0px`，两处逐词 Range 间距均为 2.698px；canonical 中国国旗仍在，文档 360/360，`0.000` 完整位于 Surface。对抗审查另发现 App 的最优开关曾在非真题来源误可用且设置行未复用 44px 结构；现 Web/App 共用 `TimerBooleanSettingRow`，OPPO 真题下 44×44、开启可用，切手动后同一偏好显示关闭且 disabled，切回真题恢复开启。App 34 files / 233 tests、Client 2 files / 16 tests、三包 typecheck、Mobile production build/Android sync 与 206-task `installDebug` 通过 | 关闭比赛名异常大空格、当前真题国旗缺失、最优设置行双实现、非真题误可操作和 App 触控目标过小五项当前反例；未重置用户偏好。只覆盖 OPPO 360×749 简中 light 当前真题/手动切换，不代表整个 Timer parity 完成 |
| 2026-09-02 | 首次按相同 360×749、简中、light、333、WCA 真题、难度开启成对运行当前 Web/App，发现下方“三项同行”只有 App 单端证据，不能代表网站：当前 Web 用既有 `toggleSlot` 把“难度”放在顶栏，用 `topControlsSlot` 把“合并”放在真题来源行，“最优打乱”位于设置页。App 已删除专属三项同行结构，直接复用同一组 `TimerWcaSourceConfig.trailingControls` 与 `TimerWcaDifficultyConfig` 两个 portal 插槽；Web/App 的最优设置行也已收敛为同一个 `TimerBooleanSettingRow` 与 setting contract，App 仅在支持最优的 WCA 真题来源允许操作。Web 两个开启态开关均为 `rgb(52, 199, 89)`；App “合并”原持久化值为关闭，临时开启后同为 `rgb(52, 199, 89)`，随后恢复关闭。App 34 files / 233 tests、App/Mobile typecheck、Mobile production build、Capacitor Android sync 与 OPPO `PFDM00` `installDebug` 通过；真机 WebView 360/360 无横向溢出，难度位于顶栏、合并位于来源行，设置页存在可操作“最优打乱” | 本行取代下方“三项同行即一致”的旧结论，并把成对 Web/App 取证写为硬门槛。开关绿色没有丢失；用户当时看到无绿色是“合并=关闭”的真实状态，未擅自重置设置。当前只关闭这三个控件的结构、颜色与可达性偏差，不代表整个计时器完成；其余状态/视口/平台继续 `ACTIVE — NOT COMPLETE` |
| 2026-09-02 | 历史证据：App 曾复用 `TimerWcaDifficultyConfig.topControlsSlot`，把共享的“合并/难度”控件 portal 到“最优打乱”所在行。App 34 files / 233 tests、App/Mobile typecheck、Mobile production build、Capacitor Android sync 与 OPPO `PFDM00` `installDebug` 通过。真机 360×749 下三组边界均为 y=106～128px；CDP 模拟 320px 时仍同为 y=106～128px，互不相交，文档宽 320/320 | 这只能证明旧 App 自身同行且无溢出，因当时没有与网站窄屏成对运行，不能作为 parity 证据；实现和结论均已由上方记录纠正 |
| 2026-09-02 | WCA 难度底色选择器继续由 Web 与五端共用 `TimerWcaDifficultyConfig`；可见文案改为本地化颜色名，`BGORWY`/`WY` 等数据库键只保留在 option value。Client 交互测试 3/3、timer-ui/app-ui/mobile typecheck、App 34 files / 233 tests、Mobile 6 files / 18 tests、production build、Capacitor Android sync 与 OPPO `PFDM00` `installDebug` 通过。真机当前值显示“六色”，展开列表显示“单色 · 白”“四色 · 蓝/绿/橙/红”等且无内部编码，页面与弹层未横向溢出 | 关闭内部底色编码泄漏到用户界面的回归，并在共享组件一次修复 Web/Android/iOS/HarmonyOS/Windows/macOS，未复制宿主补丁。该证据只覆盖当前中文竖屏难度选择器，不代表全部 Timer parity；英文、横屏、大字号、TalkBack 与其他平台仍待矩阵验收，整体继续 `ACTIVE — NOT COMPLETE` |
| 2026-09-02 | OPPO WCA 难度页实测发现计时 Surface 被 flex 压缩：修前 `0.000` 顶边 264.8px、Surface 顶边 287.7px，约 23px 字形被父级裁切，核心内容底边也越过 Surface 进入统计区。五端共用 `app-ui` CSS 改为 `min-height: min-content`，viewport 守卫 7/7、app-ui typecheck、Mobile production build、Capacitor sync、Gradle `installDebug` 通过；重装后数字 299.7～386.1px 完整位于 Surface 287.7～682.3px 内，核心底边 670.3px 小于统计顶边 682.3px，文档宽 360/360 | 关闭当前中文竖屏 idle 状态的数字裁切和预览侵入统计区；修复落在五端共享产品 CSS，没有 Android 私有尺寸。计时运行/检查/停止、英文、横屏、IME、大字号、TalkBack 与其余平台仍待验，PAR-007 和整体继续 `ACTIVE — NOT COMPLETE` |
| 2026-09-01 | “打乱图”与“3D 魔方”的 schema/default/normalizer/disabled rule 进入 `@cuberoot/shared/timer`，设置行进入 `@cuberoot/timer-ui/TimerScramblePreviewSettings`；Web 与五端 App 共用同一 UI、迁移和持久化，App effect ledger 从 11 增至 13。单人和联网均以 `TimingSurface.cornerSlot` 渲染唯一 `TimerCubePreview`，并用 `data-no-timer` 与可点击打乱条/计时输入隔离；SQ1/Megaminx 保持 shared 2D renderer。三 agent 对抗审查先发现拖动误触、联网未消费设置、布尔行与 disabled 规则重复，又构造出从预览拖到计时面松手会结束另一 pointer 的反例；根因现以共享 target guard 和 per-pointer capture ownership 收敛，三名最终 GO。App 全集 34 files / 232 tests、Client 选定 13 files / 91 tests、四包 typecheck、skill validator、Mobile/Harmony production build、Android/iOS Capacitor sync、Android `assembleDebug`、Xcode iOS Simulator build、Harmony 33-task unsigned HAP 与 macOS Tauri native bundle 均通过。最终 APK 8,789,280 bytes，SHA-256 `8d086683393d2cc203ac6aa7c0d764eb73fb5662cd78da4983e31e465a52fc00`，已覆盖安装并从 OPPO 回读一致；Harmony HAP 9,524,322 bytes / `7dc1abfb2bd2fd0c2e37a14f53a2f2e76d70313250d42c58c219c77eccc574a6`，macOS x64 DMG 6,446,983 bytes / `e24c0d4ab815e3c6671f0d41afd1deb89f4ff3862b14be8f83d6db46d38234c7` 且 `hdiutil verify` 有效 | 关闭 Web/App 的预览显示与 2D/3D 设置双事实源、App 联网漏接设置及 3D 拖动误触打乱动作的源码缺口。OPPO 仍为 `Asleep`/`NotificationShade`，进程 23966 的锁屏启动仍记录到 safe-area 注入和 Capacitor `triggerEvent` 初始化错误；exit-info 无 crash/ANR，但在解锁重跑前不把设置可见、切换、拖动、无遮挡或启动完全正常记为通过。iOS 仅模拟器编译，Harmony 未签名且无设备，macOS 未实点，Windows 未在原生电脑构建；PAR-008/PAR-012 与整体继续 `ACTIVE — NOT COMPLETE` |
| 2026-09-01 | 历史 rolling 列、strict running PB、MBLD 排除规则进入 shared；Web 与五端改为消费同一列头、picker、日期分组和逐行值/PB。FMC 继续按步数格式化，筛选后的列表只改变可见行与日计数。共享 CSS 在 ≤480px 将两列折到第二行并把成绩行触控高度补到 44px，PB tag 按 canonical 顺序优先可见；旧 App 每行日期和 `findIndex` O(n²) 查找、Web 私有 rolling 循环与固定列宽 CSS 已删除。Client 5 files / 60 tests、App 全集 34 files / 232 tests、四包 typecheck、Mobile/Next/Harmony production build、Android/iOS Capacitor sync、Android 304-task `assembleDebug`、Xcode iOS Simulator build、Harmony 33-task unsigned HAP 与 macOS Tauri native bundle 全通过。三名独立 agent 最终 GO、无 P0/P1；系统 Chrome 320/360px 的 normal/compare、四位序号、长 DNF、双 rolling+PB、多 tag 与 200% 字号均无横向溢出，picker 全控件 44px，AX 树语义完整。最终 APK 8,789,280 bytes、SHA-256 `a062067af322d8d71a2386896c44d8093c89c94a8603b00baf347d72e7a9d123`，已覆盖安装到 OPPO `PFDM00` 且设备内 `base.apk` 回读一致，进程 24998 且启动日志无 crash/ANR；Harmony HAP 9,529,610 bytes / `6135dd030d3373959288aa5e6159cf27886e46be12dac9d04907b8265f7e3a4e`；macOS x64 DMG 6,448,071 bytes / `5b453673e2f49e4d905eb5f7d2c49f32b44f452e0d3964441c7e923a0dd0bb20` 且 `hdiutil verify` 有效 | 关闭 `history.columns` 的 Web/App 双实现和源码缺口。OPPO 仍为 `Asleep`/`NotificationShade`，所以设备解锁后的真实触摸、TalkBack、横竖屏和 200% 字号仍待验，不能以等宽 Chrome/自动化冒充真机通过；Harmony 未签名且无设备，macOS 未实点，Windows 仍需原生电脑，整体继续 `ACTIVE — NOT COMPLETE` |
| 2026-09-01 | `session.switcher` 复审先发现 Web 首次空库每读一次会生成不同 ID、portal 快捷键穿透、App 删除非当前分组会因 `autoEventForSession` 意外切项目，以及 create+activate/文案/default name/联动策略重复。根因现收进 shared 的 create+activate、event/session selection 与默认名称，timer-ui 统一标签、44px、Tab trap 和 busy/rename/delete focus；Web 改为稳定单次 snapshot 与受控 overlay，App 删除私有标签文件。Client 9 files / 74 tests、App 全集 34 files / 233 tests、四包 typecheck、Mobile/Next/Harmony production build、Android/iOS Capacitor sync、Android 304-task APK、Xcode iOS Simulator、Harmony 33-task unsigned HAP 与 macOS Tauri native bundle 全通过。最终 APK 8,789,280 bytes / `c07470eddb3507ba956fb9e3ae7be235473880ea38a9859c9d71939190dc0cfd` 已覆盖安装到 OPPO `PFDM00` 并从设备回读一致，进程 27880 无 crash/ANR。OPPO 真实触摸验证：入口/弹层无溢出，IME 打开时输入/确认可见；第一次 Back 只收键盘且保留草稿，第二次关弹层且留在 History；临时分组新建、切换、重命名后焦点回正确行；删除 Back 取消不变、确认后回退默认并恢复原 7 条；默认分组清空只验证文案并取消。临时分组最终删除。Harmony HAP 9,530,319 bytes / `4cda90930301bdc351e04e009ecfbe0aa1607599d01dfd98decf4b1303f55a78`；macOS x64 DMG 6,448,538 bytes / `f2b38037ad6e555f8487c5849cf6f682a5b5954885535be949c45e3b608d4bd8`，`hdiutil verify` 有效 | 关闭分组 UI/CRUD 已存在却被文档误报缺失、Web/App 重复业务规则、首次 ID 不稳定、键盘泄漏、inactive delete 误切项目和焦点丢失。TalkBack、横屏/大字、iOS/Harmony/macOS/Windows 实体设备仍未关闭；Harmony 未签名，Windows 当前代码仍需 Windows 原生构建，整体继续 `ACTIVE — NOT COMPLETE` |
| 2026-09-01 | Web 私有 `CubePreview`/`CubingPreview`、SQ1 与 Megaminx SVG renderer 已迁为 `@cuberoot/timer-ui/TimerCubePreview`/`TimerScramblePreview` 与 `@cuberoot/puzzle-render-core` 单源；旧路径仅一行 compatibility re-export，App 私有 `ScrambleCube` 和 Mobile/App 多余 `visualcube` 依赖删除。共享 dispatcher 覆盖网站当前 NxN、Clock、Pyraminx、Skewb、SQ1、Megaminx、FTO、relay/custom 语义；手动坏题隐藏旧图并可在下一条合法题恢复，App 的 152×114/120×90/矮横屏隐藏继续由宿主外层控制。共享预览 5/5、详情 3 files / 8 tests、App 34 files / 230 tests、相关 package typecheck、Mobile/Desktop/Harmony production build、Capacitor Android sync 与 304-task `assembleDebug` 全通过；独立对抗审查最终 GO。极端 MBLD 在系统 Chromium 320/360px 实测 document/modal/title/每个数值格均 `scrollWidth=clientWidth`。8,789,280-byte Debug APK SHA-256 `b10ac740a2de615131ec3a5700443d68d4dee7a814d94fcf45340e90cf903c61` 已覆盖安装到 OPPO `PFDM00`，设备内 `base.apk` 回读一致，进程 15442 且无 crash/ANR | 关闭 Web/App 两套预览 dispatcher/renderer、非 NxN App 无图、不可解析题残留上一题、横屏隐藏被 inline style 覆盖，以及极端 MBLD 标题/数值裁切。手机仍是 `NotificationShade`，不把详情点击、项目逐项预览或 GAN 实拧冒充通过；完整复盘、预览设置/交互、TalkBack/大字/多视口和其他平台设备矩阵仍缺，整体继续 `ACTIVE — NOT COMPLETE` |
| 2026-09-01 | Web 私有 QTM/QTPS、首动延迟、最长停顿/次数算法迁入 runtime-neutral `@cuberoot/shared/timer/reconstruct/solve-metrics`，旧 `slice.ts` 只做 identity re-export 并保留尚无 App consumer 的 Web-only memo heuristic；三张指标卡进入 `@cuberoot/timer-ui/TimerReconstructMetrics`。Web 完整报告复用既有一次计算，五端 compact 详情仅在有 moves 时计算一次，full/report 分支不重复渲染。R/R、BLD execution window、严格 500/501ms、0ms、旧/新 identity、compact/full 单实例和中英文共 3 files / 6 tests 通过；架构/复用/catalog 等 5 files / 41 tests、App 34 files / 230 tests、shared/timer-ui/app-ui/client typecheck、Mobile/Desktop/Harmony production build、Capacitor sync 与 304-task `assembleDebug` 全通过。8,789,280-byte Debug APK SHA-256 `1b465438efc0c885020641c11fdd0aa7e851a06c16417f73cc43dd08f37526fe` 已覆盖安装到 OPPO `PFDM00`，设备内 `base.apk` 回读一致，进程 19717 且无 crash/ANR。最新 iOS Simulator app 已由完整 Xcode build、安装并在 iPhone 17 启动；Harmony 34-task `assembleHap` 产出 9,522,434-byte unsigned HAP（SHA-256 `5750fc207b540085267616aacec7caa41ba06087406c9854a35beb30ae37738b`）；macOS Tauri 产出并启动 `.app`，6,446,677-byte x64 DMG（SHA-256 `2c72ab75d82621cab4ebd4a9b1cd506110536a9180f02ca7f871c73445f34d32`）通过 `hdiutil verify` | 关闭基础复盘指标的 Web/App 两套算法/DOM 与 Web 同次 render 重算；完整动作谱、方法/阶段质量、时间线、回放、反馈、TalkBack/大字/多视口和其他平台设备矩阵仍缺。Android 手机仍是 `NotificationShade`；iOS 只验到最新计时页启动，Harmony 无可用设备，macOS 未点开有 moves 的详情，Windows 本批仍需在 Windows 原生构建。均不把未见的新指标视觉/点击或 GAN 实拧冒充通过；整体继续 `ACTIVE — NOT COMPLETE` |
| 2026-09-01 | 基础成绩详情的分段/BLD 派生进入 `@cuberoot/shared/timer/history`，唯一 compact/full modal 与 CSS 进入 `@cuberoot/timer-ui`；Web 删除私有详情/分段 DOM，仅以 dynamic slot 注入原重型复盘，五端 App 删除行内私有详情并消费同一 modal。字段级 repository patch 防止罚时/备注并发覆盖；session+event+solveId render gate、旧 mutation identity gate、portal、Tab/Shift+Tab 圈定、关闭恢复焦点和 Android Back 先 blur 保存均有回归。Client 3 files / 8 tests、App 3 files / 43 tests、四包 typecheck、Mobile production build、Capacitor Android sync 与 304-task `assembleDebug` 通过；两名独立 agent 最终 GO。Debug APK 为 8,788,904 bytes，SHA-256 `34996ebbcac28b655b4ad163793e522dc29ad1af7c3cd79ad2a35f9270c32c03`，已覆盖安装到 OPPO `PFDM00`，设备内 `base.apk` hash 一致；进程启动且未见 crash/ANR | 关闭 Web/App 两套基础详情 DOM、脏/partial 分段负值、字段丢更新、旧异步结果误关新详情和键盘焦点逃逸。手机前台仍是 `NotificationShade`，因此只记构建、安装、字节与进程，不记详情视觉/点击；该时点的非 NxN App 预览缺口已由上方共享预览证据关闭，完整复盘、TalkBack/大字/多视口及其他平台设备矩阵仍缺，整体继续 `ACTIVE — NOT COMPLETE` |
...
| 2026-08-30 | Mobile Vite + 系统 Chrome 对 WCA difficulty 做 340×568 / 320×568 smoke：首次发现横向 fieldset 把 Optimal/Merge/Difficulty 压成重叠三列，当时改为全宽纵向 section；Web/Mobile 共用搜索框不再为空值预留 44px 清除位。中英两档复跑均为 `root/body scrollWidth === innerWidth`、difficulty viewport overflow=0、5 个 select/range 可聚焦、键盘开关与 range `ArrowRight` 6→7 | 这是历史防重叠证据，不是最终布局要求；其纵向拆行已由 2026-09-02 的共享 portal 同行方案取代。原 smoke 中 `Search competition` 在 320px 可用 148px、中文 195px，最长英/中 loading 与 44px 刷新按钮矩形不相交；仍不替代 OPPO、IME、横屏、150% 字号、长比赛名/错误文案全矩阵 |
