...
| BASE-07 | session CRUD/项目关联、7 个 quick action、完整成绩行、快捷菜单/底部操作表、备注、Undo、8 类派生标签及筛选均由 `@cuberoot/shared/timer` 与 `@cuberoot/timer-ui` 单源维护。App 智能魔方成绩通过共用 recorder 落盘 `moves/device/stageSegments`。成绩对比与基础成绩详情也已迁入同一 shared model + timer-ui modal；详情覆盖原始/生效成绩、日期、4 罚时、打乱、CFOP/BLD/MBLD、备注、移组、删除、焦点/Backdrop/Escape/Android Back 与窄屏布局，Web 和五端只传持久化、预览与可选重型复盘 slot，旧 Web/App 私有详情已删除。非 NxN App 预览、完整复盘 UI/统计图表仍缺；网站主路径当前没有传入 `onBulkDelete`，不可达的批量删除代码不冒充既有 parity 要求 | shared history action/filter/tag/move/compare/detail 合同；Web/App 共用 History/compare/detail DOM，详情 3 files / 8 tests、App 3 files / 43 tests 与四包 typecheck 通过；Mobile build、Capacitor sync、304-task Android assembleDebug 和 OPPO 覆盖安装/字节一致通过，屏幕仍在系统通知层，详情交互待解锁 | `进行中` |
...
| PAR-009 | 左下统计与成绩历史 | shared timer schema/stats；紧凑统计、成绩行、快捷菜单/底部操作表、注释编辑、Undo toast、筛选、成绩对比和基础成绩详情已由 Web 与五端共用；bulk 当前不可达，非 NxN 预览/完整复盘/完整统计继续提取共用 | 紧凑 current/best/rolling、Mobile 持久化、行 DOM/七项动作/焦点/视口/Undo/tag/filter/compare/detail 单源回归通过；详情的异常 partial/倒序/超总时长/NaN 分段和 BLD memo 均 fail closed，App 智能魔方新成绩也已共用 move/stage producer | 本轮新 APK 与解锁后的详情、GAN、标签、TalkBack、多视口真机证据待补；iOS 和其余三端全状态矩阵仍待验 | `进行中` |
