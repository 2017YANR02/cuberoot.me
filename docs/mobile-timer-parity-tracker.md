...
| PAR-012 | 更多菜单和设置 | 菜单模型与共用设置控件单源；宿主只注入真实平台动作，未绑定项不得显示 | More 12 项与 Settings 8 类/64 字段 exact registry/条件/effect、Web 真实 consumer、App implemented/missing ledger；App 已接 More 10 项、“计时”8 字段及“点击打乱动作”共用 UI/effect，专项/replay 和其余 55 个设置字段仍待迁移 | OPPO 已显示 8/8 计时 setting ID、320px 内容边界和滚动底部；无操作/下一条/复制三动作均已实证且 360px 无横向溢出。其余设置逐字段效果、iOS 与其他三端待验 | `进行中` |
...
| 日期 | 证据 | 结论 |
| --- | --- | --- |
| 2026-09-01 | 智能魔方逐步提示、匹配判定、偏离修正 requester 进入 `@cuberoot/shared/smart-cube`，Kociemba `inverseCubie` 收敛到 `@cuberoot/puzzle-solvers`；网站只留兼容导出，五端产品层只加一个薄 Worker adapter，并与 Web 共用 `TimerScrambleStrip`。Solo、本地、联机分别消费 shared 的 3×3-shaped（排除 FMC）、仅 `333`、`333/333oh` capability；同 target pending 请求 coalesce，新 target 会在旧 Worker 后续跑；协议错会断开且拒绝晚帧；只有真实 move 到 target 才预备，连接/切模式时 state 相等不会误预备。Client 全集 572 files / 6,657 passed / 127 skipped，App 全集 34 files / 222 passed，Mobile 6 files / 18 passed；最后一行 requester 调整另有 Client 定向 5 files / 88 passed / 10 skipped；shared/app-ui/client/mobile typecheck、Desktop Vite、Harmony unsigned HAP、Android 304-task assembleDebug、Xcode Simulator build 均通过。360×749 production 预览实测本地多人 BLE 按钮 `position=static`、44px 高、边界 12～111px、文档宽 360/360，底部 544px 未压 688px 导航；联网同一真实选择器由 CSS contract 锁定，未创建生产房间。Debug APK（applicationId `me.cuberoot.app.debug`）为 8,788,904 bytes，SHA-256 `eac89c7054362915f6da93a5c8fcd5ef2054d54103bce9267ef37b81d2309b14`，已覆盖安装到 OPPO `PFDM00` 且与设备内 `base.apk` 字节一致 | 关闭提示算法、修正 requester、模式门禁与 Kociemba 运算的重复实现，并关闭非兼容项目仍被 GAN 自动起停、协议错后晚帧、state effect 误预备、pending 切题/重连丢请求及多人 BLE 按钮绝对定位遮挡的代码缺口。Web/App 两份 React lifecycle coordinator 仍是明确债务；当前只证明最新 APK 构建与安装，手机锁屏期间不能把启动画面或实体魔方操作记成通过。新版 OPPO + GAN 16 UI 逐步提示/故意偏离/修正完成、首个 Worker 冷启动延迟，及 iOS/Harmony/Windows/macOS 真机 BLE 仍待验，整体继续 `ACTIVE — NOT COMPLETE` |
