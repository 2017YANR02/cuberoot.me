...
| `server.cubeopt-runtime` / local heavy artifact | 经审阅的 CubeOpt opt5 构建与 h5 table → `CUBEOPT_ARTIFACT_DIR/current.json` 指向 `bundles/<bundle>/{manifest.json,cube48opt5.mjs,cube48opt5.wasm,h48prun31h5.dat}`；manifest 锁 schema、bundle、variant、protocol、source、bytes 与 SHA-256 | cwd `core`：`pnpm --filter @cuberoot/server cubeopt:prepare -- --store <store> <args>`；`cubeopt:promote -- --store <store> --bundle <id>`；`cubeopt:verify -- <store>`；发布后 `cubeopt:smoke -- --store <store>` | `@cuberoot/server` + `packages/server/CUBEOPT_ARTIFACT.md` | 大表和 bundle 不入 Git/普通 CI；prepare 先在同文件系统 staging 校验再原子 rename 为不可变 bundle，promote 校验后原子替换 current 指针；首次启用新 store 时，部署期 `provision.mjs` 可从旧环境指向的真实字节幂等迁移并在完整校验后原子登记，Server runtime 不保留 legacy fallback；普通测试只验小 fixture 与无 Web 路径回退，真实 daemon smoke 属制品/发布验收；不得独立覆盖 module、WASM 或 table |
