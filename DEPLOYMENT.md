...
| **Deploy Trainer** | push main 且 `trainer/` 有变更 / 手动触发 | pnpm build client → commit dist 到根目录 `_assets/` | ~1 分钟 |
...
ruiminyan.github.io/
│
│── 🤖 React SPA 根目录（由 deploy_trainer.yml CI 自动生成，勿手动修改）
...
│   └── ...                    # 其他工具
│
├── trainer/                    # 📌 CubeRoot Trainer monorepo 源码（React 19 + Vite 8 + Zustand）
...
│   └── card.css               # 落地页入口卡片样式
├── assets/js/                 # Stats 前端交互逻辑（由 trainer/packages/stats-ui/ TypeScript 编译生成）
...
| API 入口 | `https://www.cuberoot.me/trainer/api/recon/...`（Nginx 反代到 Hono）<br>`https://www.cuberoot.me/legacy/recon/api/?action=...`（PHP legacy） |
...
- **Tab 双视图**：19 个统计支持「排名」和「历史」双 Tab 切换
  ...
  - `assets/js/stats_ui.js`：`initStatsUI()` 扫描 data-label-* 自动生成按钮（由 `trainer/packages/stats-ui/src/stats_ui.ts` 编译产生）
...
```
Hono API 后端（www.cuberoot.me/trainer/api/recon/）
...
基址：`https://www.cuberoot.me/trainer/api/recon/`（Hono RESTful）
