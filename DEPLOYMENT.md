...
| 页面 | 国家数据来源 | 代码位置 |
|------|-------------|----------|
| stats（Ruby 生成页面） | WCA `country_id`（已是 ISO2） | Ruby 模板直接输出 `<span class="fi fi-xx">` |
| upcoming_comp | WCA API `country_iso2` | `upcoming_comp/index.md` JS 中 `fi fi-${comp.country.toLowerCase()}` |
| recon（复盘页面） | CSV 国家全名 → Python 映射为 ISO2 | `convert_recon_csv.py` 的 `COUNTRY_ISO2` 映射表 → JSON `countryIso2` 字段 → `recon.js` 的 `countryFlag()` |

## 新增国家

如果 recon 数据中出现了新国家，需要在 `scripts/convert_recon_csv.py` 的 `COUNTRY_ISO2` 字典中添加映射。
...
| `country` | `region_solver` | 选手国家（英文全名） |
| `countryIso2` | （Python 映射） | ISO2 代码（用于国旗渲染） |
| `scramble` | `scr*` | 最少步打乱 |
