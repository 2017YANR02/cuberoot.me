...
| 数据存储 | JSON 文件（`data/recons.json`、`data/counter.json`） |
| 部署方式 | push main → CI rsync（`data/` 目录排除，不会被覆盖） |
| 性能优化 | gzip 压缩（2.3MB → ~300KB） |
| ID 策略 | 自增数字 ID（`counter.json` 跟踪 nextId） |

> **数据目录不受部署影响**：`recon/api/data/` 在 rsync `--exclude` 列表中，CI 部署不会触碰服务器上的数据文件。
...
PHP 后端（阿里云 toolkit.cuberoot.me/recon/api/）
    │  data/recons.json（全量复盘数据，永久数字 ID）
    │  data/counter.json（自增计数器，跟踪 nextId）
    ↓
...
| Action | 方法 | 参数 | 说明 |
|--------|------|------|------|
| `list` | GET | `wcaId`（可选） | 返回全部复盘（或指定用户的），按 ID 降序 |
| `add` | POST | JSON body（复盘字段） | 添加复盘，自动分配永久数字 ID |
| `delete` | POST | `id` | 删除指定 ID 的复盘 |
| `update` | POST | `id` + JSON body | 更新指定 ID 的复盘字段 |
| `import` | POST | JSON body `{solves: [...]}` | 批量导入（保留原始 ID，更新计数器） |
| `addEdit` | POST | JSON body | 添加编辑覆盖层 |
| `deleteEdit` | POST | `id` | 删除编辑覆盖层 |
| `loadEdits` | GET | 无 | 返回所有编辑覆盖 |

> **性能**：响应已启用 gzip 压缩（~2.3MB → ~300KB）。写操作有速率限制（10 次/分钟）。
