...
| 现象 | 原因 | 修复 |
|------|------|------|
| 服务启动后立即停止 | `Uploads` 目录被删 | `New-Item -ItemType Directory -Path "C:\ProgramData\MySQL\MySQL Server 8.0\Uploads"` |
| `.ibd` 文件找不到 | WCA 导入的附属表缺失 | 加参数 `--innodb-force-recovery=1` 启动，进去后 `DROP TABLE` 清理 |
| 日志里看不到错误 | 服务启动失败的错误不写 `.err` | 必须用 `--console` 参数手动启动才能看到 |

> **关闭 MySQL**：务必用 `net stop MySQL80` 或 `mysqladmin shutdown` 干净关闭，**绝对不要强杀** `mysqld.exe`，否则会损坏 InnoDB。
