...
- **国内镜像**：[www.cuberoot.me](https://www.cuberoot.me)（自动同步，CI 配置：`.github/workflows/deploy_mirror.yml`）
...
GitHub Pages（Jekyll）+ React SPA
  ↓
上线：ruiminyan.github.io/stats/ + /wca-stats/
```
...
| **Deploy Core** | push main 且 `core/` 有变更 / 手动触发 | pnpm build client → commit dist 到根目录 `_assets/` | ~1 分钟 |
...
ruiminyan.github.io/
│
│── 🤖 React SPA 根目录（由 deploy_core.yml CI 自动生成，勿手动修改）
├── index.html                 # Vite SPA 入口
├── _assets/                   # Vite 构建产物（JS/CSS，_ 前缀避免 Jekyll 冲突）
│
│── 📌 legacy/ — 迁移自根目录的上游同步内容
│   │── 🔄 上游同步（_sync_RubiksSolverDemo.ps1 管理，勿手动改）
│   ├── solver/                # 3x3x3 Solver（主求解器页面）
│   ├── 2x2x2/                 # 2x2x2 求解器
│   ├── cross_core/         # Cross 训练器
│   ├── xcross_core/        # XCross 训练器
│   ├── xxcross_core/       # XXCross 训练器
│   ├── xcross_pairing_core/# XCross Free Pair 训练器
│   ├── eocross_core/       # EOCross 训练器
│   ├── pairing_core/       # Free Pair 训练器
│   ├── pseudo_xcross_core/ # Pseudo XCross 训练器
│   ├── pseudo_pairing_core/# Pseudo Free Pair 训练器
│   ├── algcore/            # JSON 公式训练器
│   ├── jsonEditor/            # JSON 编辑器
│   ├── documentation/         # 文档页面
│   ├── calc/                  # HTH 成绩计算器
│   ├── alg_trainers/          # 公式训练器（mihlefeld/Alg-Trainers 上游同步）
│   ├── battle/                # 1v1 对战计时器
│   ├── cstimer/               # csTimer 计时器
│   ├── recon/                 # 复盘页面（Jekyll 遗留版）
│   └── ...                    # 其他工具
│
├── core/                    # 📌 CubeRoot Trainer monorepo 源码（React 19 + Vite 8 + Zustand）
│   ├── packages/client/        # React 前端（sr-puzzlegen-pll 渲染 + Zustand 状态管理 + i18n）
│   ├── packages/shared/        # 共享类型 + PLL/OLL 数据
│   ├── packages/stats-ui/      # Stats 前端 TS 源码（编译后覆盖 assets/js/ 和 i18n/）
│   └── README.md               # 完整文档（结构 / 开发 / 部署 / 技术栈）
├── legacy/src/                 # 运行时模块（WASM、Worker、Solver 等，100% 上游）
├── icons/                     # PWA 图标
├── documents/                 # 文档资源
├── screenshots/               # 截图资源
...
├── legacy/index.html          # 原版落地页（Jekyll 遗留，已迁移到 legacy/）
├── assets/css/                # Stats 页面统一 UI 样式
│   ├── stats_ui.css           # 所有选择器/Tab/下拉菜单 CSS（从 Ruby 迁出，集中管理）
│   └── card.css               # 落地页入口卡片样式
├── assets/js/                 # Stats 前端交互逻辑（由 core/packages/stats-ui/ TypeScript 编译生成）
│   ├── stats_ui.js            # ← stats_ui.ts 编译产物：Metric/Tab/Source/Dropdown 交互 + initStatsUI()
│   ├── wr_history_chart.js    # ← wr_history_chart.ts 编译产物：WR 历史折线图
│   ├── distribution_chart.js  # ← distribution_chart.ts 编译产物：分布图（直方图/KDE/箱线图）
│   ├── logo_nav.js            # ← logo_nav.ts 编译产物：顶部 Logo 导航
...
│   ├── event_selector.js      # ← event_selector.ts 编译产物：WCA 项目选择器 + URL hash 状态持久化
...
├── _stats_build/              # WCA 统计构建脚本（Ruby legacy，已迁移到 core/packages/stats-build/ TypeScript）
├── stats/                     # 统计 Markdown 页面。由 CI 每周覆盖写入，也可本地生成后直接 push
│   ├── upcoming_comp/         # 近期比赛追踪（Jekyll 遗留，前端已迁到 React `/upcoming-comps`）
...
| 配置 | 值 |
|------|------|
| 服务器 | 阿里云 ECS（`www.cuberoot.me`） |
| API 入口 | `https://www.cuberoot.me/core/api/recon/...`（Nginx 反代到 Hono）<br>`https://www.cuberoot.me/legacy/recon/api/?action=...`（PHP legacy） |
...
| DB 连接层 | `legacy/recon/api/db.php`（PDO 单例 + JSON↔SQL 字段映射） |
| DB 凭据 | `legacy/recon/api/db_config.php`（不在 git 中，ECS 上手动创建） |

> **数据库凭据不受部署影响**：`legacy/recon/api/db_config.php` 在 rsync `--exclude` 列表中，CI 部署不会触碰。
...
| Scopes | `public` |
| Redirect URIs | `https://ruiminyan.github.io/auth/callback`<br>`https://www.cuberoot.me/auth/callback`<br>`http://localhost:5173/auth/callback`<br>（旧路径 `/callback.html` 仍保留向下兼容） |
...
88 个统计串行执行（详见 `core/packages/stats-build/src/bin/compute_all.ts`）：
...
1. 创建 `core/packages/stats-build/src/statistics/my_new_stat.ts`：
   ```typescript
...
6. 输出到 `stats/data/my_new_stat.json`，React 前端通过 `/wca-stats/my_new_stat` 渲染
...
- **Tab 双视图**：19 个统计支持「排名」和「历史」双 Tab 切换
  ...
  - `assets/js/stats_ui.js`：`initStatsUI()` 扫描 data-label-* 自动生成按钮（由 `core/packages/stats-ui/src/stats_ui.ts` 编译产生）
- 抽象基类 `statistics/abstract/wr_round_history.rb`：封装 WR 轮次衍生指标的通用模式
  - SQL 统一查询 `regional_average_record = 'WR'` 的 attempts 数据（通过 `ATTEMPTS_SUBQUERY` 子查询 `result_attempts` 表）
...
```
Hono API 后端（www.cuberoot.me/core/api/recon/）
...
React SPA（/recon 路由，Zustand 状态管理）
    ↓ 中文模式时
comp_names_zh.json（英文比赛名→中文名映射，CI 每日自动更新）
```
...
基址：`https://www.cuberoot.me/core/api/recon/`（Hono RESTful）
