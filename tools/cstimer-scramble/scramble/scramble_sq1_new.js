...
var sq1 = (function(setNPerm, getNPerm, circle, rn) {
	...
	// ─── cuberoot.me addition: solve an arbitrary scramble ────────────────────
	//
	// NOT upstream cstimer. Upstream only ever GENERATES: `Search_solution(c)`
	// returns a sequence that REACHES state `c` from solved (move2string walks the
	// found move array backwards and negates each amount), which is what a
	// random-state scrambler wants.
	//
	// A solution for scramble S falls out of that with one extra step: build
	// state(S), ask for a sequence X reaching it, and return X⁻¹. state(X) =
	// state(S), so state(S)·state(X⁻¹) = identity — X⁻¹ solves S. The inversion
	// happens in NOTATION space (reverse order, negate each turn); no search
	// internal is touched.
	//
	// The state has to be built FORWARD, not as state(S⁻¹): cstimer's shape model
	// (Shape_halflayer) can only index positions where neither slice cut has a
	// corner across it, and the inverse of a cut-clean position generally isn't
	// cut-clean. Positions that straddle a cut are real — (2,0) from solved is one,
	// you just can't slice there — so when the lookup fails we prepend the smallest
	// layer turn that aligns the cuts and solve from there (exactly what a human
	// does): solve(S) = (a,b) + solve(S + (a,b)).
	//
	// Used by /scramble/solver?event=sq1 through scrambler.worker.js's SOLVERS
	// registry (same wrap as mpyr / dino). Two-phase, so near-optimal, not
	// provably optimal — the contract is only that scramble∘solution = solved.

	function sq1Norm12(x) {
		var v = ((x % 12) + 12) % 12;
		return v > 6 ? v - 12 : v;
	}

	function sq1ParseTokens(scr) {
		var toks = [];
		var re = /(\/)|\(\s*(-?\d+)\s*,\s*(-?\d+)\s*\)/g;
		var m;
		while ((m = re.exec(scr)) !== null) {
			if (m[1]) toks.push(null); // slice
			else toks.push([parseInt(m[2], 10), parseInt(m[3], 10)]);
		}
		return toks;
	}

	// Internal move encoding (read off Search_move2string, which prints the
	// inverse): +m = top layer by +m, -m = bottom layer by +m. Both layers shift
	// their ring the same geometric way, and "clockwise" is mirrored underneath —
	// hence the sign split.
	function sq1ApplyTokens(cc, toks) {
		for (var i = 0; i < toks.length; i++) {
			var t = toks[i];
			if (t === null) { cc.doMove(0); continue; }
			var top = ((t[0] % 12) + 12) % 12;
			var bot = ((t[1] % 12) + 12) % 12;
			if (top !== 0) cc.doMove(top);
			if (bot !== 0) cc.doMove(-bot);
		}
	}

	function sq1TokensToString(toks) {
		var s = '';
		for (var i = 0; i < toks.length; i++) {
			var t = toks[i];
			s += t === null ? '/' : '(' + sq1Norm12(t[0]) + ',' + sq1Norm12(t[1]) + ')';
		}
		return s;
	}

	// 消步:相邻两刀相消,连续层转合并进一个括号(上下层转可交换,合并前后同一状态,
	// WCA 记号本来也是一对一个括号 —— 不合并会把 12c4 步数虚报)。与 shared 的
	// simplifySq1Alg 同一套规则;这份 worker 是静态 JS,跨不到那个包,只能各有一份。
	function sq1Simplify(toks) {
		var st = [];
		for (var i = 0; i < toks.length; i++) {
			var t = toks[i];
			var last = st.length > 0 ? st[st.length - 1] : undefined;
			if (t === null) {
				if (last === null) st.pop();
				else st.push(null);
			} else if (last !== undefined && last !== null) {
				var a = sq1Norm12(last[0] + t[0]);
				var b = sq1Norm12(last[1] + t[1]);
				if (a === 0 && b === 0) st.pop();
				else st[st.length - 1] = [a, b];
			} else {
				var a2 = sq1Norm12(t[0]);
				var b2 = sq1Norm12(t[1]);
				if (a2 !== 0 || b2 !== 0) st.push([a2, b2]);
			}
		}
		return st;
	}

	function sq1InvertTokens(toks) {
		var out = [];
		for (var i = toks.length - 1; i >= 0; i--) {
			var t = toks[i];
			out.push(t === null ? null : [-t[0], -t[1]]);
		}
		return out;
	}

	/** Cut-alignment candidates, nearest first: (0,0), then one layer by ±1, ... */
	var SQ1_ALIGN = (function() {
		var out = [];
		for (var d = 0; d <= 12; d++) {
			for (var a = -6; a <= 6; a++) {
				for (var b = -6; b <= 6; b++) {
					if (Math.abs(a) + Math.abs(b) === d) out.push([a, b]);
				}
			}
		}
		return out;
	...
	/** Near-optimal solution for a `(a,b)/` scramble, in the same notation. */
	function solveScramble(scr) {
		Shape_$clinit();
		Square_$clinit();
		var toks = sq1ParseTokens(scr);
		for (var i = 0; i < SQ1_ALIGN.length; i++) {
			var align = SQ1_ALIGN[i];
			var pre = align[0] === 0 && align[1] === 0 ? toks : toks.concat([align]);
			var cc = new SqCubie;
			sq1ApplyTokens(cc, pre);
			var shape = FullCube_getShapeIdx(cc);
			if (shape < 0 || ShapePrun[shape] === undefined) continue; // cut straddled
			var reached = Search_solution(new Search_Search(), cc);
			if (reached == null) continue;
			var sol = sq1InvertTokens(sq1ParseTokens(String(reached)));
			if (align[0] !== 0 || align[1] !== 0) sol.unshift(align);
			return sq1TokensToString(sq1Simplify(sol));
		}
		throw new Error('sq1 solveScramble: no solution found');
	}

	/** Validity oracle (mirrors pyraminx.js / redi.js): solve `scr`, apply the
	 *  solution on top of it, and check the cube is back to solved. */
	function selfCheck(scr) {
		var cc = new SqCubie;
		sq1ApplyTokens(cc, sq1ParseTokens(scr).concat(sq1ParseTokens(solveScramble(scr))));
		return cc.toString() === new SqCubie().toString();
	}
