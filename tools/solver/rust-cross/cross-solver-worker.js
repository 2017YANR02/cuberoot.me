...
let pocketSolver = null;  // PocketSolverWasm(2x2x2 口袋魔方整解最优),拉 opt_pocket(3.6MB)直接 from_dist
...
async function init(glueUrl, wasmUrl, tablesBase, need) {
  ...
  if (need === 'f2leo') {
    ...
    // 2x2x2 口袋魔方整解最优:拉 opt_pocket(3.6MB 全空间精确距离表)直接 from_dist,秒算。
    pocketSolver = await buildOpt(tablesBase, 'opt_pocket', mod.PocketSolverWasm);
...
self.onmessage = async (e) => {
  ...
  try {
    ...
    } else if (msg.type === 'pocket_len') {
      if (!pocketSolver) throw new Error('pocket solver not initialized');
      const t0 = performance.now();
      // 2x2x2 整解最优 HTM 步数(0..=11)。全 18 记号,D/L/B 经 24 旋转归一。
      const v = pocketSolver.solve(msg.scramble);
      self.postMessage({ type: 'variant', id: msg.id, values: [v], ms: performance.now() - t0 });
    } else if (msg.type === 'pocket_moves') {
      if (!pocketSolver) throw new Error('pocket solver not initialized');
      const t0 = performance.now();
