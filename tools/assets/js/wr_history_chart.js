...
(function () {
    ...
    function extractData(table) {
        ...
        ths.forEach(function (th, i) {
            ...
            if ((t === 'Count' || t === '次数') && rIdx === -1) {
                rIdx = i;
                isCount = true;
            }
            // NOTE: 日期列：优先 Date，回退 Start Date（屠榜历史有双日期列，取第一个 Date）
            if ((t === 'Date' || t === '日期' || t === 'Start Date' || t === '开始日期') && dIdx === -1)
                dIdx = i;
            if (t === 'Person' || t === '选手')
                pIdx = i;
            if (t === 'Improvement' || t === '进步')
                impIdx = i;
        });
        if (rIdx === -1 || dIdx === -1)
            return [];
        ...
        for (let i = 1; i < rows.length; i++) {
            ...
            if (tds.length <= Math.max(rIdx, dIdx))
                continue;
    ...
    function renderChart(canvas, points) {
        ...
        for (let i = 1; i < points.length; i++)
            ctx.lineTo(xPx(points[i].x), yPx(points[i].y));
        ...
        // ── Tooltip（鼠标 + 触摸） ──
        const tipEl = wrap.querySelector('.wr-chart-tooltip');
        ...
        function showTip(mx) {
            ...
            if (p.person)
                html += ' ' + p.person;
            if (p.imp)
                html += ' <span style="color:#6ee7b7">↓' + p.imp + '</span>';
    ...
    function createChartForPanel(panel) {
        ...
        const entry = { panel: panel, canvas: cvs, wrap: wrap };
    ...
    function init() {
        ...
        document.addEventListener('click', function (e) {
            const btn = e.target.closest('.event-btn, .stat-tab, .metric-btn, .source-btn, .metric-dropdown-item');
            if (btn) {
                setTimeout(refreshAllCharts, 0);
            }
        });
