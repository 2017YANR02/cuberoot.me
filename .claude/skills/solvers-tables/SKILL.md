...
```pwsh
Get-ChildItem D:\cube\cuberoot.me\solver\tables\*.bin | Sort Length -Desc | % { '{0}  {1}' -f $_.Length, $_.Name }
```
...
- 读 `solver/src/bin/<v>_analyzer.rs` 的 `global_init` + `solver/src/<v>_solver.rs` 的 `new()` / `global_init`, grep `ensure_` 调用。
