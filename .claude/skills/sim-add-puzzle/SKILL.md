...
- 画布两个浮层同尺寸(`--sim-float-size` 由 `layoutBackView` 一次算出落 CSS 变量,`.sim-image-overlay` / `.sim-backview` 各取):左上=图像(`PuzzleImageStudio` 预览经 `createPortal` 落 `imageHost`,`imageStudioSupported` 拼图才有),右上=背面小窗。框是硬编黑白双环(`data-board-bg` 5 档是定死艺术色不跟站点主题,主题 token 会在某些底色下隐形)。容器 `pointer-events:none` 让拖拽穿透到画布,仅 `.sim-float-close` 的 × 自带 auto。显隐入口:引擎小窗 + 图像浮层各自右上角 ×(关)+ 播放条对应角的找回钮(开,仅关着时出现);**cubing.js 原生小窗钉不上 ×**(区域归 player),播放条钮在 twisty 路径下改成常驻真开关(`aria-pressed`)—— 侧栏「小窗」toggle、「图像」折叠标题栏都已删,别再加回。

- 示意伴图(`sim_svg_export_schematic`,SR 范式):新引擎魔方给每张贴纸挂 `userData.schematicPoly`(理想晶格轮廓:局部系扁平 xyz、绕向朝外、不 inset/lift;缝烘在 scale 里→反烘如 pyra `PIECE_SHRINK`,缝烘在切割面偏移里→按 0 偏移重算如 skewb;polytope facet 类直接 `stickerGeom.schematicPolyFromFacet`;贴纸 mesh 会被压扁/带自身变换的挂 parent frame + `schematicInParent` 如 sq1);NxN instanced 走 mesh 级 `schematicInstancedPoly`(±HALF 整格 quad,mirror 时摘除)。挂了 = 伴图自动切示意导出器 + 黑边滑块出现;spec 渲染器不认识的拼图再把 simCaps `imageStudio:true`,面板走 engineOnly 模式(引擎矢量预览 + 截图组 + SVG/PNG,无 API 链接)。测试范本 `tests/sim_svg_export_schematic.test.ts`(严格多边形数 + 晶格去重数 `toBe` 锁死;镜像 det 翻转、非凸凸包守卫各有用例)。
