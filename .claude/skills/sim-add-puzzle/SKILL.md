...
- `engine/gear/` — 齿轮魔方,**复合联动范本**:一步 = 面 180° + 中层 90° 跟随 + 赤道 4 齿轮自旋 60°;每齿轮嵌套双 pivot(orbit pivot 原点 world-premultiply + spin pivot 恒定局部轴 `P₀⁻¹·r̂`,pieceAnim 零改动);状态复用 `lib/gear-solver`(BFS 双向单射锁 41,472);角块 = RoundedBox − 3 咬合环面(torus 罩住整环齿轮 sweep,构造零穿模);轮太大时球包围失效 → 测试用真齿形数值扫(全程 × 3×3 相位组合,`derive2.mjs` 二分最大净空轮径);角贴纸 = 星形 ray-march + `resampleClosed`+`chaikinClosed`(凸组合永不产生针刺;密集轮廓禁 `roundCorners` setback 圆角——会 179° 反折)。
