...
- `engine/gear/` — 齿轮魔方,**复合联动范本**:一步 = 面 180° + 中层 90° 跟随 + 赤道 4 齿轮自旋 480°(净 120° = mod-3 相位的视觉单位;相位单位角必须满足「相位归零 ⇔ 视觉归位」,pieceAnim 按 axis-angle 插值天然支持 >360° 甩转);每齿轮嵌套双 pivot(orbit 原点 world-premultiply + spin 恒定局部轴 `P₀⁻¹·r̂`,pieceAnim 零改动);状态复用 `lib/gear-solver`(BFS 双向单射锁 41,472);齿轮 = 折棱伞冠(6 等腰梯形齿板切「锥尖=棱中点、半角 45°」的锥,±90° 齿板恰落两面平面内,腰过圆心、隙 38° > 齿 22°);角块 = RoundedBox − 3 冠扫掠 lathe(自旋∪轨道解析轮廓,比球管紧)− 3 washer 环(中心臂扫掠),构造零穿模;**LatheGeometry 闭合轮廓必须 (r,y) 逆时针绕**,顺时针 = brush 内外翻转 → CSG 喷毛刺;角贴纸 = 星形 ray-march + `resampleClosed`+`chaikinClosed`(密集轮廓禁 `roundCorners` setback 圆角——会 179° 反折)。
