...
唯一入口:`core/packages/client/src/utils/useDocumentTitle.ts`。**所有新页一律走这个 hook,严禁手写 `document.title = ...`**。
...
数据加载完用 data,没加载用 fallback。参考 `pages/comp/CompDetailPage.tsx`:
...
`utils/useDocumentTitle` 的相对路径按文件深度算:
- `pages/Foo.tsx` → `../utils/useDocumentTitle`
- `pages/foo/Bar.tsx` → `../../utils/useDocumentTitle`
- `pages/foo/bar/Baz.tsx` → `../../../utils/useDocumentTitle`
