# PostToolUse hook: 改 client-next calc/ 下的文件后提醒跑回归测试
...
if ($path -match 'app[/\\][^/\\]+[/\\]calc[/\\]') {
  $msg = "你刚改了 client-next calc/ 下的文件 — 改完前必须跑 core/packages/client/tests/calc-interactions.mjs 回归测试。流程: (1) browser_navigate http://127.0.0.1:3000/zh/calc?event=333 (2) 把 mjs 内容贴进 browser_evaluate (3) failed > 0 修到全过才能说改完。"
