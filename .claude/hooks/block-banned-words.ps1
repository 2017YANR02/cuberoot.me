...
# Word list (the ONLY place to edit): <repo>/.claude/banned-words.json
...
# doc under .claude/. Never police our own machinery.
$fp = "$($j.tool_input.file_path)" -replace '\\', '/'
if ($fp -match '/\.claude/') { exit 0 }

# Only the NEWLY written text is judged (Write: content, Edit: new_string,
# MultiEdit: edits[].new_string) — never the old content being replaced.
...
$reason = "写入内容命中站内违禁词:`n" + ($hits -join "`n") `
  ...
  + "`n词表在 .claude/banned-words.json(用户说「添加违禁词:xx」就是加到那里)。"
