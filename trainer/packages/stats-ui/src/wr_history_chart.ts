...
(function () {
  ...
  interface ChartPoint {
    x: number;       // 年份小数（X 轴）
    y: number;       // 成绩秒数或 Count（Y 轴）
    person: string;  // 选手名
    imp: string;     // 进步百分比
    label: string;   // 原始成绩字符串（tooltip 显示）
  }
  ...
  function extractData(table: HTMLTableElement): ChartPoint[] {
    const ths = table.querySelectorAll('tr:first-child th');
    let rIdx = -1, dIdx = -1, pIdx = -1, impIdx = -1;
    let isCount = false; // NOTE: 是否用 Count 而非秒数作为 Y 轴
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      const date = parseDate(tds[dIdx].textContent!);
      if (!isNaN(result) && result > 0 && !isNaN(date)) {
        const person = pIdx >= 0 && tds[pIdx] ? tds[pIdx].textContent!.trim() : '';
        const imp = impIdx >= 0 && tds[impIdx] ? tds[impIdx].textContent!.trim() : '';
        const resultStr = tds[rIdx].textContent!.trim();
  ...
  // NOTE: 窄屏降低图表高度
  function getChartH(): number { return window.innerWidth < 600 ? 140 : 180; }

  function renderChart(canvas: HTMLCanvasElement, points: ChartPoint[]): void {
    const wrap = canvas.parentElement!;
    if (points.length < 2) { wrap.style.display = 'none'; return; }
    wrap.style.display = '';

    const CHART_H = getChartH();
    ...
    function xPx(v: number): number { return PAD.left + (v - xMin) / (xMax - xMin) * plotW; }
    function yPx(v: number): number { return PAD.top + (1 - (v - yMin) / (yMax - yMin)) * plotH; }
    ...
    // ── Tooltip（鼠标 + 触摸） ──
    const tipEl = wrap.querySelector('.wr-chart-tooltip') as HTMLElement;

    // NOTE: 根据 X 坐标找最近的数据点并显示 tooltip
    function showTip(mx: number): void {
      if (mx < PAD.left || mx > w - PAD.right) { tipEl.style.display = 'none'; return; }
      let best = 0, bestDist = Infinity;
      for (let i = 0; i < points.length; i++) {
        const d = Math.abs(xPx(points[i].x) - mx);
  ...
  // ── 找到面板内当前可见的表格并提取数据 ─────────────────────
  function getVisibleData(panel: HTMLElement): ChartPoint[] {
    const tables = panel.querySelectorAll('table');
    for (let i = 0; i < tables.length; i++) {
      // NOTE: event_selector 通过 display:none 隐藏不选中的 section
      if ((tables[i] as HTMLElement).style.display !== 'none' && (tables[i] as HTMLElement).offsetParent !== null) {
        return extractData(tables[i] as HTMLTableElement);
      }
    }
    // 回退: 如果都可见(初始状态), 只取第一个
    if (tables.length > 0) return extractData(tables[0] as HTMLTableElement);
    return [];
  }

  // ── 为每个 history stat-panel 创建一个图表 ─────────────────
  const chartRegistry: ChartEntry[] = [];

  function createChartForPanel(panel: HTMLElement): ChartEntry {
    ...
    const entry: ChartEntry = { panel: panel, canvas: cvs, wrap: wrap };
  ...
  function refreshAllCharts(): void {
    chartRegistry.forEach(function (entry) {
      ...
      if (entry.panel.classList.contains('active') || entry.panel.offsetParent !== null) {
        const data = getVisibleData(entry.panel);
  ...
  // ── 初始化 ─────────────────────────────────────────────
  function init(): void {
    const panels = document.querySelectorAll('.stat-panel[id$="-history"]');
    if (!panels.length) return;

    panels.forEach(function (panel) {
      createChartForPanel(panel as HTMLElement);
    });
    ...
    document.addEventListener('click', function (e) {
      const btn = (e.target as HTMLElement).closest('.event-btn, .stat-tab, .metric-btn, .source-btn, .metric-dropdown-item');
      if (btn) {
        setTimeout(refreshAllCharts, 0);
      }
    });
    ...
    window.addEventListener('resize', function () {
      ...
      resizeTimer = window.setTimeout(refreshAllCharts, 200);
    });
  }
