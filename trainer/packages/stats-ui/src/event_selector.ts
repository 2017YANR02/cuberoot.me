...
// NOTE: stats_ui.ts 中的全局函数声明（通过 onclick 属性调用）
declare function switchMetric(e: Event, id: string): void;
declare function switchTab(e: Event, id: string): void;
declare function switchSource(btn: HTMLElement, id: string): void;
...
(function () {
   ...
   */
  function collectSections(container: HTMLElement): Section[] {
  ...
  /**
   * NOTE: 创建图标选择器 DOM
   * @param eventIds - 页面中出现的 eventId 列表（保持顺序）
   * @param onSelect - 点击回调 (eventId) => void
   * @returns HTMLElement
   */
  function createSelector(eventIds: string[], onSelect: (id: string) => void, disabledIds?: Set<string>): HTMLElement {
    disabledIds = disabledIds || new Set<string>();
   ...
   */
  function showEvent(sections: Section[], eventId: string): void {
  ...
  function updateHash(newParams: Record<string, string>): void {
  ...
  let _allSelectors: SelectorEntry[] = [];

  // NOTE: 选中选择器中的指定项目按钮
  function selectEventInBar(selector: HTMLElement, eventId: string): boolean {
   ...
   * @param allPanelSections - 多组 sections（每组来自一个容器）
   * @param insertBeforeNode - 选择器 DOM 的插入点
   */
  function setupSelector(allPanelSections: Section[][], insertBeforeNode: HTMLElement, onEventChanged: ((id: string) => void) | null = null): void {
    ...
    const selector = createSelector(fullIds, (id: string) => {
   ...
   */
  function handleGroupedPage(container: HTMLElement, sections: Section[]): void {
    setupSelector([sections], sections[0].h3);
  }

  /**
   * NOTE: 处理 TabUi 页面（h3 在 .stat-panel 容器内）
   * @param scope - 限定搜索范围（.metric-panel 或 document）
   */
  function handleTabbedPage(scope: HTMLElement | Document): void {
    const panels = Array.from(scope.querySelectorAll('.stat-panel')) as HTMLElement[];
    if (panels.length === 0) return;
    const allPanelSections = panels.map(p => collectSections(p)).filter(s => s.length > 0);
    const insertBefore = (scope.querySelector('.stat-tabs') || panels[0]) as HTMLElement;
    setupSelector(allPanelSections, insertBefore);
  }

  // NOTE: 给 tab/metric 按钮叠加 click 监听，写入 hash
  function attachHashTracking(): void {
    ...
    document.querySelectorAll('.source-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (m) {
          ...
          const panel = btn.closest('.metric-panel') as HTMLElement | null;
  ...
  function restoreFromHash(): void {
    ...
    if (h.metric && typeof switchMetric === 'function') {
      ...
      const ddItem = document.querySelector(`.metric-dropdown-item[data-id="${h.metric}"]`) as HTMLElement | null;
      const btn = document.querySelector(`.metric-btn[onclick*="switchMetric"][onclick*="'${h.metric}'"]`) as HTMLElement | null;
    ...
    if (h.source && typeof switchSource === 'function') {
      ...
      const prefix = (metricScope as HTMLElement).id ? (metricScope as HTMLElement).id.replace('metric-', '') + '-' : '';
      const fullId = prefix + h.source;
      const btn = metricScope.querySelector(`.source-btn[onclick*="switchSource(this,'${fullId}')"]`) as HTMLElement | null;
      if (btn) btn.click();
    }
   ...
   */
  function handleMetricPage(metricPanels: NodeListOf<Element>): void {
    const allPanelSections: Section[][] = [];
    const metricSupportedEvents = new Map<HTMLElement, Set<string>>();

    metricPanels.forEach(mp => {
      const panels = Array.from(mp.querySelectorAll('.stat-panel')) as HTMLElement[];
      ...
      metricSupportedEvents.set(mp as HTMLElement, supported);
    });

    const onEventChanged = (eventId: string): void => {
      ...
      if (activeMetricDisabled && validMetrics.length > 0 && typeof switchMetric === 'function') {
        ...
        const ddFallback = document.querySelector('.metric-dropdown-item[data-id="' + fallbackId + '"]') as HTMLElement | null;
        const btnFallback = document.querySelector(`.metric-btn[onclick*="switchMetric"][onclick*="'${fallbackId}'"]`) as HTMLElement | null;
    ...
    const insertBefore = (document.querySelector('.metric-dropdown') || document.querySelector('.metric-selector') || metricPanels[0]) as HTMLElement;
  ...
  function init(): void {
    ...
    if (panels.length > 0) {
      ...
      const container = (document.querySelector('.container') || document.body) as HTMLElement;
