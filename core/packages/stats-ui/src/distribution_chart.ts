...
(function () {
    ...
    interface ChartController {
        update: (newDatasets: Dataset[]) => void;
        destroy: () => void;
     ...
     * 创建直方图+KDE 分布图
     */
    function createDistChart(container: HTMLElement, datasets: Dataset[], options?: Partial<ChartOptions>): ChartController | null {
        const opts: ChartOptions = { ...DEFAULTS };
        if (options) {
            Object.assign(opts, options);
        }
        // 合并 padding
        if (options && options.padding) {
            opts.padding = { ...DEFAULTS.padding, ...options.padding };
        }
        ...
        let BIN_WIDTH: number = typeof opts.binWidth === 'number' ? opts.binWidth : 0;
        ...
        function onLocaleChange(): void { draw(); }
        ...
        return {
            ...
            update: function (newDatasets: Dataset[]): void {
            ...
            setActive: function (indices: number[]): void {
        ...
        // ── 直方图 ──
        function drawHistogram(selected: Dataset[], gMin: number, gMax: number): void {
            ...
            allBins.forEach(bins => {
                bins.forEach(c => { if (c > maxCount) maxCount = c; });
            });
            ...
            for (let si = 0; si < nP; si++) {
                ...
                for (let b = 0; b < binCount; b++) {
                    ...
                    rect.addEventListener('mouseenter', function (e) {
                        tooltip.textContent = (e.target as SVGElement).getAttribute('data-info');
                        tooltip.style.opacity = '1';
                    });
                    rect.addEventListener('mousemove', function (e) {
                        const cr = wrapper.getBoundingClientRect();
                        tooltip.style.left = ((e as MouseEvent).clientX - cr.left + 12) + 'px';
                        tooltip.style.top = ((e as MouseEvent).clientY - cr.top - 30) + 'px';
                    });
        ...
        // ── KDE ──
        function drawKDE(selected: Dataset[], gMin: number, gMax: number): void {
        ...
        // ── 箱线图 ──
        function drawBoxPlot(selected: Dataset[], gMin: number, gMax: number): void {
        ...
        // ── 图例 + 统计信息 ──
        function drawLegend(selected: Dataset[]): void {
        ...
        // ── 轴标题 ──
        function axisLabels(yLabelEn: string, yLabelZh: string): void {
    ...
    function initStatsDistChart(table: HTMLTableElement, aoLabel: string): void {
        ...
        // 构建数据集（带动态名字 getter）
        const datasets: Dataset[] = players.map(p => ({
            name: p.name,
            times: p.times,
            color: p.color,
            // NOTE: 从表格单元格动态读取名字（语言切换后 i18n 会更新 <a> 文本）
    ...
    function svgEl(tag: string, attrs: Record<string, string>, parent?: SVGElement): SVGElement {
    ...
    function createToggleBtn(enText: string, zhText: string, active: boolean): HTMLButtonElement {
    ...
    // ── 暴露公共 API ──
    (window as any).DistributionChart = {
