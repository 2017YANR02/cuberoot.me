 ...
 * 但 `parseClockMoves` 把 y2 折成了每步公式的绝对 `side`(= 此前 y2 的奇偶),它索引的是
 * **起手帧**的两个半区。两者要同时成立,只能像 `clockStateFromAlg` 那样:整段按起手帧算,
 * 末了再按翻面与否对调一次。
 *
 * 逐步播放的引擎不能"末了再说",所以这里把两个帧分开存:
 *   `raw`     —— 起手帧的 18 个盘位,公式直接作用于它,中途永不对调
 *   `flipped` —— 当前是否翻着面
 *   `state`   —— 对外(渲染 / 求解 / 求打乱)的规范形式,读时才按 `flipped` 对调
 * 于是「先 y2 再拧」与「整段一次算完」逐盘相等,`tests/sim_clock_board.test.ts` 锁这条。
 */
import * as THREE from 'three';
import MoveHistory from '@cuberoot/puzzle-render-core/engine/MoveHistory';
import TweenTwister, { type TweenCube } from '@cuberoot/puzzle-render-core/engine/TweenTwister';
import { makeAnim, type PieceAnim } from '@cuberoot/puzzle-render-core/engine/pieceAnim';
import {
...
export type { ClockStep } from '../../support/clock-notation';
