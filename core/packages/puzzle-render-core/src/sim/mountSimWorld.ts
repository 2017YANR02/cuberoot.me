...
export interface SimMountOpts<W extends World = World> {
  createWorld?: () => W;
...
export interface SimMount<W extends World = World> {
  world: W;
...
export function mountSimWorld<W extends World = World>(opts: SimMountOpts<W>): SimMount<W> {
  const {
    host,
    puzzle = 3,
    interactive = false,
  ...
  const dispose = (): void => {
    if (disposed) return;
    disposed = true;
    // Release everything acquired so far, including a partially mounted view.
    // One failed cleanup must not strand the remaining GPU/listener resources.
    for (const cleanup of cleanups.reverse()) {
      try { cleanup(); } catch { /* continue releasing the other resources */ }
    }
  };

  try {
    const world = opts.createWorld?.() ?? new World() as W;
    cleanups.push(() => (world.cube as { dispose?: () => void }).dispose?.());
    if (interactive) cleanups.push(() => world.controller?.stop());
    ...
    cleanups.push(() => darkQuery.removeEventListener('change', invalidateHintBackdrop));
