...
import { derivePieceBlocks } from '@cuberoot/puzzle-solvers/piece-blocks';
...
/** Canonical TNOODLE sticker-state permutation (axes 0=R, 1=U, 2=L, 3=B). */
export class SkewbState {
  image: number[][] = Array.from(
    { length: 6 },
    (_, face) => Array.from({ length: 5 }, (_, slot) => face * 5 + slot),
  );

  private static swap3(
    image: number[][],
    face1: number,
    slot1: number,
    face2: number,
    ...
    const prior = image[face1][slot1];
    image[face1][slot1] = image[face2][slot2];
    image[face2][slot2] = image[face3][slot3];
    image[face3][slot3] = prior;
  }

  turnOnce(axis: number): void {
    const image = this.image;
    switch (axis) {
      case 0:
        SkewbState.swap3(image, 2, 0, 3, 0, 1, 0);
        SkewbState.swap3(image, 2, 4, 3, 2, 1, 3);
        SkewbState.swap3(image, 2, 2, 3, 1, 1, 4);
        SkewbState.swap3(image, 2, 3, 3, 4, 1, 1);
        SkewbState.swap3(image, 4, 4, 5, 3, 0, 4);
        break;
      case 1:
        SkewbState.swap3(image, 0, 0, 1, 0, 5, 0);
        SkewbState.swap3(image, 0, 2, 1, 2, 5, 1);
        SkewbState.swap3(image, 0, 4, 1, 4, 5, 2);
        SkewbState.swap3(image, 0, 1, 1, 1, 5, 3);
        SkewbState.swap3(image, 4, 1, 2, 2, 3, 4);
        break;
      case 2:
        SkewbState.swap3(image, 4, 0, 5, 0, 3, 0);
        SkewbState.swap3(image, 4, 3, 5, 4, 3, 3);
        SkewbState.swap3(image, 4, 1, 5, 3, 3, 1);
        SkewbState.swap3(image, 4, 4, 5, 2, 3, 4);
        SkewbState.swap3(image, 2, 3, 0, 1, 1, 4);
        break;
      case 3:
        SkewbState.swap3(image, 1, 0, 3, 0, 5, 0);
        SkewbState.swap3(image, 1, 4, 3, 4, 5, 3);
        SkewbState.swap3(image, 1, 3, 3, 3, 5, 1);
        SkewbState.swap3(image, 1, 2, 3, 2, 5, 4);
        SkewbState.swap3(image, 0, 2, 2, 4, 4, 3);
        break;
      default:
        break;
    }
  ...
  applyMove(token: string): void {
    const match = /^([RULBrulb])(['2]?)$/.exec(token.trim());
    if (!match) return;
    const axis = 'RULB'.indexOf(match[1].toUpperCase());
    if (axis < 0) return;
    const turns = match[2] === "'" ? 2 : 1;
    for (let turn = 0; turn < turns; turn++) this.turnOnce(axis);
  }

  applyAlgorithm(scramble: string): void {
    for (const token of scramble.split(/\s+/).filter(Boolean)) this.applyMove(token);
  }
}
...
function buildCoord(slots: readonly number[]): Coord {
  ...
  const localPerm = MOVE_PERMS.map((p) => Int32Array.from(slots, (s) => {
    ...
    if (src === undefined) throw new Error('skewb-solver: 转动把角格换到了中心格(置换表读错了)');
    return src;
  }));
...
/**
 * 取逆一段斜转公式串:倒序 + 每步取逆。记号只有 4 个字母 + 可选撇,`X2` 按 tnoodle 的口径
