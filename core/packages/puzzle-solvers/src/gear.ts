...
/** csTimer `gearso` starts its exact-length search at four face turns. */
export const GEAR_TIMER_MIN_LENGTH = 4;
/**
...
function exactGearSolution(
  ...
  if (remaining === 0) return key === g.states[0];
  const distance = g.dist.get(key);
  if (distance === undefined || distance > remaining) return false;
  for (const move of MOVES) {
    // csTimer's gear search never emits two adjacent turns of one axis.
    if (move.axis === lastAxis) continue;
    path.push(move.name);
    if (exactGearSolution(g, applyMove(state, move), remaining - 1, move.axis, path)) return true;
    path.pop();
  }
  return false;
}

function gearScrambleAtLeast(g: GearGraph, key: number, minimum: number): string {
  const state = decodeKey(key);
  const optimal = g.dist.get(key);
  if (optimal === undefined) throw new Error('gear-solver: sampled an unreachable state');
  for (let length = Math.max(minimum, optimal); length <= minimum + GEAR_GODS_NUMBER; length++) {
    const solution: string[] = [];
    if (!exactGearSolution(g, state, length, -1, solution)) continue;
    return solution
      .reverse()
      .map((move) => MOVE_BY_NAME.get(move)!.inverse)
      .join(' ');
  }
  throw new Error('gear-solver: no competition-style path for sampled state');
}

/**
 * CubeRoot Timer's shared equivalent of csTimer `gearso`:
 *
 * - one uniformly sampled reachable state (including the identity, matching
 *   csTimer's historical array-vs-number identity check);
 * - the exact 33-token U/R/F alphabet;
 * - an exact path of at least four face turns with no repeated adjacent axis.
 *
 * The state remains uniformly distributed; only its displayed path is padded
 * to the competition-style minimum instead of exposing the shortest solution.
 */
export function generateGearTimerScramble(rng: () => number = Math.random): string {
  const sample = rng();
  if (!Number.isFinite(sample) || sample < 0 || sample >= 1) {
    throw new Error('gear-solver: rng must return a number in [0,1)');
  }
  const g = graph();
  const key = g.states[Math.floor(sample * g.states.length)];
  return gearScrambleAtLeast(g, key, GEAR_TIMER_MIN_LENGTH);
}
