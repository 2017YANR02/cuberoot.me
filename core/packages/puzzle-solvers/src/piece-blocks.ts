/**
 * piece-blocks —— 从「转动的贴纸置换」自动推出「哪几张贴纸长在同一块上」。
 *
 ...
 * 展开图各面朝向不同,肉眼数极易翻)。
 *
 * 好在块划分是被置换群唯一确定的:块是**群不变的划分**(a、b 同块 ⇒ 转动后 p(a)、p(b) 仍同块)。
...
/**
 * 含全部给定「同块」约束的最小群不变划分。`perms[m][slot]` = 转动 m 之后该 slot 上的内容来自哪个
...
function closure(
  ...
  while (work.length) {
    ...
    // a~b ⇒ 每个转动下 p(a)~p(b);传递闭包由并查集自己兜住。
