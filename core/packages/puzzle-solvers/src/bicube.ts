/*
 * Bicube (联体魔方 / Bicube) optimal solver core — pure TS. TIER B (offline precomputed exact-distance table).
 ...
 *   • OFFLINE: the scramble-stats build job BFS-es the closure ONCE
 ...
 *   • RUNTIME: this core decodes the bytes into TYPED ARRAYS (~10MB resident: Float64Array of 1.1M
 *     sorted ranks + Uint8Array of 1.1M dist bytes), then solves by GRADIENT DESCENT (from the scrambled
 *     state pick the legal neighbour whose looked-up optimal distance is current−1). The client adapter
 *     owns fetch + gzip inflation. Still PROVABLY OPTIMAL; no in-browser BFS and no 510MB Map.
 *
