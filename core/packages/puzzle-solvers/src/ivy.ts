...
/** csTimer `ivyso` starts its exact-length search at six corner turns. */
export const IVY_TIMER_MIN_LENGTH = 6;
...
/** Convert standard /sim and scramble-sheet notation to cstimer's inverted Ivy powers. */
export function ivyStandardToCstimer(scramble: string): string {
  const out: string[] = [];
  for (const tok of scramble.trim().split(/\s+/)) {
    if (!tok) continue;
    const match = TOKEN_RE.exec(tok);
    if (!match) throw new Error(`bad: ${tok}`);
...
/** Apply standard /sim and scramble-sheet notation instead of cstimer's inverted powers. */
...
export function generateIvyByDistance(lo: number, hi: number, rng: () => number): string {
  ...
  const g = graph();
  const index = sampleIvyIndex(g, lo, hi, sample);
  // Timer 以空字符串表示「还在生成」；0 步状态需用非空恒等序列。
  return indexToScramble(g, index) || "R R'";
}

function exactIvySolution(
  g: IvyGraph,
  index: number,
  remaining: number,
  lastAxis: number,
  ...
  if (remaining === 0) return index === SOLVED_IDX;
  if (g.dist[index] < 0 || g.dist[index] > remaining) return false;
  for (let move = 0; move < MOVES.length; move++) {
    const axis = MOVES[move].axis;
    // mathlib.Solver, used by csTimer `ivyso`, rejects repeated adjacent axes.
    if (axis === lastAxis) continue;
    path.push(move);
    if (exactIvySolution(g, nextIdx(g, index, move), remaining - 1, axis, path)) return true;
    path.pop();
  }
  return false;
}

function ivyScrambleAtLeast(g: IvyGraph, index: number, minimum: number): string {
  const optimal = g.dist[index];
  if (optimal < 0) throw new Error('ivy-solver: sampled an unreachable state');
  for (let length = Math.max(minimum, optimal); length <= minimum + IVY_GODS_NUMBER; length++) {
    const solution: number[] = [];
    if (!exactIvySolution(g, index, length, -1, solution)) continue;
    return solution.reverse().map((move) => MOVES[move ^ 1].name).join(' ');
  }
  throw new Error('ivy-solver: no competition-style path for sampled state');
}

/**
 * CubeRoot Timer's shared equivalent of csTimer `ivyso`: uniformly sample a
 * reachable state beyond distance one, then emit its legal R/L/D/B path using
 * at least six turns and no repeated adjacent axis.
 */
export function generateIvyTimerScramble(rng: () => number = Math.random): string {
  const sample = rng();
  if (!Number.isFinite(sample) || sample < 0 || sample >= 1) {
    throw new Error('ivy-solver: rng must return a number in [0,1)');
  }
  const g = graph();
  const index = sampleIvyIndex(g, 2, IVY_GODS_NUMBER, sample);
  return ivyScrambleAtLeast(g, index, IVY_TIMER_MIN_LENGTH);
}
