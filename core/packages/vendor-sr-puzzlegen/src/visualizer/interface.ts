...
export interface PuzzleOptions {
   ...
   * ```
   */
  rotations?: { x?: number; y?: number; z?: number }[] | null;
...
export function validatePuzzleOptions(options: PuzzleOptions) {
  ...
  if (options.scheme) {
    if (typeof options.scheme !== "object" || Array.isArray(options.scheme)) {
      ...
      Object.keys(scheme).forEach((face) => {
        const faceColor = scheme[face];
        if (
          faceColor == null ||
  ...
  if (options.mask) {
    if (typeof options.mask !== "object" || Array.isArray(options.mask)) {
      ...
      Object.keys(mask).forEach((face) => {
        const maskValues = mask[face];
  ...
  if (options.stickerColors) {
    if (
      ...
      const stickerColors = options.stickerColors;
      Object.keys(stickerColors).forEach((face) => {
        const faceColors = stickerColors[face];
