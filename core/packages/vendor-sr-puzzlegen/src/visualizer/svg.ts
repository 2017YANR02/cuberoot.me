...
export interface SVGVisualizerOptions {
  ...
  /**
   * camera z-distance (透视). Smaller = stronger perspective, larger = flatter;
   * on-screen size stays constant. Default = sr's native camera (5).
   * Fork addition — replaces the old module-level setSrPerspective patch.
   */
  cameraDist?: number;
}
...
export class SvgVisualizer extends Visualizer {
  ...
  constructor(
    ...
    super(renderer, type, options.puzzle, options.cameraDist);
  ...
  setSvgOptions(options: SVGVisualizerOptions) {
    ...
    svgElement.setAttributeNS(null, "width", `${this.svgOptions.width}`);
    svgElement.setAttributeNS(null, "height", `${this.svgOptions.width}`);
