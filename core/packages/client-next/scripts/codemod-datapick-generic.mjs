#!/usr/bin/env node
// Generic type-gated dataPick 3-way for `COND ? X.<p>Zh : X.<p>En` (and zh/en):
//   i18n.language === 'zh-Hant' ? (X.<p>ZhHant ?? X.<p>Zh) : (COND ? X.<p>Zh : X.<p>En)
// Fires only when the zh/en fields are `string` AND X's type carries the
// matching <p>ZhHant member (added by generic-zhhant-data.mjs). Run AFTER it.
//
// Run: node scripts/codemod-datapick-generic.mjs
...
let changed = 0, skipNoType = 0; const touched = new Set();
for (const sf of project.getSourceFiles()) {
  ...
  for (const ce of sf.getDescendantsOfKind(SyntaxKind.ConditionalExpression).reverse()) {
    ...
    try {
      ...
      if (!base.getType().getProperty(hn)) { skipNoType++; continue; }
    } catch { continue; }
    const orig = ce.getText();
    ce.replaceWithText(`i18n.language === 'zh-Hant' ? (${base.getText()}.${hn} ?? ${zhN.getText()}) : (${orig})`);
...
await project.save();
console.log(`datapick-generic: rewrote ${changed} ternaries across ${touched.size} files (skipped ${skipNoType} no-type)`);
