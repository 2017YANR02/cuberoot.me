...
export abstract class Storage {
    ...
    save() {
        // NOTE(port): SSR-safe — getInitialState() constructs a ColorScheme
        // (which would .save()/.load()) during the useReducer lazy-init; on the
        // server there is no window/localStorage, so no-op / fall back to default.
        if (typeof window === 'undefined') return;
        window.localStorage.setItem(this.name, this.serialize());
    }
    load() {
        if (typeof window === 'undefined') { this._setDefault(); return; }
