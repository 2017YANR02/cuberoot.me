...
- 12 个一级模块 + 28+ 路由,**全部**搬到 `packages/client-next/`
...
| `packages/client-next/` | Next.js 16.2.6 + React 19.2.4 + Turbopack POC,已跑通 /frame-count |
...
4. **本文档**(你正在读)
5. `packages/client-next/README.md` — POC 已验证 + 已踩坑清单
6. `packages/client-next/app/frame-count/page.tsx` — 唯一 POC 页参考
...
| 场景 | 工具 |
|---|---|
| 跑 dev server | `pnpm --filter @cuberoot/client-next dev`(端口 3000,127.0.0.1)|
| typecheck | `pnpm --filter @cuberoot/client-next typecheck` |
| build | `pnpm --filter @cuberoot/client-next build` |
...
- [ ] `pnpm --filter @cuberoot/client-next build` 干净
...
- [ ] 主域名切流量后 24h 无回滚
- [ ] `packages/client/` 删除,`packages/client-next/` 改名 `packages/client`(或保留双名)
- [ ] CLAUDE.md 更新

## 给接手 AI 的开场动作建议

1. `git log --oneline -5` + `git status` 确认当前 branch 是 `poc/nextjs-frame-count`
2. `pnpm --filter @cuberoot/client-next dev`(背景跑)
