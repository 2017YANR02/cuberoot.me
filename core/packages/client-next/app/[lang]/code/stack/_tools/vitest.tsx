...
export const VITEST: StackTool = {
  ...
  zh: {
    ...
    cuberootDesc: <>cuberoot.me 的 <code>@cuberoot/client-next</code> 包用 Vitest 跑全集 (<code>pnpm --filter @cuberoot/client-next test</code>) 或 watch (<code>test:watch</code>)。测试统一放 <code>packages/client-next/tests/*.test.ts</code> (不与源码并排, 避开 App Router 的 <code>app/</code> 路由目录), 源文件走 <code>@/</code> alias import;worker / 算法回归测试配一个 <code>_*_runner.cjs</code> (node:worker_threads + classic-worker globals shim)。CI 在 <code>.github/workflows/test.yml</code> PR + push main 触发。</>,
    outlookTitle: '当下与前景',
  ...
  en: {
    ...
    cuberootDesc: <>cuberoot.me's <code>@cuberoot/client-next</code> package runs Vitest — full suite via <code>pnpm --filter @cuberoot/client-next test</code>, watch via <code>test:watch</code>. All tests live in <code>packages/client-next/tests/*.test.ts</code> (not next to source — clear of the App Router <code>app/</code> directory), importing source through the <code>@/</code> alias; worker / algorithm regression tests pair with a <code>_*_runner.cjs</code> (node:worker_threads + a classic-worker globals shim). CI fires on PR + push to main via <code>.github/workflows/test.yml</code>.</>,
  ...
  cuberoot: {
    zh: (
      <>
        <p><code>@cuberoot/client-next</code> 用 Vitest 跑测试。两条入口:<code>pnpm --filter @cuberoot/client-next test</code> 跑全集 (CI 用), <code>test:watch</code> 留 dev 用。Next 16 那份 <code>vitest.config.ts</code> 直接复用 Vite 的 transform, 不需要单独写 jest.config —— 这是选 Vitest 而不是 Jest 的核心动机。退役的 <code>@cuberoot/client</code> 还保留同一份测试作回滚兜底, 但不进 CI。</p>
        <p>测试统一放在 <code>packages/client-next/tests/*.test.ts</code>, 不与源码并排 —— 避开 Next App Router 的 <code>app/</code> 路由目录, 源文件一律走 <code>@/</code> alias import (<code>vitest.config.ts</code> 里配的)。按用途分两类:</p>
    ...
    en: (
      <>
        <p><code>@cuberoot/client-next</code> runs the tests with Vitest. Two entry points: <code>pnpm --filter @cuberoot/client-next test</code> for the full suite (CI uses this), <code>test:watch</code> for dev. The Next 16 <code>vitest.config.ts</code> reuses Vite's transform directly — there's no separate jest.config to maintain, which is the core reason we picked Vitest over Jest. The retired <code>@cuberoot/client</code> still keeps the same tests as a rollback fallback, but stays out of CI.</p>
        <p>All tests live in <code>packages/client-next/tests/*.test.ts</code>, never beside the source — clear of the Next App Router <code>app/</code> directory, with source imported through the <code>@/</code> alias (configured in <code>vitest.config.ts</code>). Two flavors by purpose:</p>
