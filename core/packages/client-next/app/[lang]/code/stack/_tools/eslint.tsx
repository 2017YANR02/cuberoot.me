...
export const ESLINT: StackTool = {
  ...
  zh: {
    ...
    role: '本站 client-next 的代码检查。一条 flat config 规则: 强制 playwright 脚本走 headless.mjs 包装, 不直接 import 浏览器引擎。',
    ...
    cuberootDesc: <>本站 client-next 跑 ESLint 9.39 flat config, 但只立了一条规则 —— 把它当 "护栏" 而非 "风格警察"。</>,
    outlookTitle: '当下与前景',
  ...
  en: {
    ...
    role: "Code-checks the site's client-next. One flat-config rule: force playwright scripts through the headless.mjs wrapper instead of importing a browser engine directly.",
    ...
    cuberootDesc: <>The site's client-next runs ESLint 9.39 flat config, but with exactly one rule — treating it as a guardrail, not a style cop.</>,
  ...
  adopters: [
    ...
    { name: 'cuberoot.me', highlight: true, zhNote: '本站 client-next flat config, 一条护栏规则', enNote: "This site — client-next flat config, one guardrail rule" },
  ],
  outlook: [
  ...
  cuberoot: {
    zh: (
      <>
        <p>本站 client-next 跑 ESLint 9.39 的 flat config (<code>eslint.config.mjs</code>), 命令是 <code>pnpm --filter @cuberoot/client-next lint</code> → <code>eslint .</code>。但这份 config 刻意极简: 整个项目<strong>只立了一条规则</strong>。它不当风格警察 (格式靠约定 + Prettier 心智), 而是当一道护栏。</p>
