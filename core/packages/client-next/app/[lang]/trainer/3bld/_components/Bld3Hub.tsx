    ...
    <>
      {!embedded && (
        <div className="bld-topbar">
          <h1><EventIcon event="333bf" /> {isZh ? '盲拧训练' : '3BLD Trainer'}</h1>
      ...
      <p className="bld-hub-intro">
        {isZh
          ? '三阶盲拧全套训练：读码还原、棱角公式、浮动缓冲、编组、奇偶，以及记忆默写与公式库。'
          : 'Full 3BLD training suite: read-code helper, edge/corner drills, floating buffer, 2-2 swaps, parity, plus memory recall and a commutator library.'}
      </p>
      ...
      {GROUPS.map((group) => (
        <section key={group.en} className="bld-hub-group">
          <h2 className="bld-hub-group-title">{isZh ? group.zh : group.en}</h2>
          <div className="bld-hub-grid">
            {group.modules.map((m) => (
              <Link key={m.href} href={m.href} className="bld-hub-card">
                ...
                <div className="bld-hub-card-title">{isZh ? m.zh : m.en}</div>
              </Link>
            ))}
          </div>
        </section>
