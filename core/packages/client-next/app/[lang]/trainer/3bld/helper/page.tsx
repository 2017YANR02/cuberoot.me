...
export default function HelperPage(): JSX.Element {
  ...
  const generateFromState = useCallback(async () => {
    ...
    try {
      ...
      setGenScramble(isZh ? '(生成失败)' : '(failed)');
    } finally {
      setGenBusy(false);
    }
  }, [scramble, comms, isZh]);
  ...
  const copyGen = useCallback(async () => {
    ...
    try {
      ...
      window.prompt(isZh ? '复制下面的打乱:' : 'Copy the scramble below:', genScramble);
    }
  }, [genScramble, isZh]);
  ...
  return (
    <div className="bld-trainer-root">
      <div className="bld-topbar">
        <h1>{isZh ? '读码还原助手' : 'Read & Restore Helper'}</h1>
      </div>
      ...
      <div className="bld-helper-grid">
        ...
        <section className="bld-helper-col">
          <div className="bld-helper-row-head">
            <span className="bld-section-title">{isZh ? '打乱公式' : 'Scramble'}</span>
            <button
              type="button"
          ...
          <div className="bld-input-wrap bld-helper-scr-wrap">
            <textarea
              ...
              placeholder={isZh ? "例: R U R' U' ..." : "e.g. R U R' U' ..."}
              spellCheck={false}
              rows={3}
            />
            {rawScramble && (
          ...
          {scramble.length === 0 && rawScramble.length > 0 && (
            <p className="bld-modal-msg is-error">
              {isZh ? '无法识别打乱(检查空格或转动记号)' : 'Unrecognized scramble (check spaces / move tokens)'}
            </p>
          )}
        </section>
        ...
        <section className="bld-helper-col">
          <span className="bld-section-title">{isZh ? '读码结果' : 'Read-code'}</span>
          <div className="bld-helper-reads">
            <LetterReadout label={isZh ? '棱块读码' : 'Edges'} cells={read.edges} />
            <div className="bld-readout">
              <span className="bld-readout-label">{isZh ? '棱块翻色' : 'Edge flips'}</span>
              <span className={read.flips ? 'bld-readout-cells' : 'bld-readout-empty'}>
                {read.flips || (isZh ? '无' : 'none')}
              </span>
            </div>
            <LetterReadout label={isZh ? '角块读码' : 'Corners'} cells={read.corners} />
            <div className="bld-readout">
              <span className="bld-readout-label">{isZh ? '角块翻色' : 'Corner twists'}</span>
          ...
          <div className="bld-helper-restore-head">
            <span className="bld-section-title">
              {isZh ? '请输入还原编码' : 'Restore codes'}
            </span>
            <span className="bld-helper-restore-hint">
              {isZh ? '(拖拽可交换执行顺序)' : '(drag to reorder execution)'}
            </span>
          </div>
        ...
        <section className="bld-helper-col">
          <span className="bld-section-title">{isZh ? '3D 预览' : '3D preview'}</span>
          ...
          <div className="bld-helper-gen">
            <div className="bld-helper-gen-actions">
              ...
              >
                ...
                {isZh ? '生成当前状态打乱' : 'Generate scramble from state'}
              </button>
              {genScramble && (
