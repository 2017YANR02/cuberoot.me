...
// the official article (aligned EN / 简体 / 繁体, built by scripts/build-reg-clauses.mjs
...
export interface RegClause { id: string; depth: number; en: string; zh: string; }
...
export default function FullClauses({ data }: { data: RegClauseDoc }) {
  ...
  const pick = (c: { en: string; zh: string; }) => (isZh ? c.zh : c.en);
    ...
    <details className="reg-full">
      <summary className="reg-full-summary">
        ...
        <span className="reg-full-summary-title">
          {t('完整条款', 'Full regulations')}
        </span>
        <span className="reg-full-count">{t(`${n} 条`, `${n} clauses`)}</span>
      </summary>
      ...
      <div className="reg-full-body">
        ...
        <p className="reg-full-note">
          {t(
            ...
            `Clauses from the WCA Regulations (${ver}). Translations are for reference only; the official English text is authoritative.`
          )}{' '}
          <a
