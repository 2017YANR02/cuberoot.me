 ...
 *    , (next scramble).
 *
 * Next.js port: 1:1 mirror of packages/client/src/pages/timer/TimerPage.tsx.
 * Imports rewired to ./_lib/* and ./_components/*. router-dom Link → next/link;
 * i18n helpers imported from @/i18n/i18n-client.
 */
...
import { formatMs, bestSingle, bestAverageOfN } from './_lib/stats';
...
export default function TimerPage() {
  ...
  useEffect(() => {
    ...
    // Preserve other params (e.g. ?lang=zh) when stripping ?replay=.
  ...
  return (
    <div className={`timer-root ${fullscreen ? 'fullscreen' : ''} ${settings.hideAllUiWhileRunning && timer.phase === 'running' ? 'ui-hidden' : ''}`}>
      ...
      <div className="timer-topbar">
        <div className="left">
          <Link className="home-link" href={`/${getLangQuery()}`} title={isZh ? '返回首页' : 'Home'}>
