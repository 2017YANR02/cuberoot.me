...
// mediainfo.js default browser entry is `dist/esm-bundle/index.js` which contains
// `new URL('MediaInfoModule.wasm', import.meta.url)` that Turbopack tries to statically
// resolve at compile time and fails. The package's `exports` field blocks subpath imports
// to the unbundled esm/index.js. Workaround: lazy load at runtime via Function('return import...')
// to bypass static analysis. The locateFile callback we pass below intercepts the wasm fetch.
let mediaInfoFactory: typeof mediaInfoFactoryT | null = null;
async function loadMediaInfo() {
  if (mediaInfoFactory) return mediaInfoFactory;
  const dyn = new Function('p', 'return import(p)') as (p: string) => Promise<{ default: typeof mediaInfoFactoryT }>;
  const mod = await dyn('mediainfo.js');
  mediaInfoFactory = mod.default;
...
async function detectFpsFromFile(file: File): Promise<number | null> {
  try {
    const factory = await loadMediaInfo();
