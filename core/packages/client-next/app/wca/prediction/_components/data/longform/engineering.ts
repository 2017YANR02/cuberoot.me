...
The mechanical cause of a corner twist is that the corner piece is held in place by three contact surfaces (one with each adjacent center) plus the spring tension on those centers. If the outward inertial loading on the corner during a fast turn exceeds the holding force (so the holding force cannot supply the required centripetal acceleration), the corner can slip and rotate around its body diagonal axis by 120 degrees. The introduction of magnets reduces corner twists by adding additional alignment forces, but does not eliminate them entirely. Some premium cubes (notably the GAN 12 series with what GAN calls "corner stability" geometry) include additional mechanical features specifically designed to resist corner twists, such as a small ridge on the corner that mates with a recess on the center.
