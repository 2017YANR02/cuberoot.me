 ...
 *
 * Renders the state the cube ITSELF reports, one of four ways:
 *
 ...
 *
 * The React lazy boundary lives HERE rather than in a host on purpose:
...
import {
  mirrorForBrand, sensorBasisForBrand,
 ...
 *
 * A host may explicitly enable this for development without a physical cube —
 * `window.__cuberootFakeQuat` (a Quat or a `(tMs) => Quat`) is polled at BLE
 * cadence and fed in exactly where a real sample would go. Disabled by default;
 * installed App hosts never enable it. See orientation.ts for the shape.
 */
