...
export function TimerScramblePreview({
  ...
  useEffect(() => {
    ...
    try {
      ...
      applyScramble(player, host, scramble || '', plan.cubingPuzzle);
    } catch (err) {
  ...
  useLayoutEffect(() => {
    ...
    applyScramble(player, host, scramble || '', plan?.cubingPuzzle ?? event);
  }, [Ctor, event, plan?.cubingPuzzle, scramble]);

  if (!plan) {
    return <div className={className} style={{ display: 'none' }} aria-hidden />;
  }

  if (plan.inlineSvg && !portedSvg) {
    return <div className={className} style={{ display: 'none' }} aria-hidden />;
  }
