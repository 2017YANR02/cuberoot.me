...
export function makeStickerColors(options: ResolvedCubeOptions): string[] {
  ...
  if (!stickerColors) {
    stickerColors = ([] as string[]).concat.apply(
      [],
      AllFaces.map(face => {
  ...
  let faceMappedStickers = AllFaces.reduce<{ [face: number]: string[] }>((acc, face) => {
  ...
  return ([] as string[]).concat.apply([], AllFaces.map(face => cubeData.faces[face].slice()))
}
