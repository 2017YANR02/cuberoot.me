...
import { useVizStore, MARGIN } from '../_stores/viz_store';
import { drawHistogramView } from '../_renderers/histogram_view';
import { drawLineView } from '../_renderers/line_view';
import { drawCumHistView } from '../_renderers/cumhist_view';
import { stddev } from '../_engine/kde';
import { fmtVal, isFMC, isMBLD, isHigherBetter, formatCompName } from '../_engine/data_fetch';
import { vizSurfaceBg } from '../_renderers/draw_utils';
...
export default function VizCanvas({ onStats, onRidgeHighlight }: VizCanvasProps) {
  ...
  const setupCanvas = useCallback(() => {
    ...
    // NOTE: 既看 store(逻辑尺寸)也看 canvas 元素自身像素尺寸。切走 tab 再回来时 store 的
    // cw/ch 仍是上次的值,而新挂载的 canvas 元素是默认 300×150;只看 store 会早退、画布不被
    // 定尺,内容全画到 300×150 缓冲外 = 空白。故元素尺寸不符时也要重新定尺。
    const elemSized = canvas.width === Math.round(w * dpr) && canvas.height === Math.round(h * dpr);
    if (elemSized && s.cw === w && s.ch === h) return;
    ...
    // NOTE: 只在逻辑尺寸真变化时写 store，打破 setCanvasSize→subscribe→重绘 的无限循环
    if (s.cw !== w || s.ch !== h) useVizStore.getState().setCanvasSize(w, h);
  }, []);
  ...
  useEffect(() => {
    setupCanvas();
    // NOTE: 按当前明暗主题设 canvas 调色板(必须在首帧 drawFrame 前,避免浅色下闪一帧白 ink)
    applyVizPalette();
    // NOTE: 初始化后立即绘制一帧
    drawFrame();
    ...
    // 主题切换 → 重设调色板并重绘
    const unwatchTheme = watchVizTheme(() => drawFrame());
