...
import type { KDEPoint, HistogramBin } from '../_engine/kde';
import { pickNiceStep } from '../_engine/kde';
import type { PlayerData } from '../_engine/data_fetch';
import {
  playerHSL, getShiftedHSL, isFMC, isMBLD, fmtVal,
} from '../_engine/data_fetch';

// ─── 主题感知调色板 ───
// canvas 里前景(网格/坐标/文字 = "ink")与底面色原本硬编码成白字深底。改成模块级
// 可切换:VizCanvas / RidgelineCanvas 绘制前按站点明暗主题调用 setVizPalette()。
// 暗色 = 原值(白 ink + 近黑底);浅色 = 深 ink + 浅底。所有渲染器走 ink()/vizSurfaceBg()/
// vizSurfaceTip(),不再写 rgba(255,255,255,…) / #0c0c18 字面量。
interface VizPalette { inkRGB: string; surfaceBg: string; surfaceTip: string }
const DARK_PALETTE: VizPalette = { inkRGB: '255, 255, 255', surfaceBg: '#0c0c18', surfaceTip: 'rgba(12, 12, 30, 0.88)' };
let PALETTE: VizPalette = DARK_PALETTE;
export function setVizPalette(p: VizPalette): void { PALETTE = p; }
/** 前景 ink(网格/文字/坐标),按当前主题给定 alpha。 */
export function ink(a: number): string { return `rgba(${PALETTE.inkRGB}, ${a})`; }
/** 画布清底色(KDE 重定心 / 脊线图整块底)。 */
export function vizSurfaceBg(): string { return PALETTE.surfaceBg; }
/** 悬浮卡片 / tooltip 底色。 */
export function vizSurfaceTip(): string { return PALETTE.surfaceTip; }
