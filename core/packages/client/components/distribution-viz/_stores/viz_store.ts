...
export interface VizState {
  ...
  loadSingle: (wcaId: string, eventId: string) => Promise<void>;
...
export const useVizStore = create<VizState>((set, get) => ({
  ...
  /**
   * NOTE: 只保留单个选手并在指定项目加载(person 页「动态分布」用 —— 进入 tab /
   * 切换选手时把该人设为唯一主选手,清掉之前的对比集)。
   */
  loadSingle: async (wcaId: string, eventId: string) => {
    set({ players: [], isPlaying: false, currentEventId: eventId, currentFrame: 0, maxFrame: 0 });
    const data = await fetchPlayerDataFn(wcaId, eventId);
    if (data) {
      data.colorIdx = 0;
      set({ players: [data], activePlayerIdx: 0 });
      get().rebuildAllChannels();
    }
  },
