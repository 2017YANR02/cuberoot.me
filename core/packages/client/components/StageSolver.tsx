...
export default function StageSolver({ scramble, lang, initialMethod = 'std', initialStage = 0, initialFace, onSelectionChange, initialSlot = '', onSlotChange, initialBase = '', onBaseChange, compact = false }: Props) {
  ...
  const computeAll = useCallback(async (): Promise<(number | null)[]> => {
    ...
    try {
      if (kind === 'std' && useCrRef.current) {
        ...
        const vals = await getXcrPool().solveXCrossRestrictGrid(scr, crMaskRef.current.lo, crMaskRef.current.hi, crMaxRotRef.current, engineStage);
        ...
        await ensureXCrossTables(pool, kind, engineStage);
        if (computeReq.current !== my) return result;
        await Promise.all(FACES.map(async (_f, f) => {
          try {
            const r = await pool.solveFace(scr, engineStage, f, activeMaskRef.current);
  ...
  const fetchMoves = useCallback(async (f: number) => {
    ...
    try {
      ...
      const res = kind === 'std'
        ? (useCrRef.current
          ? await getXcrPool().solveXCrossRestrictMoves(scr, f, crMaskRef.current.lo, crMaskRef.current.hi, crMaxRotRef.current, slack, cap, engineStage, combo, onPartial)
          : await pool.solveMoves(scr, engineStage, f, { extra: slack, cap, combo, mask: movesMask }, onPartial))
        : kind === 'f2leo'
