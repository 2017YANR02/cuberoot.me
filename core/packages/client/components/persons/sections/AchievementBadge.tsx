...
export function AchievementBadge({ kind, event, name, description, children, recordCount }: {
  kind: AchievementKind; event?: string; name?: string; description: string; children?: ReactNode; recordCount?: number;
}) {
  const t = useT();
  const id = useId();
  ...
  return <>
    <button ref={trigger} type="button" className="wp-achievement"
      ...
      <AchievementMedal kind={kind} event={event} recordCount={recordCount} />
