...
| 贴纸遮罩(`U:0,2;F:3-5` + 点选编辑) | facelet 级遮罩 | 同上直映层;点选编辑 UI 照搬 | ◐→**NxN ✅**(2026-07-21)。pyra/skewb/mega 直映 + 点选联动已实证。**NxN**:`engine/nxn/netIndex.ts` 的 `engineHomeSid`(纯,零依赖 → instanced.ts 可引不拖 visualcube)给每贴纸 instance 打 HOME canonical sid(`${面字母}${netIndex}`);`toEngineMask('cube')` 恒等透传(引擎 key 即 sid,无需派生表);示意导出器 instanced 分支查 `schematicInstanceKeys[i] ∈ mask.keys` 灰化。贴纸实例跟物理块走 → 键 HOME = 灰随块(piece-following,同 sr/其它拼图)。锁:net_index.test.ts 双射(每面外层 → 恰好该面全 N² sid);实证 `U:0,2;F:3-5` → 伴图灰 U 后 2 角 + F 中排 3 连 + R 无,位置全对。**sq1 待做** |
