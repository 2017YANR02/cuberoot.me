...
export interface PuzzleImageStudioProps {
   ...
   * SimPage regenerates it on camera settle (GPU depth-map path). null ⇒ fall back to
   * the PuzzleImage spec render (also the pre-first-frame placeholder).
   */
    ...
    <div className={`vc-studio vc-studio-${mode}${className ? ` ${className}` : ''}`}>
      ...
      <section className="vc-preview-wrap" ref={previewRef}>
        {/* Page mode: interactive (drag-to-rotate, paint editor). Panel mode: a passive
            mirror — the sim's own 左右 / 上下 / 透视 drive it. For the sr-puzzlegen puzzles
            (sq1/mega/pyra/skewb) that can't track the sim, SimPage feeds a live vector
            projection of the sim's own scene (livePreviewSvg), an exact zoom-crisp mirror
            of the left 3D; everything else uses PuzzleImage (also the pre-first-frame
            placeholder before the live SVG lands). */}
        {livePreviewSvg != null
