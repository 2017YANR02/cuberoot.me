...
let favListManager = function() {
    ...
    let getFavList = () => {
        ...
        // NOTE(port): SSR-safe — getInitialState() runs this during a useReducer
        // lazy-init; on the server window/localStorage don't exist, so return a
        // fresh copy of the empty initial fav list instead of touching storage.
        if (typeof window === 'undefined') {
            return initialFavList.map((c) => ({ ...c }))
        }
...
let configManager = function() {
    ...
    let getConfig = () => {
        ...
        // NOTE(port): SSR-safe — see getFavList. Return a deep copy of the
        // initial config (round-trip through the (de)serializer so Selectors stay
        // class instances) without writing to localStorage on the server.
        if (typeof window === 'undefined') {
            return deserializeConfig(serializeConfig(initialConfig)) as Config
        }
        const item = window.localStorage.getItem(key);
