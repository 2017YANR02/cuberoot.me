...
// 覆盖全部 WCA 项目:每项目按「打乱长度」分桶;222 / 金字塔 / 斜转 + 单手 / 脚拧 / 最少步 / 三盲
// 额外按「难度」(整解最优步数)分桶,难度视图显示最优等态打乱。
// plain 3x3 只取这里的长度桶(其打乱长度不定,12–23 步);3x3 的难度(变体×类型×底色)仍走
...
export interface RecentEventBuckets {
  ...
  difficulty?: {
    metric: string;                 // 'htm'(222/pyram/skewb + 单手/脚拧/最少步/三盲 整解最优步数口径)
...
export interface RecentScramblesEventsJson {
  ...
  // id -> 最优等态打乱(与原打乱同态、步数最短,其长度即难度值)。222/金字塔/斜转 + 单手/脚拧/最少步/三盲。
  // 难度视图强制显示这份;长度视图仍用 scr(那按的就是原打乱长度)。三盲为纯面转最优(相差整体旋转)。
