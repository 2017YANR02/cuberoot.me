 ...
 *   - `approx`  d = 16..20:cube20.org 只给到**两位有效数字**。估计项相加比真实尾部大 1.03%
 *               —— 那是四舍五入的必然结果,不是数据错。
 *   - d = 20 的 490,000,000 同样是估计，原文明确说 d ≥ 16 均非精确计数。
 *
 * 真实尾部总和 `GOD_TAIL_TOTAL = |G| − Σ(d ≤ 15)` 反而是**精确**的 —— 这是唯一能对 d ≥ 16
 * 说的硬话。要画占比 / 算期望的调用方走 `GOD_DIST_333_NORMALIZED`:把五个估计值等比缩到
...
export const GOD_DIST_333: GodBin[] = [
  ...
  { d: 20, count: '490000000', kind: 'approx' },
];
...
/**
 * 归一化分布:将 d=16..20 五个估计档等比缩到尾部真值，余数放入最大档 d=18。
 * d ≤ 15 的精确计数保持不变；Σ 由构造恰等于 |G|，不意味着估计档变成精确值。
 */
