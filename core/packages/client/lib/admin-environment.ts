...
export function adminEnvironment(hostname: string, device: Pick<Navigator, 'userAgent' | 'platform' | 'maxTouchPoints'>) {
  ...
  return {
    ...
    current: ['localhost', '127.0.0.1', '[::1]', 'dev.cuberoot.me', 'dev-mac-mini.cuberoot.me', 'dev-alienware.cuberoot.me', 'dev-macbook-pro.cuberoot.me'].includes(hostname) ? 'local' : 'prod',
  };
}
