...
export const PLATFORM_ROUTES: readonly PlatformRouteDefinition[] = [
  ...
  ...PLATFORM_COURSE_SECTIONS.map(section => route({ id: `course-section-${section.slug}`, pattern: `courses/:id/sections/${section.slug}`, area: 'learning', access: section.slug === 'introduction' ? 'public' : 'account', kind: 'detail', title: section.title, resource: 'courses', description: text('选择课时，开始学习。', 'Choose a lesson and start learning.') })),
  ...
  route({ id: 'online-competitions', pattern: 'events/online', area: 'discover', access: 'public', kind: 'landing', title: text('线上比赛', 'Online competitions'), description: text('选择比赛和监督场次，报名参加 CubeRoot 线上魔方比赛。', 'Choose a competition and supervised session to enter a CubeRoot online cubing competition.') }),
  route({ id: 'online-competition', pattern: 'events/online/:id', area: 'discover', access: 'public', kind: 'landing', title: text('线上比赛详情', 'Online competition details'), description: text('查看比赛项目、监督场次、报名状态和成绩。', 'View events, supervised sessions, registration status and results.') }),
  ...
  route({ id: 'account-invites', pattern: 'account/invites', area: 'account', access: 'account', kind: 'collection', title: text('兑换码', 'Redemption codes'), resource: 'account-invites', actions: ['redeem-invite'] }),
  ...
  route({ id: 'admin-invites', pattern: 'admin/invites', area: 'admin', access: 'admin', kind: 'collection', title: text('兑换码管理', 'Redemption code management'), resource: 'admin-invites', actions: ['admin-save', 'admin-delete', 'admin-invite-batch', 'admin-invite-order', 'admin-invite-revoke'] }),
  ...
  route({ id: 'admin-qr-cards', pattern: 'admin/qr/cards', area: 'admin', access: 'admin', kind: 'collection', title: text('二维码卡片', 'QR cards'), description: text('选择二维码，编辑双面卡片，然后下载 SVG 或排版到 A4 纸张。', 'Choose QR codes, edit their two-sided cards, then download SVG files or lay them out on A4 paper.'), resource: 'admin-qr' }),
