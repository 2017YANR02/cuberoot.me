 ...
 * ## 用哪个镜面
 *
 * 常说的「左右镜」(M 平面,R↔L)**不保 LSLL 定义域**:它把最后一槽从 FR 送到 FL,
 * 出来的状态压根不是 LSLL 态。要在「槽固定 FR」这套坐标里谈镜像,得挑一个**把 FR 槽
 * 钉在原地**的镜面 —— 过 FR 与 BL 两条棱的那个对角面:
 *
 *     U → U    D → D    F ↔ R    L ↔ B
 *
 * 它就是「左右镜 ∘ y 转体」(把 FL 转回 FR),即 §5.1 说的那件事:归一坐标下左右镜与
 * 前后镜给出同一个伙伴,区别只在公式落到哪个 y-view。
 *
 * ## 为什么它在 AUF 商上良定义
 *
