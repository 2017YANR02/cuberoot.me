...
// 四面体 S4 群去重到 39,035 本质状态;一次性生成静态 JSON,永不变化 → 直接 commit)。
// 主文件小(边际 + 联合 V×H + 全空间边际 + 致谢/记号),状态大文件(39,035 条)按需懒加载。
...
// 联合 V×H 表:grid[vi][hi] = (V=v[vi], H=h[hi]) 的本质状态数。
...
export interface PyramEssentialJson {
  meta: {
    ...
    god_htm: number;           // 11
    avg_h: number;             // 本质状态平均 H
    avg_v: number;             // 本质状态平均 V
  ...
  h: PyramHist;      // 本质状态(39,035)的 H 边际
  v: PyramHist;      // 本质状态的 V 边际
  joint: PyramJoint; // 本质状态的联合 V×H
  full_h: PyramHist; // 全空间(933,120)H 边际 —— 随机打乱的真实难度分布
  full_v: PyramHist; // 全空间 V 边际
}

// 状态:每条 = [idx, alg, V, H]。alg = 自产整解最优解(H 步,不含小角)。
...
// 状态存的是「解法」(还原到复原态);打乱 = 解法的逆序(金字塔无 180° 转,只翻转 ')。
