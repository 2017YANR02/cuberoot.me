 ...
 * 动画保留完整调整；顶层公式的显示、复制和 PDF 去掉收尾 U/y。补哪个调整由 `auf` 返回。
 *
...
export async function validateAlgCase(
  ...
  if (options.storedAlg && puzzle === 'sq1') {
    ...
    if (!trace.ok) {
      return {
        ...
        reason: trace.reason === 'unsliceable'
          ? tr({
              zh: `SQ1 公式第 ${trace.step ?? '?'} 步无法切片，不能入库`,
              en: `SQ1 move ${trace.step ?? '?'} cannot be sliced; this alg cannot be saved`,
            })
          : tr({
              zh: 'SQ1 公式含有无法识别的记号，不能入库',
              en: 'The SQ1 alg contains unrecognized notation and cannot be saved',
            }),
      };
    }
    if (goalKind === 'sq1-ep') {
