// /dev/components + /dev/utils registry guards. These two reference pages are
...
//
// Fix when red: register the missing hook in app/[lang]/dev/utils/_catalog.tsx,
...
import { CATALOG as UTILS } from '@/app/[lang]/dev/utils/_catalog';
...
describe('hooks/ are all registered in /dev/utils', () => {
  ...
  it('every exported hook is in the utils catalog', () => {
    ...
    expect(missing, `unregistered hooks — add to app/[lang]/dev/utils/_catalog.tsx:\n${missing.join('\n')}`).toEqual([]);
  });
...
const src = readFileSync(join(ROOT, 'app', '[lang]', 'dev', 'components', '_catalog.tsx'), 'utf8');
