// renderCubeNetSvg:引擎驱动展开图(net / wca)导出器。与 tnoodle 参照
...
import {
  renderCubeNetSvg, netFaceOffsets, NET_GAP, NET_FACE_ORDER, type NetFaceLetter,
...
describe('renderCubeNetSvg', () => {
  ...
  it('lays out 6·N² sticker rects in the URFDLB cross with viewBox 4N+5GAP × 3N+4GAP', () => {
    const N = 3;
    const svg = renderCubeNetSvg({ serialized: solvedSerialized(N), order: N, faceColors: COLORS });
  ...
  it('paints each facelet with its engine face colour (solved = 9 of each)', () => {
    const svg = renderCubeNetSvg({ serialized: solvedSerialized(3), order: 3, faceColors: COLORS });
  ...
  it('54 格逐格归属:serialize 任一格改字母 → 恰该格的 rect 换色,落在自己面的偏移处', () => {
    ...
    const base = renderCubeNetSvg({ serialized: solved, order: N, faceColors: COLORS });
    ...
    for (let g = 0; g < 6 * N * N; g++) {
      ...
      const svg = renderCubeNetSvg({ serialized: mutated, order: N, faceColors: COLORS });
      const rects = svg.match(/<rect [^>]*>/g)!;
