...
describe('detectRotations', () => {
  it('一次 y:数出来一次,名字对,时刻是新朝向第一次被采到的那一刻', () => {
    ...
    // 记谱用开始离开旧格的时刻，不能等落进新格才把 y 插到后面几手。
    expect(evs[0].startMs).toBeGreaterThan(300);
    expect(evs[0].startMs).toBeLessThan(evs[0].tMs);
...
describe('死区压缩过的录像(真编码器产出的形状)', () => {
   ...
   * 35° 的无人区),两次 blip 之间的时间差会把保持时长凑够 —— 一次纯晃动被记成转体。
   */
 ...
 *   基准取格里**最后**那一下 → 每次超前约 35°,四个 y 只数出**三**个。
 *
...
describe('连着转不漂(真 GyroRecorder 压过的流)', () => {
   ...
   * 待够 HOLD_MS」,而一次慢半转**确实**在中间那一格的 35° 带里待了一百多毫秒。
...
describe('未知的传感器姿态:消掉的和消不掉的', () => {
  ...
  it('给了 brand 就吃那张表 —— 现在全表是 Z-up,记号跟着换', () => {
    ...
    //
    // 今天表里是 `rotX270`:传感器 z 就是魔方朝上那根轴(渲染器的 y),没有别的
    // 修正 —— 曾经多出来的那 90° 偏航是开机航向,不是装配姿态,见 orientation.ts。
    // 所以一个绕**样本 y 轴**的合成动作在真机语义下根本不是 y,而是 z'。
