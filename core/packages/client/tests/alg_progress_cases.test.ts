...
const marks = (m: Record<string, { s?: 'learning' | 'mastered' }>): CaseMarks =>
...
describe('matchesFilter', () => {
  ...
  it('未学 = 没有状态', () => {
    expect(matchesFilter(base, 'none')).toBe(true);
    expect(matchesFilter({ ...base, status: 'learning' }, 'none')).toBe(false);
  });

  it('状态档精确匹配', () => {
    expect(matchesFilter({ ...base, status: 'learning' }, 'learning')).toBe(true);
    expect(matchesFilter({ ...base, status: 'mastered' }, 'learning')).toBe(false);
  });
});
...
describe('collectCases', () => {
  const src = (over: Partial<SetCaseSource> = {}): SetCaseSource => ({
    puzzle: '3x3', set: 'oll',
    marks: marks({ 'a|1': { s: 'learning' }, 'a|2': { s: 'mastered' } }),
    ...over,
  });
  ...
  it('按档拍平,墓碑不出现在任何一档', () => {
    ...
    expect(collectCases([withTomb], 'none').map(c => c.key)).toEqual(['a|9']);
  });

  it('拿不到整套 key 时无法列出未学', () => {
    // 虚拟集(LSLL):57 万 case 枚举不了,补集算不出来
    expect(collectCases([src()], 'none')).toEqual([]);
...
describe('sortCases', () => {
  const mk = (key: string, r?: SrsRec): ProgressCase =>
    ({ ps: '3x3/oll', puzzle: '3x3', set: 'oll', key, rec: r });
  ...
  it('同分不抖:全序兜底到 ps + key', () => {
    const a: ProgressCase = { ps: '3x3/pll', puzzle: '3x3', set: 'pll', key: 'b|1' };
    const b: ProgressCase = { ps: '3x3/oll', puzzle: '3x3', set: 'oll', key: 'b|1' };
...
describe('drillQueue', () => {
  const mk = (key: string, over: Partial<ProgressCase>): ProgressCase =>
    ({ ps: '3x3/oll', puzzle: '3x3', set: 'oll', key, ...over });
  ...
  it('两层:到期且忘过 → 不熟', () => {
    ...
    expect(q.map(c => c.key)).toEqual(['due-lapsed', 'shaky']);
  });
...
describe('groupByPuzzle', () => {
  it('按 puzzle 分组 —— 合练路由跨不了 puzzle', () => {
    const g = groupByPuzzle([
      { ps: '3x3/oll', puzzle: '3x3', set: 'oll', key: 'a|1' },
      { ps: 'skewb/l2l', puzzle: 'skewb', set: 'l2l', key: 'b|1' },
      { ps: '3x3/pll', puzzle: '3x3', set: 'pll', key: 'c|1' },
    ]);
