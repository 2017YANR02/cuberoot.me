...
describe('mergeMarks (LWW)', () => {
  it('local newer wins and is uploaded', () => {
    ...
    expect(toUpload).toEqual([{ k: 'T|T1', s: 'mastered', t: 200 }]);
  });
  ...
  it('local-only real mark is uploaded; local-only tombstone is not', () => {
    const local: CaseMarks = {
      ...
      'T|T2': { t: 100 }, // 墓碑:云端本来就没有,不用传
    };
    const { merged, toUpload } = mergeMarks(local, {});
    expect(Object.keys(merged).sort()).toEqual(['T|T1', 'T|T2']);
    expect(toUpload.map(i => i.k)).toEqual(['T|T1']);
  });
  ...
  it('newer local tombstone clears an older cloud mark (upload the clear)', () => {
    ...
    expect(toUpload).toEqual([{ k: 'T|T1', s: null, t: 300 }]);
  });
...
describe('summarizeMarks', () => {
  it('counts each status; tombstones and legacy star fields are ignored', () => {
    ...
    expect(summarizeMarks(marks)).toEqual({ learning: 1, mastered: 2 });
  });

  it('empty marks → all zero', () => {
    expect(summarizeMarks({})).toEqual({ learning: 0, mastered: 0 });
  });
});

describe('combineOverviews', () => {
  it('cloud wins per set; local-only sets are kept', () => {
    const cloud: MarkOverview = { '3x3/pll': { learning: 0, mastered: 5 } };
    const local: MarkOverview = {
      '3x3/pll': { learning: 3, mastered: 1 }, // 被云端覆盖
      '3x3/oll': { learning: 2, mastered: 0 }, // 云端没有 → 保留
    };
    expect(combineOverviews(cloud, local)).toEqual({
      '3x3/pll': { learning: 0, mastered: 5 },
      '3x3/oll': { learning: 2, mastered: 0 },
    });
  });
});
