// /dev/solvers fleet guard. The dashboard's NONWCA_TS array (app/[lang]/dev/
...
// app/[lang]/dev/solvers/_fleet.ts (data from the solver's file header +
// solver/NONWCA_PUZZLE_LOOP.md §1/§2); and drop it from NONWCA_TS_PLANNED.
// guard-registry: tracked at /dev/guards (app/[lang]/dev/guards/_guards.ts)
import { describe, it, expect } from 'vitest';
import { NONWCA_TS, NONWCA_TS_PLANNED } from '@/app/[lang]/dev/solvers/_fleet';
import { CSTIMER_SOLVABLE_IDS } from '@/lib/cstimer-scramble';

describe('/dev/solvers fleet stays in sync with the pure-TS non-WCA solvers', () => {
  ...
  it('NONWCA_TS event ids exactly equal CSTIMER_SOLVABLE_IDS', () => {
    ...
    expect(
      { missing, extra },
      `\nNONWCA_TS (/dev/solvers/_fleet.ts) is out of sync with CSTIMER_SOLVABLE_IDS:` +
