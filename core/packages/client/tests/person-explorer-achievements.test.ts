...
it('unlocks travel at five countries and two continents without counting duplicates or multi-country venues', () => {
  ...
  expect(get(rows.slice(0, 4), 'traveler', comps)).toEqual([]);
  expect(get(rows.slice(0, 5), 'traveler', comps).map(a => [a.count, a.tier])).toEqual([[5, 5]]);
  ...
  expect(EXPLORER_ACHIEVEMENTS.traveler.tiers).toEqual([5, 10, 20, 50]);
