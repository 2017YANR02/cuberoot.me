...
import {
  solveCuboid337,
  cuboid337Apply,
  parseCuboid337Scramble,
  cuboid337Heuristic,
  cuboid337DbStats,
  randomCuboid337Scramble,
  CUBOID337_MOVE_NAMES,
...
// We rebuild the 102-facelet permutations FROM 3D geometry here — NOT importing the solver's MOVE_PERM —
...
const N = REF_STICKERS.length; // 102
...
const refYTurn = (ly: number) => refPerm((s) => s.y === ly, (s) => [icx(cz(s.z)), s.y, icz(-cx(s.x)), s.nz, s.ny, -s.nx]);
const RU = refYTurn(6), Ru = refYTurn(5), Rd = refYTurn(1), RD = refYTurn(0);
...
// rigid apply: stop at " / " separator (the cstimer 333 suffix has no rigid realisation on a 3×3×7).
...
describe('cuboid337 reference (independent geometry)', () => {
  it('exposes the exact 16-token rigid alphabet', () => {
    expect(CUBOID337_MOVE_NAMES.length).toBe(16);
    expect([...CUBOID337_MOVE_NAMES].sort()).toEqual(
      ['B2', 'D', "D'", 'D2', 'F2', 'L2', 'R2', 'U', "U'", 'U2', 'd', "d'", 'd2', 'u', "u'", 'u2'].sort(),
    );
  });

  it('base move orders match the physics (U⁴=u⁴=d⁴=D⁴=id, all lateral²=id)', () => {
    for (const t of ['U', 'u', 'd', 'D']) expect(keyOf(refApplyTokens([t, t, t, t])), `${t}⁴`).toBe(REF_SOLVED_KEY);
    for (const t of ['R2', 'L2', 'F2', 'B2']) expect(keyOf(refApplyTokens([t, t])), `${t}²`).toBe(REF_SOLVED_KEY);
  });

  it('the 16 move effects are reproduced move-for-move by the solver apply', () => {
    const rnd = mulberry32(0x337a11);
    for (const tok of CUBOID337_MOVE_NAMES) {
      expect(Array.from(cuboid337Apply(tok))).toEqual(refApplyTokens([tok]));
      for (let t = 0; t < 20; t++) {
        const pre: string[] = []; const n = 1 + Math.floor(rnd() * 8);
        for (let i = 0; i < n; i++) pre.push(CUBOID337_MOVE_NAMES[Math.floor(rnd() * 16)]);
        expect(Array.from(cuboid337Apply([...pre, tok].join(' '))), `${tok} after ${pre.join(' ')}`).toEqual(refApplyTokens([...pre, tok]));
      }
    }
  });

  it('U/D act on the caps only and u/d on the inner slices only (disjoint y-layers)', () => {
    // U touches y=6 stickers only; u touches y=5 only; etc. — proves the slice moves are genuine.
    const touchedYs = (tok: string): Set<number> => {
      const after = refApplyTokens([tok]); const ys = new Set<number>();
      for (let i = 0; i < N; i++) if (after[i] !== REF_SOLVED[i]) ys.add(REF_STICKERS[i].y);
      return ys;
    };
    expect([...touchedYs('U')].sort((a, b) => a - b)).toEqual([6]);
    expect([...touchedYs('u')].sort((a, b) => a - b)).toEqual([5]);
    expect([...touchedYs('d')].sort((a, b) => a - b)).toEqual([1]);
    expect([...touchedYs('D')].sort((a, b) => a - b)).toEqual([0]);
  });
});

describe('cuboid337 pattern databases (measured orbit sub-spaces)', () => {
  it('the seven movable orbit DBs cover the measured full sizes, 180° subgroup sizes, and phase depths', () => {
    const { sizes, inHCounts, p2MaxDepths, intoHMaxDepths } = cuboid337DbStats();
    // full reachable sub-state count per movable orbit (under all 16 moves)
    expect(sizes).toEqual([40320, 40320, 40320, 2520, 24, 24, 24]);
    // size of the 180°-only subgroup H per orbit (phase-2 lives here)
    expect(inHCounts).toEqual([96, 576, 96, 36, 24, 24, 24]);
    // diameter of each per-orbit 180° subgroup (phase-2 admissible PDB depth)
    expect(p2MaxDepths).toEqual([4, 7, 4, 4, 4, 4, 4]);
    // max moves to enter H from any sub-state (phase-1 admissible heuristic depth)
    // orbits 0,2 (corner-type) need ≤11; orbits 1,3 (edge-type) ≤4; wings already in H (0).
    expect(intoHMaxDepths).toEqual([11, 4, 11, 4, 0, 0, 0]);
  });
});

describe('cuboid337 state-space constants', () => {
  it('locks the Schreier-Sims reachable count and the orbit-product strings (BigInt-verified)', () => {
    expect(CUBOID337_STATE_COUNT_STR).toBe('126,859,598,081,556,480,000');
    expect(CUBOID337_ORBIT_PRODUCT_STR).toBe('2,283,472,765,468,016,640,000');
    // the naive orbit product = 40320³·2520·24³, verified with BigInt; the reachable count divides it by
    // exactly 18 (the measured parity coupling).
    const product = BigInt(40320) ** 3n * 2520n * BigInt(24) ** 3n;
    const reachable = BigInt(CUBOID337_STATE_COUNT_STR.replace(/,/g, ''));
    expect(product.toString()).toBe(CUBOID337_ORBIT_PRODUCT_STR.replace(/,/g, ''));
    expect(product % reachable).toBe(0n);
    expect(product / reachable).toBe(18n);
  });
});

describe('solveCuboid337 — validity (round-trip via INDEPENDENT geometry)', () => {
  it('handles solved / empty input', () => {
    expect(solveCuboid337('')).toEqual({ solution: '', length: 0, optimal: true });
    expect(solveCuboid337("U U'")).toEqual({ solution: '', length: 0, optimal: true });
  });

  it('single-move scrambles solve optimally in one move with the inverse token', () => {
    expect(solveCuboid337("U'")).toMatchObject({ solution: 'U', optimal: true });
    expect(solveCuboid337('U')).toMatchObject({ solution: "U'", optimal: true });
    expect(solveCuboid337('u2')).toMatchObject({ solution: 'u2', optimal: true });
    expect(solveCuboid337("d'")).toMatchObject({ solution: 'd', optimal: true });
    expect(solveCuboid337('D')).toMatchObject({ solution: "D'", optimal: true });
    for (const t of ['R2', 'L2', 'F2', 'B2']) {
      const r = solveCuboid337(t);
  ...
  it('rejects invalid tokens in the cuboid part', () => {
    expect(() => solveCuboid337('U3')).toThrow();
    expect(() => parseCuboid337Scramble('Rw')).toThrow();
    expect(() => parseCuboid337Scramble("M2'")).toThrow();
    // a 90° side turn BEFORE any "/" is not a rigid 3×3×7 move → rejected
    expect(() => parseCuboid337Scramble('R')).toThrow();
    expect(() => parseCuboid337Scramble('3u')).toThrow();
  });

  it('parsing stops at the " / " separator (the 333 suffix is ignored)', () => {
    expect(parseCuboid337Scramble('U / R U F')).toEqual(parseCuboid337Scramble('U'));
    expect(Array.from(cuboid337Apply('u / R U F'))).toEqual(Array.from(cuboid337Apply('u')));
  });

  it('short random scrambles (depth ≤ 8) all solve optimally and round-trip (60 trials)', () => {
    const rnd = mulberry32(0x337bf5);
    for (let trial = 0; trial < 60; trial++) {
      const len = 1 + Math.floor(rnd() * 8);
      const scramble = randomRefScramble(len, rnd);
      const { solution, length, optimal } = solveCuboid337(scramble);
      ...
      if (length > 0) expect(cuboid337Heuristic(scramble)).toBeLessThanOrEqual(length);
    }
  });

  it('deep random states ALWAYS solve (never throw), round-trip, and stay within the hard bound (6 trials)', () => {
    const rnd = mulberry32(0x337d77);
    let solved = 0;
    for (let trial = 0; trial < 6; trial++) {
      const scramble = randomRefScramble(30 + Math.floor(rnd() * 15), rnd); // depth 30–44 (near-diameter)
      const res = solveCuboid337(scramble); // must NOT throw — there is no too-deep anymore
      const after = refApply(res.solution ? `${scramble} ${res.solution}` : scramble);
      expect(keyOf(after), `round-trip: ${scramble}`).toBe(REF_SOLVED_KEY);
      expect(res.length, `bounded: ${scramble}`).toBeLessThanOrEqual(CUBOID337_MAX_LENGTH);
      if (res.length > 0) expect(cuboid337Heuristic(scramble)).toBeLessThanOrEqual(res.length);
      solved++;
    }
...
describe('solveCuboid337 — provable optimality on the shallow ball', () => {
  it('exhaustive: every state within radius 3 solves to EXACTLY its BFS depth (optimal flag set)', () => {
    const maxDepth = 3;
    const INV = (name: string) => ({ U: "U'", "U'": 'U', u: "u'", "u'": 'u', d: "d'", "d'": 'd', D: "D'", "D'": 'D' } as Record<string, string>)[name] ?? name;
    ...
    for (const [key, depth] of dist) {
      ...
      const r = solveCuboid337(scramble);
    ...
    // radius-3 ball over the 16-move generator (U/u/d/D 90° + 4 side 180°), inverse-pruned — exact lock.
    expect(dist.size).toBe(1749);
    expect(checked).toBeGreaterThan(1740);
  });
});

describe('renderCuboid337ScrambleSvg', () => {
  const fills = (svg: string) => [...svg.matchAll(/fill="([^"]+)"/g)].map((m) => m[1]).filter((f) => f !== 'none');

  it('solved → 102 stickers, exactly 6 colors (self-proving uniform faces)', () => {
    const svg = renderCuboid337ScrambleSvg('');
    expect(fills(svg).length).toBe(102);
    expect(new Set(fills(svg)).size).toBe(6);
  });

  it('a turn breaks the solved render; round-trip restores it', () => {
    const solved = renderCuboid337ScrambleSvg('');
    expect(renderCuboid337ScrambleSvg('U')).not.toEqual(solved);
    expect(renderCuboid337ScrambleSvg('u')).not.toEqual(solved);
    expect(renderCuboid337ScrambleSvg("U' U")).toEqual(solved);
    expect(renderCuboid337ScrambleSvg('R2 R2')).toEqual(solved);
    expect(renderCuboid337ScrambleSvg("d d'")).toEqual(solved);
  });

  it('preview tracks the solver: scramble ∘ solution renders the solved net (10 random, solvable depth)', () => {
    const rnd = mulberry32(0x337c0d);
    const solved = renderCuboid337ScrambleSvg('');
    for (let trial = 0; trial < 10; trial++) {
      const scramble = randomRefScramble(1 + Math.floor(rnd() * 8), rnd);
      const { solution } = solveCuboid337(scramble);
      const combined = solution ? `${scramble} ${solution}` : scramble;
      expect(renderCuboid337ScrambleSvg(combined), `after solving "${scramble}"`).toEqual(solved);
    }
  });

  it('sticker color counts are invariant under any scramble (faithful group action)', () => {
    const rnd = mulberry32(0x337d11);
    const tally = (scr: string) => { const m = new Map<string, number>(); for (const f of fills(renderCuboid337ScrambleSvg(scr))) m.set(f, (m.get(f) ?? 0) + 1); return m; };
    const base = tally('');
...
function loadCstimer(): CstimerCtx | null {
  ...
  try {
    ...
    // 337 = `<cuboid> / ${333}` → also need min2phase + the 333 scrambler (like 335).
    ...
    console.warn('[cuboid337_solver.test] cstimer vm load failed — skipping oracle', e);
    CSTIMER = null; return null;
  }
}

describe('cstimer 337 oracle (real engine via node:vm)', () => {
  it('real 337 scrambles parse + the cuboid part round-trip solves; rigid tokens ⊆ the 16-token alphabet', () => {
    const c = loadCstimer();
    if (!c) { console.warn('[cuboid337_solver.test] cstimer unavailable — independent geometry already covered validity'); return; }
    const fn = c.scrMgr.scramblers && c.scrMgr.scramblers['337'];
    if (!fn) { console.warn('[cuboid337_solver.test] 337 scrambler unavailable'); return; }
    const alphabet = new Set(CUBOID337_MOVE_NAMES);
    let generated = 0, parseFails = 0; const tokSet = new Set<string>();
    for (let i = 0; i < 20; i++) {
      let scr: string;
      try { scr = String(fn('337', 23)).trim(); } catch { continue; }
      if (!scr) continue;
      generated++;
      // a real 337 scramble must contain the " / " separator (cuboid part + 333 part)
      ...
      // every cuboid-part token must be one of our 16 rigid moves
      let toks: number[];
      try { toks = parseCuboid337Scramble(cuboidPart); } catch { parseFails++; continue; }
    ...
    // 337 mixes caps AND slices, so a decent sample exercises >5 distinct rigid tokens.
    expect(tokSet.size).toBeGreaterThan(5);
  });

  it('EVERY real cstimer 337 scramble solves — 100% solve-rate, round-trips, bounded length', () => {
    // The prime requirement: a real cstimer state must RETURN a valid solution. We solve 8 FULL real
    // scrambles (the solver consumes the whole string, applying the rigid cuboid part), assert none throw,
    // each round-trips via INDEPENDENT geometry, and length ≤ the hard bound. (A larger ≥300 stress is run
    // locally outside the committed test; see the build report.)
    const c = loadCstimer();
    if (!c) { console.warn('[cuboid337_solver.test] cstimer unavailable — skipping real-scramble solve-rate'); return; }
    const fn = c.scrMgr.scramblers && c.scrMgr.scramblers['337'];
    if (!fn) { console.warn('[cuboid337_solver.test] 337 scrambler unavailable'); return; }
    let attempted = 0, solved = 0;
    let maxLen = 0;
    for (let i = 0; i < 8; i++) {
      const scr = String(fn('337', 30)).trim();
      if (!scr) continue;
      attempted++;
      const res = solveCuboid337(scr); // must NOT throw — solver consumes the full string (cuboid part)
      ...
      expect(res.length, `bounded real scramble: ${scr}`).toBeLessThanOrEqual(CUBOID337_MAX_LENGTH);
    ...
    expect(maxLen).toBeLessThanOrEqual(CUBOID337_MAX_LENGTH);
  });
});

describe('randomCuboid337Scramble', () => {
  it('emits only rigid alphabet tokens and stays reachable from solved', () => {
    const rnd = mulberry32(0x337e00);
    const alphabet = new Set(CUBOID337_MOVE_NAMES);
    for (let trial = 0; trial < 30; trial++) {
      const scr = randomCuboid337Scramble(40, rnd);
