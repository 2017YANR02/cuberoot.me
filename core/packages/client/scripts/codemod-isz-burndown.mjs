...
const files = dirArg
  ...
  : project.getSourceFiles().filter((f) => /client[\\/](app|components|lib|hooks)[\\/]/.test(f.getFilePath()));
...
const bump = (sf) => { const f = sf.getFilePath().replace(/.*client[\\/]/, ''); perFile[f] = (perFile[f] || 0) + 1; changed++; };
