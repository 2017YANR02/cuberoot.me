...
const requirementsPath = join(outputDir, '../../../shared/src/data/account_location_requirements.ts');
const requirements = {};
...
const requirementsSource = '// Generated by client/scripts/build-account-locations.mjs; do not edit.\n'
  + 'export const ACCOUNT_LOCATION_REQUIREMENTS: Readonly<Record<string, Readonly<Record<string, boolean>>>> = '
  + JSON.stringify(requirements, null, 2) + ';\n';
if (check) {
  if (await readFile(requirementsPath, 'utf8') !== requirementsSource) throw new Error('account location requirements are stale');
} else {
  await writeFile(requirementsPath, requirementsSource);
}
