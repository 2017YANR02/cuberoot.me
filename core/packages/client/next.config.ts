...
const nextConfig: NextConfig = {
  ...
  async rewrites() {
    return {
      ...
      afterFiles: [
        ...
        // Pick domains with LOCAL_DOMAINS=alg,wiki (comma list); every other /v1
        // endpoint still hits prod via the rule below — so local pg13 only needs the
        // seeded tables for the chosen domains. Seed + run: server/scripts/README.md.
        ...(process.env.NODE_ENV === "development"
          ? (process.env.LOCAL_DOMAINS ?? "")
              .split(",")
              .map((d) => d.trim())
              .filter(Boolean)
              .map((d) => ({
                source: `/v1/${d}/:path*`,
                destination: `http://127.0.0.1:3001/v1/${d}/:path*`,
              }))
          : []),
  ...
  async redirects() {
    return [
      ...
      { source: "/dev/ts", destination: "/dev/language/ts", permanent: true },
      { source: "/dev/rust", destination: "/dev/language/rust", permanent: true },
      { source: "/dev/go", destination: "/dev/language/go", permanent: true },
      { source: "/dev/python", destination: "/dev/language/python", permanent: true },
      { source: "/dev/c", destination: "/dev/language/c", permanent: true },
      { source: "/dev/cpp", destination: "/dev/language/cpp", permanent: true },
      { source: "/dev/zig", destination: "/dev/language/zig", permanent: true },
      { source: "/dev/swift", destination: "/dev/language/swift", permanent: true },
      { source: "/dev/kotlin", destination: "/dev/language/kotlin", permanent: true },
      { source: "/dev/java", destination: "/dev/language/java", permanent: true },
      { source: "/dev/javascript", destination: "/dev/language/javascript", permanent: true },
      { source: "/dev/mojo", destination: "/dev/language/mojo", permanent: true },
      { source: "/dev/compare", destination: "/dev/language/compare", permanent: true },
      { source: "/dev/scramble", destination: "/dev/language/scramble", permanent: true },
