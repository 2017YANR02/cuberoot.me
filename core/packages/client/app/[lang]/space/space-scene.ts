...
import { movePosition, PEDESTALS, VILLA_ROOMS, floorHeight, type Destination, type Level, type Layout, type SpaceObject, type Vec3 } from './space-state';
...
export class SpaceScene {
  ...
  private camera = new THREE.PerspectiveCamera(42, 1, 0.05, 1000);
  ...
  constructor(private host: HTMLDivElement, private callbacks: {
    ...
    new HDRLoader().load('/assets/space/v1/sunset.hdr.bin', texture => {
      if (!this.disposed) {
        ...
        this.scene.environment = this.room?.style === 'company' ? this.environment.texture : this.outdoorEnvironment.texture; generator.dispose(); this.render();
        texture.mapping = THREE.EquirectangularReflectionMapping; this.sky = texture;
        if (this.room?.style !== 'cyberpunk' && this.room?.style !== 'company') this.scene.background = texture;
    ...
    this.observer = new ResizeObserver(() => {
      ...
      this.camera.fov = ['interior', 'study', 'bedroom', 'bathroom', 'courtyard'].includes(this.currentView) ? (width < 600 ? 76 : this.room?.style === 'company' ? 65 : 58) : width < 600 ? 60 : 46;
  ...
  private render = () => {
    ...
    this.frame = requestAnimationFrame(time => {
      ...
      // One shadow update per frame, shared by the reflection and AO passes.
  ...
  private settle(entry: Entry, snap = this.snap) {
    ...
    for (const surface of this.room?.surfaces ?? []) {
      if (!surface.visible || surface.type === 'Reflector') continue;
      const bounds = new THREE.Box3().setFromObject(surface);
      if (bounds.max.y < level * 5 - 0.1 || bounds.max.y > level * 5 + 1.5) continue;
      if (position[0] >= bounds.min.x && position[0] <= bounds.max.x && position[1] >= bounds.min.z && position[1] <= bounds.max.z) height = Math.max(height, bounds.max.y);
    }
  ...
  sync(layout: Layout, selected: string | null, mode: Mode, snap: boolean, placing: boolean) {
    ...
    if (entry && !placing && mode !== 'twist') this.transform.attach(entry.root);
  ...
  private cameraChanged = () => {
    ...
    if (r && this.room?.style !== 'company' && !this.flight) {
  ...
  view(view: View) {
    ...
    const inside = ['interior', 'study', 'bedroom', 'bathroom', 'courtyard'].includes(view);
    this.scene.backgroundIntensity = 0.12;
    ...
    if (view === 'exterior' && !mobile) { position.set(23, 8, 34); target.set(-9, 3.7, -2); }
    if (view === 'exterior' && (this.room?.style === 'penthouse' || this.room?.style === 'cyberpunk')) {
      target.set(-9, -7, -2);
      position.copy(target).addScaledVector(direction.set(0.85, 0.45, 1).normalize(), mobile ? 130 : 112);
    }
  ...
  private move = (event: PointerEvent) => {
    ...
    if (this.placing) {
      this.cast(event);
      const hit = this.ray.intersectObjects([...this.surfaces, ...this.room?.surfaces ?? []].filter(o => o.visible), false)[0];
  ...
  private up = (event: PointerEvent) => {
    ...
    if (this.drag?.id === event.pointerId) {
      ...
      if (this.placing) {
        this.cast(event);
        const hit = this.ray.intersectObjects([...this.surfaces, ...this.room?.surfaces ?? []].filter(o => o.visible), false)[0];
        if (hit) this.callbacks.place(movePosition([hit.point.x, hit.point.z], this.snap), hit.point.y > 4 ? 1 : 0, this.room?.style === 'company' ? 0.06 : 1);
