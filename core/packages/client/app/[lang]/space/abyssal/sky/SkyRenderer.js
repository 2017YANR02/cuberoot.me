...
void main(){
  ...
  if (uCloudEnabled > 0.5) {
    vec4 cl = vec4(0.0);
    if (uScreenClouds) {
      vec2 texel = 1.0 / vec2(textureSize(uCloudTex, 0));
      // Spatial resolve for the fixed march jitter; no stale temporal history on orbit.
      for (int y = -1; y <= 1; y++) for (int x = -1; x <= 1; x++) {
        float weight = (x == 0 ? 2.0 : 1.0) * (y == 0 ? 2.0 : 1.0);
        cl += texture(uCloudTex, vUv + vec2(float(x), float(y)) * texel) * weight / 16.0;
      }
    } else cl = texture(uCloudEnvTex, dirToEquirect(dir));
    sky = sky * cl.a + cl.rgb;
  }
...
export class SkyRenderer {
  constructor(renderer, atmosphere, U) {
    this.U = U;
    ...
    this.bgMaterial = new THREE.RawShaderMaterial({
      ...
      uniforms: {
        ...
        uViewProjNJ: this.U.uViewProjNJ,
        uPrevViewProjNJ: this.U.uPrevViewProjNJ,
        uResolution: this.U.uResolution,
        uFogDensity: this.U.uFogDensity,
        uCloudTex: { value: null },
        uCloudEnvTex: { value: null },
        uScreenClouds: { value: false },
      },
      depthTest: false,
      depthWrite: false,
    });
    ...
    this.bgMaterial.uniforms.uCamPos = { value: new THREE.Vector3() };
    this.mesh.onBeforeRender = (_r, _s, camera) => {
      this.bgMaterial.uniforms.uScreenClouds.value = camera === this.mainCamera;
      this.bgMaterial.uniforms.uCamPos.value.setFromMatrixPosition(camera.matrixWorld);
      this.bgMaterial.uniforms.uInvViewProj.value.multiplyMatrices(camera.projectionMatrix, camera.matrixWorldInverse).invert();
    };
  ...
  dispose() { this.envRT.dispose(); this.envPass.dispose(); this.bgMaterial.dispose(); this.mesh.geometry.dispose(); this.mesh.removeFromParent(); }
