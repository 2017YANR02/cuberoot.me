...
void main(){
  float coastDepth = uSeaLevel - islandHeight(vWorldPos.xz);
  if (coastDepth < -2.) discard;
  ...
  if (uRain > 0.01) {
    ...
    if (f > 0.004) {
      ...
      N = normalize(N + vec3(dir.x, 0.0, dir.y) * ring * .28 * f);
      rainRip = ring * f;
    }
  }
  ...
  float haze = 1.-exp(-vDist * max(.000025,uFogDensity*.045));
  vec3 horizonDir = normalize(vec3(vWorldPos.x-uCamPos.x, 120., vWorldPos.z-uCamPos.z));
  vec4 ap = vec4(textureLod(uEnvMap, dirToEquirect(horizonDir), 4.).rgb * haze / max(uSunIntensity,.01), 1.-haze);
...
export class OceanMesh {
  constructor(oceanFFT, atmosphere, quality, cloudShared, U) {
    this.fft = oceanFFT;
    this.gridX = 0;
    this.gridY = 0;
    ...
    uniforms.uViewProjNJ = { value: new THREE.Matrix4() };
    uniforms.uPrevViewProjNJ = { value: new THREE.Matrix4() };
    uniforms.uInvViewProjNJ = { value: new THREE.Matrix4() };
    ...
    this.mesh.name = 'abyssal-ocean';
    const rayProjection = new THREE.PerspectiveCamera();
    this.mesh.onBeforeRender = (_renderer, _scene, camera) => {
      // Reflectors alter the near plane. Reconstruct ordinary perspective for
      // ray intersection, but keep the renderer's oblique matrix for clipping.
      rayProjection.copy(camera, false); rayProjection.updateProjectionMatrix();
      uniforms.uCamPos.value.setFromMatrixPosition(camera.matrixWorld);
      uniforms.uViewProjNJ.value.multiplyMatrices(rayProjection.projectionMatrix, camera.matrixWorldInverse);
      uniforms.uInvViewProjNJ.value.copy(uniforms.uViewProjNJ.value).invert();
      uniforms.uPrevViewProjNJ.value.copy(uniforms.uViewProjNJ.value);
    };
