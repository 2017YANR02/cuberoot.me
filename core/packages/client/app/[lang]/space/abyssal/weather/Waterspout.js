...
float funnelShape(vec3 p, float h, out float rn, out float ang, out float cascade){
  ...
  body *= 1.0 - smoothstep(0.70, 1.0, h);
...
void main(){
  ...
  for (int i = 0; i < 96; ++i) {
    ...
    if (dens > 0.002) {
      ...
      // The residential tornado sits beneath an overcast cloud deck.
      vec3 lum = sunCol * lt * phase * 0.2;
...
export class Waterspout {
  constructor(U) {
    ...
    this.mesh.onBeforeRender = (_r, _s, camera) => this.uniforms.uCamPos.value.setFromMatrixPosition(camera.matrixWorld);
  }
