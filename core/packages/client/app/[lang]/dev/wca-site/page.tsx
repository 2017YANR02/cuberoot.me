...
const HISTORY: HistoryItem[] = [
  ...
  },
  {
    ...
    zh: { title: <>仍在高速开发, cuberoot.me 收录此页</>, desc: <>本地快照里最近一条提交是 <strong>2026 年 6 月 5 日</strong>。仓库累计 <strong>约 16,071 次提交、201 位贡献者</strong>, 由 <strong>WCA 软件团队 (WST)</strong> 领衔。同月 <strong>cuberoot.me</strong> 在 <code>/dev</code> 写下这一页, 作为下游数据消费者对它的技术侧写。</> },
    en: { title: <>Still shipping fast — profiled here by cuberoot.me</>, desc: <>The latest commit in this local snapshot is <strong>5 Jun 2026</strong>. The repo has <strong>~16,071 commits from 201 contributors</strong>, led by the <strong>WCA Software Team (WST)</strong>. The same month, <strong>cuberoot.me</strong> wrote this page under <code>/dev</code> as a downstream data consumer's technical profile of it.</> },
  },
];
...
const ECO: EcoItem[] = [
  ...
  {
    href: '/dev/wca-export', internal: true,
  ...
  {
    href: '/dev/wca-rest-api', internal: true,
  ...
  {
    href: '/dev/cubingchina', internal: true,
  ...
  {
    href: '/dev', highlight: true, internal: true,
        ...
        <main id="top">
          ...
          <section className="section" id="eco">
            <header className="sec-head">
              ...
              <p className="sec-desc"><L
                zh={<>WCA 官网是<strong>上游</strong>, 一整圈东西长在它下面: <strong>GitHub</strong> 仓库、<strong>Docs</strong> 生态总览、每日<strong>数据导出</strong>、<strong>WCIF</strong> 格式、<strong>非官方 REST API</strong>、下游镜像<strong>粗饼网</strong>, 以及本站 <strong>cuberoot.me</strong>。带"本站另有一页"的可点进 <code>/dev</code> 里的对应专页。</>}
                en={<>The WCA site is <strong>upstream</strong>, with a whole ring growing beneath it: the <strong>GitHub</strong> repo, the <strong>Docs</strong> ecosystem overview, the daily <strong>data export</strong>, the <strong>WCIF</strong> format, the <strong>unofficial REST API</strong>, downstream mirror <strong>CubingChina</strong>, and this site <strong>cuberoot.me</strong>. Cards marked "own page here" link to the matching page under <code>/dev</code>.</>}
              /></p>
            </header>
            ...
            <div className="spotlight">
              ...
              <div className="spotlight-grid">
                <div>
                  ...
                  <p><L
                    zh={<><strong>和单人站的对比</strong>: 同在 <code>/dev</code> 里的<a href="/dev/cubingchina">粗饼网</a>约 94% 提交出自一个人——那是巧劲, 也是 bus factor。WCA 官网走的是另一条路: <strong>一支持续轮换的团队 + 强 CI + 严格 lint(RuboCop / overcommit)+ 高测试覆盖</strong>, 用流程把"人"这个单点风险摊薄。</>}
                    en={<><strong>Versus one-person sites</strong>: <a href="/dev/cubingchina">CubingChina</a>, also under <code>/dev</code>, has ~94% of commits from a single person — efficient, but a bus factor. The WCA site takes the other road: <strong>a rotating team + strong CI + strict lint (RuboCop / overcommit) + high test coverage</strong>, using process to dilute the single-person risk.</>}
                  /></p>
                </div>
        ...
        <footer className="footer">
          <div className="footer-grid">
            ...
            <div className="footer-col">
              ...
              <ul>
                <li><a href="/dev/wca-export"><L zh="WST 数据导出" en="WST Export" /></a></li>
                <li><a href="/dev/wcif"><L zh="WCIF 比赛格式" en="WCIF format" /></a></li>
                <li><a href="/dev/cubingchina"><L zh="粗饼网 — 下游镜像" en="CubingChina — downstream" /></a></li>
                <li><a href="/dev"><L zh="← 回 /dev" en="← back to /dev" /></a></li>
              </ul>
            </div>
