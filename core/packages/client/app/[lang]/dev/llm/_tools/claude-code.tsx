...
export const CLAUDE_CODE: StackTool = {
  ...
  history: [
    ...
    { year: '2025·11', highlight: true, zh: { title: <>Subagents (Agent tool)</>, desc: <>Agent tool 上线 —— 父会话能 spawn 子会话, 每个有独立 context window, 可以并行跑。本页 /dev/stack 整批工具数据就是 5 个 subagent 并发产出的。</> }, en: { title: <>Subagents (Agent tool)</>, desc: <>The Agent tool ships: a parent can spawn child sessions, each with its own context window, executable in parallel. The /dev/stack tool data set on this page was generated by five subagents concurrently.</> } },
  ...
  concepts: [
    ...
    { tag: 'G', zh: { title: <>Subagents (Agent tool)</>, desc: <>父会话用 Agent tool spawn 子会话, 每个独立 context window。可以串行 (链式) 或并行 (一条消息发 N 个)。本页 /dev/stack 数据就是 5 个并行 subagent 写出来的。</> }, en: { title: <>Subagents (Agent tool)</>, desc: <>The parent uses the Agent tool to spawn child sessions, each with its own context window. They can chain or run in parallel (N invocations in one message). The /dev/stack data set on this page is five parallel subagents.</> }, code: <code>{c('// parent message')}{'\n'}{f('Task')}({'{'}{'\n'}  {p('subagent_type')}: {s('"general-purpose"')},{'\n'}  {p('prompt')}: {s('"Write stack_tools/claude.tsx ..."')}{'\n'}{'}'});</code> },
  ...
  whyCards: [
    ...
    { icon: '⚙', zh: { title: <>Subagent 并行</>, desc: <>一个长任务可以拆成 5 个子会话并发跑, 每个有独立 1M context。本页 /dev/stack 的 N 个 .tsx 就是这样产出的, 总时间从串行 ~50 分钟降到 ~12 分钟。</> }, en: { title: <>Subagent parallelism</>, desc: <>A long task splits into N parallel child sessions, each with its own 1M context window. The N .tsx files of /dev/stack are produced this way — total time from ~50 minutes serial to ~12 minutes.</> }, code: <>{f('Task')} × {n('5')} → wait → merge</> },
  ...
  links: [
    { label: 'claude.com/dev', href: 'https://www.claude.com/dev' },
