        ...
        <main id="top">
          ...
          <section className="section" id="vs">
            <header className="sec-head">
              ...
              <p className="sec-desc"><L
                zh={<>跟 <a href="/dev/language/bash">bash</a> 比: 文本流 vs 对象流, 两种世界观, 不是优劣是分工 — Linux 服务器跑 bash, Windows 全栈跑 pwsh。跟 <a href="/dev/language/python">Python</a> 比: pwsh 是 shell + 语言合一, Python 是<em>不是 shell</em>的语言。跟 <strong>Nushell</strong> 比: Nushell 复用了 pwsh "<em>结构化数据进管道</em>" 的想法但用 Rust 重写、跨平台、不要 .NET。</>}
                en={<>vs <a href="/dev/language/bash">bash</a>: text streams vs object streams — two worldviews, not a winner-loser comparison but a division of labour. Linux servers run bash; the Windows stack runs pwsh. vs <a href="/dev/language/python">Python</a>: pwsh merges shell + language, Python is a language that is <em>not</em> a shell. vs <strong>Nushell</strong>: Nushell takes pwsh's "structured data in pipelines" idea and rewrites it in Rust, cross-platform, no .NET dependency.</>}
              /></p>
            </header>
        ...
        <footer className="footer">
          <div className="footer-grid">
            ...
            <div className="footer-col">
              ...
              <ul>
                <li><a href="/dev/language/bash"><L zh="Bash — 文本管道对照组" en="Bash — the text-pipeline counterpart" /></a></li>
                <li><a href="/dev/language/csharp"><L zh="C# — pwsh 的母语" en="C# — pwsh's parent language" /></a></li>
                <li><a href="/dev/language/python"><L zh="Python — 跨平台脚本备选" en="Python — cross-platform fallback" /></a></li>
                <li><a href="/dev/language/rust"><L zh="Rust — nushell 的载体" en="Rust — what powers nushell" /></a></li>
              </ul>
            </div>
