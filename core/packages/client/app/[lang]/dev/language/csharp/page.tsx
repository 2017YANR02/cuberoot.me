...
const HISTORY: HistoryItem[] = [
  ...
  },
  {
    ...
    zh: { title: <>C# 5.0 — <code>async</code>/<code>await</code></>, desc: <>C# <strong>把 async/await 第一次推到主流</strong>: 用 state machine 重写 continuation-passing, 写起来跟同步代码一样。3 年后 <a href="/dev/language/ts">TypeScript</a> 抄了它, 5 年后 JavaScript ES2017 抄了它, 之后 Python / Rust / Swift 全部跟上——<em>这是 C# 留给整个工业界最大的礼物</em>。</> },
    en: { title: <>C# 5.0 — <code>async</code>/<code>await</code></>, desc: <>C# <strong>brings async/await to the mainstream</strong>: a state-machine rewrite of continuation passing that reads like synchronous code. <a href="/dev/language/ts">TypeScript</a> copies it three years later, JavaScript (ES2017) five years later, then Python, Rust, Swift all follow — <em>this is C#'s single biggest gift to the industry</em>.</> },
  },
  {
  ...
  {
    year: '2026',
    zh: { title: <>26 岁 — top-5 全球, 企业 + 游戏 + 跨平台</>, desc: <>2026 年的 C# 现状: TIOBE / IEEE / RedMonk 长期<strong>top-5</strong>; 企业后端 (尤其北美 + 欧洲) 是稳态主力; <strong>Unity 仍是游戏行业 #1 脚本语言</strong> (大量统计长期给到 60–70% in-use rate); MAUI 让"一个 C# 写 4 个 OS"成为现实; Blazor / WASM 在前端继续上量。<em>Hejlsberg 自己 2012 已经把重心移向 <a href="/dev/language/ts">TypeScript</a>, 但 C# 的产线已经能自跑</em>。</> },
    en: { title: <>26 years in — global top-5, enterprise + games + cross-platform</>, desc: <>C# in 2026: long-time <strong>top-5</strong> on TIOBE / IEEE / RedMonk; the steady backbone of enterprise back ends, especially in North America and Europe; <strong>Unity remains the #1 scripting language in the games industry</strong> (most surveys hover at a 60–70% in-use rate); MAUI delivers "one C# codebase, four OSes"; Blazor / WASM keeps climbing on the front end. <em>Hejlsberg shifted focus to <a href="/dev/language/ts">TypeScript</a> back in 2012, but C# now sustains itself</em>.</> },
  },
];
...
const CS_CARDS: CsCard[] = [
  ...
  {
    tag: 'B',
    zh: { title: <><code>async</code> / <code>await</code> — C# 的世界级输出</>, desc: <>2012 年 C# 5 第一次把 async/await 推到工业语言。<strong>看起来同步、底下是 state machine</strong>, 异步 IO 写起来跟同步无差。<em><a href="/dev/language/ts">TypeScript</a> → JS → Python → Rust → Swift 都跟着抄</em>。</> },
    en: { title: <><code>async</code> / <code>await</code> — C#'s industry export</>, desc: <>C# 5 in 2012 brought async/await to a mainstream industrial language. <strong>It reads synchronously; underneath it's a state machine</strong>. Async I/O looks like sync I/O. <em><a href="/dev/language/ts">TypeScript</a> → JS → Python → Rust → Swift all copied it</em>.</> },
    code: (
      <code>
...
const WHY_CARDS: WhyCard[] = [
  ...
  {
    icon: '&',
    zh: { title: <>Hejlsberg 履历 — 4 门语言 4 个时代</>, desc: <><strong>Turbo Pascal (1983) → Delphi (1995) → C# (2000) → <a href="/dev/language/ts">TypeScript</a> (2012)</strong>。每一门都<strong>定义了一个时代的工具</strong>; 同一个人, 横跨 40 年的工业语言史——<em>这种连续战绩在语言设计圈是孤本</em>。</> },
    en: { title: <>Hejlsberg's track record — 4 languages, 4 eras</>, desc: <><strong>Turbo Pascal (1983) → Delphi (1995) → C# (2000) → <a href="/dev/language/ts">TypeScript</a> (2012)</strong>. Each <strong>defined its era's tooling</strong>; one person spans 40 years of industrial language history — <em>a track record without peer in the field</em>.</> },
...
const FUTURE_CARDS: FutureCard[] = [
  ...
  {
    tag: 'JAVA-RIVALRY',
    zh: { title: <>跟 Java 渐行渐远</>, body: <><p>2002 年 C# 跟 <a href="/dev/language/java">Java</a> 是镜像兄弟, 2026 年是<strong>两个完全不同的语言</strong>: C# 走 record / pattern / async / span, Java 走 virtual threads / sealed / records (晚 5 年抄到 C#)。<em>哪边赢? 取决于你看哪一面</em>: Java 的服务器侧依然大、生态依然广; C# 的语言细节依然新、工具一体化依然强。</p></> },
    en: { title: <>Drifting away from Java</>, body: <><p>In 2002 C# and <a href="/dev/language/java">Java</a> were mirror siblings; in 2026 they are <strong>two distinctly different languages</strong>. C# went all in on records, patterns, async, Span; Java's virtual threads / sealed types / records (five years late to C#) are catching up but never aligning. <em>Who wins? Depends which side you measure</em>: Java's server footprint and ecosystem are still vast; C#'s language pace and integrated tooling still lead.</p></> },
  },
  {
    tag: 'HEJLSBERG',
    zh: { title: <>Hejlsberg 自己 — 已在 TypeScript</>, body: <><p>有趣的注脚: <strong>2012 年 Hejlsberg 已经把主要精力转向了 <a href="/dev/language/ts">TypeScript</a></strong>。他在微软的位置一直是"语言架构师", C# 的演化早就交给 Mads Torgaard 主导的 LDM 团队。<em>同一个人, 同时在主导 C# 1.0 和 TypeScript 1.0 之间的 12 年——这是工业语言史上非常少见的事</em>。</p></> },
    en: { title: <>Hejlsberg himself — already on TypeScript</>, body: <><p>An interesting footnote: <strong>by 2012 Hejlsberg had moved his main focus to <a href="/dev/language/ts">TypeScript</a></strong>. His title at Microsoft is "language architect," and C#'s evolution has long been steered by the LDM team under Mads Torgaard. <em>One person stewarding both C# 1.0 and TypeScript 1.0 within twelve years is extraordinarily rare in industrial language history</em>.</p></> },
  },
];
        ...
        <main id="top">
          ...
          <section className="section" id="hejlsberg">
            ...
            <div className="trilogy">
              ...
              <div className="tri-step">
                ...
                <div className="tri-name"><a href="/dev/language/ts">TypeScript</a></div>
                <div className="tri-desc"><L
          ...
          <section className="section" id="vs">
            <header className="sec-head">
              ...
              <p className="sec-desc"><L
                zh={<>跟 <a href="/dev/language/java">Java</a> 是 25 年前的镜像兄弟, 跟 <a href="/dev/language/kotlin">Kotlin</a> 是 2010 年后的"现代 JVM 同龄人"。这里把三者放同一张表里, 看哪些一致、哪些根本两个方向。</>}
                en={<>The mirror sibling of <a href="/dev/language/java">Java</a> 25 years back, and the "modern JVM peer" of <a href="/dev/language/kotlin">Kotlin</a> from the 2010s. Putting all three in one table makes alignment versus divergence obvious.</>}
              /></p>
            </header>
        ...
        <footer className="footer">
          <div className="footer-grid">
            ...
            <div className="footer-col">
              ...
              <ul>
                <li><a href="/dev/language/ts"><L zh="TypeScript — Hejlsberg 第四语言" en="TypeScript — Hejlsberg's fourth language" /></a></li>
                <li><a href="/dev/language/java"><L zh="Java — 25 年前的镜像兄弟" en="Java — mirror sibling, 25 years on" /></a></li>
                <li><a href="/dev/language/kotlin"><L zh="Kotlin — 现代 JVM 同代" en="Kotlin — modern JVM peer" /></a></li>
                <li><a href="/dev/language/swift"><L zh="Swift — 苹果的 C# 答卷" en="Swift — Apple's answer to C#" /></a></li>
                <li><a href="/dev/language/mojo"><L zh="Mojo — AI 时代的 Hejlsberg-代" en="Mojo — the AI-era equivalent" /></a></li>
              </ul>
            </div>
