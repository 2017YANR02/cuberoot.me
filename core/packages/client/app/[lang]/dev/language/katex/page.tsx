...
const HISTORY: HistoryItem[] = [
  {
    ...
    zh: { title: <>TeX → LaTeX — KaTeX 的祖先</>, desc: <>Donald Knuth 1977 写 <strong>TeX</strong>, Leslie Lamport 1985 在 TeX 上做 <strong>LaTeX</strong> 宏包。KaTeX 实现的<em>正是 LaTeX 数学子集</em>的语法。完整故事见 <a href="/dev/language/latex">/dev/language/latex</a>。</> },
    en: { title: <>TeX → LaTeX — KaTeX's ancestors</>, desc: <>Donald Knuth wrote <strong>TeX</strong> in 1977; Leslie Lamport built <strong>LaTeX</strong> as macros on top in 1985. KaTeX implements <em>exactly the LaTeX math subset</em>. The full story lives at <a href="/dev/language/latex">/dev/language/latex</a>.</> },
  },
  {
    year: '2003',
...
const PROJECTS: Project[] = [
  ...
  },
  {
    ...
    zhNote: '上游 LaTeX · 见 /dev/language/latex', enNote: 'Upstream LaTeX · see /dev/language/latex',
        ...
        <main id="top">
          ...
          <section className="hero">
            ...
            <p className="hero-sub">
              <L
                zh={<><strong>KaTeX = 浏览器里 100× MathJax 速度的 LaTeX 数学渲染器</strong>。Khan Academy 2013 立项, 2014 开源, <em>同步</em>返回 HTML 字符串, 1ms 渲一条公式, 250KB 一次缓存。<strong>本页所有公式</strong> (包括下面这条) 都是 KaTeX 实时渲——见 <a href="/dev/language/latex">/dev/language/latex</a> 看它实现的 LaTeX 子集来自哪里。</>}
                en={<><strong>KaTeX is a LaTeX math renderer for the browser at 100× MathJax speed</strong>. Started inside Khan Academy in 2013, open-sourced in 2014. Returns an HTML string <em>synchronously</em>, renders an equation in ~1ms, caches a 250KB bundle once. <strong>Every equation on this page</strong> (the one below included) is live KaTeX — see <a href="/dev/language/latex">/dev/language/latex</a> for the LaTeX it implements a subset of.</>}
              />
            </p>
          ...
          <section className="section" id="what">
            ...
            <div className="def-grid">
              {[
                { h: <L zh="不是完整 LaTeX 引擎" en="Not a full LaTeX engine" />, tag: 'math-only', p: <L zh={<>只渲<strong>数学模式</strong>。没有 <code>\documentclass</code>、TikZ、BibTeX、页面布局。<em>这是设计选择, 不是缺陷</em>——scope 收紧才能 250KB 进浏览器。完整 LaTeX 见 <a href="/dev/language/latex">/dev/language/latex</a>。</>} en={<>Math mode only. No <code>\documentclass</code>, TikZ, BibTeX, or page layout. <em>A design choice, not a defect</em> — tight scope is the only way you get 250KB in the browser. For the full LaTeX story see <a href="/dev/language/latex">/dev/language/latex</a>.</>} /> },
          ...
          <section className="section" id="history">
            <header className="sec-head">
              ...
              <p className="sec-desc"><L
                zh={<>KaTeX 的故事可分 4 段: <strong>祖先 TeX/LaTeX</strong> (1977-2003) → <strong>jsMath / MathJax 时代</strong> (2003-2013) → <strong>Khan Academy 立项 + 开源</strong> (2013-2018) → <strong>主流落地 + AI 时代</strong> (2019-2026)。上游 LaTeX 故事完整版见 <a href="/dev/language/latex">/dev/language/latex</a>。</>}
                en={<>KaTeX's arc breaks into four phases: <strong>the TeX/LaTeX ancestors</strong> (1977–2003) → <strong>the jsMath / MathJax era</strong> (2003–2013) → <strong>Khan Academy starts it + open-sources it</strong> (2013–2018) → <strong>mainstream adoption + the AI era</strong> (2019–2026). The full upstream LaTeX story is at <a href="/dev/language/latex">/dev/language/latex</a>.</>}
              /></p>
            </header>
            ...
            <div className="bwars">
              ...
              {BENCH_TIME.map((row, i) => (
                <div className="bwars-row" key={i}>
                  <div className="bwars-yr">{tr(row)}</div>
          ...
          <section className="section" id="projects">
            <header className="sec-head">
              ...
              <p className="sec-desc"><L
                zh={<>2026 用 KaTeX 的产品名单: <strong>GitHub、Notion、Discourse、Stack Exchange、Obsidian、Substack、Khan Academy、Quora、Logseq、Hashnode、Roam Research、几乎所有静态站生成器</strong> (Hugo / Astro / Eleventy / Docusaurus / VitePress)。下面是工具 / wrapper / 上下游核心节点。上游 LaTeX 见 <a href="/dev/language/latex">/dev/language/latex</a>。</>}
                en={<>2026 KaTeX users in production: <strong>GitHub, Notion, Discourse, Stack Exchange, Obsidian, Substack, Khan Academy, Quora, Logseq, Hashnode, Roam Research, every modern static-site generator</strong> (Hugo / Astro / Eleventy / Docusaurus / VitePress). The 12 cards below cover tools, wrappers and upstream / downstream anchors. Upstream LaTeX at <a href="/dev/language/latex">/dev/language/latex</a>.</>}
              /></p>
            </header>
        ...
        <footer className="footer">
          <div className="footer-grid">
            ...
            <div className="footer-col">
              ...
              <ul>
                <li><a href="/dev/language/latex"><L zh="LaTeX — 上游语言" en="LaTeX — the upstream" /></a></li>
                <li><a href="/dev/language/html"><L zh="HTML — 渲染目标" en="HTML — the render target" /></a></li>
                <li><a href="/dev/language/css"><L zh="CSS — KaTeX 的姊妹层" en="CSS — KaTeX's sibling layer" /></a></li>
                <li><a href="/dev/language/javascript"><L zh="JavaScript — KaTeX 的宿主" en="JavaScript — KaTeX's host" /></a></li>
                <li><a href="/dev/language"><L zh="返回语言索引" en="Back to language index" /></a></li>
              </ul>
            </div>
