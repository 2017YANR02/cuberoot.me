...
export const HONO: StackTool = {
  ...
  cuberoot: {
    zh: (
      <>
        <p>本站后端 2025-03-24 从 Fastify 切到 Hono, 24 小时完成迁移。<code>apps/api</code> 整个壳 ~400 行入口 + 22 个 route handler 文件, 跑在 Node 22 LTS 上, pm2 cluster mode 单 instance。云 VM 上 nginx 把 <code>api.cuberoot.me</code> 反代到 <code>127.0.0.1:3001</code>。</p>
        ...
        <p>CORS allowlist 写在 <code>core/apps/api/src/index.ts</code>, 通过 <code>cuberoot.me</code> / <code>www.cuberoot.me</code> / <code>dev.cuberoot.me</code> 几个域。Validator 走 <code>@hono/zod-validator</code> + zod schema, 错误统一通过 <code>app.onError</code> 翻译成 <code>{`{ error: string }`}</code> JSON。session token 在 PostgreSQL 13 <code>sessions</code> 表, middleware 提早 reject 未带 cookie 的请求。</p>
        <p>客户端 fetch 全部走 <code>lib/api-base.ts</code> 的 <code>apiUrl()</code>。<code>process.env.NODE_ENV</code> 切 dev/prod API base,dev 请求由 Next rewrites 同源转发 (不能用 hostname 检查,LAN / 隧道域名都不匹配)。RPC 类型暂时没启用 — <code>typeof app</code> 在 monorepo 里通过 <code>@cuberoot/shared</code> 包导出,但 client 仍用 plain <code>fetch</code>,因为大多数端点是手写的 retry / cache 逻辑包裹,RPC 反而麻烦。</p>
      </>
    ),
    en: (
      <>
        <p>The backend swapped from Fastify to Hono on 2025-03-24, migration done in 24 hours. The <code>apps/api</code> shell is ~400 lines of entry plus 22 route handler files, running on Node 22 LTS under pm2 cluster mode, one instance. On the cloud VM, nginx reverse-proxies <code>api.cuberoot.me</code> to <code>127.0.0.1:3001</code>.</p>
        ...
        <p>The CORS allowlist lives in <code>core/apps/api/src/index.ts</code>, permitting <code>cuberoot.me</code> / <code>www.cuberoot.me</code> / <code>dev.cuberoot.me</code>. Validation goes through <code>@hono/zod-validator</code> + zod schemas, with <code>app.onError</code> translating errors uniformly into <code>{`{ error: string }`}</code> JSON. Session tokens are stored in the PostgreSQL 13 <code>sessions</code> table; middleware rejects cookie-less requests early.</p>
        <p>Client fetches all go through <code>apiUrl()</code> in <code>lib/api-base.ts</code>. <code>process.env.NODE_ENV</code> toggles dev/prod API base, with Next rewrites keeping development requests same-origin (a hostname check would miss LAN / tunnel domains). RPC types aren't enabled yet — <code>typeof app</code> can be exported via <code>@cuberoot/shared</code>, but the client still uses plain <code>fetch</code> because most endpoints wrap hand-rolled retry / cache logic that RPC just complicates.</p>
      </>
    ),
  },
  links: [
