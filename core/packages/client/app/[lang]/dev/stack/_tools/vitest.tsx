...
export const VITEST: StackTool = {
  ...
  zh: {
    ...
    heroSub: <>用独立测试配置获得 Vite 驱动的 ESM 转换和快速反馈,又不绑定网站的开发服务器与生产打包器。Anthony Fu 在 2021 年底开源,2024 年 1.0 GA,十八个月里把 Jest 的份额吃掉一大半。Vue / Nuxt 钦定,Astro / Solid 模板默认,大量 React 项目从 Jest 迁过来。</>,
    ...
    conceptsDesc: <>Jest 用户可以零成本上手:断言、mock、生命周期钩子名字一模一样。差异主要在配置 (<code>vitest.config.ts</code>, Vite 项目也可合并配置) 和 ESM 行为 (不需要 transformIgnorePatterns 这种 hack)。</>,
    whyDesc: <>选 Vitest 而不是 Jest,三件事:<strong>不用再配 ts-jest / babel-jest</strong>、<strong>原生处理 ESM + TS</strong>、<strong>watch mode 反馈极快</strong>。最近一年又加上 Browser Mode 这条 Jest 永远做不出来的路线。</>,
    adoptersTitle: '谁在用',
  ...
  en: {
    ...
    heroSub: <>Use an independent test configuration with Vite-powered ESM transforms and fast feedback, without tying the website to the same development server or production bundler. Anthony Fu open-sourced it in late 2021, shipped 1.0 GA in 2024, and over eighteen months ate a large chunk of Jest's share. Vue / Nuxt make it official; Astro / Solid templates default to it; React projects migrate over from Jest in droves.</>,
    ...
    conceptsDesc: <>Jest users are at home immediately: assertions, mocks, and lifecycle hooks share names. The differences are mostly configuration (<code>vitest.config.ts</code>, optionally merged in Vite projects) and ESM behavior (no transformIgnorePatterns hack).</>,
    whyDesc: <>Picking Vitest over Jest comes down to three things: <strong>no more ts-jest / babel-jest configuration</strong>, <strong>native ESM + TypeScript handling</strong>, and <strong>blistering watch-mode feedback</strong>. Add Browser Mode — a road Jest will never walk — and the gap keeps widening.</>,
  ...
  concepts: [
    ...
    { tag: 'F', zh: { title: <>coverage</>, desc: <>V8 或 istanbul 两种 provider。<code>--coverage</code> 一开就出。V8 快但少几个边界, istanbul 准但慢一档。</> }, en: { title: <>coverage</>, desc: <>V8 or istanbul providers. Toggle with <code>--coverage</code>. V8 is fast but misses some edges; istanbul is accurate but slower.</> }, code: <code>{c('// vitest.config.ts')}{'\n'}{p('test')}: {'{ '}{p('coverage')}: {'{ '}{p('provider')}: {s('"v8"')} {'} }'}</code> },
    { tag: 'G', zh: { title: <>workers</>, desc: <>测试默认在 worker threads 池里并行跑, CPU 满载。<code>--no-parallel</code> 退回单线程便于调试。</> }, en: { title: <>workers</>, desc: <>Tests default to a worker-thread pool, saturating CPU. <code>--no-parallel</code> falls back to single-threaded for debugging.</> }, code: <code>{c('// vitest.config.ts')}{'\n'}{p('test')}: {'{ '}{p('pool')}: {s('"threads"')} {'}'}</code> },
  ...
  whyCards: [
    { icon: '⚡', zh: { title: <>Vite 驱动的转换</>, desc: <>Vitest 内部使用 Vite 的 ESM transform,但可由独立 <code>vitest.config.ts</code> 驱动,不用让网站开发和生产构建也采用 Vite。</> }, en: { title: <>Vite-powered transforms</>, desc: <>Vitest uses Vite's ESM transform internally while allowing an independent <code>vitest.config.ts</code>; the website's development and production builds do not need to use Vite.</> }, code: <>{c('// vitest.config.ts')}{'\n'}{c('// test-only transform configuration')}</> },
  ...
  cuberoot: {
    zh: (
      <>
        <p><code>@cuberoot/client</code> 用 Vitest 跑测试。两条入口:<code>pnpm --filter @cuberoot/client test</code> 跑全集 (CI 用), <code>test:watch</code> 留 dev 用。测试有独立的 <code>vitest.config.ts</code>;它只负责测试转换, 不参与 Next 16 网站的开发服务器或生产构建。</p>
        <p>测试统一放在 <code>packages/client/tests/*.test.ts</code>, 不与源码并排 —— 避开 Next App Router 的 <code>app/</code> 路由目录, 源文件一律走 <code>@/</code> alias import (<code>vitest.config.ts</code> 里配的)。按用途分两类:</p>
        <p><strong>纯函数 / 算法回归</strong>:formatWcaResult / displayCuberName / scramble 解析 / kociemba 这类直接 import 跑断言。analyzer / scramble 这种回归用 <code>expect().toBe(具体数)</code> 锁死 baseline —— 改算法 = 主动改 baseline, 把它当一种 review signal, 而不是放宽到 <code>toBeGreaterThan</code> 蒙混。</p>
        <p><strong>worker</strong>:走 <code>tests/*.test.ts</code> + 一个配套 <code>_*_runner.cjs</code>。runner 用 <code>node:worker_threads</code> 启动, 在文件顶部 shim 浏览器 worker 的全局 (<code>self</code> / <code>postMessage</code> / <code>onmessage</code>), 然后 require 真正的 classic-worker 源文件。典型例子 <code>tests/analyzer_worker.test.ts</code> —— analyzer 在 node 端全空间枚举跑 ~225s, 占了全集 99% 的时间。所以只改它以外的东西时单跑 <code>pnpm --filter @cuberoot/client exec vitest run &lt;path&gt;</code>, 别用 <code>pnpm test -- &lt;path&gt;</code> (那个 <code>--</code> 被 pnpm 透传后会被 vitest 吞掉, 反而跑全集)。</p>
    ...
    en: (
      <>
        <p><code>@cuberoot/client</code> runs tests with Vitest. Two entry points: <code>pnpm --filter @cuberoot/client test</code> for the full suite (CI uses this), and <code>test:watch</code> for development. Tests have an independent <code>vitest.config.ts</code>; it handles test transforms only and does not participate in the Next 16 development server or production build.</p>
        <p>All tests live in <code>packages/client/tests/*.test.ts</code>, never beside the source — clear of the Next App Router <code>app/</code> directory, with source imported through the <code>@/</code> alias (configured in <code>vitest.config.ts</code>). Two flavors by purpose:</p>
        <p><strong>Pure-function / algorithm regression</strong>: formatWcaResult / displayCuberName / scramble parsing / kociemba — imported directly and asserted. Analyzer / scramble regression locks baselines with <code>expect().toBe(specific number)</code> — when the algorithm changes, change the baseline (that's a review signal), never loosen to <code>toBeGreaterThan</code>.</p>
        <p><strong>Worker</strong>: routed through <code>tests/*.test.ts</code> + a companion <code>_*_runner.cjs</code>. The runner spawns <code>node:worker_threads</code>, shims the browser worker globals (<code>self</code> / <code>postMessage</code> / <code>onmessage</code>) at the top, then requires the real classic-worker source. The canonical example is <code>tests/analyzer_worker.test.ts</code> — the analyzer enumerates the full space in node for ~225s, about 99% of the suite's runtime. So when touching anything else, run a single file with <code>pnpm --filter @cuberoot/client exec vitest run &lt;path&gt;</code> — never <code>pnpm test -- &lt;path&gt;</code> (the <code>--</code> is passed through and swallowed by vitest, running the whole suite instead).</p>
