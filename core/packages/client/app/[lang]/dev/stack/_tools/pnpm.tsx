...
export const PNPM: StackTool = {
  ...
  concepts: [
    ...
    { tag: 'F', zh: { title: <>workspace:* 协议</>, desc: <>跨包依赖写 <code>workspace:*</code>, 装包时连成 symlink, 发版时 pnpm 自动替换成真实版本号。</> }, en: { title: <>workspace:* protocol</>, desc: <>Cross-package deps spell <code>workspace:*</code> — at install time they're symlinked; at publish time pnpm rewrites them to a real version.</> }, code: <code>{c('// apps/api/package.json')}{'\n'}{p('"dependencies"')}: {'{'}{'\n'}  {p('"@cuberoot/shared"')}: {s('"workspace:*"')}{'\n'}{'}'}</code> },
  ...
  whyCards: [
    ...
    { icon: '⎇', zh: { title: <>workspace 是一等公民</>, desc: <><code>pnpm-workspace.yaml</code> + <code>--filter</code> 让 monorepo 不再需要 Lerna / Nx 当外层包装。本仓库所有活动 workspace 都走这套配置。</> }, en: { title: <>Workspaces, first-class</>, desc: <><code>pnpm-workspace.yaml</code> + <code>--filter</code> means monorepos no longer need Lerna / Nx as a wrapper. Every active workspace in this repo uses this configuration.</> }, code: <>pnpm --filter @cuberoot/client {f('dev')}{'\n'}pnpm -r {f('typecheck')}</> },
  ...
  adopters: [
    ...
    { name: 'cuberoot.me', highlight: true, zhNote: '本站 core/ 即 pnpm workspace 根, 包清单由 workspace 配置与各 package.json 定义', enNote: 'This site — core/ is the pnpm workspace root; workspace config and package manifests define the inventory' },
  ],
  outlook: [
  ...
  cuberoot: {
    zh: (
      <>
        <p>本仓库根是一个静态 HTML 集合 (来自多个 fork), 真正的开发都在 <code>core/</code> 里。<code>core/</code> 是 pnpm workspace 根, <code>pnpm-workspace.yaml</code> 动态纳入 <code>packages/*</code>；应用、共享库与离线任务分别由各自的 <code>package.json</code> 声明, 当前清单以 workspace 配置和 package manifests 为准。</p>
        ...
        <p>日常命令一律 <code>pnpm --filter @cuberoot/client …</code>,不进子目录:<code>dev</code> 起 Next.js (绑 127.0.0.1:3000),<code>typecheck</code> 走 tsgo,<code>build</code> 出 Next standalone 产物,<code>lint</code> 跑 ESLint。这些 script 由 Turbo 编排和缓存。</p>
        <p>有一条铁规:<strong><code>pnpm add</code> 必须在 <code>core/</code> 里跑</strong>,不能在仓库根。仓库根没有 <code>pnpm-workspace.yaml</code>,在那里 add 会生成错误 lockfile,CI 立刻挂。这条规则写在项目 <code>AGENTS.md</code> 里,多 AI 并行时所有 agent 都得遵守。</p>
      </>
    ),
    en: (
      <>
        <p>The repo root is a collection of static HTML from several forks; active development lives under <code>core/</code>. <code>core/</code> is the pnpm workspace root, and <code>pnpm-workspace.yaml</code> dynamically includes <code>packages/*</code>. Apps, shared libraries, and offline jobs each declare their own <code>package.json</code>; the workspace configuration and package manifests are the current inventory.</p>
        ...
        <p>Daily commands always run as <code>pnpm --filter @cuberoot/client …</code> without changing directories: <code>dev</code> starts Next.js on 127.0.0.1:3000, <code>typecheck</code> uses tsgo, <code>build</code> emits the Next standalone output, and <code>lint</code> runs ESLint. Turbo orchestrates and caches these scripts.</p>
        <p>One hard rule: <strong><code>pnpm add</code> must run inside <code>core/</code></strong>, never from the repo root. The repo root has no <code>pnpm-workspace.yaml</code>; adding there produces a wrong lockfile and CI fails immediately. The rule is in the project's <code>AGENTS.md</code> so every AI agent working in parallel respects it.</p>
      </>
    ),
  },
  links: [
