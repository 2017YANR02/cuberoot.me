...
const ENDPOINTS: Ep[] = [
  ...
  { d: 'auth', m: 'POST', p: '/v1/auth/identity/complete', g: 'public', c: 'no-store', zh: '单次核销首次认证票据；创建需明确选择，绑定需当前 Bearer 或抖音专用绑定码，并核对目标 UID', en: 'Consume a first-time identity ticket once; creation requires explicit choice, linking requires Bearer or a Douyin-specific link code and a matching target UID' },
  { d: 'auth', m: 'POST', p: '/v1/auth/identity/link-code', g: 'login', c: 'no-store', zh: '核对 expectedUid 后为当前账号生成 10 分钟抖音小程序绑定码，与合并码权限隔离', en: 'Check expectedUid and issue a 10-minute Douyin Mini Program linking code, isolated from merge codes' },
  { d: 'auth', m: 'POST', p: '/v1/auth/identity/link-code/preview', g: 'public', c: 'no-store', zh: '验证抖音待确认身份与绑定码后预览目标账号，确认前不绑定或登录', en: 'Verify a pending Douyin identity and link code to preview the target account, without linking or signing in before confirmation' },
  ...
  // ---- drive ----
  { d: 'drive', m: 'POST', p: '/v1/drive/compressions/:id/cancel', g: 'login', c: 'no-store', zh: '所有者或超级管理员取消排队、压缩或画质检查中的任务；保留原片及已完成副本', en: 'Owners or superadministrators cancel queued, encoding or validating jobs; retain the original and any completed copy' },
  ...
  { d: 'drive', m: 'PATCH', p: '/v1/drive/nodes/:id', g: 'login', c: 'no-store', zh: '重命名、移动本人项目或设置文件夹成员共享（memberShared）', en: 'Rename or move owned items, or set folder member sharing (memberShared)' },
  ...
  { d: 'drive', m: 'POST', p: '/v1/drive/files/:id/access', g: 'login', c: 'no-store', zh: '生成两小时有效的本人或成员共享文件票据；下载时重新核验成员及目录权限', en: 'Create a two-hour owned or member-shared file ticket; downloads recheck membership and folder access' },
  ...
  { d: 'timer', m: 'POST', p: '/v1/battle/rooms', g: 'public', zh: '按项目建联机对战房间；打乱由服务端共用生成器产生，并单次返回 playerId + 私有 capability + 房间码', en: 'Create an online battle room for an event; the shared server-side generator creates the scramble, and playerId, a private capability, and the room code are returned once' },
  { d: 'timer', m: 'POST', p: '/v1/battle/rooms/:code/join', g: 'public', zh: '加入对战房间；单次签发该玩家的私有 capability', en: 'Join a battle room and receive that player’s private capability once' },
  { d: 'timer', m: 'GET', p: '/v1/battle/rooms/:code', g: 'public', c: 'no-store', zh: '公开读取房态；带 pid 刷心跳时必须同时带 X-Battle-Token', en: 'Read public room state; a pid heartbeat also requires X-Battle-Token' },
  { d: 'timer', m: 'POST', p: '/v1/battle/rooms/:code/status', g: 'public', zh: '凭 X-Battle-Token 上报准备/观察/计时状态；全员准备才落同时起表时刻', en: 'Report ready/inspection/solving state with X-Battle-Token; synchronized start requires everyone ready' },
  { d: 'timer', m: 'POST', p: '/v1/battle/rooms/:code/name', g: 'public', zh: '凭 X-Battle-Token 修改自己的房内名字与 WCA 身份', en: 'Update your room name and WCA identity with X-Battle-Token' },
  { d: 'timer', m: 'POST', p: '/v1/battle/rooms/:code/event', g: 'public', zh: '凭 X-Battle-Token 在交卷前改自己的项目；缺少的本轮打乱由服务端共用生成器补齐', en: 'Change your event before submission with X-Battle-Token; the shared server-side generator fills a missing round scramble' },
  { d: 'timer', m: 'POST', p: '/v1/battle/rooms/:code/scramble', g: 'public', zh: '凭 X-Battle-Token 请求服务端 set-if-absent 生成自己当前项目的本轮打乱；不接受客户端打乱正文', en: 'Request a server-generated, set-if-absent round scramble for your current event with X-Battle-Token; client-authored scramble text is not accepted' },
  { d: 'timer', m: 'POST', p: '/v1/battle/rooms/:code/result', g: 'public', zh: '凭 X-Battle-Token 交本轮成绩；重复提交仅改罚时', en: 'Submit the round result with X-Battle-Token; resubmission only adjusts the penalty' },
  { d: 'timer', m: 'POST', p: '/v1/battle/rooms/:code/next', g: 'public', zh: '凭 X-Battle-Token 在本轮参与者全部交卷后原子结算，并用服务端共用生成器创建下一轮打乱；显式“不等了”可强制推进', en: 'Atomically settle after every round participant submits and create the next scramble with the shared server-side generator; an explicit skip-waiting action may force advancement' },
  { d: 'timer', m: 'POST', p: '/v1/battle/rooms/:code/settings', g: 'public', zh: '房主凭 X-Battle-Token 修改同时开始计时设置', en: 'Host updates synchronized-start settings with X-Battle-Token' },
  { d: 'timer', m: 'POST', p: '/v1/battle/rooms/:code/admin', g: 'public', zh: '房主凭 X-Battle-Token 转让房主', en: 'Transfer host with the current host’s X-Battle-Token' },
  { d: 'timer', m: 'POST', p: '/v1/battle/rooms/:code/kick', g: 'public', zh: '房主凭 X-Battle-Token 移出玩家，并轮换媒体房 generation 隔离旧凭证', en: 'Remove a player with the host’s X-Battle-Token and rotate the media-room generation to isolate old credentials' },
  { d: 'timer', m: 'POST', p: '/v1/battle/rooms/:code/leave', g: 'public', zh: '凭 X-Battle-Token 离开；空房即删', en: 'Leave with X-Battle-Token; an empty room is deleted' },
