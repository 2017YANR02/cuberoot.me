...
// 复用 /dev/wcif 的文档样式（.wcif-* 类）。事实据本地导入的 developer dump 实测 + 官方导出页。
  ...
  return (
    <div className="wcif-page wcif-page-wide">
      <div className="wcif-topbar">
        ...
        <AppLink href="/dev" className="wcif-back">/dev</AppLink>
        <AppLink href="/dev/wcif" className="wcif-back">WCIF</AppLink>
      </div>

      <header className="wcif-hero">
        <div className="wcif-hero-route">/dev/wca-export</div>
...
WHERE ce.competition_id = ? AND r.linked_round_id IS NOT NULL;`}</code></pre>
      ...
      <section className="wcif-section">
        ...
        <p className="wcif-note">
          {tr({
            zh: '整条流水线在 stats.yml CI 里每天 UTC 20:00 跑（跟着 WCA 上游天更），产物 commit 进 stats/，再由 sync_toolkit 同步到静态服务器。未来比赛拿不到 dump（还没结束），那部分走 WCIF 公开端点实时补 —— 见 /dev/wcif。',
            en: 'The whole pipeline runs daily at 20:00 UTC in stats.yml CI (tracking WCA upstream), commits artifacts into stats/, and sync_toolkit ships them to the static server. Upcoming comps aren’t in the dump yet, so that slice is filled live from the WCIF public endpoint — see /dev/wcif.'
        })}
        </p>
      </section>
      ...
      <footer className="wcif-foot">
        ...
        <span className="wcif-meta-sep">·</span>
        <AppLink href="/dev/wcif">{tr({ zh: 'WCIF（未来比赛实时格式）', en: 'WCIF (live upcoming format)'
        })}</AppLink>
        <span className="wcif-meta-sep">·</span>
        <AppLink href="/dev/wca-rest-api">{tr({ zh: 'WCA REST API（同源的现成接口）', en: 'WCA REST API (off-the-shelf over the same data)'
        })}</AppLink>
        <span className="wcif-meta-sep">·</span>
