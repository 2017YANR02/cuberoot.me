...
const TABLES: Table[] = [
  ...
  { name: 'role_preview_sessions', domain: 'account', origin: '0217', evolved: [245, 246], purpose: { zh: '可撤销的 30 分钟角色测试或指定用户只读查看会话，记录实际操作者与查看理由', en: 'Revocable 30-minute role-test or specific-user read-only sessions with the real actor and viewing reason' } },
  { name: 'role_preview_events', domain: 'account', origin: '0217', evolved: [246], purpose: { zh: '角色测试写入及指定用户查看中的已拦截写入审计，不记录正文或凭据', en: 'Audit role-test writes and blocked writes during specific-user viewing without bodies or credentials' } },
  // ── WCA mirror ──────────────────────────────────────────
  ...
  { name: 'battle_rooms', domain: 'studio', origin: '0086', purpose: { zh: '/timer 联机对战房间:每人可选自己项目(同项目共享打乱),当前轮各项目打乱 + 玩家实时状态 + 每轮战绩历史 + 胜场(1s 轮询 + jsonb 原子合并)', en: '/timer online battle rooms: each player picks their own event (same-event players share a scramble), per-event current scrambles + live status + per-round history + win tallies (1s polling, atomic jsonb merges)' }, cols: [
    { name: 'code (PK), revision, video_generation, round_roster, event, round', note: { zh: 'revision 拒绝旧快照；video_generation 隔离旧媒体 token；round_roster 固定同步起表参赛者', en: 'revision rejects stale snapshots; video_generation isolates old media tokens; round_roster freezes synchronized-start participants' } }, { name: 'scrambles JSONB', note: { zh: '{event:scramble} 当前轮各项目打乱(同项目共享)', en: '{event:scramble} current-round scramble per event (shared)' } }, { name: 'players JSONB', note: { zh: '{pid:{name,seen,ph,at,event}} 心跳 + 实时状态 + 所选项目', en: '{pid:{name,seen,ph,at,event}} heartbeat + live phase + chosen event' } }, { name: 'results JSONB', note: { zh: '{round:{pid:{t,p}}} 当前轮成绩', en: '{round:{pid:{t,p}}} current-round results' } }, { name: 'history JSONB', note: { zh: '[{round,scrambles,playerEvents,results,winners}] 最近 50 轮', en: '[{round,scrambles,playerEvents,results,winners}], last 50 rounds' } }, { name: 'scores JSONB' }, { name: 'player_auth JSONB', note: { zh: '{pid:SHA-256} 私有玩家 capability 的摘要；绝不进入房间状态响应', en: '{pid:SHA-256} digests of private player capabilities; never included in room-state responses' } }, { name: 'admin, sync_start, start_at', note: { zh: '房主 pid(离场回落最早加入者)+「同时开始计时」开关 + 本轮同时起表时刻', en: 'host pid (falls back to the earliest joiner) + synchronized-start toggle + this round’s shared start instant' } },
  ] },
...
const MIGRATIONS: { n: number; slug: string; desc: Bi }[] = [
  ...
  { n: 94, slug: 'lsll_cases', desc: { zh: '新表 lsll_cases:LSLL 那 148,384 个 case 的整方 HTM 最优解落库,case 页那块「待批量求解管道回填」终于有东西可显示。数据不进 migration —— 本地 solver/lsll 用 cubeopt/h48 跑完,export_cases.mjs 出 CSV,update_lsll.mts 按 sha1 行清单增量灌。关键是 exhaustive 这一列诚实:h48 的 wasm 吐不出全部最优解(它那个第三参数是「同时解几条」,不是解数上限),所以阶段 1 每个 case 只有一条最优解,qtm 是这一条的 qtm 而不是并列里最小的,前端照实说', en: 'New lsll_cases table: whole-cube HTM-optimal solutions for all 148,384 LSLL cases, so the case page’s “pending the batch pipeline” placeholder finally has something to show. The rows never ride in a migration — the local solver/lsll run (cubeopt/h48) produces them, export_cases.mjs writes a CSV, and update_lsll.mts loads only the lines whose sha1 changed. The load-bearing column is exhaustive: the h48 wasm cannot enumerate every optimal solution (its third argument is “how many at once”, not a solution cap), so stage 1 stores one solution per case and qtm is that solution’s, not the minimum across ties — and the page says so' } },
