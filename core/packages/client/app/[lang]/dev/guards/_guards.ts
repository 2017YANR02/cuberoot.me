// Data for /dev/guards, shared with tests/dev-guards-drift.test.ts (which keeps this
...
export const PAIRED_GUARDS: PairedGuard[] = [
  {
    ...
    en: { title: 'Architecture boundaries and public entry points', desc: 'One manifest records runtime, build, test, artifact, subprocess, and deployment relationships across active workspaces. The 223 legacy identities occur 238 times: removal and reduction are allowed, while additions, substitutions, and duplicate violations fail. In a new session, the project hook gives immediate feedback for visible shared root imports, private cross-app source imports, unexported subpaths, and complete subprocess calls in the added fragment; CI is the authoritative full-file check using the same AST scanner.' },
  },
  {
  ...
  },
  {
    ...
    en: { title: 'Date-input reimplementations', desc: 'Date-only values use DateInput and ranges use DateRangeInput, with fixed yyyy-mm-dd display and the site-localized calendar. Codex blocks raw type="date" and text lookalikes at write time; CI scans all app code.' },
  },
  {
...
export const CI_GUARDS_UI: CiGuard[] = [
  ...
  },
  {
    ...
    zh: { title: '公式库 case 详情另造结构或漏掉静态主图', desc: 'PG 公式库的标准 case 详情统一由 AlgCaseView 承载：CaseThumb 是稳定辨识图，AlgPlayer 固定在公式左侧；meta case 的关联图与打乱仍在播放器上方，训练弹窗保持紧凑。写入 Hook 会拦回行内播放器、条件隐藏主图和只给部分公式集启用的布局，CI 同时钉住共享路由与响应式结构。' },
    en: { title: 'Rebuilt formula-library case detail or missing identity image', desc: 'Canonical PG formula-library case details go through AlgCaseView: CaseThumb remains the identity image and AlgPlayer stays fixed beside the alg list. Meta-case relations and scramble remain above it, while training modals stay compact. The write hook blocks inline players, conditional thumbnails, and set-specific layout gates; CI pins the shared route and responsive structure.' },
  },
];
...
export const CI_GUARDS_DRIFT: CiGuard[] = [
  {
    id: 'catalog-sync',
    test: 'dev-catalog-sync.test.ts',
    zh: { title: '/dev/components + /dev/utils 登记表漂移', desc: 'hooks/ 里每个导出的 use* hook 必须在 /dev/utils 登记表里出现;两个登记表写的 import 路径必须在磁盘上真实存在。漏登记或路径改名各自直接红。' },
    en: { title: '/dev/components + /dev/utils registry drift', desc: 'Every exported use* hook in hooks/ must appear in the /dev/utils catalog; every import path either catalog references must resolve on disk. Forgetting to register or a stale renamed path both turn CI red.' },
  },
  {
    id: 'tokens-drift',
    test: 'dev-tokens-drift.test.ts',
    zh: { title: '/dev/tokens 数值漂移', desc: '/dev/tokens 页面手工抄了 globals.css 的颜色值做展示,这条测试逐条重新对比 —— globals.css 改了令牌却忘了同步页面,直接红。' },
    en: { title: '/dev/tokens value drift', desc: '/dev/tokens hand-mirrors color values from globals.css for display. This re-diffs every value — change a token in globals.css without updating the page and CI goes red.' },
  },
  {
  ...
  },
  {
    ...
    en: { title: '/dev/solvers fleet table drift', desc: '/dev/solvers’ NONWCA_TS table must exactly equal CSTIMER_SOLVABLE_IDS (the real "already solvable" set); the not-yet-built PLANNED list must be disjoint from it.' },
  },
  {
  ...
  {
    id: 'guards-drift',
    test: 'dev-guards-drift.test.ts',
    zh: { title: '/dev/guards 自身漂移', desc: '这页也是一份手工快照,所以也有自己的漂移守卫:每个带 guard-registry 标记注释的 CI 测试必须在这页列出,这页列出的每个测试必须真的存在且带标记。新增一对守卫忘了登记 → 直接红。' },
    en: { title: '/dev/guards self-drift', desc: 'This page is itself a hand-maintained snapshot, so it gets its own drift guard: every CI test carrying a guard-registry marker comment must be listed here, and every test listed here must actually exist and carry that marker. Add a new guard pair and forget to register it → CI red.' },
  },
];
...
export const CI_GUARDS_API: CiGuard[] = [
  ...
  },
  {
    ...
    zh: { title: '会议码统一为 4 位数字', desc: '/meet 的客户端归一与校验、服务端分配与校验必须都只接受正好 4 位数字,包括带前导零的 0000。守卫从服务端源码读取正则,钉住客户端字符表与码长,并覆盖裸码、邀请链接和无 room 参数链接的粘贴路径。新建会议由服务端避开活跃房与刚分配的号码;短码便于口述,但不应当作秘密。' },
    en: { title: 'Meeting codes stay four-digit numeric', desc: 'The /meet client normalizer and validator and the server allocator and validator must all accept exactly four decimal digits, including leading-zero codes such as 0000. The guard reads the server regex from source, pins the client alphabet and length, and covers pasted bare codes, invite links, and links without a room parameter. New meetings are allocated server-side around active and recently issued codes; the short code is easy to share but is not a secret.' },
  },
  {
