...
// /dev/guards — 汇总散落仓库各处的「写入即拦」PreToolUse hook + CI 棘轮测试。
// 自包含静态内容,跟 /dev 系其他页独立设计(art-directed dark,guard-red 主题)。
// 数据在 ./_guards.ts(与 tests/dev-guards-drift.test.ts 共享,改这页先改那份数据)。
    ...
    <div className="gd-page">
      ...
      <div className="gd-shell">
        <div className="gd-topbar">
          <Link href="/dev" className="gd-back">← /dev</Link>
        </div>
        ...
        <section className="gd-section">
          ...
          <p className="gd-ci-group-note">
            {tr({
              zh: '/dev 下好几页是手工镜像真实源码状态的快照(这页本身也是一个)—— 这组测试逐条重新对比,源码改了快照忘同步就直接红。',
              en: 'Several /dev pages are hand-maintained mirrors of real source state (this page is one too) — these tests re-diff every claim and turn red the moment the source moves but the snapshot doesn’t.',
            })}
          </p>
          <div className="gd-ci-list">
          ...
          <p className="gd-ci-aside">
            ...
            <Link href="/dev/dead-code">/dev/dead-code</Link>。
          </p>
        </section>
        ...
        <footer className="gd-foot">
          ...
          <Link href="/dev" className="gd-foot-link">/dev</Link>
        </footer>
      </div>
    </div>
  );
