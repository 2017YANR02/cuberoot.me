...
// /dev/utils 这页直接读这份 CATALOG 渲染。让下一个人 / 下一个 AI 写新代码前
...
export const CATALOG: UtilEntry[] = [
  ...
  },
  {
    ...
    category: 'cube',
    zh: '魔方图片状态(ImageSpec)的 URL 编解码,单一来源。只写非默认值(干净页面就一个 ?pzl=3),prefix 给每个 key 加前缀,好让已经占了 puzzle/alg/view 的宿主(/sim)把图片状态挂到 img_* 下不撞车。prefix 为 \'\' 时就是 /v1/visualcube.svg 认的那套裸 key(含只读旧别名 puzzle=)。',
    en: 'Single source for the URL codec of a puzzle-image state (ImageSpec). Emits only non-default values (a pristine page is just ?pzl=3); `prefix` namespaces every key so a host that already owns puzzle/alg/view (/sim) can mount the image state under img_*. At prefix \'\' it is the bare key set the /v1/visualcube.svg endpoint speaks, legacy read-only `puzzle=` alias included.',
  },
  {
    name: 'renderSpecSvg',
