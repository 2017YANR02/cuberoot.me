...
/* auth-doc-review
{"fingerprint": "6807e17d36ddbcf8c13f359bdac8fece74c04735cf0f70735078221cc809c1fc", "reason": "复核抖音网站扫码、小程序登录、绑定和解绑：两端分别取同主体 AlliedID 识别同一用户，历史小程序身份在再次登录后补齐跨应用别名；不同账号持有相同标识时拒绝合并，解绑抖音时清除全部别名。六位绑定码仍限制尝试次数。并核对最新 App.tsx 变动仅涉及桌面智能魔方设备扫描，不改变安装版登录流程；微信及其他登录方式不变。本次短信改动仅将阿里云标准短信发送实现换为固定版本共享传输包，本站仍负责验证码生成、冷却、存储与核销，手机号登录及绑定流程不变；供应商受理不代表手机实际送达。本次 index.ts 增加比赛数据访问凭证校验：只保护 cubing-live 与直播流，CN 来源由 nginx 识别后豁免；凭证不包含账号身份，不修改登录、绑定、合并、退出、注销或角色预览的认证规则，因此账号流程图保持不变。本次把比赛自动验证改为手动输入图片验证码，新增获取与一次性核销端点；仅签发比赛访问凭证，账号流程与权限不变。"}
*/
...
export default function AuthFlowPage() {
  ...
  return <main className="auth-map-page">
    <header className="auth-map-header">
      ...
      <p className="auth-map-note">{t('比赛页的图片验证码只授予短时数据访问权限，不代表登录，不授予账号或管理员权限；账号退出与账号会话仍按下图处理。', 'Competition image verification grants only short-lived data access. It does not sign in a user or grant account or administrator permissions; account sessions and sign-out follow the flows below.')}</p>
