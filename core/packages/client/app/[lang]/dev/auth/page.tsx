...
export default function AuthFlowPage() {
  ...
  return <main className="auth-map-page">
    <header className="auth-map-header">
      ...
      <p className="auth-map-note">{t('全站图片验证码只授予短时页面及受保护数据访问权限，不代表登录，不授予账号或管理员权限；第三方登录回调保留通路，账号退出与账号会话仍按下图处理。', 'Site image verification grants only short-lived page and protected data access. It does not sign in a user or grant account or administrator permissions; OAuth callbacks remain reachable, and account sessions and sign-out follow the flows below.')}</p>
