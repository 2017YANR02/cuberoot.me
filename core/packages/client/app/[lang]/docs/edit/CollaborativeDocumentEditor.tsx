...
export function CollaborativeDocumentEditor({ session, details, title, onError, onShare, onSubscriptionChange }: {
  ...
  return <>
    ...
    <div className="doc-toolbar" aria-label={tr({ zh: '格式工具栏', en: 'Formatting toolbar' })}>
      ...
      <label className="doc-color-tool" title={tr({ zh: '文字颜色', en: 'Text color' })}><span>A</span><input type="color" disabled={!canEdit} value={editor.getAttributes('textStyle').color || '#111111'} onChange={(event) => editor.chain().focus().setColor(event.target.value).run()} aria-label={tr({ zh: '文字颜色', en: 'Text color' })} /></label><span className="doc-tool-separator" />
    ...
    {searchOpen && <div className="doc-searchbar"><Search size={16} /><input value={searchText} onChange={(event) => setSearchText(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') findNext(); }} placeholder={tr({ zh: '查找', en: 'Find' })} autoFocus /><input value={replaceText} onChange={(event) => setReplaceText(event.target.value)} placeholder={tr({ zh: '替换为', en: 'Replace with' })} /><button type="button" onClick={findNext}><T zh="下一个" en="Next" /></button><button type="button" disabled={!canEdit} onClick={replaceCurrent}><T zh="替换" en="Replace" /></button><button type="button" disabled={!canEdit} onClick={replaceAll}><T zh="全部替换" en="Replace all" /></button><ClearButton variant="standalone" ariaLabel={tr({ zh: '关闭查找', en: 'Close find' })} onClick={() => setSearchOpen(false)} /></div>}
