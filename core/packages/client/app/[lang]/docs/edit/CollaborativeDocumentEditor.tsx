...
export function CollaborativeDocumentEditor({ session, details, title, onError, onShare, onSubscriptionChange }: {
  ...
  const addActivity = useCallback((kind: DocumentActivity['kind'], summary: string) => {
    if (readOnly) return;
    activityArray.push([{ id: nextDocumentRecordId('activity'), authorKey: ownerKey, authorName, authorUserId, kind, summary, createdAt: Date.now() }]);
  }, [activityArray, authorName, authorUserId, ownerKey, readOnly]);
  ...
  useEffect(() => {
    ...
    const range = editorRanges(editor, searchText)[0];
    // 保留搜索框焦点,用户可以继续输入;不触发任何替换。
    if (range) editor.chain().setTextSelection(range).scrollIntoView().run();
  }, [editor, searchOpen, searchText]);
  ...
  const addComment = () => {
    ...
    const item: DocumentComment = { id: nextDocumentRecordId('comment'), authorKey: ownerKey, authorName, authorUserId, body, createdAt: Date.now(), resolvedAt: null, anchor, replies: [] };
  ...
  const reply = (comment: DocumentComment) => {
    ...
    commentMap.set(comment.id, { ...comment, replies: [...comment.replies, { id: nextDocumentRecordId('reply'), authorKey: ownerKey, authorName, authorUserId, body, createdAt: Date.now() }] });
  ...
  const addSuggestion = () => {
    ...
    const item: DocumentSuggestion = {
      id: nextDocumentRecordId('suggestion'), authorKey: ownerKey, authorName, authorUserId, anchor, beforeText, replacement,
