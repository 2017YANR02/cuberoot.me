...
function DirectoryEntry({ entry, canEdit }: { entry: TeacherDirectoryEntry; canEdit: boolean }) {
  ...
  return (
    <article className="directory-entry">
      <div className="directory-entry-heading">
        ...
        {canEdit && <AppLink href={`/teachers/edit?id=${entry.id}`} prefetch={false} className="directory-edit-button"><Pencil size={15} />{tr({ zh: '编辑', en: 'Edit' })}</AppLink>}
      </div>
  ...
  return (
    <main className="teachers-page">
      ...
      <section className="directory-list" aria-label={tr({ zh: '老师与机构名录', en: 'Teacher and school directory' })}>
        ...
        {filtered.map((entry) => <DirectoryEntry key={entry.id} entry={entry} canEdit={isAdmin || myIds.has(entry.id)} />)}
      </section>
    </main>
  );
}
