    ...
    <div className="live-guide-layout">
      ...
      <article className="live-guide-body">
        ...
        <section id="review" tabIndex={-1}>
          ...
          <div className="live-guide-copy-row"><h3>{tr({ zh: '复盘记录模板', en: 'Session review template' })}</h3><button type="button" onClick={() => copy(tr(reviewTemplate))}>{copied ? <Check size={16} aria-hidden /> : <Copy size={16} aria-hidden />}<span aria-live="polite">{copied ? tr({ zh: '已复制', en: 'Copied' }) : tr({ zh: '复制模板', en: 'Copy template' })}</span></button></div>
