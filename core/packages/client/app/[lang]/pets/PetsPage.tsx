  ...
  return <main className="pets-page">
    <header className="pets-header">
      ...
      <nav aria-label={tr({zh:'宠物导航',en:'Pet navigation'})}>
        <AppLink href={`/pets?pet=${pet?.id ?? 'rootbeast'}`} prefetch={false} aria-current={!gallery?'page':undefined}><Heart size={15}/>{tr({zh:'领养与陪伴',en:'Adopt & care'})}</AppLink>
        <AppLink href={`/pets/gallery?pet=${pet?.id ?? 'rootbeast'}`} prefetch={false} aria-current={gallery?'page':undefined}><Sparkles size={15}/>{tr({zh:'宠物图鉴',en:'Gallery'})}</AppLink>
        <button type="button" aria-label={tr({zh:'分享',en:'Share'})} onClick={()=>void share()} disabled={!pet}>{shareState==='copied'?<Check size={16}/>:<Share2 size={16}/>}<span role="status">{shareState==='copied'?tr({zh:'已复制',en:'Copied'}):tr({zh:'分享',en:'Share'})}</span></button>
      </nav>
    </header>
    ...
    {shareState==='failed' && <label className="pets-share-fallback">{tr({zh:'复制这个链接分享',en:'Copy this link to share'})}<input readOnly value={shareUrl} onFocus={e=>e.currentTarget.select()}/></label>}
    {failed ? <div className="pets-empty" role="alert"><p>{tr({zh:'小伙伴暂时没能赶来。',en:'Our little friends could not load.'})}</p><button className="pet-primary" onClick={()=>setRetry(v=>v+1)} type="button">{tr({zh:'再试一次',en:'Try again'})}</button></div>
