...
- [ ] **批量求解管道**(本地,solver/ 大表,≤14 线程):
      **范围 = 两步可达那批,不做全量**(用户 2026-07-26 定):302 × 494 = 149,188 条路线,
      去重后 **148,384 个 canonical key**(804 条重复见上节),占全量 583,284 的 25.4%;
      其余 434,900 个 case 不算,case 页照旧诚实占位。
      每 case 全部 HTM 最优解 + 加深到 ≥100 条候选(自适应 opt+n,上限 opt+4;
      13.34^n 增长)。先 PoC 实测 100 case 单例耗时再外推,决定分批节奏。
      引擎口子已就位:`lsll_solver::enumerate(st, extra, cap, node_cap)` 返回
      `Enumeration { best, sols, nodes, hit_cap, hit_node_cap }`,**落库前先看 `complete()`** ——
      截断的解集是按 move 序的偏样本,当完整集喂 MCC 会有偏(2026-07-27 修,见下)。
      **进度显示**(用户要求):单行原地刷新(`\r`,~200ms 一次)报 已完成/总数、%、
      case/s、ETA、截断计数;里程碑(每批 / 每 1%)才打**换行**的持久行;禁每 case 一行刷屏。
      非 TTY(重定向到日志)用 `IsTerminal` 判掉 `\r`,退化成周期性单行。14 线程并发,
      进度必须走单一 reporter(原子计数 + 一个打印线程),别让 worker 各印各的交错。
...
**旧估(0.1–2s/case、全量 1–24 小时)作废** —— 那是纯外推,和实测差 3–5 个数量级。

2026-07-27 实测(release 单线程,本机,`lsll_solver` 现有 4 张投影 PDB):
...
- **候选数比预想少得多**:opt+2 只有 8 / 204 条,离 4096 的 cap 很远 —— cap 不是瓶颈。
  「一般 opt+2 即 ≥100」对深 case 成立(T-perm 204),对浅 case 不成立(Sune 才 8)。
- **节点数每深 1 步 ×10.5**(两点实测:7→11 步、opt+2 从 66,794 涨到 826M,×12,370)。
  LSLL 峰值 12–14 步,外推单 case opt+2 ≈ **40 分钟 – 7 小时**。148,384 个 ÷ 14 线程
  = **年**级别。**当前引擎跑不了这条管道**,不是分批能解决的。
- 卡点是启发式太弱(4 张独立投影取 max,11 步最优就要 8.7M 节点)。要跑通得先换刀:
  更强的剪枝表(issue #40 T6「LSLL 大剪枝表」正是这件事)/ 复用站内已有的三阶最优求解器 /
  或把「每 case ≥100 候选」降成「每 case 全部最优解」(T-perm 20 条、1.7s,量级上可行)。
  先定这个再谈跑批。
