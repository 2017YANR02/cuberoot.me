...
- [ ] **批量求解管道**(本地,≤14 线程)。**引擎 = cubeopt/h48 `cube48opt9` + 15.6G 表**
      ...
      **范围**:整个空间 583,284,**分两批**跑(用户 2026-07-27 定:先 14 万,再 43 万):
      第一批 = 302 × 494 = 149,188 条路线去重后的 **148,384 个 canonical key**(804 条重复见上节);
      第二批 = 其余 **434,900 个**(含 O 类那 3,916 个纯顶层,页面不列但定位功能会撞上)。
      两批同一个 `out.csv`、按 key 续跑,谁先谁后都行。第二批**不比第一批难**(各 300 条抽样:
      HTM 均 14.13 vs 14.07,h6 表 21.8 vs 19.7 解/s ⇒ 5.5h vs 2.1h)。
      ...
      **阶段 1 已落地**(2026-07-27):`solver/lsll/`(runbook 见其 `README.md`)。
      语料两个文件都由 `scripts/lsll-corpus.mts` 一次生成:`corpus.txt` 148,384 行 —— 只两阶段解
      302+494=796 个基件,路线打乱靠 `composeState(zbll, zbls) ⇒ setup(zbll)+setup(zbls)` 拼接,
      149,188 条逐条回放校验;`corpus_rest.txt` 434,900 行 —— 拼不出来,但也不用逐个跑两阶段
