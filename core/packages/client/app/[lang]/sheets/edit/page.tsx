...
function SpreadsheetGrid({ session, readOnly, activeSheetId, onActiveSheet, onError }: {
  ...
  return <div className="sheet-editor-area">
    <div className="sheet-toolbar">
      ...
      <label className="sheet-color-label"><T zh="填充" en="Fill" /><input className="sheet-color-input" type="color" value={selectedStyle.fill || '#ffffff'} disabled={readOnly} onChange={(event) => patchStyle({ fill: event.target.value })} /></label>
      <label className="sheet-color-label"><T zh="文字" en="Text" /><input className="sheet-color-input" type="color" value={selectedStyle.color || '#111111'} disabled={readOnly} onChange={(event) => patchStyle({ color: event.target.value })} /></label>
