    ...
    <form className="org-form" onSubmit={submit} onChange={() => { operationKey.reset(); setMessage(''); }}>
      <fieldset disabled={submitting || versions.length === 0}>
        <label>{t('模板版本', 'Template version')}
          <select name="templateVersionId" defaultValue={selectedVersion?.id ?? ''} required>
            {versions.map((version) => <option key={version.id} value={version.id}>v{version.versionNumber} {version.title}</option>)}
          </select>
        </label>
        <label>{t('任务标题', 'Assignment title')}<input name="title" required maxLength={160} defaultValue={initial?.assignment.title ?? ''} /></label>
        <label className="org-field-wide">{t('任务说明', 'Instructions')}<textarea name="instructions" required maxLength={8_000} defaultValue={initial?.assignment.instructions ?? ''} /></label>
        <label>{t('频率', 'Schedule')}
          <select name="scheduleKind" defaultValue={initial?.assignment.scheduleKind ?? 'once'}>
            <option value="once">{t('一次性', 'Once')}</option>
            <option value="daily">{t('每天', 'Daily')}</option>
          </select>
        </label>
        <label>{t('期望次数', 'Expected count')}<input name="expectedCount" type="number" min={1} max={1_000} step={1} required defaultValue={initial?.assignment.expectedCount ?? 1} /></label>
        <label>{t('开始时间', 'Starts at')}<input name="startsAt" type="datetime-local" required defaultValue={localDateTime(initial?.assignment.startsAt)} /></label>
        <label>{t('结束时间（可留空）', 'Ends at (optional)')}<input name="endsAt" type="datetime-local" defaultValue={localDateTime(initial?.assignment.endsAt)} /></label>
        <label>{t('班级目标（可多选）', 'Class targets (multiple)')}
          <select name="groupIds" multiple size={Math.min(6, Math.max(3, groups.length))} defaultValue={initialGroupIds}>
        ...
        <label>{t('个别学员（可多选）', 'Individual students (multiple)')}
          <select name="studentIds" multiple size={Math.min(6, Math.max(3, students.length))} defaultValue={initialStudentIds}>
        ...
        <label>{t('证据条数目标（可留空）', 'Evidence goal (optional)')}<input name="evidenceCount" type="number" min={1} step={1} defaultValue={goal('evidence_count') ?? ''} /></label>
        <label>{t('训练分钟目标（可留空）', 'Minutes goal (optional)')}<input name="durationMinutes" type="number" min={1} step={1} defaultValue={goal('duration_ms') ? Math.round(goal('duration_ms')! / 60_000) : ''} /></label>
        <label>{t('成功次数目标（可留空）', 'Success goal (optional)')}<input name="successCount" type="number" min={1} step={1} defaultValue={goal('success_count') ?? ''} /></label>
        <label>{t('最佳成绩秒数（仅计时器）', 'Best seconds (timer only)')}<input name="bestResultSeconds" type="number" min="0.001" step="0.001" defaultValue={goal('best_result_ms') ? goal('best_result_ms')! / 1_000 : ''} /></label>
        <div className="org-form-actions"><button type="submit">{submitting ? t('保存中…', 'Saving…') : initial ? t('保存草稿', 'Save draft') : t('创建任务', 'Create assignment')}</button></div>
      </fieldset>
