...
type SolveState =
  | { kind: 'idle' }
  | { kind: 'solving' }
  | { kind: 'done'; result: Sia123Solution }
  | { kind: 'error'; message: string; tableError?: boolean };

export default function Sia123SolverPage() {
  useDocumentTitle('联体 1×2×3 求解器', 'Siamese 1×2×3 Solver');

  const [scramble, setScramble] = useQueryState('scramble', parseAsString.withDefault(''));
  const [mode, setMode] = useQueryState('mode', parseAsStringEnum(['single', 'batch'] as const).withDefault('single'));
  ...
  const randomScramble = async () => {
    ...
    setGenerating(true);
    try {
      const s = await cstimerScramble('sia123');
  ...
  const batchSpec: BatchSpec = useMemo(() => ({
    event: 'sia123',
    metricLabel: 'moves',
    placeholder: { zh: "每行一条打乱,如 U R r' z2 U2 R r", en: "one scramble per line, e.g. U R r' z2 U2 R r" },
    validate: (line) => badTokenIn(line),
    solveOne: async (s) => { const o = await solveSia123(s); return { len: o.length, solution: o.solution }; },
  ...
  return (
    <div className="pos-page">
      <SolveTabs puzzle="sia123" mode="solve" />
        ...
        <>
          ...
          <div className="pos-input-row">
            <SearchInput
              ...
              placeholder={tr({ zh: "输入打乱,如 U R r' z2 U2 R r", en: "Enter a scramble, e.g. U R r' z2 U2 R r" })}
              spellCheck={false}
              autoComplete="off"
              autoCapitalize="off"
            />
            ...
            <div className="pos-result" aria-live="polite">
              {state.kind !== 'error' && (
                <div className="ivy-preview">
                  <ScramblePreview2D event="sia123" scramble={trimmed} size={64} />
                </div>
              )}
              {state.kind === 'solving' && (
                <p className="pos-result-solved">
                  <LoaderCircle size={16} className="pos-spin" aria-hidden style={{ verticalAlign: '-3px', marginRight: 6 }} />
                  {tr({ zh: '求解中(首次会加载剪枝表)…', en: 'Solving (the first call loads the databases)…' })}
                </p>
              )}
              ...
              {state.kind === 'error' && !state.tableError && (
                <p className="pos-error">
                  {tr({ zh: '打乱记号无法识别(应为 U/R/r 招式,中间 z2 分隔两个立方)', en: 'Unrecognized notation (expected U/R/r moves with a z2 separator)' })}: <code>{state.message}</code>
                </p>
              )}
