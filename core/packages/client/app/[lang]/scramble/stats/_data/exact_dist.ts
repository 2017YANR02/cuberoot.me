 ...
 * `variants[*].stages` 逐字相同,可与经验分布直接对照；另含尚未单独灌注的 UI 别名。
 */
 ...
 *
 * 这也是伪十字**不**在这条路上的原因,虽然它长得和十字一模一样:它单面最深 8 步、六色底最深
 * 却是 7 步,于是「取最优 = 7」只意味着每个面都 ≥ 7,里面混着某些面是 8 的状态 —— 逐面取
 * 「正好 7 步那一层」求交会把它们漏掉。标准十字两边都是 8,才轮得到这句话成立。
 */
