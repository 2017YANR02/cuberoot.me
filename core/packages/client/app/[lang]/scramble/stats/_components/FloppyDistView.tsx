...
// 1×3×3 花型(Floppy)整解最优步数分布 —— 理论全空间(全 192 态,精确枚举,非抽样)。
// 数据 = lib/floppy-solver 的 FLOPPY_LENGTH_DISTRIBUTION(与求解器页同一份),示例由
// floppyExamplesByLength 枚举每个步数档的真实状态、反推最短打乱生成(Floppy 不是 WCA 项目,
// 没有比赛语料,但全状态空间可枚举)。
...
import {
  FLOPPY_LENGTH_DISTRIBUTION, FLOPPY_GODS_NUMBER, floppyExamplesByLength, floppyAllScramblesByLength,
} from '@/lib/floppy-solver';
import { tr } from '@/i18n/tr';

const FLOPPY_COLOR = '#3a86ff'; // 长方体蓝(数据色,非 UI 灰阶)
...
export default function FloppyDistView({ isZh }: { isZh: boolean }) {
  ...
  const counts = useMemo<Record<string, number>>(() => {
    ...
    FLOPPY_LENGTH_DISTRIBUTION.forEach((n, d) => { c[String(d)] = n; });
    return c;
  }, []);

  const examples = useMemo(() => floppyExamplesByLength(12), []);
  ...
  const series = useMemo<HistSeries[]>(() => [{
    name: tr({ zh: '1×3×3 花型', en: '1×3×3 Floppy' }),
    fillColors: [FLOPPY_COLOR],
    counts,
  }], [counts]);

  const shown = effectiveBin !== null ? (examples[effectiveBin] ?? []) : [];
  const solverHref = (scr: string) => `/scramble/solver?${new URLSearchParams({ event: '133', scramble: scr })}`;

  // 全量穷举(所有 191 个非平凡状态的最短打乱)按需懒算一次,供下载。
  const allRef = useRef<Record<number, string[]> | null>(null);
  const getAll = () => (allRef.current ??= floppyAllScramblesByLength());

  // 下载全部状态:CSV,按最优步数分类(含还原态一行 0,)。
  const downloadAll = () => {
    const all = getAll();
    const lines = ['optimal_length,scramble', '0,'];
    for (let d = 1; d <= FLOPPY_GODS_NUMBER; d++) for (const s of all[d] ?? []) lines.push(`${d},${s}`);
    downloadText('floppy_133_all_states.csv', lines.join('\n'));
  };
  // 下载某步数全部:txt,每行一条打乱。
  const downloadBin = (d: number) => {
    downloadText(`floppy_133_${d}-move.txt`, (getAll()[d] ?? []).join('\n'));
  };
  const binCount = effectiveBin !== null ? (FLOPPY_LENGTH_DISTRIBUTION[effectiveBin] ?? 0) : 0;

  return (
    <>
      <div className="scramble-stats-controls">
        <div className="scramble-stats-puzzle-meta">
          <span>{tr({ zh: '全 192 态(精确枚举)', en: 'All 192 states (exact enumeration)' })}</span>
          <span className="scramble-stats-puzzle-metric">{tr({ zh: '每转一面 180° = 1 步', en: 'one 180° face turn = 1 move' })}</span>
        </div>
            ...
            <ul className="scramble-stats-examples-list">
              {shown.map((scr, i) => (
                <li key={i}>
                  ...
                  >
                    <ScramblePreview2D event="133" scramble={scr} size={26} />
                  </Link>
      ...
      {st && (
        <div className="scramble-stats-panel">
          ...
          <div className="scramble-stats-stat-grid">
            ...
            <Cell label={tr({ zh: '上帝之数', en: "God's number" })} value={String(FLOPPY_GODS_NUMBER)} />
          </div>
        </div>
      )}
