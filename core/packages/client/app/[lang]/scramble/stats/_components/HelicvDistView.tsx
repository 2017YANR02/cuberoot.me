...
// 弧面直升机魔方(helicv)解步数分布 —— **离线预计算的采样直方图**(非全空间精确曲线):helicv 可达状态
// ≈ 3.03×10²¹(= 8!·3⁷·(6!)⁴·2¹²/2⁵,Schreier-Sims 实算,是直升机魔方的 256 倍),太大无法整图枚举。分布由
// build 脚本(scramble-stats-build/src/build_puzzle_sampled_dist.ts)**离线**采 N 条 cstimer 同款随机打乱、逐条用
// 从零对易子约简求解器求解、把返回解步数分桶,落静态 stats/scramble/dist_helicv.json;本页**只 fetch + 渲染**,
...
const HELICV_COLOR = '#0ea5e9';   // 数据天蓝(非 UI 灰阶)
...
export default function HelicvDistView({ isZh }: { isZh: boolean }) {
  ...
  useEffect(() => {
    let alive = true;
    fetch(statsUrl('/stats/scramble/dist_helicv.json') + `?v=${V}`)
  ...
  const solverHref = (scr: string) => `/scramble/solver?${new URLSearchParams({ event: 'helicv', scramble: scr })}`;

  const series = useMemo<HistSeries[]>(() => [{
    name: tr({ zh: '弧面直升机(采样, 有界, 非全空间)', en: 'Curvy Copter (sampled, bounded, not full-space)' }),
    fillColors: [HELICV_COLOR],
    counts,
  }], [counts]);
  ...
  const downloadSample = () => {
    ...
    downloadText('helicv_sample.csv', lines.join('\n'));
  };

  const stateCountStr = json?.stateCountStr ?? '3,033,257,372,496,691,200,000';
          ...
          <ul className="scramble-stats-examples-list">
            {shown.map((scr, i) => (
              <li key={i}>
                ...
                >
                  <ScramblePreview2D event="helicv" scramble={scr} size={26} />
                </Link>
