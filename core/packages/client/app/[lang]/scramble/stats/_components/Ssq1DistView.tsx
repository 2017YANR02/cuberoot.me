...
const SSQ1_COLOR = '#0A84FF';   // 数据蓝(非 UI 灰阶)
...
export default function Ssq1DistView({ isZh }: { isZh: boolean }) {
  ...
  useEffect(() => {
    let alive = true;
    fetch(statsUrl('/stats/scramble/dist_ssq1.json') + `?v=${V}`)
  ...
  const solverHref = (scr: string) => `/scramble/solver?${new URLSearchParams({ event: 'ssq1', scramble: scr })}`;

  const series = useMemo<HistSeries[]>(() => [{
    name: tr({ zh: 'Super Square-1(采样,有界,非全空间)', en: 'Super Square-1 (sampled, bounded, not full-space)' }),
    fillColors: [SSQ1_COLOR],
    counts,
  }], [counts]);
  ...
  const downloadSample = () => {
    ...
    downloadText('ssq1_sample.csv', lines.join('\n'));
  };

  const stateCountStr = json?.stateCountStr ?? '≈1.15×10²⁵ (two coupled Square-1 mechanisms)';
          ...
          <ul className="scramble-stats-examples-list">
            {shown.map((scr, i) => (
              <li key={i}>
                ...
                >
                  <ScramblePreview2D event="ssq1" scramble={scr} size={26} />
                </Link>
