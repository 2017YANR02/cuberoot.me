...
// 3×3×4(334)解步数分布 —— **离线预计算的采样直方图**(非全空间精确曲线):3×3×4 状态空间
// ≈ 165,181,768,335,360,000(facelet 群阶 2,642,908,293,365,760,000,Schreier-Sims 实算),太大无法整图枚举。
// 求解器走两阶段约简(先把所有块归约进全 180° 子群,再只用 180° 转还原),任何随机态都能解出一条有界的近最优解
//(很浅的态另给可证最优解)。分布由 build 脚本(scramble-stats-build/src/build_puzzle_sampled_dist.ts)**离线**
// 采 N 个 cstimer 同款随机打乱、逐条求解、把返回解步数分桶,落静态 stats/scramble/dist_334.json;本页**只 fetch +
...
export default function Cuboid334DistView({ isZh }: { isZh: boolean }) {
  ...
  useEffect(() => {
    let alive = true;
    fetch(statsUrl('/stats/scramble/dist_334.json') + `?v=${V}`)
  ...
  const stateCountStr = json?.stateCountStr ?? '165,181,768,335,360,000';
