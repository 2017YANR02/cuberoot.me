...
// UFO(UFO 魔方)整解最优步数分布 —— 理论全空间(全 60,480 态,精确枚举,非抽样)。
// 数据 = lib/ufo-solver 的 UFO_LENGTH_DISTRIBUTION(与求解器页同一份),示例由 ufoExamplesByLength
// 枚举每个步数档的真实状态、反推最短打乱生成(UFO 不是 WCA 项目,没有比赛语料,但全状态空间可枚举)。
// 态数 60,480(< 2M),「下载全部」CSV 直接 Blob(无需 confirm / 流式)。
...
import {
  UFO_LENGTH_DISTRIBUTION, UFO_GODS_NUMBER, UFO_TOTAL_STATES,
  ufoExamplesByLength, ufoAllStates, ufoAllScramblesByLength,
} from '@/lib/ufo-solver';
import { tr } from '@/i18n/tr';

const UFO_COLOR = '#8338ec'; // 异形扭转紫(数据色,非 UI 灰阶)
...
export default function UfoDistView({ isZh }: { isZh: boolean }) {
  ...
  const counts = useMemo<Record<string, number>>(() => {
    ...
    UFO_LENGTH_DISTRIBUTION.forEach((n, d) => { c[String(d)] = n; });
    return c;
  }, []);

  const examples = useMemo(() => ufoExamplesByLength(12), []);
  ...
  const series = useMemo<HistSeries[]>(() => [{
    name: tr({ zh: 'UFO', en: 'UFO' }),
    fillColors: [UFO_COLOR],
    counts,
  }], [counts]);

  const shown = effectiveBin !== null ? (examples[effectiveBin] ?? []) : [];
  const solverHref = (scr: string) => `/scramble/solver?${new URLSearchParams({ event: 'ufo', scramble: scr })}`;

  // 下载全部状态:CSV,按最优步数分类(含还原态一行 0,)。60,480 行 < 2M,直接 Blob。
  const downloadAll = () => {
    const lines = ['optimal_length,scramble'];
    for (const { depth, scramble } of ufoAllStates()) lines.push(`${depth},${scramble}`);
    downloadBlob('ufo_all_states.csv', lines.join('\n'));
  };
  // 下载某步数全部:txt,每行一条打乱。
  const downloadBin = (d: number) => {
    const all = ufoAllScramblesByLength();
    downloadBlob(`ufo_${d}-move.txt`, (all[d] ?? []).join('\n'));
  };
  const binCount = effectiveBin !== null ? (UFO_LENGTH_DISTRIBUTION[effectiveBin] ?? 0) : 0;

  return (
    <>
      <div className="scramble-stats-controls">
        <div className="scramble-stats-puzzle-meta">
          <span>{tr({ zh: `全 ${UFO_TOTAL_STATES.toLocaleString()} 态(精确枚举)`, en: `All ${UFO_TOTAL_STATES.toLocaleString()} states (exact enumeration)` })}</span>
          <span className="scramble-stats-puzzle-metric">{tr({ zh: '一次转动 = 1 步(A/B/C 翻球,U 转盘)', en: 'one turn = 1 move (A/B/C ball flips, U wheel)' })}</span>
        </div>
        <button
          type="button"
          className="ivy-dl-all"
          onClick={downloadAll}
          title={tr({ zh: '下载全部 60,479 条(CSV)', en: 'Download all 60,479 scrambles (CSV)' })}
        >
            ...
            <ul className="scramble-stats-examples-list">
              {shown.map((scr, i) => (
                <li key={i}>
                  ...
                  >
                    <ScramblePreview2D event="ufo" scramble={scr} size={26} />
                  </Link>
      ...
      {st && (
        <div className="scramble-stats-panel">
          ...
          <div className="scramble-stats-stat-grid">
            ...
            <Cell label={tr({ zh: '上帝之数', en: "God's number" })} value={String(UFO_GODS_NUMBER)} />
          </div>
        </div>
      )}
