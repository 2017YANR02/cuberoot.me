...
// 八数码(8-Puzzle)整解最优步数分布 —— 理论全空间(全 181,440 态,精确枚举,非抽样)。
// 数据 = lib/slide8-solver 的 SLIDE8_LENGTH_DISTRIBUTION(与求解器页同一份),示例由
// slide8ExamplesByLength 枚举每个步数档的真实状态、反推最短打乱生成(8 数码不是 WCA 项目,
// 没有比赛语料,但全状态空间可枚举)。
...
import {
  SLIDE8_LENGTH_DISTRIBUTION, SLIDE8_GODS_NUMBER, SLIDE8_STATE_COUNT,
  slide8ExamplesByLength, slide8AllScramblesByLength,
} from '@/lib/slide8-solver';
import { tr } from '@/i18n/tr';

const SLIDE8_COLOR = '#3a86ff'; // 数据蓝(非 UI 灰阶)
...
export default function Slide8DistView({ isZh }: { isZh: boolean }) {
  ...
  const counts = useMemo<Record<string, number>>(() => {
    ...
    SLIDE8_LENGTH_DISTRIBUTION.forEach((n, d) => { c[String(d)] = n; });
    return c;
  }, []);

  const examples = useMemo(() => slide8ExamplesByLength(12), []);
  ...
  const series = useMemo<HistSeries[]>(() => [{
    name: tr({ zh: '八数码', en: '8-Puzzle' }),
    fillColors: [SLIDE8_COLOR],
    counts,
  }], [counts]);

  const shown = effectiveBin !== null ? (examples[effectiveBin] ?? []) : [];
  const solverHref = (scr: string) => `/scramble/solver?${new URLSearchParams({ event: '8p', scramble: scr })}`;

  // 全量穷举(所有 181,439 个非平凡状态的最短打乱)按需懒算一次,供下载。
  const allRef = useRef<Record<number, string[]> | null>(null);
  const getAll = () => (allRef.current ??= slide8AllScramblesByLength());

  // 下载全部状态:CSV,按最优步数分类(含还原态一行 0,)。
  const downloadAll = () => {
    const all = getAll();
    const lines = ['optimal_length,scramble', '0,'];
    for (let d = 1; d <= SLIDE8_GODS_NUMBER; d++) for (const s of all[d] ?? []) lines.push(`${d},${s}`);
    downloadText('slide8_8p_all_states.csv', lines.join('\n'));
  };
  // 下载某步数全部:txt,每行一条打乱。
  const downloadBin = (d: number) => {
    downloadText(`slide8_8p_${d}-move.txt`, (getAll()[d] ?? []).join('\n'));
  };
  const binCount = effectiveBin !== null ? (SLIDE8_LENGTH_DISTRIBUTION[effectiveBin] ?? 0) : 0;

  return (
    <>
      <div className="scramble-stats-controls">
        <div className="scramble-stats-puzzle-meta">
          <span>{tr({ zh: '全 181,440 态(精确枚举)', en: 'All 181,440 states (exact enumeration)' })}</span>
            ...
            <ul className="scramble-stats-examples-list">
              {shown.map((scr, i) => (
                <li key={i}>
                  ...
                  >
                    <ScramblePreview2D event="8p" scramble={scr} size={26} />
                  </Link>
      ...
      {st && (
        <div className="scramble-stats-panel">
          ...
          <div className="scramble-stats-stat-grid">
            ...
            <Cell label={tr({ zh: '上帝之数', en: "God's number" })} value={String(SLIDE8_GODS_NUMBER)} />
          </div>
        </div>
      )}
