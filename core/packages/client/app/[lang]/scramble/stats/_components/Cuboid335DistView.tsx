...
// 3×3×5(335)解步数分布 —— **离线预计算的采样直方图**(非全空间精确曲线):3×3×5 可达状态
// 156,067,430,400(≈1.56×10¹¹,Schreier-Sims 实算),太大无法整图枚举。分布由 build 脚本
// (scramble-stats-build/src/build_puzzle_sampled_dist.ts)**离线**采 N 条 cstimer 同款随机打乱、逐条用
// 两阶段约简求解器求解、把返回解步数分桶,落静态 stats/scramble/dist_335.json;本页**只 fetch + 渲染**,
...
export default function Cuboid335DistView({ isZh }: { isZh: boolean }) {
  ...
  useEffect(() => {
    let alive = true;
    fetch(statsUrl('/stats/scramble/dist_335.json') + `?v=${V}`)
  ...
  const stateCountStr = json?.stateCountStr ?? '156,067,430,400';
    ...
    <>
      <div className="scramble-stats-controls">
        <div className="scramble-stats-puzzle-meta">
          <span>
            {tr({
              zh: `${sampleCount.toLocaleString()} 条随机打乱样本(离线预计算,两阶段近最优,非全空间)`,
