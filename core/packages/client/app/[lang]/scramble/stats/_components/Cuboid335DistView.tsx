...
// 3×3×5(335)解步数分布 —— **采样 / 近最优**(非全空间精确曲线):3×3×5 可达状态 156,067,430,400
//(≈1.56×10¹¹,Schreier-Sims 实算),太大无法整图枚举。求解器走**两阶段约简**(先把所有块归约进全 180°
// 子群,再只用 180° 转还原),任何随机态都能解出一条有界的**近最优**解(很浅的态另给可证最优解)。这里现场
// 生成 N 个 cstimer 同款随机打乱(SCRAMBLE_LEN 步刚体打乱),逐条求解,把**返回解的步数**分桶 —— 即随机态的
// 返回解长度分布(主要为近最优,浅态为最优),不是全空间最优分布。下载提供「下载样本」CSV(length,scramble,
// optimal),不提供「下载全部状态」(1.56×10¹¹ 态不可枚举)。
import { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import { Download, Loader2, RotateCw } from 'lucide-react';
import Link from '@/components/AppLink';
import DiscreteHistogram, { type HistSeries } from './DiscreteHistogram';
import { ScramblePreview2D } from '@/components/ScramblePreview2D';
import { randomCuboid335Scramble, solveCuboid335, CUBOID335_STATE_COUNT_STR, CUBOID335_ORBIT_PRODUCT_STR } from '@/lib/cuboid335-solver';
import { tr } from '@/i18n/tr';

const C335_COLOR = '#0ea5e9';   // 数据天蓝(非 UI 灰阶)
const DEFAULT_SAMPLE = 120;     // 默认采样个数(两阶段每条数毫秒至数百 ms)
const SCRAMBLE_LEN = 40;        // cstimer 同款随机刚体打乱长度(近均匀随机态)
...
export default function Cuboid335DistView({ isZh }: { isZh: boolean }) {
  ...
  const run = useCallback(() => {
    ...
    const step = () => {
      ...
      for (let i = 0; i < BATCH && collected.length < sampleN; i++) {
        try {
          const scramble = randomCuboid335Scramble(SCRAMBLE_LEN);
          const out = solveCuboid335(scramble); // two-phase: every state returns a bounded solution
  ...
  const solverHref = (scr: string) => `/scramble/solver?${new URLSearchParams({ event: '335', scramble: scr })}`;

  const series = useMemo<HistSeries[]>(() => [{
    name: tr({ zh: '3×3×5(采样,近最优,非全空间)', en: '3×3×5 (sampled, near-optimal, not full-space)' }),
    fillColors: [C335_COLOR],
    counts,
  }], [counts]);
  ...
  const downloadSample = () => {
    ...
    downloadText('335_sample.csv', lines.join('\n'));
  };
              ...
              <ul className="scramble-stats-examples-list">
                {shown.map((scr, i) => (
                  <li key={i}>
                    ...
                    >
                      <ScramblePreview2D event="335" scramble={scr} size={26} />
                    </Link>
