...
// 大金字塔(Master Pyraminx,随态)整解【近最优】步数分布 —— **离线预计算的采样直方图**,非全空间精确、非可证最优。
// 状态空间约 4.6×10¹¹,无法整图枚举。分布由 build 脚本(scramble-stats-build/src/build_puzzle_sampled_dist.ts)
// **离线**采 N 个 cstimer 同款随机打乱、逐条用 cstimer 自带两阶段求解器求近最优解、把解的步数分桶,落静态
// stats/scramble/dist_mpyrso.json;本页**只 fetch + 渲染**,进页不做任何求解(铁律:TIER C/D 分布严禁浏览器现场采样)。
// **这是抽样估计的近最优长度**:既不是全空间精确曲线(TIER A/B),也不是可证最优(TIER C)。下载样本来自 JSON 内嵌的 generatedSamples。
...
export default function MpyrDistView({ isZh }: { isZh: boolean }) {
  ...
  useEffect(() => {
    let alive = true;
    fetch(statsUrl('/stats/scramble/dist_mpyrso.json') + `?v=${V}`)
  ...
  const stateCountStr = json?.stateCountStr ?? '461,078,666,256';
    ...
    <>
      <div className="scramble-stats-controls">
        <div className="scramble-stats-puzzle-meta">
          <span>
            {tr({
              zh: `${sampleCount.toLocaleString()} 个随机态样本(离线预计算,近最优,非全空间)`,
              en: `${sampleCount.toLocaleString()} random-state samples (precomputed offline, near-optimal, not full-space)`,
            })}
          </span>
