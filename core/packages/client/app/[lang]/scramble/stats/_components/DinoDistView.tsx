...
// 恐龙魔方(Dino Cube,随态)整解【近最优】步数分布 —— **采样**,非全空间精确、非可证最优。
// 状态空间 A12 = 12!/2 = 239,500,800(只有棱),超 TIER A/B 上限,所以这里在浏览器里现场生成 N 个
// 随机打乱(cstimer 随态生成器),再用 cstimer 自带的求解器(worker)逐条求近最优解,把解的步数分桶。
// **这是抽样估计的近最优长度**:既不是全空间精确曲线(TIER A/B),也不是可证最优 —— 是 cstimer
// IDA* 解(配三张棱组合剪枝表)的长度分布(恐龙魔方上帝之数为 10)。求解走 worker 异步,故采样分批跑、
// 显示进度、可取消;下载提供「下载样本」CSV(near_optimal_length,scramble),不提供「下载全部状态」。
...
export default function DinoDistView({ isZh }: { isZh: boolean }) {
  ...
  const run = useCallback(() => {
    ...
    const step = async () => {
      ...
      try {
        const scramble = (await cstimerScramble('dino')).trim();
        if (cancelRef.current) { setRunning(false); return; }
        const out = await solveDino(scramble);
    ...
    <>
      ...
      {samples.length === 0 ? (
        <div className="scramble-stats-loading">
          {tr({
            zh: `求解 ${done.toLocaleString()} / ${sampleN.toLocaleString()} 个随机态…(每个用 cstimer 自带求解器现场求近最优解,首个会构建剪枝表)`,
            en: `Solving ${done.toLocaleString()} / ${sampleN.toLocaleString()} random states… (each solved on the fly by cstimer's own solver; the first call builds prune tables)`,
          })}
        </div>
      ) : (
        <>
      ...
      <div className="scramble-stats-meta">
        <span>
          {tr({
            zh: `这是采样估计的【近最优】长度分布,既不是全空间精确分布,也不是可证最优:恐龙魔方有 ${STATE_COUNT.toLocaleString()} 个状态(A12 = 12!/2,只有棱),太大无法整图枚举,所以这里现场生成并用 cstimer 自带求解器解了 ${sampleN.toLocaleString()} 个随机态,把解的步数分桶。解长度接近最优(上帝之数为 10)但本工具不断言每条最短(样本量越大分布越稳)。它不是 WCA 项目,示例即采样到的真实随机打乱。`,
            en: `This is a SAMPLED estimate of the NEAR-OPTIMAL solution-length distribution — neither the exact full-space curve nor a provably-optimal one: the Dino Cube has ${STATE_COUNT.toLocaleString()} states (A12 = 12!/2, edges only), far too many to enumerate, so ${sampleN.toLocaleString()} random states were generated and solved on the fly with cstimer's own solver, then bucketed. Solution lengths are near-optimal (the God's number is 10) but this tool does not assert each is shortest (a larger sample stabilizes the curve). It is not a WCA event; the examples are the actual sampled random scrambles.`,
          })}
        </span>
      </div>
    </>
