...
// 受限方块一(Bandaged Square-1)解步数分布 —— **离线预计算的采样直方图**(非全空间精确曲线):受限方块一
// 是一只物理 Square-1,但记号限制成 < / , (1,0) >(只转顶层 + 切片,底层不直接转),可达状态是 Square-1 群的
// 一个大子群(无法整图枚举);分布由 build 脚本(scramble-stats-build/src/build_puzzle_sampled_dist.ts)
// **离线**采 N 条 cstimer 同款随机打乱、逐条用三阶段约简求解器求解、把返回解步数分桶,落静态
// stats/scramble/dist_bsq.json;本页**只 fetch + 渲染**,进页不做任何求解(铁律:TIER C/D 分布严禁现场采样)。
...
const BSQ_COLOR = '#a855f7';   // 数据紫(非 UI 灰阶)
...
export default function BsqDistView({ isZh }: { isZh: boolean }) {
  ...
  useEffect(() => {
    let alive = true;
    fetch(statsUrl('/stats/scramble/dist_bsq.json') + `?v=${V}`)
  ...
  const solverHref = (scr: string) => `/scramble/solver?${new URLSearchParams({ event: 'bsq', scramble: scr })}`;

  const series = useMemo<HistSeries[]>(() => [{
    name: tr({ zh: '受限方块一(采样,有界,非全空间)', en: 'Bandaged Square-1 (sampled, bounded, not full-space)' }),
    fillColors: [BSQ_COLOR],
    counts,
  }], [counts]);
  ...
  const downloadSample = () => {
    ...
    downloadText('bsq_sample.csv', lines.join('\n'));
  };

  const stateCountStr = json?.stateCountStr ?? 'huge (large subgroup of the Square-1 group; cube-shape coset = 518,400 = 720²)';
    ...
    <>
      <div className="scramble-stats-controls">
        <div className="scramble-stats-puzzle-meta">
          <span>
            {tr({
              zh: `${sampleCount.toLocaleString()} 条随机打乱样本(离线预计算,三阶段约简有界,非全空间)`,
              en: `${sampleCount.toLocaleString()} random-scramble samples (precomputed offline, three-stage reduction, bounded, not full-space)`,
            })}
          </span>
          <span className="scramble-stats-puzzle-metric">{tr({ zh: '一个切片 = 1 步', en: 'one slice = 1 move' })}</span>
        </div>
          ...
          <ul className="scramble-stats-examples-list">
            {shown.map((scr, i) => (
              <li key={i}>
                ...
                >
                  <ScramblePreview2D event="bsq" scramble={scr} size={26} />
                </Link>
      ...
      {stats && (
        <div className="scramble-stats-panel">
          <div className="scramble-stats-panel-title">{tr({ zh: '摘要统计(随机态样本,三阶段约简,有界非最优)', en: 'Summary stats (random-state sample, three-stage reduction, bounded not optimal)' })}</div>
