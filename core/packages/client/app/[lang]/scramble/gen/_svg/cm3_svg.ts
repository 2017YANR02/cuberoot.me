/**
 * Cmetrick (cm3) state preview — a 2D schematic of the puzzle's 9 balls, derived entirely from the
 * solver's exact 9-ball state (lib/cm3-solver). The Cmetrick is a 3×3 grid of spheres, NOT a cube, so a
 * WCA net doesn't fit; instead we draw 9 circles in a 3×3 layout (row-major 0..8). Each ball shows its 3
 * visible faces (front / up / right) as a small 3-facelet rosette colored by the 6 standard cube colors
 * mapped through that ball's current orientation. This is the 3×3 scaling of cm2_svg's 2×2 schematic.
 *
 ...
 * so all 9 balls render the SAME 3-color rosette — a self-proving render. Any scramble rolls the balls
...
export const CM3_DEFAULT_COLORS = {
...
function frontColor(orient: number): string { return CM3_DEFAULT_COLORS.stickers[ROT_PERM[orient][4]]; }
function upColor(orient: number): string { return CM3_DEFAULT_COLORS.stickers[ROT_PERM[orient][2]]; }
function rightColor(orient: number): string { return CM3_DEFAULT_COLORS.stickers[ROT_PERM[orient][0]]; }

const VIEW = 200;
const R_BALL = 28;
// 3×3 ball centers, row-major 0..8. Evenly spaced across the 200 viewport.
const CENTERS: ReadonlyArray<[number, number]> = (() => {
  const pos = [40, 100, 160];
  const out: [number, number][] = [];
  for (let r = 0; r < 3; r++) for (let c = 0; c < 3; c++) out.push([pos[c], pos[r]]);
  return out;
})();

function fmt(n: number): string { return Number(n.toFixed(2)).toString(); }

/** A small 3-facelet rosette inside the ball: up wedge / front wedge / right wedge. */
function ballRosette(cx: number, cy: number, orient: number): string {
  const C = CM3_DEFAULT_COLORS;
  const out: string[] = [];
...
export function renderCm3ScrambleSvg(scramble: string): string {
  let st: number[] = [...CM3_SOLVED];
  try {
    st = cm3Apply(scramble);
  } catch (e) {
    console.warn('[cm3_svg] apply failed', scramble, e);
  }
  const C = CM3_DEFAULT_COLORS;
  const out: string[] = [
