/**
 * Siamese 2×2×2 (sia222) state preview — a 2D net of BOTH glued 3×3×3 cubes, colored ENTIRELY from the scramble
 * applied to an INDEPENDENT re-derivation of the geometry (NOT imported from the solver's internal tables). The
 * fused puzzle is two 3×3×3 cubes joined along a shared 2×2×2 corner block; cstimer turns each with {U, R, F}
 * and a literal `z2 y` (whole-puzzle reorientation that swaps the two boxes about the shared block) separates
 * cube-A moves from cube-B moves. We split the scramble on `z2 y`: tokens before → cube A, after → cube B. By the
 * direct-product structure each cube's own tokens act on it like a plain restricted {U,R,F} cube, so we draw each
 * cube as a standard 6-face cross-net, painting every facelet with the home-face color of the sticker now there.
 * At solved each face is a single color (self-proving); the shared corner block never moves. (2D net, NOT iso —
 * per the loop convention.)
 *
 * Geometry re-derived here from 3D rotation matrices, independent of lib/sia222-solver (U=layer y=2 about +y,
 * R=x=2 about +x, F=z=2 about +z; cube centre (1,1,1)).
 */
...
// Restricted {U,R,F}: U = y=2 about +y, R = x=2 about +x, F = z=2 about +z.
const BASE: Record<string, BaseMove> = { U: layerMove(1, 2, RY), R: layerMove(0, 2, RX), F: layerMove(2, 2, RZ) };
...
// ── facelet model: 6 faces × 9 cells → (cubie, outward normal). Net: U / L F R B / D. ────────────────
const FACES = [
...
export const SIA222_FACE_COLORS: Record<string, string> = {
  U: '#FFD500', // yellow
  D: '#FFFFFF', // white
  R: '#EE0000', // red
  L: '#FF8800', // orange
...
function cubeSvg(st: St, baseX: number): string {
  ...
  for (const F of FACES) {
    ...
    for (const fl of FACELETS) {
      ...
      const color = SIA222_FACE_COLORS[colorOf(st, fl)] ?? '#888888';
...
/** Split a fused sia222 scramble on `z2 y` into cube-A and cube-B token lists. */
function split(scramble: string): { a: string[]; b: string[] } {
  const s = scramble.trim();
  const idx = s.indexOf('z2 y');
  if (idx < 0) return { a: s.split(/\s+/).filter((t) => /^[URF](2|')?$/.test(t)), b: [] };
  const a = s.slice(0, idx).trim().split(/\s+/).filter((t) => /^[URF](2|')?$/.test(t));
  const b = s.slice(idx + 'z2 y'.length).trim().split(/\s+/).filter((t) => /^[URF](2|')?$/.test(t));
  return { a, b };
}

export function renderSia222ScrambleSvg(scramble: string): string {
  ...
  try {
    ...
    console.warn('[sia222_svg] apply failed', scramble, e);
  }
  return [
