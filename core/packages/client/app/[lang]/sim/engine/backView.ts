...
export function createBackView(
  ...
  return {
    ...
    render(world: World) {
      ...
      // 手开着时:取景对齐主视图 3.9,near/far 按手部真包络 ±6×cubelet 放宽,且 near
      // 钳正 —— perspective 滑杆低段 distance 仅 7.8×cubelet,「distance − 8」为负 =
      // 投影矩阵损坏乱切面(与主视图 resize() 同款修法,细节见 world.resize 注释)。
      ...
      camera.aspect = 1;
      camera.near = Math.max(distance - cubeletSize * (handsOn ? 6 : isSq1 ? 5 : 4), cubeletSize * 0.4);
