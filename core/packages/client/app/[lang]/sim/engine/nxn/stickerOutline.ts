...
import { COLORS, STICKER_INNER, STICKER_CORNER_RADIUS } from '../define';
    ...
    mat.onBeforeCompile = (shader) => {
      ...
      shader.fragmentShader = shader.fragmentShader
        ...
        .replace(
          ...
          `#include <color_fragment>
          if (vOutline > 0.5) {
            vec2 q = abs(vOutlinePos) - vec2(${(HALF - R).toFixed(3)});
            float sdf = min(max(q.x, q.y), 0.0) + length(max(q, vec2(0.0))) - ${R.toFixed(3)};
