...
export function homeRight(): HandPose {
  return {
    ...
    fingers: fingerPose(
      ...
      // 「平摊」搭在 B 面(c2≤0.55、c3≤0.35 姿态上界,钩成爪 = 假),同时每个
      // 关节保留轻微自然屈曲(下界 c2≥0.15、c3≥0.12;全伸直 = 木棍,用户指定
      // 「不要完全直线」—— 加下界后以「指尖钉回原接触点」为目标重解,残差
      // ≤2.2U,接触布局不变)、拇指压 FR 棱贴纸(F 面右列中排;别解到棱下角 ——
      // 会藏进掌后视觉上拇指消失)。目标为「贴面平面 + x/y 软区间」而非定点
      // (定点 + 姿态上界几何不可达,解会卡关节上界);注意平贴面姿态下别开
      // 关节穿模罚(平贴时骨心离面本就只剩肉厚,罚项会把解推离目标)。手位
      // (pos/quat 俯仰+yaw)与指弯联合解出;拇指弯曲平面 roll 烘在
      // handModelGltf 的 THUMB_CURL_PLANE_ROLL。拇指尖 (55,2,101) 落 FR 贴纸
      // 中心带。改指长/缩放/roll/HAND_SCALE 必须重跑标定(方法见 memory
      // project_sim_hands_rig)。
      fc(0.522, 0.497, 0.156, 1.005), // 拇指:压 FR 棱贴纸(用户指位规格),
                                      // 指尖 (55,2,101) 中排,指腹盖 FR 贴纸中带
      fc(0.953, 0.166, 0.134, -0.45), // 食指:微拱,指腹平搭 B 面上部 (87,62,-100)
      fc(1.031, 0.355, 0.126, -0.45), // 中指:微拱,平搭 B 面中部 (82,-11,-100)
      fc(1.003, 0.267, 0.12, -0.404), // 无名指:微拱,平搭 B 面下部 (88,-66,-100)
