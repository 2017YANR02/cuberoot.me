...
/**
 * 右手 home 握持(前后钳形,按用户指位规格):掌贴 R 面,指列竖排绕过右后
 * 竖棱 —— 食指尖压 BUR 角块 B 面 (64,64,-96),中指压 BR 棱 B 面 (64,0,-96),
 * 无名指压 BDR 角块 B 面 (64,-64,-96),拇指从掌根前下绕出压 FR 棱 F 面
 * (64,0,96);小指无目标,收拢在无名指下方。左手镜像同规格(BUL/BL/BDL/FL)。
...
export function homeRight(): HandPose {
  return {
    pos: new THREE.Vector3(122, -8, 25),
    quat: quatFromWorldRots([
      ["y", 90],   // 指列转向 -z(后)
      ["z", 180],  // 食指侧翻到上方,掌心落向 -x(魔方)
    ]),
    fingers: fingerPose(
      // 数值 = 浏览器内坐标下降求解(指尖圆帽心落面外一半径处 + 指腹朝按压向 +
      // 指链不进 96+r 立方包络 + 解剖正则),见 memory project_sim_hands_rig。
      fc(0.75, 1.41, 0.49, -0.26),  // 拇指:近节折在掌根后,末节沿 F 面朝左上压 FR
      fc(0.14, 0.38, 0.53, 0.31),   // 食指:上展,绕右后棱压 BUR 角 B 面
      fc(0.13, 0.58, 0.69, -0.03),  // 中指:压 BR 棱 B 面
      fc(0.15, 0.37, 0.5, -0.36),   // 无名指:下展压 BDR 角 B 面
      fc(-0.01, 0.57, 0.18, -0.92), // 小指:下收,悬在 R 面右下后缘外侧
    ),
  };
}
