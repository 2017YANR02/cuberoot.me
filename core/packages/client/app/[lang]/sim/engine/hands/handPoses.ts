...
export function homeRight(): HandPose {
  return {
    ...
    fingers: fingerPose(
      ...
      // 2026-07-10 r7 拇指绕指尖下压(用户抓「根部翘太高」,规格:指尖/指甲
      // 原位不动,以指尖为圆心顺时针转、根侧下沉):c1/splay 2 通道雅可比
      // 最小二乘(浏览器在体解),与 handModelGltf.THUMB_PITCH_MOUNT 挂点
      // 平移配对合成绕指尖刚体旋转 —— CMC 落 ~15U(y 88→73,x 外漏 +6.5),
      // 指尖钉回误差 0(右手另含 +0.6U cube-y 整体微抬回填 E 带,tip −9.5→
      // −8.9)。E 带蒙皮顶点复核:R yVis [−31.8,29.1] / L [−31.9,28.0] ⊂ ±32。
      // 改任一侧必须连动重解另一侧。
      fc(-0.0063, -0.167, 0.5768, 1.2603), // 拇指:平压 FR 贴纸中带(E 层),甲面朝前
...
const LEFT_CURL_OFFSET: Record<FingerName, [number, number, number, number?]> = {
  ...
  // 拇指 c1/splay:2026-07-10 r7 绕指尖下压左手独立解 − 右手解(左 CMC 落
  // 15.9U、指尖钉回误差 0,c2/c3 不动)。
  thumb: [0.1485, -0.15, 0.05, 0.0013],
