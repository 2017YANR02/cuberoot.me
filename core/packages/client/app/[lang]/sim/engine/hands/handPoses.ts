 ...
 * (BUL/BL/BDL/FL)。**掌心不贴 R 面**:掌体外移悬空,与面之间留 ~16U 空腔
 * (参考站同款「指尖抓面、掌心成拱」),指尖接触点相应滑向各自贴纸的后棱端
 * (食指/无名指 x≈94,中指 x≈81,拇指滑向 FR 棱外端 x≈72,均在原块贴纸上)。
...
export function homeRight(): HandPose {
  return {
    pos: new THREE.Vector3(136, -8, 15),
    ...
    fingers: fingerPose(
      ...
      fc(0.75, 1.22, 0.53, -0.26),  // 拇指:近节折在掌根后,末节沿 F 面压 FR 棱外端
      fc(0.03, 0.56, 0.8, 0.36),    // 食指:上展,基节平伸架空、远端钩压 BUR 角 B 面
      fc(0.07, 0.76, 0.82, -0.03),  // 中指:压 BR 棱 B 面
      fc(0.04, 0.55, 0.78, -0.41),  // 无名指:下展钩压 BDR 角 B 面
