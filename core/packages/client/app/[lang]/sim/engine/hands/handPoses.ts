...
function manoHomeRight(): HandPose {
  ...
  // TIPZLO=-120(PENW 加压无效,pen 停 0.00 被契约项顶死)。终解:拇指
  // |x|min 36.1(镜像净距 72U,原 M 列墙 34.5 也达标)、贴 F fGap 1.92
  // 接触 (84.7,0.0) Je 带心、水平 0.0° 甲 0.999 CMC −30;四指接触
  // (94.3,77.3)/(92.7,2.9)/(92.9,−87.3) 在 Q/Te/T,三尖 z 展布 2.3。
  // ⚠ pen 0.00 零呼吸余量(待机呼吸或浅蹭 ≤1.7U,实机复核);改此值
  // 必须重跑 SOLVE=R(env 同上)。
  return {
    pos: new THREE.Vector3(368.63, -75.98, -15.28),
    quat: new THREE.Quaternion(-0.554011, -0.109, 0.815782, 0.125261),
    fingers: fingerPose(
      { curl: [1.0402, -0.4952, 0.0812], splay: 0.4628, twist: 2.3394, mid: [-0.8237, -0.0834] }, // 拇指:水平 0.0°,贴 F fGap 1.92 @Je 带心
      { curl: [0.4412, 0.9868, 0.1645], splay: -0.086, twist: -0.1136 },   // 食指:tip x 86.4 @Q,gap 3.57
      { curl: [0.7224, 0.7336, 0.4433], splay: 0.1405, twist: 0.0238 },    // 中指:tip x 72.7 @Te,gap 2.87
      { curl: [0.6964, 0.7826, 0.1398], splay: 0.1616, twist: -0.1206 },   // 无名指:tip x 76.9 @T,gap 2.03
      { curl: [1.0442, 0.3856, 0.2697], splay: 0.5106, twist: 0.0165 },    // 小指:∅ 悬空
    ),
  };
}
