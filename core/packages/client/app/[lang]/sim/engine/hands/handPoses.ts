...
export function homeRight(): HandPose {
  return {
    ...
    fingers: fingerPose(
      ...
      // 后按「贴面 2.4~3.2U + 肉 |x|≥34.5 + 接触心留在 FR 贴纸内(|x|∈[33,95],
      // y∈[−31,31])+ 甲背·ẑ 最大」联合解:MCP 近伸直 + IP 微过伸,末节近乎
      // 平躺贴面,甲背·ẑ≈0.85(贴纸内可行域极限;放开贴纸下缘可到 0.90,
      // 见 THUMB_CURL_PLANE_ROLL 注释)。
      fc(0.12, -0.175, 0.55, 1.47),   // 拇指:平压 FR 贴纸下带,甲面朝正前
...
const LEFT_CURL_OFFSET: Record<FingerName, [number, number, number]> = {
  thumb: [0.06, -0.06, 0.055], // 2026-07-08 随甲面∥F 重解(L 独立解−R 解;splay 仍共享)
