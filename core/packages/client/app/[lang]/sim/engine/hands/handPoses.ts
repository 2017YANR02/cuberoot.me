...
function manoHomeRight(): HandPose {
  ...
  // CURLW=300,拇指+手根冻结。
  // 2026-07-11 r26(用户三规格:①食/中/无名再内收一点;②小指别上翘,压平到
  // 无名指同级;③拇指改斜置 —— 指尖不动,MCP 下沉,正面看指尖→MCP 左上到
  // 右下):A/B 拆解(四指/拇指几乎不耦合,全通道解两连翻车 —— 水平项陡梯度
  // 走「抬整只拇指」宽盆地)。四指:TIPX 收 [40,58](ring 豁免 TIPXR=68,其上
  // 摆钉中线天然推外)、TILTP=14 压平小指(PBELOW hi 80 放宽,防平化小指经
  // below 带反拽 ring 掉中线;PINKZ 圈 z ∈ [−105,−40] 防深探背后)。拇指:c1
  // 扫描证单通道是整链刚转(加深 c1 MCP 沉但尖跟着掉),正解 = c1 钉深(1.28
  // →1.44 两级)+ 远端 c2/mid/twist 回钉指尖,THCY 收 [−12,−4] 钉尖高度,末
  // 轮 c1 放开局部续抛。终解:MCP −46.2、horiz +13.8°、尖 (95.5,−4.6) 漂
  // 3.6U、fGap 0.84、甲 0.988。env:THORIZ='8,18' THCY='-12,-4' TTX='40,80'。
  // 2026-07-11 r27(用户续调:①拇指大鱼际再下沉;②食/中再内收、更平):
  // 拇指 c1 1.56→1.67(MCP −46→−55,鱼际下沿 −84→−89,尖 y −11 带内,
  // fGap 3.6 轻触);四指 TIPX 收 [40,52](尖 53.9/53.4,中指倾角 6.1→4.8)。
  return {
    pos: new THREE.Vector3(372.21, -75.81, -29.55),
    quat: new THREE.Quaternion(-0.536805, -0.106353, 0.827205, 0.127516),
    fingers: fingerPose(
      { curl: [1.668, 0.161, 0.0194], splay: 0.6163, twist: 2.3403, mid: [-0.2678, -0.3677] }, // 拇指:MCP −55 沉,尖→MCP +13.9°,贴 F @Je
      { curl: [0.7377, 0.5779, 0.0266], splay: -0.123, twist: -0.1413 },   // 食指:接触心 y 61.6 @Q,tip x 53.9
      { curl: [0.8754, 0.6122, 0.1923], splay: 0.1882, twist: 0.2163 },    // 中指:接触心 y 0.5 @Te,tip x 53.4
      { curl: [0.8291, 0.5874, 0.0171], splay: 0.2616, twist: -0.1054 },   // 无名指:接触心 y −68.5 @T(中线守住)
      { curl: [1.1206, 0.2156, -0.0256], splay: 0.3918, twist: 0.2883 },   // 小指:∅ 悬空,倾角 14.8° 平化
    ),
  };
}
