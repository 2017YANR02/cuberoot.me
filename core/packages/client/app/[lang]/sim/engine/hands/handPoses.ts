...
export function homeRight(): HandPose {
  return {
    ...
    fingers: fingerPose(
      ...
      fc(0.183, -0.167, 0.5705, 1.313), // 拇指:平压 FR 贴纸中带(E 层),甲面朝前
      fc(0.7199, 0.38, 0.28, 0.0075), // 食指:水平环抱,贴 B 面 Q 角排,−0.1°
      fc(0.7902, 0.42, 0.26, 0.03),   // 中指:水平环抱,T 棱排,0.1°
      fc(0.6691, 0.40, 0.28, 0.055),  // 无名指:水平环抱,T 角排,3.0°
      fc(0.98, 0.42, 0.30, -0.05),    // 小指:随无名指并拢弯收,悬空不接触,7.8°
    ),
  };
}
...
const LEFT_CURL_OFFSET: Record<FingerName, [number, number, number, number?]> = {
  // 2026-07-10 随四指水平硬规格重解(SOLVE=L 12 维偏移解,L 独立解−R 解):
  // 倾角 食 0.0 / 中 −0.0 / 无名 1.8 / 小 7.4°,贴面 2.6/2.7/2.6(y 84/22/−38),
  // 拇指 fGap 3.2、|x|≥34.5、yVis [−31.6,31.5] ⊂ ±32。
  thumb: [0.1672, -0.15, 0.0625, 0.005],
  index: [0.0104, 0.02, 0.03],
  middle: [-0.0082, 0.025, -0.02, 0.0025],
  ring: [0.0189, -0.03, 0.02, 0.02],
