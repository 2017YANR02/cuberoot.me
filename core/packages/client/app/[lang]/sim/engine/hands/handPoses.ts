...
function manoHomeRight(): HandPose {
  ...
  // (pivot (170,0,-10),世界 y)俯视顺时针刚体转 26°(Ry(−26°) 烘进
  // quat),使食/中/无名指尖排与 B 面平行;左手经 homeLeft 镜像自动反向。
  // 指参由 SOLVE=R SEED 直通抛光(FREEZE=0,31,32 锁死朝向)恢复契约:
  // 贴纸位 R1:Je R2:Q R3:Te R4:T 不变(接触 (95.2,60.2)/(90.4,−0.4)/
  // (95.1,−91.8) 全在原贴纸),三尖 z −113.7/−113.5/−115.5 展布 2.0
  // (tipzEq 新硬项,平行达成)。CW26 几何强制的规格让步(5 连解证明
  // 不可兼容,env 化:TIPX/TTX/THUMBMX/TIPZLO):① r5 尖端内缘带
  // [36,46]→[36,84](tip x 82/71/73,仍在贴纸带);② 拇指 M 列墙 34.5
  // 废(拇指必入 M 列,扫掠避让交指法侧,同中指 @pin 先例),现 |x|min
  // 0.9 ⇒ 左右拇指净距仅 1.8U;③ 尖越面放宽到 −120(斜穿观感,预览已
  // 认可)。⚠ pen −0.86 <呼吸余量 1.2,待机呼吸可能浅蹭 ~0.9U(ring
  // gap 0.86 同源);改此值必须重跑 SOLVE=R(env 同上)。
  return {
    pos: new THREE.Vector3(340.83, -75.96, 41.85),
    quat: new THREE.Quaternion(-0.641440, -0.122276, 0.748988, 0.112339),
    fingers: fingerPose(
      { curl: [1.0582, -0.5752, 0.0812], splay: 0.4628, twist: 2.3394, mid: [-0.7379, -0.0834] }, // 拇指:水平 2.4°,贴 F fGap 2.41,contact (33.3,−9.2)
      { curl: [0.4762, 0.9387, 0.1691], splay: -0.11, twist: -0.0759 },    // 食指:tip x 82.2 @Q,gap 2.91
      { curl: [0.7612, 0.689, 0.4725], splay: 0.2305, twist: 0.1881 },     // 中指:tip x 70.6 @Te,gap 2.43
      { curl: [0.7384, 0.7293, 0.114], splay: 0.1588, twist: -0.1206 },    // 无名指:tip x 73.3 @T,gap 0.86(紧)
      { curl: [1.0429, 0.3563, 0.2697], splay: 0.4856, twist: 0.0165 },    // 小指:∅ 悬空,低于无名指 29.7U
    ),
  };
}
