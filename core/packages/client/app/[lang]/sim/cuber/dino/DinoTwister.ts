/**
 * DinoTwister — animation orchestrator for DinoCube.
 *
 * Same shape as Sq1Twister: uses the global tweener so DirectorPanel's offline
 * stepping records dino too; one tween per move, frames synced with the speed
 * slider via CubeGroup.frames. Each frame the callback rotates the 3 moving pivots
 * by q(axis, angle·v) · startQuat. Setup/push parse with the dino parser.
 */
...
import { parseDinoMoves, applyDinoMove, type DinoMove } from './dinoState';

export default class DinoTwister {
  cube: DinoCube;
  queue: DinoMove[] = [];
  private activeTween: Tween | null = null;
  lastSetupCpuMs = 0;

  constructor(cube: DinoCube) {
    this.cube = cube;
  }
  ...
  setup(scramble: string): void {
    ...
    this.finish();
    this.cube.reset();
    const moves = parseDinoMoves(scramble);
   ...
   */
  twist(move: DinoMove, fast: boolean, force: boolean): boolean {
    if (fast) {
  ...
  undo(): void {
    ...
    this._silentApply(parseDinoMoves(replay));
  }
  ...
  private _animate(move: DinoMove): void {
