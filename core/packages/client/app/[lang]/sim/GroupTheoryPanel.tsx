...
export default function GroupTheoryPanel({
  ...
  // Facts-only puzzles (cubing.js "explore": rendered by TwistyPlayer, no engine cube) show
  // |G| + structure but no live state / solve / scramble.
  ...
  useEffect(() => {
    ...
    void import('./engine/pgBindings').then(({ createBinding }) => {
      ...
      // Facts-only binding (no in-house engine to mirror): stop here — show |G| + structure,
      // never attach to a cube (there is none) or the rAF loop would spin forever.
      if (binding.hasLiveState === false) {
        setFactsOnly(true);
        return;
      }
        ...
        <>
          ...
          {factsOnly && (
            <div className="gt-actions-note">
              {t('cubing.js 渲染,无自有引擎 → 仅展示群结构(|G| 为外层招式生成的忠实群)',
                'rendered by cubing.js, no in-house engine → structure only (|G| is the faithful group generated by the outer-layer turns)')}
            </div>
          )}
