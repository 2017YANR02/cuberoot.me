      ...
      <div className="pred-layout">
        ...
        <article className="pred-article">
          ...
          <Section id="tldr" titleZh="一句话结论" titleEn="Top Line" isZh={isZh}>
            <div className="pred-tldr pred-tldr-333">
              <p className="pred-tldr-lede">
                {<T zh={<>三阶魔方单次 WR 在 <strong>2026 年 2 月 8 日</strong> 由 9 岁波兰选手 <strong>Teodor Zajder</strong> 以 <strong>2.76 秒</strong> 拿下,
                                                                                                                                                          人类首次跌破 3 秒;两个月后,8 岁的中国选手 <strong>Xuanyi Geng</strong> 用全 ZB 方法把 Ao5 WR 压到 <strong>3.71 秒</strong>。
                                                                                                                                                          本文综合曲线拟合 (Exp+floor / Gompertz / 幂律), 物理下界 (STM × TPS + R), 极值理论 (Gumbel/GEV reverse-Weibull) 三轨预测:</>} en={<>The 3x3 single WR was set at <strong>2.76 s</strong> by 9-year-old <strong>Teodor Zajder</strong> (Poland) on <strong>2026-02-08</strong>, the first sub-3 ever;
          ...
          <Section id="history" titleZh="23 年 WR 编年史" titleEn="23-Year WR Chronicle" isZh={isZh}>
            <p>
              {<T zh={<>WCA 2003 年 8 月 23-24 日在多伦多 (Ontario Science Centre) 世锦赛上成立,同时改写了停了 21 年的单次 WR — 美国选手 Dan Knights 用 <strong>16.71 秒</strong> 打破了 Minh Thai 1982 年的 22.95。此后 23 年,单次 WR 一路从 16.71 压到 Zajder 2.76,<strong>缩了 6.06 倍</strong>,平均每年下降 ~7%。注意 1982 那场不在本文趋势模型中 — 中间 21 年的「无数据空窗」对拟合没意义。</>} en={<>The WCA was founded at the August 23-24, 2003 World Championship in Toronto. Dan Knights (USA) ran a <strong>16.71</strong> on opening day — finally breaking Minh Thai's 22.95 from 1982 that had stood for 21 years. Over the next 23 years the single fell to Zajder's 2.76, a <strong>6.06× compression</strong>, ~7% per year compounded. The 1982 mark is excluded from the trend models — different era, 21-year data gap.</>} />}
            </p>
            <LineChart
            ...
            />
            <p>
              {<T zh={<>下方表格按时间排列,共 <strong>{WR_SINGLE_HISTORY.length} 次单次 WR 改写</strong>。STM/TPS 来自社区复盘。</>} en={<>The table below lists all <strong>{WR_SINGLE_HISTORY.length} single WR drops</strong>. STM/TPS from community reconstructions.</>} />}
            </p>
            ...
            <p>
              {<T zh={<>WCA 从 2007 年起以 Ao5 作为正式排名指标 (之前是 Mo3)。本表是 Ao5 时代的完整 WR,共 <strong>{WR_AO5_HISTORY.length} 次</strong>。Feliks Zemdegs 一人霸榜 9 年 (2010-2019)。</>} en={<>WCA adopted Ao5 as the official ranking metric in 2007 (Mo3 prior). Complete Ao5 WR table, <strong>{WR_AO5_HISTORY.length} drops</strong>. Zemdegs held it solo for ~9 years (2010-2019).</>} />}
            </p>
            <LineChart
          ...
          <Section id="reconstructions" titleZh="著名复盘 (STM / TPS)" titleEn="Famous Reconstructions" isZh={isZh}>
            <p>
              {<T zh={<>每次单次 WR 都对应一个 (打乱, 解, STM, TPS) 四元组。摆到一起看最清楚: <strong>TPS 一路涨,STM 因为幸运打乱 + 方法跃迁而出现非线性跳水</strong>。Wang 45 STM × 14.61 TPS (高 TPS 路径);Geng 33 STM × 10.81 TPS (高效路径);Zajder 29 STM × 10.50 TPS (高效 + ZBLL 组合)。</>} en={<>Every WR single = (scramble, solution, STM, TPS) signature. Stacked: <strong>TPS rises continuously; STM drops in jumps from lucky scrambles + method shifts</strong>. Wang 45 STM × 14.61 TPS (TPS path); Geng 33 STM × 10.81 TPS (efficient path); Zajder 29 STM × 10.50 TPS (combo path).</>} />}
            </p>
            ...
            <p className="pred-note">
              {<T zh={<><strong>核心观察:</strong> Du 3.47 的 27 STM 比 Zajder 2.76 的 29 STM 还少,但 Du 的 TPS 只有 7.78。如果 Du 那把打乱让 Zajder 来跑,理论上能跑出 27/14 ≈ 1.9 秒 — 这就是「百年内可达 ~1.5 秒」这条预测的来源: <strong>Du 的步数 × Wang 的 TPS</strong> = 把现役顶级选手叠在一起的最优值。</>} en={<><strong>Key cross-comparison:</strong> Du's 27 STM is fewer than Zajder's 29, but Du was only 7.78 TPS. Hand Du's scramble to Zajder-level TPS: 27/14 ≈ 1.9 s. This is the source of "100-yr reachable ~1.5 s" — <strong>Du's STM + Wang's TPS</strong> = "stacked optimum of living cubers".</>} />}
            </p>

          </Section>

          <Section id="state-space" titleZh="状态空间 4.3×10¹⁹" titleEn="State Space 4.3×10¹⁹" isZh={isZh}>
            <p>{<T zh={<>3x3 魔方的可达状态总数 <strong>43,252,003,274,489,856,000 ≈ 4.3 × 10¹⁹</strong>:</>} en={<>3x3 has <strong>43,252,003,274,489,856,000 ≈ 4.3 × 10¹⁹</strong> reachable states:</>} />}</p>
...
素因子分解: |G| = 2^27 · 3^14 · 5^3 · 7^2 · 11`}</pre>
            <ul>
              <li>{<T zh={<><strong>角朝向和 ≡ 0 mod 3。</strong> 每个角有 3 种扭向,第 8 个由前 7 个决定。</>} en={<><strong>Corner orientation sum ≡ 0 mod 3.</strong> Each has 3 twist states; the 8th is forced.</>} />}</li>
              <li>{<T zh={<><strong>棱朝向和 ≡ 0 mod 2。</strong> F/B 的 1/4 转翻 4 个棱,第 12 个由前 11 个强制确定。</>} en={<><strong>Edge orientation sum ≡ 0 mod 2.</strong> F/B quarter turns flip 4 edges; 12th is forced.</>} />}</li>
              <li>{<T zh={<><strong>角棱奇偶性耦合。</strong> 一次面的 1/4 转同时是角和棱的 4-cycle (奇置换),两个奇偶性永远同步 — 这就是 /2 的来源。</>} en={<><strong>Corner-edge parity coupling.</strong> Every face quarter turn is a 4-cycle on both corner and edge ring (both odd) — they stay locked. This is the /2.</>} />}</li>
            </ul>
            <p>
              {<T zh={<><strong>宇宙年龄对比。</strong> 宇宙年龄 ~4.36 × 10¹⁷ 秒。每秒数 1 个魔方状态,要 ~1.37 万亿年 ≈ <strong>宇宙年龄的 100 倍</strong>。</>} en={<><strong>Universe scale.</strong> Universe is ~4.36 × 10¹⁷ s old. Enumerate one state per second: <strong>1.37 trillion years ≈ 100× the age of the universe</strong>.</>} />}
            </p>
            <p>
              {<T zh={<><strong>信息论。</strong> 唯一编码一个状态需要 log₂(4.3 × 10¹⁹) ≈ <strong>65.22 bits</strong> ≈ 9 字节。</>} en={<><strong>Info-theoretic.</strong> Identifying a state takes log₂(4.3 × 10¹⁹) ≈ <strong>65.22 bits</strong> ≈ 9 bytes.</>} />}
            </p>
          </Section>

          <Section id="gods-number" titleZh="God's number 演化 (1981-2010)" titleEn="God's Number Evolution (1981-2010)" isZh={isZh}>
            <p>
              {<T zh={<>「God's number」是魔方群在指定度量下的 Cayley graph 直径,也就是最坏情况下的最优解长度。<strong>1981 年 Thistlethwaite 给出第一个上界 52 HTM</strong>,29 年后 Rokicki / Kociemba / Davidson / Dethridge (2010) 用 ~35 CPU-年的 Google 算力把 HTM 上界证到 <strong>20</strong>,而且至少有一个状态 (superflip composite) 恰好需要 20。</>} en={<>God's number is the Cayley graph diameter of the cube group under the chosen metric. <strong>Thistlethwaite (1981) gave the first upper bound, 52 HTM</strong>. 29 years later Rokicki et al. (2010) used ~35 CPU-years of Google compute to push HTM to <strong>exactly 20</strong>, with at least one position (superflip composite) requiring exactly 20.</>} />}
            </p>
            <div className="pred-method-table-wrap">
            ...
            <p>
              {<T zh={<><strong>Superflip</strong> (8 角正确,12 棱原位翻转) 是著名的 20-HTM-难态,位于魔方群的中心。STM 下只要 16 步即可解 — 这是「STM God's number 不可能小于 16」的下界。</>} en={<><strong>Superflip</strong> (8 corners correct, 12 edges flipped in place) is the classic 20-HTM-hard position; sits in the center of the cube group. Solvable in just 16 STM — source of the 16-STM lower-bound.</>} />}
            </p>
            <p>
              {<T zh={<><strong>Two-phase 算法 (Kociemba 1992)。</strong> Phase 1 把状态约简到子群 H = ⟨U, D, R², L², F², B²⟩,Phase 2 在 H 内求解。Phase 1 余类数 = |G|/|H| = 2,217,093,120。现代 Cube Explorer 和 nissy 是最优解器的两个标杆。</>} en={<><strong>Two-phase (Kociemba 1992).</strong> Phase 1 reduces to subgroup H = ⟨U, D, R², L², F², B²⟩; phase 2 within H. Phase 1 coset count = 2,217,093,120. Cube Explorer + Tronto's nissy are modern optimal-solver standards.</>} />}
            </p>
          </Section>

          <Section id="optimal-dist" titleZh="随机打乱的最优 HTM 分布" titleEn="Random Scramble Optimal HTM Distribution" isZh={isZh}>
            <p>
              {<T zh={<>cube20.org 全状态枚举给出的分布。<strong>67% 的随机打乱需要正好 18 HTM 最优解</strong>,平均 17.92 HTM,中位 18。只有 ~4.9 × 10⁸ 个状态需要 20 HTM (~10⁻¹¹)。</>} en={<>cube20.org full enumeration: <strong>67% need exactly 18 HTM</strong>; mean 17.92, median 18. Only ~4.9 × 10⁸ states need 20 HTM (~10⁻¹¹).</>} />}
            </p>
            <LineChart
            ...
            />
            <p>
              {<T zh={<><strong>对单次 WR 的含义。</strong> 顶级 CFOP 选手实际 50-58 STM,ZB 选手 48-52 STM,都是最优 HTM 的 2-3 倍。「幸运打乱」一般意味着抽到一个能用 X-cross / XX-cross 或者天然跳过 OLL/PLL 的低最优值打乱。Du 3.47 那把打乱最优是 18 HTM,Du 实际走了 28 STM 的方法路径 — 这不违反 God's number,因为 CFOP/ZB 本身就要为方法结构额外付出 10-20 步。</>} en={<><strong>Implication for single WRs.</strong> Top CFOP 50-58 STM, ZB 48-52 STM — both 2-3× optimal. A "lucky scramble" = low-optimal scramble admitting X-cross / XX-cross or natural skips. Du's scramble had optimal HTM = 18, his CFOP path was 28 STM — no violation of God's number; CFOP pays a +10-20 STM method tax.</>} />}
            </p>
          ...
          <Section id="metrics" titleZh="度量学: HTM vs STM vs QTM vs ATM" titleEn="Metrics: HTM vs STM vs QTM vs ATM" isZh={isZh}>
            ...
            <p>
              {<T zh={<><strong>STM 是速拧的自然度量。</strong> M / M' / M2 是「拇指 + 无名指」的单指动作,物理上一个动作。HTM 把它当 2 步,跟手部动作的实际负担对不上。本文 TPS 和步数全用 STM。HTM 用于学术,QTM 用于群论。</>} en={<><strong>STM is the natural cubing metric.</strong> M is a single thumb-ring fingertrick — HTM counting it as 2 face turns is wrong for biomech. All TPS / move counts use STM here. HTM for academic, QTM for group theory.</>} />}
            </p>
            <p>
              {<T zh={<><strong>HTM → STM 换算。</strong> CFOP 用 slice 少,HTM ≈ STM。Roux 的 M-slice 多,HTM 比 STM 高 5-10%。跨方法比较步数必须先统一到 STM,否则 Roux 的 48 STM 会被误显示成 53 HTM。</>} en={<><strong>HTM→STM conversion.</strong> CFOP slice-rare so HTM ≈ STM. Roux relies on M-slice; HTM exceeds STM by 5-10%. Cross-method comparison must normalize to STM, or Roux's 48 STM shows as 53 HTM.</>} />}
            </p>
          ...
          <Section id="method-cfop" titleZh="CFOP 解剖学: Cross → F2L → OLL → PLL" titleEn="CFOP Anatomy: Cross → F2L → OLL → PLL" isZh={isZh}>
            <p>
              {<T zh={<>CFOP 由 Jessica Fridrich 1997 年公开,跟 Hans Dockhorn 和 Anneke Treep 等人 1980 年代的独立工作汇流,共 <strong>119 个核心算法</strong> (41 F2L + 57 OLL + 21 PLL),顶级实际跑 ~<strong>57.5 HTM</strong>。23 年 WR 历程里绝对的主流方法。</>} en={<>CFOP, published by Jessica Fridrich 1997 with 1980s independent work by Hans Dockhorn and Anneke Treep, <strong>119 core algorithms</strong> (41 F2L + 57 OLL + 21 PLL), <strong>~57.5 HTM</strong> at speed. Dominant method behind 23 years of WRs.</>} />}
            </p>
            <div className="pred-method-table-wrap">
              <table className="pred-fit-table pred-method-table">
            ...
            <p>
              {<T zh={<><strong>色中性 (CN) 的收益。</strong> 固定颜色 cross 平均 5.81 HTM,全色中性 (CN) <strong>4.81 HTM</strong>。关键不是均值差,是 P(≤ 4 步 cross) 从固定色 5.99% 跳到 CN 29.17% — <strong>5 倍简单 cross</strong>。这就是 CN 在顶级普及的原因。</>} en={<><strong>Cross color neutrality (CN).</strong> Fixed color 5.81 HTM avg, full CN <strong>4.81</strong>. The win isn't the mean — P(≤4-move cross) jumps from 5.99% to <strong>29.17%</strong>, ~5× as many easy crosses. Why CN is universal at the top.</>} />}
            </p>
            <p>{<T zh={<><strong>F2L 41 种 case</strong> 平均 6.7 STM/槽,4 槽合计 26.8 STM。顶级选手的 F2L:</>} en={<><strong>F2L 41 cases</strong>, 6.7 STM/slot avg, 26.8 total. Elite F2L tricks:</>} />}</p>
            <ul>
              <li>{<T zh={<><strong>Pseudoslotting</strong>: 把 F 或 B 面错位 90° 当作起手。</>} en={<><strong>Pseudoslotting</strong>: misalign F or B by 90° as setup.</>} />}</li>
              <li>{<T zh={<><strong>Multislotting</strong>: 一组动作同时解两对 F2L。</>} en={<><strong>Multislotting</strong>: one sequence inserts two pairs.</>} />}</li>
              <li>{<T zh={<><strong>EO-during-F2L</strong>: 即 ZBLS,最后一对 F2L 同时控 EO。</>} en={<><strong>EO-during-F2L</strong>: i.e. ZBLS, last pair while orienting LL edges.</>} />}</li>
            </ul>
          ...
          <Section id="method-oll" titleZh="OLL 57 case 全表" titleEn="OLL: All 57 Cases" isZh={isZh}>
            <p>{<T zh={<><strong>57 种 case</strong>,算法长度 STM 7-14 不等:</>} en={<><strong>57 cases</strong>, alg lengths 7-14 STM:</>} />}</p>
            <LineChart
            ...
            />
            <p>{<T zh={<><strong>双峰</strong>: 9 STM (14 个 case) 和 11 STM (10 个 case) 是高点;14 STM 只 1 个 (Dot OLL #57)。<strong>Sune / Anti-Sune (#26/27) 是经典的 7 步</strong>。</>} en={<><strong>Bimodal</strong>: 9 STM (14) and 11 STM (10) peak; only OLL #57 (Dot) at 14. <strong>Sune / Antisune (#26/27) classic 7-move</strong>.</>} />}</p>
          </Section>

          <Section id="method-pll" titleZh="PLL 21 case 全表" titleEn="PLL: All 21 Cases" isZh={isZh}>
            <p>{<T zh={<><strong>21 种 case</strong> (4 对镜像 → 17 个独立算法)。均长 12.5 STM:</>} en={<><strong>21 cases</strong> (4 mirror pairs → 17 independent algs). Mean 12.5 STM:</>} />}</p>
            <div className="pred-method-table-wrap">
              <table className="pred-fit-table pred-method-table">
            ...
            <p>{<T zh={<><strong>4 个 PLL 视觉样本</strong>:</>} en={<><strong>4 iconic PLL previews</strong>:</>} />}</p>
          ...
          <Section id="method-zb" titleZh="ZB / ZBLS / ZBLL — 顶级方法栈" titleEn="ZB / ZBLS / ZBLL — The Elite Stack" isZh={isZh}>
            <p>
              {<T zh={<>ZB 方法把 CFOP 的 OLL + PLL 替换为 <strong>ZBLS (302-303 个 case)</strong> + <strong>ZBLL (493 个 case)</strong>。ZBLS 解最后一对 F2L 的同时把 LL 4 棱朝向也解掉;ZBLL 在 EO 已知的前提下 1 个算法解掉整个 LL。</>} en={<>ZB replaces CFOP's OLL + PLL with <strong>ZBLS (302-303 cases)</strong> + <strong>ZBLL (493 cases)</strong>. ZBLS solves last pair while orienting LL edges; ZBLL solves entire LL in one alg given EO.</>} />}
            </p>
            <p>{<T zh={<><strong>ZBLL 8 个子集</strong>:</>} en={<><strong>ZBLL 8 sub-families</strong>:</>} />}</p>
            <div className="pred-method-table-wrap">
              <table className="pred-fit-table">
            ...
            <p>
              {<T zh={<><strong>ZBLS + ZBLL 合计 ~795-798 个算法</strong>。现役全 ZB 用户: <strong>Xuanyi Geng</strong> (Ao5 WR 3.71),<strong>Qixian Cao</strong> (Worlds 2025 5.07),<strong>Tymon Kolasiński</strong>。Wang 3.08 和 Park 3.13 走的是 CFOP + 部分 ZBLL,不是全 ZB。</>} en={<><strong>Total ZBLS + ZBLL = ~795-798 algs</strong>. Full-ZB users: <strong>Xuanyi Geng</strong> (Ao5 WR 3.71), <strong>Qixian Cao</strong> (Worlds 2025 5.07), <strong>Tymon Kolasiński</strong>. Wang and Park are CFOP + partial ZBLL.</>} />}
            </p>
            <p>
              {<T zh={<><strong>1LLL</strong> — 把整个 OLL+PLL 合到一个算法里,<strong>3,915 个 case</strong>。<strong>Eduardo Silva Damasceno 2022 年第一个完整学完</strong>。<em>WR 水平上没人用</em> — 识别要 0.8-1.4 秒,比 OLL+PLL 拆两步 (0.4+0.4) 还慢。1LLL 撞到的是「算法学习极限」,不是「速拧极限」。</>} en={<><strong>1LLL</strong> — full OLL+PLL in one alg, <strong>3,915 cases</strong>. <strong>Eduardo Silva Damasceno first fully learned in 2022</strong>. <em>Nobody uses it at WR speed</em> — recognition 0.8-1.4s slower than OLL+PLL split. 1LLL is the "alg-learning limit" not the "speedcubing limit".</>} />}
            </p>
          </Section>

          <Section id="method-roux" titleZh="Roux / ZZ / Petrus / Mehta — 非 CFOP 派" titleEn="Roux / ZZ / Petrus / Mehta — Non-CFOP" isZh={isZh}>
            <p>
              {<T zh={<><strong>Roux (Gilles Roux, 2003)。</strong> 4 步: 左 1×2×3 → 右 1×2×3 → CMLL (42 个算法) → LSE (用 M, U 解最后 6 棱)。平均 <strong>~48 STM</strong>。<strong>单次从未破过 4 秒</strong>;M-slice 持续 TPS &lt; 7。顶级 Roux 用户: Kian Mansour, Sean Patrick Villanueva, Alexey Tsvetkov (3.95 — 唯一一次 sub-4 Roux 单次)。</>} en={<><strong>Roux (Gilles Roux 2003).</strong> 4 steps: left 1x2x3 → right 1x2x3 → CMLL (42) → LSE (M, U for last 6 edges). <strong>~48 STM</strong>. <strong>Never broke sub-4 single</strong>; M-slice sustained TPS &lt; 7. Top Roux: Kian Mansour, Sean Patrick Villanueva, Alexey Tsvetkov (3.95 — only sub-4 Roux ever).</>} />}
            </p>
            <p>
              {<T zh={<><strong>ZZ (2006)。</strong> 起手 <strong>EOLine</strong> 把 12 棱朝向 + DF/DB pair 一次性解掉,F2L 只用 R/U/L。算法量 28 (ZZ-A) 到 493 (ZZ+ZBLL)。<strong>从未拿过 WR</strong> — EOLine 在 15 秒 inspection 内规划极难。</>} en={<><strong>ZZ (2006).</strong> Opens with <strong>EOLine</strong> (12 edges oriented + DF/DB pair in one step); F2L uses only R/U/L. Algs 28-493. <strong>Never held WR</strong> — EOLine inspection planning is brutally hard.</>} />}
            </p>
            <p>
              {<T zh={<><strong>Petrus (Lars Petrus, 1981)。</strong> 7 步: 2×2×2 → 2×2×3 → EO → F2L → COLL → CPLL → EPLL,~45 STM。CFOP 兴起前是用户数第二的方法,2004 之后式微。</>} en={<><strong>Petrus (Lars Petrus 1981).</strong> 7 steps; ~45 STM. #2 method pre-CFOP era; declined after 2004.</>} />}
            </p>
            <p>
              {<T zh={<><strong>Mehta (Yash Mehta, 2020)。</strong> FB → 3QB → EOLE → 4 种结束。Mehta-TDR 的 843 个算法是目前提出过的速拧方法里算法量最大的。社区不大,top-50 里没人用。</>} en={<><strong>Mehta (Yash Mehta 2020).</strong> FB → 3QB → EOLE → 4 finishes. Mehta-TDR's 843 algs is the highest-alg-count speed method proposed. Small community, no top-50 user.</>} />}
            </p>
          </Section>

          <Section id="lookahead" titleZh="F2L lookahead — 顶级速度的真正瓶颈" titleEn="F2L Lookahead — The Real Bottleneck" isZh={isZh}>
            <p>
              {<T zh={<><strong>Lookahead</strong> = 执行第 N 对的同时,眼睛已经在追第 N+1 对。<strong>故意把执行放慢,给眼睛留出反应时间,总时间反而更快</strong>。Zemdegs 的 CubeSkills 分三阶段:</>} en={<><strong>Lookahead</strong> = executing pair N while eyes track pair N+1. <strong>Deliberately slow execution; total time drops</strong>. Zemdegs CubeSkills splits into 3 stages:</>} />}
            </p>
            <ul>
              <li>{<T zh={<><strong>Spotting</strong>: 暂停后用眼睛找 (新手 / 中阶)。</>} en={<><strong>Spotting</strong>: pause, find with eyes (beginner/intermediate).</>} />}</li>
              <li>{<T zh={<><strong>Tracking</strong>: 转动中眼睛追 (sub-12+)。</>} en={<><strong>Tracking</strong>: eyes follow during turns (sub-12+).</>} />}</li>
              <li>{<T zh={<><strong>Knowing</strong>: 不看就能推断位置 (sub-8 顶尖)。</>} en={<><strong>Knowing</strong>: predict location no-look (sub-8 elite).</>} />}</li>
            </ul>
            <p>{<T zh={<><strong>lookahead 失效的三种情形</strong>: 幸运打乱, multislotting, pseudoslotting。</>} en={<><strong>3 lookahead failure modes</strong>: lucky scrambles, multislotting, pseudoslotting.</>} />}</p>
          </Section>

          <Section id="inspection" titleZh="Inspection 运筹 — 15 秒怎么用" titleEn="Inspection — How to Spend 15 Seconds" isZh={isZh}>
            <p>{<T zh={<><strong>WCA A3</strong>: inspection 上限 15 秒,16-17 秒 +2,≥ 17 秒 DNF。顶级选手流程:</>} en={<><strong>WCA A3</strong>: 15s cap, 16-17s +2, ≥17s DNF. Elite flow:</>} />}</p>
            <ol>
              <li>{<T zh={<><strong>0-5 秒</strong>: 扫色中性,锁定最优 cross 颜色。</>} en={<><strong>0-5 s</strong>: CN scan, lock best cross color.</>} />}</li>
              <li>{<T zh={<><strong>5-11 秒</strong>: 规划完整 cross 序列 (6-8 步,ergo-optimal)。</>} en={<><strong>5-11 s</strong>: plan full cross (6-8 moves, ergo-optimal).</>} />}</li>
              <li>{<T zh={<><strong>11-14 秒</strong>: 定位第一对 F2L。</>} en={<><strong>11-14 s</strong>: locate first F2L pair.</>} />}</li>
              <li>{<T zh={<><strong>15 秒</strong>: 确认握姿,启动 StackMat。</>} en={<><strong>15 s</strong>: confirm grip, start StackMat.</>} />}</li>
            </ol>
            <p>{<T zh={<><strong>X-cross inspection</strong> 是难度最高也回报最大的。<strong>CN 选手比固定色看到 X-cross 的概率多 ~5 倍</strong>。</>} en={<><strong>X-cross inspection</strong> is the hardest, most rewarding skill. <strong>CN cubers see X-cross ~5× more often</strong>.</>} />}</p>
          </Section>
          ...
          <Section id="skips" titleZh="幸运打乱 + skip 概率" titleEn="Lucky Scrambles + Skip Probability" isZh={isZh}>
            ...
            <p>
              {<T zh={<><strong>P(LL skip) = 1/216 × 1/72 = 1/15,552 ≈ 0.00643%</strong>。顶级选手一生 ~10⁶ 次解里,期望出现 60 次。Du 3.47 和 Park 3.13 都吃到 PLL skip。<strong>Zajder 2.76 没靠 skip</strong> — 而是用 XX-cross + ZBLL 把 STM 压到 29。</>} en={<><strong>P(LL skip) = 1/216 × 1/72 = 1/15,552 ≈ 0.00643%</strong>. Top cuber career ~10⁶ solves → ~60 expected. Du 3.47 / Park 3.13 had PLL skip. <strong>Zajder 2.76 didn't use skip</strong> — XX-cross + ZBLL gave 29 STM.</>} />}
            </p>
          </Section>

          <Section id="hardware" titleZh="硬件 1980-2026" titleEn="Hardware 1980-2026" isZh={isZh}>
            <p>
              {<T zh={<>46 年硬件演化分四段: <strong>1980-2010 原始期</strong> (原版 Rubik / Type-A) → <strong>2010-2014 现代速拧诞生</strong> (DaYan GuHong / ZhanChi / MoYu AoLong) → <strong>2016-2020 磁铁革命</strong> (Cubicle Mod / GAN 356 Air UM / GAN 11 M Pro 核心磁) → <strong>2021-2026 磁场网络 + 智能化</strong> (GAN 12-16 MagLev / MoYu Super RS3M)。</>} en={<>46 years in 4 eras: <strong>1980-2010 primitive</strong> → <strong>2010-2014 modern speedcube birth</strong> → <strong>2016-2020 magnet revolution</strong> → <strong>2021-2026 magnetic networks + smart cubes</strong>.</>} />}
            </p>
            <div className="pred-method-table-wrap">
              <table className="pred-fit-table pred-method-table">
            ...
            <p>
              {<T zh={<><strong>值得注意一点。</strong> Zajder 2.76 用的是 <strong>2021 年的 GAN 12 M</strong>,不是最新的 GAN 16。<strong>硬件已经不是顶级 WR 的瓶颈了</strong>,后续突破来自方法 (ZBLL) 和训练 (智能魔方)。</>} en={<><strong>Key finding.</strong> Zajder's 2.76 used a <strong>2021 GAN 12 M</strong>, not the latest GAN 16. <strong>Hardware is no longer the binding constraint at the top</strong>; gains come from methods (ZBLL) + training (smart cubes).</>} />}
            </p>
          ...
          <Section id="smart-cube" titleZh="智能魔方革命 (2019-2026)" titleEn="Smart Cube Revolution (2019-2026)" isZh={isZh}>
            <p>
              {<T zh={<>智能魔方 = BLE + 陀螺仪 + 电池 + 配套 App。<strong>GAN 356 i (2019)</strong> 是首款大规模商业化的 BLE 速拧魔方。之后有 i2 (2021), i3 (2022), i Carry 2 (2024 — 无底座,700 小时电池)。配套 App: GAN Cube Station, Cubeast (第三方专业级), csTimer (开源,2020+ 支持 BLE)。</>} en={<>Smart cube = BLE + gyro + battery + app. <strong>GAN 356 i (2019)</strong> the first mainstream BLE speedcube. Iterations i2 (2021), i3 (2022), i Carry 2 (2024 — dockless, 700h battery). Apps: GAN Cube Station, Cubeast (3rd-party speedcubing-grade), csTimer (open source, BLE since 2020).</>} />}
            </p>
            <ul>
              <li>{<T zh={<><strong>初 / 中级 (sub-20 → sub-12)</strong>: 价值最大。Cubeast 给按步切分的时间,sub-15 → sub-12 周期缩短 30-50%。</>} en={<><strong>Beginner/intermediate</strong>: highest value. Cubeast per-step splits; sub-15 → sub-12 progression 30-50% faster.</>} />}</li>
              <li>{<T zh={<><strong>高级 (sub-10 → sub-6)</strong>: 训练新方法 (ZBLL 识别) + 监测平台期。</>} en={<><strong>Advanced</strong>: new method drilling + plateau detection.</>} />}</li>
              <li>{<T zh={<><strong>顶级 (sub-5 → WR)</strong>: 只用作训练分析 (WCA 不允许比赛使用)。反馈循环更短,刷 PR 节奏更快。</>} en={<><strong>Elite</strong>: analysis only (WCA disallows in comp). Faster feedback shortens PR cadence.</>} />}</li>
            </ul>
            <p>
              {<T zh={<><strong>2022 后的中国速拧少年群体。</strong> Wang (生于 2013),Geng (生于 2017),Zajder (~2016) — 学习过程几乎完全在智能魔方时代。预计 2030+ 进入 sub-3 的主要是这一代。</>} en={<><strong>Post-2022 Chinese child cohort.</strong> Wang (b. 2013), Geng (b. 2017), Zajder (~2016) — learning trajectory entirely in smart-cube era. Expect post-2030 sub-3 contenders predominantly from this generation.</>} />}
            </p>
          </Section>
          ...
          <Section id="biomech" titleZh="生物力学: TPS 的硬天花板" titleEn="Biomech: The TPS Ceiling" isZh={isZh}>
            ...
            <ul>
              <li>{<T zh={<><strong>钢琴单指敲击</strong>: Aoki & Kinoshita 2001 (Ergonomics 44(15)) — 钢琴家单指 6.0-6.7 Hz (食指 / 中指),5.0-5.5 Hz (无名指 / 小指)。</>} en={<><strong>Piano single-finger tap</strong>: Aoki & Kinoshita 2001 (Ergonomics 44(15)) — pianists 6.0-6.7 Hz (index/middle), 5.0-5.5 Hz (ring/little).</>} />}</li>
              <li>{<T zh={<><strong>双手交替击鼓</strong>: Keita Hattori 2024 Guinness — 22.2 strokes/s (1334/min)。双手交替的上限。</>} en={<><strong>Dual-hand drum stroke</strong>: Keita Hattori 2024 Guinness — 22.2 strokes/s (1334/min). Two-hand alternating ceiling.</>} />}</li>
              <li>{<T zh={<><strong>速拧持续 TPS</strong>: Wang 3.08 全程 14.61 — 现役 WR 最高。Feliks 2012 估的 11-12 上限,已被 Wang 这一代打破。</>} en={<><strong>Sustained cubing TPS</strong>: Wang 3.08 = 14.61 sustained. Feliks 2012 estimated 11-12 ceiling, broken by Wang generation.</>} />}</li>
            </ul>
            <p>
              {<T zh={<><strong>速拧 TPS 顶端估计</strong>: 持续 17 TPS,突发 20-22。17 × 50 STM = 2.94 秒,17 × 28 STM (Zajder 路径) = 1.65 秒 — 跟「百年内可达 1.5 秒」一致。</>} en={<><strong>Cubing TPS ceiling estimate</strong>: sustained 17, burst 20-22. 17 × 50 STM = 2.94 s, 17 × 28 STM (Zajder path) = 1.65 s — matches "100-yr reachable 1.5 s".</>} />}
            </p>
            <p>{<T zh={<><strong>步数-TPS 等高线</strong> (T = STM/TPS + 0.05 秒反应):</>} en={<><strong>STM-TPS contours</strong> (T = STM/TPS + 0.05 s reaction):</>} />}</p>
            <div className="pred-method-table-wrap">
              <table className="pred-fit-table">
            ...
            <p>
              {<T zh={<><strong>读图法。</strong> Zajder 2.76 落在 29 STM × 10.5 TPS → 表中 30/10 = 3.05 (实测 2.76 反映 ZBLL skip)。100 年渐近 ~24 STM × 16 TPS ≈ 1.55 秒。数学硬墙 ~16 STM × 17 TPS = 1.0 秒。</>} en={<><strong>Reading.</strong> Zajder 2.76 ≈ 29 STM × 10.5 TPS → cell 30/10 = 3.05. 100-yr asymptote ≈ 24 STM × 16 TPS ≈ 1.55 s. Math wall ≈ 16 STM × 17 TPS = 1.0 s.</>} />}
            </p>
          ...
          <Section id="training" titleZh="训练学方法 — 量化的练习路径" titleEn="Training Methodology — Quantitative Practice" isZh={isZh}>
            <p>{<T zh={<><strong>累计 solves 与 PB 阈值关系</strong> (社区共识):</>} en={<><strong>Cumulative solves vs PB threshold</strong> (community consensus):</>} />}</p>
            <div className="pred-method-table-wrap">
              <table className="pred-fit-table pred-method-table">
            ...
            <p>{<T zh={<><strong>训练分配 (顶级共识)</strong>:</>} en={<><strong>Practice composition (consensus)</strong>:</>} />}</p>
            <ul>
            ...
            <p>{<T zh={<><strong>训练 PB 跟 WCA PB 的差距</strong>: 5-10%。原因: 魔方冷启动, 打乱验证延迟, 紧张, 没热身。</>} en={<><strong>Training-comp gap</strong>: 5-10%. Causes: cube cooldown, scramble verification delay, anxiety, no warmup.</>} />}</p>
          </Section>
          ...
          <Section id="stats" titleZh="统计建模: 4 个独立模型" titleEn="Statistical Modeling: 4 Independent Models" isZh={isZh}>
            ...
            <ol>
              <li>{<T zh={<><strong>Exp + floor。</strong> T(t) = L + A · exp(−k(t−t₀))。L 用网格搜索。优势: floor 可解释;劣势: 单 floor 假设。</>} en={<><strong>Exp + floor.</strong> Grid search L. Pro: floor interpretable. Con: single-floor assumption.</>} />}</li>
              <li>{<T zh={<><strong>Gompertz 衰减。</strong> S 形,比 exp+floor 多一个拐点。</>} en={<><strong>Gompertz decay.</strong> S-shaped; adds an inflection vs exp+floor.</>} />}</li>
              <li>{<T zh={<><strong>幂律。</strong> T(t) = a · t^(−b)。无 floor,长期撞 0。</>} en={<><strong>Power law.</strong> No floor; hits 0 long-term.</>} />}</li>
              <li>{<T zh={<><strong>GEV reverse-Weibull。</strong> 极值参数 ξ &lt; 0 给有限 endpoint,拟合 1982-2026 WR 得 τ̂ ≈ 2.0 ± 0.4 秒。</>} en={<><strong>GEV reverse-Weibull.</strong> Shape ξ &lt; 0 gives finite endpoint; fit on 1982-2026 WR yields τ̂ ≈ 2.0 ± 0.4 s.</>} />}</li>
            </ol>
            <p>{<T zh={<><strong>Walk-forward backtest</strong> (训练 2003-2020, 预测 2021-2026):</>} en={<><strong>Walk-forward backtest</strong> (train 2003-2020, forecast 2021-2026):</>} />}</p>
            <div className="pred-method-table-wrap">
              <table className="pred-fit-table">
            ...
            <p>{<T zh={<><strong>BMA 结果:</strong> 磁铁时代加速 (post-2017) 让 GEV (重尾) 最准。权重: GEV 0.55, Exp-floor 0.30, Gompertz 0.15。</>} en={<><strong>Key finding.</strong> Magnet-era acceleration (post-2017) makes GEV most accurate. BMA: GEV 0.55, Exp-floor 0.30, Gompertz 0.15.</>} />}</p>
          </Section>

          <Section id="gev" titleZh="GEV 极值理论" titleEn="GEV Extreme Value Theory" isZh={isZh}>
            <p>{<T zh={<>WR 单次本质上是 <strong>N 次独立尝试的样本最小值</strong>。Gumbel 渐近:</>} en={<>WR single = <strong>sample minimum of N attempts</strong>. Gumbel asymptotic:</>} />}</p>
...
T_min ≈ 4.5 × exp(−0.12 × √(2 × 16.12))
      = 4.5 × exp(−0.68) ≈ 2.27 s`}</pre>
            <p>{<T zh={<><strong>3 个失效模式</strong>: (1) log-normal 下尾被物理 floor 截断,公式没这约束;(2) 各次解不独立 (硬件代际 / 同期组非平稳);(3) N → ∞ 时 Gumbel 不停外推,真实极值受 16-STM 下界硬截。严格建模用 <strong>reverse-Weibull (ξ &lt; 0)</strong>,给出有限端点 τ ≈ 2.0 秒,跟物理 floor 一致。</>} en={<><strong>Three failure modes</strong>: (1) lower tail truncated by physical floor; (2) non-iid (hardware era / cohort non-stationary); (3) Gumbel extrapolates to 0 as N→∞ vs cube20.org 16-STM floor. Rigorous modeling uses <strong>reverse-Weibull (ξ &lt; 0)</strong> giving finite endpoint τ ≈ 2.0 s, matching physical floor.</>} />}</p>
          </Section>
          ...
          <Section id="forecast" titleZh="综合预测 — Single + Ao5 (BMA Ensemble)" titleEn="Final Forecast — Single + Ao5 (BMA Ensemble)" isZh={isZh}>
            ...
            <p className="pred-note">
              {<T zh={<><strong>建模。</strong> BMA 集成 (GEV 0.55 + Exp-floor 0.30 + Gompertz 0.15),80% CI 来自残差 bootstrap。Ao5 / 单次 比例由 σ_log = 0.10-0.12 + Ao5 trimmed mean √5 收缩推导,顶级同轮 Ao5 / min-single ≈ 1.25-1.35。<strong>下界不超过物理 floor 0.99 秒 和 Ao5 1.9 秒</strong>。</>} en={<><strong>Methodology.</strong> BMA ensemble (GEV 0.55 + Exp-floor 0.30 + Gompertz 0.15), 80% CI from residual bootstrap. Ao5/single ratio from σ_log = 0.10-0.12 + Ao5 trimmed-mean √5 shrinkage; top same-round Ao5/min-single ≈ 1.25-1.35. <strong>Lower bounds capped at physical floor 0.99 s and Ao5 1.9 s</strong>.</>} />}
            </p>
          ...
          <Section id="caveats" titleZh="局限与陷阱" titleEn="Caveats & Pitfalls" isZh={isZh}>
            <ol>
              <li>{<T zh={<><strong>未来的「方法革命」是离散事件。</strong> ZB → 1LLL 或者全新方法不会出现在已有的曲线趋势里。历史上每 8-10 年一次方法革命。</>} en={<><strong>Future "method revolutions" are discrete.</strong> ZB→1LLL or new methods aren't in the trend. Cadence ~8-10 years.</>} />}</li>
              <li>{<T zh={<><strong>极值统计的样本依赖。</strong> N 增长 → 期望最小值下移 1-2% (N = 10⁷)。</>} en={<><strong>Extreme-value sample dependence.</strong> More attempts → expected min drops 1-2% (N = 10⁷).</>} />}</li>
              <li>{<T zh={<><strong>WCA 规则未来不可预测。</strong> 历史每 5-10 年出现 1-3% 的规则不连续。</>} en={<><strong>WCA rule unpredictability.</strong> 1-3% historical discontinuities every 5-10 yrs.</>} />}</li>
              <li>{<T zh={<><strong>训练 - 比赛差距 ~5-10%。</strong> 即使训练 PB sub-2,比赛仍要拿走一截。</>} en={<><strong>Training-comp gap stays at 5-10%.</strong> Even if training sub-2, comp takes a slice.</>} />}</li>
              <li>{<T zh={<><strong>「未知的未知」。</strong> 顶级选手的老化曲线?心理上限?都没有同行评审的数据。</>} en={<><strong>"Unknown unknowns".</strong> Top-cuber aging curves? Psychological ceiling? No peer-reviewed data.</>} />}</li>
            </ol>
            <p>{<T zh={<><strong>最终评估。</strong> 所有预测都是最佳估计,5 年 ±10%,25 年 ±30%,50 年的误差量级与当前数值相当。这套预测不是预言,是一个框架 — 数据变了就跟着迭代。</>} en={<><strong>Final assessment.</strong> All forecasts best-estimate; 5-yr ±10%, 25-yr ±30%, 50-yr order-of-magnitude. Not an oracle — a framework to iterate.</>} />}</p>
...
function Section({ id, titleZh, titleEn, isZh, children }: { id: string; titleZh: string; titleEn: string;
 isZh: boolean; children: React.ReactNode }) {
