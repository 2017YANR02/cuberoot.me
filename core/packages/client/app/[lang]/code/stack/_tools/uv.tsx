...
import { k, s, f, c } from '../_lib/stack_tool_types';
...
export const UV: StackTool = {
  ...
  zh: {
    ...
    role: '管理 Python 工具链 + 依赖 + venv。本站评估过, 最终没用——把那几个 Python 脚本直接移植成 TS 了。',
    ...
    cuberootDesc: <>本站原来 <code>scripts/</code> 下有 3 个 Python 脚本 (拉 WCA 比赛、生成中文翻译、build attempts)。2026-06 评估 uv 后选了反方向:整个仓库已是 pnpm + TS monorepo, 与其再引一套 Python 工具链, 不如把这 3 个脚本忠实移植成 TS, 把 Python 整个从 CI 去掉。</>,
    outlookTitle: '当下与前景',
  ...
  en: {
    ...
    role: 'Python toolchain + deps + venvs in one binary. This site evaluated it but went the other way — ported its Python scripts to TypeScript instead.',
    ...
    cuberootDesc: <>The site used to have 3 Python scripts in <code>scripts/</code> (fetch WCA comps, generate zh translations, build attempts JSON). In June 2026, after weighing uv, it went the other way: the repo is already a pnpm + TS monorepo, so rather than add a Python toolchain it ported all three faithfully to TypeScript and dropped Python from CI entirely.</>,
  ...
  whyCards: [
    ...
    { icon: '✺', zh: { title: <>PEP 723 让脚本独立</>, desc: <>单文件 Python 工具不再需要"先 pip install xxx"前置说明。脚本自带依赖声明, 给同事一份 .py 就能跑。"几个一次性 build 脚本"这种场景特别合适。</> }, en: { title: <>PEP 723 makes scripts self-contained</>, desc: <>Single-file Python tools no longer need "first pip install xxx" preambles. Scripts ship with their deps inline; send a coworker one .py and they can run it. A perfect fit for ad-hoc build scripts.</> }, code: <>{c('# /// script')}{'\n'}{c('# dependencies = ["httpx"]')}{'\n'}{c('# ///')}</> },
  ...
  adopters: [
    ...
    { name: 'cuberoot.me', highlight: true, zhNote: '评估 uv 后反向:Python 脚本移植成 TS, CI 去 Python', enNote: 'Weighed uv, went the other way — ported Python scripts to TS, dropped Python from CI' },
  ],
  outlook: [
  ...
  cuberoot: {
    zh: (
      <>
        <p>本站原来 <code>scripts/</code> 下有三个 Python 脚本 —— <code>build_wca_attempts.py</code> (从 WCA dump 生成 attempts JSON)、<code>fetch_comp_names_zh.py</code> (拉中文比赛名翻译)、<code>fetch_upcoming_comps.py</code> (从 WCA API 拉将来比赛) —— 都是 CI 里跑的数据管道, 依赖很轻 (基本就 requests + json + 标准库)。</p>
        <p>这篇本来想拿"下一步迁 uv"当结尾, 但真到决策时选了反方向。原因很直接:整个仓库已经是 pnpm + TypeScript monorepo, CI 早就在装 Node / pnpm。这三个脚本若迁 uv, CI 就得同时养两套工具链 (Node + Python/uv);而脚本本身又轻到没有任何非 Python 不可的依赖。两相权衡, 与其引 uv, 不如把 Python 整个去掉。</p>
        <p>2026-06 把这三个脚本忠实移植成 TypeScript, 放进 <code>packages/stats-build/src/bin/</code>, CI 直接 <code>npx tsx</code> 跑。逐行对照 + 多轮对抗性审查 + 字节级对拍 (一个坑值得记:Python 的 <code>requests</code> timeout 是读超时, JS 的 <code>AbortController</code> 是硬总超时, 大响应端点得放宽总预算否则会 abort 丢数据)。迁完 <code>scripts/</code> 整个删掉, CI 不再有 Python。</p>
        <p>这不是说 uv 不好 —— 对一个真正以 Python 为主语言的项目, uv 仍是 2026 年的正确默认。只是 cuberoot 这种"几个轻量数据脚本挂在一个全 TS 仓库上"的具体形态, 消掉语言边界比统一 Python 工具链更省事。换个技术栈 (脚本依赖 numpy / pandas / PyTorch), 结论就会反过来。</p>
      </>
    ),
    en: (
      <>
        <p>This site used to have three Python scripts in <code>scripts/</code> — <code>build_wca_attempts.py</code> (turns the WCA dump into attempts JSON), <code>fetch_comp_names_zh.py</code> (pulls Chinese comp-name translations), <code>fetch_upcoming_comps.py</code> (pulls future comps from the WCA API) — all CI data-pipeline jobs with light deps (basically requests + json + the standard library).</p>
        <p>This section was going to end with "uv is the next step," but at decision time the site went the other way. The reasoning is direct: the repo is already a pnpm + TypeScript monorepo and CI already installs Node / pnpm. Moving these scripts to uv would mean CI maintaining two toolchains (Node + Python/uv), while the scripts were light enough to have nothing that genuinely required Python. On balance, rather than add uv, it made more sense to remove Python entirely.</p>
        <p>In June 2026 the three scripts were ported faithfully to TypeScript, dropped into <code>packages/stats-build/src/bin/</code>, and CI now runs them with <code>npx tsx</code>. The port was line-by-line with several adversarial review rounds and byte-level output diffing (one trap worth recording: Python’s <code>requests</code> timeout is a per-read timeout, while JS <code>AbortController</code> is a hard total deadline — large-response endpoints need a generous total budget or the fetch aborts mid-body and drops data). Once migrated, <code>scripts/</code> was deleted outright; CI has no Python left.</p>
        <p>None of this is a knock on uv — for a project where Python is genuinely the primary language, uv is still the right 2026 default. It’s just that for cuberoot’s specific shape (a handful of light data scripts hanging off an all-TS repo), erasing the language boundary was less work than unifying on a Python toolchain. Change the stack (scripts needing numpy / pandas / PyTorch) and the conclusion flips.</p>
      </>
    ),
  },
  links: [
