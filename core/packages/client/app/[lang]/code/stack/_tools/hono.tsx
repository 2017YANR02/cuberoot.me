  ...
  cuberoot: {
    zh: (
      <>
        ...
        <p>客户端 fetch 全部走 <code>utils/api_base.ts</code> 的 <code>apiUrl()</code>。<code>import.meta.env.DEV</code> 切 dev/prod API base (不能用 hostname 检查, LAN / 隧道域名都不匹配)。RPC 类型暂时没启用 — <code>typeof app</code> 在 monorepo 里通过 <code>@cuberoot/shared</code> 包导出, 但 client 仍用 plain <code>fetch</code> 因为大多数端点是手写的 retry / cache 逻辑包裹, RPC 反而麻烦。</p>
      </>
    ),
    en: (
      <>
