  ...
  cuberoot: {
    zh: (
      <>
        ...
        <p><strong>Agent 线</strong>:开发期用 <strong>Playwright MCP</strong> 做 ad-hoc 验证。Codex 在改完 UI 后可以直接 <code>browser_navigate</code> 到 <code>http://127.0.0.1:3000/code/stack</code>, <code>browser_snapshot</code> 读 accessibility tree, 或者 <code>browser_take_screenshot</code> 给作者看视觉差异。交互与布局改动默认按风险主动验证;纯文案或很小的 CSS 调整可按节奏决定。</p>
    ...
    en: (
      <>
        ...
        <p><strong>Agent track</strong>: during development, <strong>Playwright MCP</strong> handles ad-hoc verification. After a UI edit, Codex can <code>browser_navigate</code> to <code>http://127.0.0.1:3000/code/stack</code>, <code>browser_snapshot</code> to read the accessibility tree, or <code>browser_take_screenshot</code> to show visual diffs. Interaction and layout changes are verified proactively according to risk; pure copy or very small CSS edits can be handled proportionally.</p>
