...
export const NGINX: StackTool = {
  ...
  zh: {
    ...
    cuberootDesc: <>cuberoot.me 的入口就是一个 nginx 进程, 同时干四件事:SPA 静态 + try_files 兜 React Router, 反代 api.cuberoot.me 到 :3001 同时挂 24h proxy_cache, 在 <code>/scramble/(solver|analyzer)</code> 两条路径通过 map+add_header 注入 COOP/COEP 让 cubeopt-wasm 拿到 SharedArrayBuffer, blog 子路径 alias 到静态归档目录。</>,
    outlookTitle: '当下与前景',
  ...
  en: {
    ...
    cuberootDesc: <>The cuberoot.me edge is one nginx process doing four jobs at once: SPA static + try_files fallback for React Router, reverse-proxy <code>api.cuberoot.me</code> to :3001 with a 24h proxy_cache layer, inject COOP/COEP via map+add_header on just <code>/scramble/(solver|analyzer)</code> so cubeopt-wasm can use SharedArrayBuffer, and alias the blog subpath to a static archive directory.</>,
