...
export const VITE: StackTool = {
  ...
  zh: {
    ...
    role: '本站已退役的前端开发服务器与生产打包器。现役网站统一使用 Next.js 16 与 Turbopack。',
  ...
  en: {
    ...
    role: 'Retired frontend dev server and production bundler. The current site uses Next.js 16 and Turbopack throughout.',
  ...
  heroStats: [
    ...
    { num: '0.3', unit: 's', zh: <>冷启动到能交互 <em>旧版 SPA 实测</em></>, en: <>cold start to interactive <em>retired SPA, measured</em></>
    },
  ...
  concepts: [
    ...
    { tag: 'H', zh: { title: <>library mode</>, desc: <><code>build.lib</code> 让 Vite 输出 esm、cjs 与类型声明，适合发布 npm 包。本站旧版前端曾用它构建 VisualCube。</> }, en: { title: <>library mode</>, desc: <><code>build.lib</code> emits esm, cjs, and declarations for npm packages. The retired frontend once used it to build VisualCube.</> }, code: <code>{p('build')}: {'{'}{'\n'}  {p('lib')}: {'{'}{'\n'}    {p('entry')}: {s('"src/index.ts"')},{'\n'}    {p('formats')}: [{s('"es"')}, {s('"cjs"')}],{'\n'}  {'}'},{'\n'}{'}'},</code> },
  ],
  whyCards: [
    { icon: '⚡', zh: { title: <>冷启动 0.3s</>, desc: <>不打包, 浏览器原生 import 源文件。本站旧版 100 多个组件的 SPA 冷启动到能交互约 300ms。</> }, en: { title: <>0.3s cold start</>, desc: <>No bundling: the browser imports source files natively. The retired site SPA with 100+ components cold-started to interactive in about 300ms.</> }, code: <>{c('// vite dev')}{'\n'}{c('// ready in 287 ms')}</> },
    ...
    { icon: '⏚', zh: { title: <>静态 SPA 最佳拍档</>, desc: <>build 出来就是一组 hashed JS、CSS 与资源，可直接部署到静态主机，不需要 Node runtime。本站旧版 SPA 曾使用这条部署链路。</> }, en: { title: <>Best fit for static SPAs</>, desc: <>The build output is hashed JS, CSS, and assets that can go straight to static hosting with no Node runtime. The retired site SPA used this deployment path.</> }, code: <>{c('// dist/assets/*.[hash].js')}{'\n'}{c('// dist/index.html')}</> },
  ...
  adopters: [
    ...
    { name: 'cuberoot.me', highlight: true, zhNote: '2023 至 2026-05 使用，现已迁至 Next.js', enNote: 'Used from 2023 to 2026-05, now migrated to Next.js' },
  ],
  outlook: [
  ...
  cuberoot: {
    zh: (
      <>
        <p><strong>已退役。</strong>2023 年立项到 2026-05 之间, 本站前端一直是跑在 Vite 上的 SPA;Phase 4 迁到 Next 16 App Router 后, dev server 与生产打包都交给了 Turbopack, <code>vite.config.ts</code> 连同整个 <code>client-vite</code> 包一起删除。Vite 本身没有从仓库消失 —— Vitest 4 仍然带着它跑全部单测的 transform。下面记的是它在职那三年的样子。</p>
        <p>本站 SPA 从 2023 年立项就跑在 Vite 上。<code>vite.config.ts</code> 大约 60 行: <code>@vitejs/plugin-react</code> + 一个自写的 <code>serveRepoRoot</code> 插件 (~30 行, dev 时把仓库根的 <code>/tools/</code> 和 <code>/stats/</code> 也 serve 出来, 这样 fork 来的 upstream 静态页能在本地一起跑)。</p>
        <p>dev server 死死绑 <code>127.0.0.1:5173</code>。Vite 默认绑 <code>[::1]</code> IPv6, Windows Chrome 经常打不开, 直接在 <code>vite.config.ts</code> 写死 <code>host: '127.0.0.1'</code>。HMR 跨设备调试时 (蜂窝 / 外网走 dev.cuberoot.me) <code>clientPort</code> 不写, 让客户端从页面 URL 自己推导端口 — 这是两入口 (本机 / 隧道域名) 各自正确的唯一办法, 写死任何一个都会另一个挂。</p>
        ...
        <p>边角:vite worker 配置让 <code>analyzer_worker.ts</code> / <code>scramble_cstimer_worker.ts</code> 等 worker 自动走独立 chunk + classic-worker 协议。<code>import.meta.env.DEV</code> 是切 dev/prod API base 的唯一信号 (不能用 <code>hostname === 'localhost'</code>, LAN / 隧道域名都不匹配)。<code>@cuberoot/visualcube</code> 通过 <code>build.lib</code> 单独 publish。</p>
      </>
    ),
    en: (
      <>
        <p><strong>Retired.</strong> From the 2023 start until 2026-05 the frontend was a Vite SPA; once Phase 4 moved it onto the Next 16 App Router, both the dev server and the production build went to Turbopack, and <code>vite.config.ts</code> was deleted along with the whole <code>client-vite</code> package. Vite itself never left the repo — Vitest 4 still uses it to transform every unit test. What follows is how it worked during those three years.</p>
        <p>The SPA ran on Vite from its 2023 inception. <code>vite.config.ts</code> was ~60 lines: <code>@vitejs/plugin-react</code> plus a hand-written <code>serveRepoRoot</code> plugin (~30 lines) that exposes the repo's <code>/tools/</code> and <code>/stats/</code> during dev so the upstream fork static pages run locally alongside the SPA.</p>
        <p>The dev server is hard-pinned to <code>127.0.0.1:5173</code>. Vite's default <code>[::1]</code> IPv6 bind often refuses Windows Chrome connections, so <code>host: '127.0.0.1'</code> is fixed in <code>vite.config.ts</code>. For cross-device HMR (cellular / external via dev.cuberoot.me), <code>clientPort</code> is intentionally omitted — letting the client derive the port from the page URL is the only way both entrypoints (localhost / tunnel domain) work; hard-coding either breaks the other.</p>
        ...
        <p>Edge bits: the Vite worker config sends <code>analyzer_worker.ts</code> / <code>scramble_cstimer_worker.ts</code> through separate chunks with the classic-worker protocol. <code>import.meta.env.DEV</code> is the only signal that flips dev/prod API base (using <code>hostname === 'localhost'</code> would miss LAN / tunnel URLs). <code>@cuberoot/visualcube</code> is shipped via <code>build.lib</code> as its own npm package.</p>
      </>
    ),
  },
  links: [
