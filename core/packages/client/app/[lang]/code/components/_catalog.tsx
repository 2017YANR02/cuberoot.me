...
export const CATALOG: ComponentEntry[] = [
  ...
  },
  {
    ...
    category: 'more',
    zh: '一条公式的三份镜像,行内展开(issue #40 T5)。槽统一在 FR 之后,左右镜(M 平面)和前后镜(S 平面)给的是同一个伙伴 case,区别只在落到它的哪个 y 视角(FL / BR);y² 那份不是镜像,是本 case 自己的 BL 视角。重写走 lib/cube3 的 mirrorAlg / relabelY,规则表在 @cuberoot/shared/alg-notation —— 落在镜面法向轴上的 M/m/x 不取反,别自己重推。用 hasMirror(puzzle, set) 决定出不出这个入口,名单是 shared 的 MIRROR_SYNC_SETS(只有 f2l / zbls / cls,逐一核过,理由见 docs/issue-40-alg-mirror-plan.md §5.7)。',
    en: 'The three mirrors of a single alg, expanded inline (issue #40 T5). With the slot normalised to FR, the left-right (M plane) and front-back (S plane) mirrors land on the same partner case — they differ only in which y view of it (FL / BR); the y² row is not a mirror at all but this case seen from BL. Rewriting goes through mirrorAlg / relabelY in lib/cube3, whose rule tables live in @cuberoot/shared/alg-notation — M/m/x sit on the mirror plane’s normal and keep their sign, so do not re-derive them. Gate the entry point on hasMirror(puzzle, set); the list is shared’s MIRROR_SYNC_SETS (f2l / zbls / cls only, each audited — see docs/issue-40-alg-mirror-plan.md §5.7).',
  },
  {
    name: 'CompPicker',
