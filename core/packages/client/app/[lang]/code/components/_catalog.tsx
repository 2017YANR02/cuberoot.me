...
export const CATALOG: ComponentEntry[] = [
  ...
  },
  {
    ...
    category: 'more',
    zh: `内联 3D 魔方动画预览,播放给定 (alg, puzzle, set, setup)。按魔方分流引擎:NxN 走站内 /sim 引擎(AlgSimPlayer),sq1 / 五魔 / 金字塔 / 斜转走懒加载的 cubing.js TwistyPlayer。支持固定尺寸或 fillPane 撑满父容器。`,
    en: `Inline animated 3D cube preview for a given (alg, puzzle, set, setup). Picks the engine by puzzle: NxN runs the in-house /sim engine (AlgSimPlayer), while sq1 / megaminx / pyraminx / skewb still lazy-load the cubing.js TwistyPlayer. Fixed size or fillPane.`,
  ...
  },
  {
    ...
    category: 'more',
    zh: `公式动画的 /sim 引擎版(NxN 专用):setup 瞬时落位、逐步播放、进度条可拖,顶层遮罩复用 /sim 的阶段遮罩。一般不直接用 —— 走 AlgPlayer 的分流。`,
    en: `The /sim-engine take on the alg animation (NxN only): the setup lands instantly, moves play one at a time, the scrubber seeks, and the last-layer mask reuses /sim's stage stickering. Usually reached through AlgPlayer rather than directly.`,
