...
export const CI_GUARDS_API: CiGuard[] = [
  ...
  },
  {
    ...
    zh: { title: '视频码率两处一致', desc: '对战房视频的单路码率有两个副本:服务端 video_rooms.ts 的 PER_STREAM_MBPS 用来守带宽预算,客户端 video-room-api.ts 的 VIDEO_MAX_BITRATE 用来真发流,分属两个包谁也不 import 谁。分叉后失败是静默的 —— 客户端偏大则服务端低估占用、放进来的房间推爆实例峰值(全体卡顿而监控显示预算未满);客户端偏小则服务端高估、明明有带宽却回「视频带宽已满」。顺带钉住第三处:VideoStrip 的 videoEncoding 必须引用那个常量而不是就地写数字(写死了改常量不生效)。' },
    en: { title: 'Video bitrate agrees on both sides', desc: 'The per-stream bitrate for battle-room video exists twice: PER_STREAM_MBPS in the server’s video_rooms.ts guards the bandwidth budget, VIDEO_MAX_BITRATE in the client’s video-room-api.ts is what actually gets published — different packages, neither imports the other. Divergence fails silently: too high on the client and the server under-counts, admitting rooms that blow past the instance’s peak bandwidth (everyone stutters while the budget looks unused); too low and the server over-counts, refusing tokens with “video capacity full” while the server sits idle. It also pins a third site: VideoStrip’s videoEncoding must reference the constant rather than an inline number, since a hardcoded value makes changing the constant a no-op.' },
  },
];
