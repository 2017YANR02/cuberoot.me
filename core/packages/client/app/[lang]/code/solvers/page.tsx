...
const BROWSER: BrowserSolver[] = [
  ...
  { key: 'First Layer', zhEngine: 'FirstLayerSolverWasm (零下载,底面/底层共享移动表与 PDB,首次查询惰性现场建)', enEngine: 'FirstLayerSolverWasm (zero download; First Face and First Layer share move tables and PDBs, built lazily in-WASM)', zhLatency: '首次建表后:底面查表,底层严格最优搜索', enLatency: 'after the first build: First Face lookup, First Layer exact search' },
...
const TABLES: Record<string, SolverTbls> = {
  ...
  first_layer: {
    move: [{ n: 'mt_edge4', b: 18247692 }],
    prune: [],
    builtZh: '零落盘剪枝:现场建角4/棱4移动表、44,906,400 字节底面精确表、角4/棱4/联合排列 PDB。底面 O(1) 直查,底层用这些可采纳表做严格最优 IDA*；底面 God 数 10,底层直径只报已证 11..20',
    builtEn: 'zero on-disk prune: builds corner-4/edge-4 moves, a 44,906,400-byte exact First Face table, and corner-4/edge-4/joint-permutation PDBs in RAM. First Face is O(1); First Layer runs exact IDA* on these admissible tables. Face God\'s number 10; Layer diameter reported only as the proven 11..20 interval',
  },
  std: {
