...
const TABLES: Table[] = [
  ...
  { name: 'battle_rooms', domain: 'studio', origin: '0086', purpose: { zh: '/timer 联机对战房间:每人可选自己项目(同项目共享打乱),当前轮各项目打乱 + 玩家实时状态 + 每轮战绩历史 + 胜场(1s 轮询 + jsonb 原子合并)', en: '/timer online battle rooms: each player picks their own event (same-event players share a scramble), per-event current scrambles + live status + per-round history + win tallies (1s polling, atomic jsonb merges)' }, cols: [
    { name: 'code (PK), event, round' }, { name: 'scrambles JSONB', note: { zh: '{event:scramble} 当前轮各项目打乱(同项目共享)', en: '{event:scramble} current-round scramble per event (shared)' } }, { name: 'players JSONB', note: { zh: '{pid:{name,seen,ph,at,event}} 心跳 + 实时状态 + 所选项目', en: '{pid:{name,seen,ph,at,event}} heartbeat + live phase + chosen event' } }, { name: 'results JSONB', note: { zh: '{round:{pid:{t,p}}} 当前轮成绩', en: '{round:{pid:{t,p}}} current-round results' } }, { name: 'history JSONB', note: { zh: '[{round,scrambles,playerEvents,results,winners}] 最近 50 轮', en: '[{round,scrambles,playerEvents,results,winners}], last 50 rounds' } }, { name: 'scores JSONB' },
  ] },
...
const MIGRATIONS: { n: number; slug: string; desc: Bi }[] = [
  ...
  { n: 87, slug: 'page_notice_icon', desc: { zh: 'page_notices 加可选 icon 列(lucide 图标 key,空=按 level 回退):每页顶部通知条不再永远一个圆圈 i,预设与图标选择器让每条通知带自己的语义图标', en: 'page_notices gains an optional icon column (lucide icon key, empty = fall back to the level icon): the per-page notice bar is no longer always a single circle-i — presets and an icon picker let each notice carry its own semantic icon' } },
];
