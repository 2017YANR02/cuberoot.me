...
// Port of roux-trainers src/components/CubeSim.tsx (three.js virtual cube).
// Faithful to upstream; surgical changes only:
//  1) 'use client' (Next).
...
import { FaceletCubeT, Face } from '@/lib/roux/Defs';
...
// lrgb mix of two #rrggbb colors (chroma.mix default mode). Falls back to the
// base color if either input isn't parseable.
function parseHex(c: string): [number, number, number] | null {
    let m = /^#?([0-9a-fA-F]{6})$/.exec(c.trim())
    if (m) {
        let n = parseInt(m[1], 16)
        return [(n >> 16) & 255, (n >> 8) & 255, n & 255]
    }
    let m3 = /^#?([0-9a-fA-F]{3})$/.exec(c.trim())
    if (m3) {
        let s = m3[1]
...
function mixHexLrgb(a: string, b: string, t: number): string {
    let ca = parseHex(a), cb = parseHex(b)
    if (!ca || !cb) return a
    let ch = (i: number) => Math.round(Math.sqrt((1 - t) * ca![i] * ca![i] + t * cb![i] * cb![i]))
    let toHex = (v: number) => Math.max(0, Math.min(255, v)).toString(16).padStart(2, '0')
...
const redraw_cube = function (cube: FaceletCubeT, config: ConfigT ) {
    let { width, height, colorScheme, mode, faces, theme} = config
    ...
    function drawCube(faces: FaceletCubeT, colorScheme: Array<string>): THREE.Group {
        ...
        let hint_mesh = Array(7).fill(0).map((_, i) => {
            let color = (theme === "bright")
                ? mixHexLrgb(colorScheme[i], '#ffffff', 0.18)
