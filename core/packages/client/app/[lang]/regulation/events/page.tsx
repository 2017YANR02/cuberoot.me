      ...
      <div className="evt-strip-row">
        {times.map((a, i) => (
          <div key={i} className={`evt-chip evt-chip-${a.state}`}>
            ...
            {a.state === 'best' && <span className="evt-chip-tag">{t('最优', 'best')}</span>}
            {a.state === 'worst' && <span className="evt-chip-tag">{t('最差', 'worst')}</span>}
          </div>
        ))}
      </div>
      <div className="evt-strip-calc">
        <span className="evt-strip-calc-label">{t('保留中间 3 把取平均', 'mean of the middle 3')}</span>
...
export default function EventsChapter() {
  ...
  return (
    <RegArticleLayout slug="events">
      ...
      <RegSection
        eyebrow={t('总共多少', `${TOTAL} in all`)}
        title={t('全部 WCA 项目', 'Every official WCA event')}
        lede={t(
          'WCA 目前承认 17 个官方项目。每个项目都有一个标准赛制(一轮里你拧几把)和一个排名依据(用哪一把、还是几把的平均来给你排名)。按“魔方家族”分组看最清楚。',
          'The WCA currently recognizes 17 official events. Each has a standard round format (how many attempts you get in a round) and a ranking method (which attempt — or which average — decides your place). Grouping by puzzle family makes the pattern clear.'
        )}
      >
        <div className="evt-family">
          <h3 className="evt-family-title">
            <Trophy size={17} />
            {t('NxN 与其它常规项目 · 取 5 次平均', 'NxN and other standard events · Average of 5')}
          </h3>
          <p className="evt-family-lede">
            {t(
              '速拧的“主力军”:拧 5 把,去掉最快和最慢,剩下 3 把取平均。这套规则把运气压到最低 —— 一把超神或一次失误都不会单独决定名次。',
              'The bulk of speedsolving: five attempts, drop the fastest and slowest, average the middle three. This squeezes out luck — neither one lightning solve nor one disaster alone decides your placement.'
            )}
          </p>
        ...
        <div className="evt-family">
          ...
          <p className="evt-family-lede">
            {t(
              ...
              'A single 6×6×6 or 7×7×7 solve already runs minutes long, so five attempts would take too long. Instead you get three attempts and all of them count toward the mean — nothing is dropped, so consistency matters more.'
            )}
          </p>
          <EventTable rows={BIG} />
        </div>
        ...
        <div className="evt-family">
          <h3 className="evt-family-title">
            <ListChecks size={17} />
            {t('三阶变体 · 单手与最少步', '3×3×3 variants · One-Handed & Fewest Moves')}
          </h3>
          <p className="evt-family-lede">
            {t(
              '同样是三阶,但玩法不同:单手沿用取 5 次平均;最少步是用纸笔写解法、比谁步数少,拧 3 次取均值(步数也能算平均)。',
              'Still the 3×3×3, but solved differently: One-Handed keeps Average of 5, while Fewest Moves is a pen-and-paper event scored on move count over three attempts, with the move counts themselves averaged into a mean.'
            )}
          </p>
        ...
        <div className="evt-family">
          <h3 className="evt-family-title">
            ...
            {t('盲拧家族 · 取最优', 'Blindfolded family · Best of X')}
          </h3>
          <p className="evt-family-lede">
            {t(
              '盲拧太难,平均没意义 —— 一把 DNF 就会拖垮整组。所以盲拧只看你最好的那一把。多盲更特别:它比“净还原数”,即还原成功的魔方数减去失败的,差值越大越靠前。',
              'Blindfolded solving is so error-prone that an average is meaningless — a single DNF would wreck a whole set. So these events rank you on your single best attempt. Multi-Blind is special again: it ranks on net puzzles — cubes solved minus cubes missed — with a bigger difference ranking higher.'
            )}
          </p>
        ...
        <Callout tone="info" label={t('想看每个项目的世界纪录与现场成绩', 'See world records and live results per event')}>
          ...
          {t(
            ...
            ' to browse records, rankings and history by event; the real scramble-length distribution for each event lives at '
          )}
      ...
      <RegSection
        ...
        title={t('赛制:Bo · Ao · Mo 到底怎么算', 'Formats: how Bo, Ao and Mo work')}
        lede={t(
          '一轮里你拧固定次数(称作一次“尝试”),结果按下面三种方式之一汇总。看懂这三个缩写,成绩单就全看明白了。',
          'In a round you make a fixed number of attempts (a “solve”), and the results are combined in one of three ways. Learn these three abbreviations and any scorecard becomes readable.'
        )}
      >
          ...
          <div className="evt-fmt-card">
            ...
            <p className="evt-fmt-text">
              {t(
                ...
                'Three attempts, all three counted, arithmetic mean. Nothing is dropped — so a single DNF or DNS makes the whole mean a DNF. Used by 6×6×6, 7×7×7 and Fewest Moves.'
              )}
            </p>
          ...
          <div className="evt-fmt-card evt-fmt-card-wide">
            ...
            <p className="evt-fmt-text">
              {t(
                ...
                'Five attempts; drop the single fastest and the single slowest, then average the middle three. Almost every speed event uses it.'
              )}
            </p>
            <Ao5Strip />
          </div>
        ...
        <RegQuote num="9f8 · 9f10">
          {t(
            ...
            'Average of 5: of five attempts the best and worst are removed, and the arithmetic mean of the remaining three determines ranking. Mean of 3: the arithmetic mean of the three attempts determines ranking.'
          )}
        </RegQuote>

        <h3 className="reg-sub-title">{t('成绩怎么记:精度与 DNF/DNS', 'How a result is recorded: precision and DNF/DNS')}</h3>
        <RegList
          items={[
            <span key="prec">
              <strong>{t('计时精度:单次截断,平均四舍五入。', 'Precision: singles truncate, averages round.')}</strong>{' '}
              {t(
                '单次成绩向下截断(不四舍五入):10 分钟以内截到百分之一秒,10 分钟及以上、以及全部多盲成绩截到整秒。而平均、去尾平均(Mo3 / Ao5)则是四舍五入:10 分钟以内取到百分之一秒,超过 10 分钟取到整秒。',
                'A single result is truncated (rounded down), not rounded: under 10 minutes to the hundredth of a second, and at 10 minutes or more — plus all Multi-Blind results — to whole seconds. Averages and means (Mo3 / Ao5), by contrast, are rounded to the nearest unit: to the hundredth under 10 minutes, and to the whole second above 10 minutes.'
              )}{' '}
            ...
            <span key="dnf">
              ...
              {t(
                ...
                'An attempt that is disqualified, stopped at the time limit, or left unsolved is a DNF. Under Best of X a DNF is the worst possible result.'
              )}{' '}
              <span className="evt-tag">9f4 · 9f7</span>
            </span>,
            <span key="dns">
              <strong>DNS（{t('未开始', 'Did Not Start')}）。</strong>{' '}
              {t(
                '你有资格拧这把、却没有开始(比如已退赛、或前面已无法晋级而放弃),记 DNS。和 DNF 一样视为最差结果。',
                'You were eligible for the attempt but never started it (you withdrew, or there was nothing left to play for) — recorded as DNS, treated like a DNF as the worst result.'
              )}{' '}
              <span className="evt-tag">9f5</span>
            </span>,
            <span key="mbf">
              <strong>{t('多盲怎么排。', 'How Multi-Blind ranks.')}</strong>{' '}
              {t(
                '一把里你同时记忆并蒙眼还原多个三阶。排名看“净得分”=还原成功数 − 未还原数;净得分相同时,用时少者靠前。净得分小于 0、或只成功还原了 1 个魔方,都记 DNF。',
                'In one attempt you memorize and blind-solve several 3×3×3 cubes at once. You rank on net score = cubes solved minus cubes not solved; ties break on time. The attempt is a DNF if the net score is less than 0, or if only 1 cube is solved.'
              )}{' '}
      ...
      <RegSection
        eyebrow={t('一轮的边界', 'The edges of a round')}
        title={t('晋级线与时间上限', 'Cutoffs and time limits')}
        lede={t(
          '两条规则决定一轮怎么收口:晋级线(cutoff)决定你能不能拧完全部把数,时间上限决定单把(或累计)能拧多久。它们常常被写在成绩单顶部。',
          'Two rules shape how a round plays out: the cutoff decides whether you get all your attempts, and the time limit decides how long each solve (or all of them together) may run. Both are printed at the top of the scorecard.'
        )}
      >
        <h3 className="evt-block-title">
          <Scissors size={18} />
          {t('晋级线 / 合并轮', 'The cutoff / combined round')}
        </h3>
        <p className="evt-block-lede">
          {t(
            '为了让一轮跑得快,组委会可以给一轮加一个“前段”:先拧若干把(取最优作前段),只有在前段里至少有一把达到“晋级线”的人,才能继续拧完剩下的把数。',
            'To keep a round moving, the organizers can give it a cutoff phase: you first make a few attempts (a Best of X), and only if at least one of them beats the cutoff do you continue to the remaining attempts.'
          )}
        </p>
        <div className="evt-cutoff">
          <div className="evt-cutoff-head">
            ...
            <span className="evt-cutoff-cap">
              {t('前段取 2 次最优,达标后补满 5 把走平均', 'A Best-of-2 phase, then fill out 5 attempts for the average')}
            </span>
          </div>
          <div className="evt-cutoff-flow">
            <div className="evt-cutoff-stage">
              ...
              <p className="evt-cutoff-stage-text">
                {t('先拧 2 把。任意一把好于晋级线即“达标”。', 'Make 2 attempts. Beat the cutoff on either one to qualify.')}
              </p>
            </div>
            <div className="evt-cutoff-gate" aria-hidden>→</div>
            <div className="evt-cutoff-branch">
              <div className="evt-cutoff-pass">
                <strong>{t('达标', 'Met cutoff')}</strong>{' '}
                {t('→ 补拧第 3、4、5 把,五把走完整的取平均。', '→ make attempts 3, 4 and 5; the full Average of 5 stands.')}
                <div className="evt-cutoff-attempts">
              ...
              <div className="evt-cutoff-fail">
                <strong>{t('未达标', 'Missed cutoff')}</strong>{' '}
                {t('→ 到此为止,本轮成绩就是前 2 把的最优。', '→ you stop; your result for the round is the best of those 2 attempts.')}
              </div>
            </div>
          </div>
        </div>
        <RegQuote num="9g">
          {t(
            ...
            'A combined round has a Best-of-X cutoff phase and a cutoff requirement; if a competitor meets the requirement on at least one cutoff-phase attempt, they are eligible for the remaining attempts.'
          )}
        </RegQuote>

        <h3 className="evt-block-title">
          <Timer size={18} />
          {t('时间上限:单把与累计', 'Time limits: per-attempt and cumulative')}
        </h3>
        <p className="evt-block-lede">
          {t(
            '每一轮都必须有至少一种时间上限。最常见的是“单把上限”:这一把拧到上限还没还原,裁判立刻叫停并记 DNF。默认上限是 10 分钟。',
            'Every round must carry at least one kind of time limit. The usual one is a per-attempt limit: if a solve reaches it unsolved, the judge stops it at once and records a DNF. The default is 10 minutes.'
          )}
        </p>
        <div className="evt-limit-grid">
          <div className="evt-limit-card">
            <div className="evt-limit-head">
              <Hourglass size={16} />
              {t('单把上限', 'Per-attempt limit')}
            </div>
            <p>
              {t(
                '写成一个时间,例如 10:00。每一把都独立计时;到点未还原 = 这把 DNF。',
                'Written as a single time, e.g. 10:00. Each attempt is timed on its own; reaching it unsolved means a DNF for that attempt.'
              )}
            </p>
          </div>
          <div className="evt-limit-card">
            <div className="evt-limit-head">
              <Hourglass size={16} />
              {t('累计上限', 'Cumulative limit')}
            </div>
            <p>
              {t(
                '一个时间值管住一轮里若干把的总和,例如 “20:00 累计 / 4 把”。某一把可用的时间 = 累计上限 − 已用掉的时间;总时间用尽后,余下没拧完的把记 DNF。',
                'One value caps the sum across several attempts, e.g. “20:00 cumulative for 4 attempts”. The time available for any one attempt is the cumulative limit minus the time already spent; once the budget runs out, the unfinished attempts are DNFs.'
              )}
            </p>
          </div>
        </div>
        <Callout tone="danger" label={t('到点 = DNF', 'Hit the limit = DNF')}>
          {t(
            '时间上限不是“扣 2 秒”那种罚时,而是硬线 —— 一旦达到,裁判当场停表,这把直接记 DNF,没有补救。上限会在赛前公布,且一轮开始后通常不再改动。',
            'A time limit is not a +2-style penalty — it is a hard line. The moment it is reached the judge stops the solve and the attempt is a flat DNF, with no recovery. Limits are announced before the competition and normally are not changed once an affected round has begun.'
          )}
        </Callout>
      </RegSection>
      ...
      <RegSection
        eyebrow={t('谁能继续', 'Who moves on')}
        title={t('一轮淘汰一批:晋级与轮次上限', 'Each round cuts the field: advancing and round caps')}
        lede={t(
          '热门项目会分多轮,每轮砍掉一批人,把最强的选手送进决赛。能开几轮、每轮留多少人,都有硬性规定。',
          'Popular events run several rounds, each trimming the field and funnelling the strongest into the final. How many rounds you may hold, and how many advance, are both fixed by rule.'
        )}
      >
        <h3 className="evt-block-title">
          <Users size={18} />
          {t('每轮至少淘汰 25%', 'At least 25% are eliminated each round')}
        </h3>
        <RegList
          items={[
            <span key="how">
              <strong>{t('两种晋级方式。', 'Two ways to set who advances.')}</strong>{' '}
              {t(
                '按名次取(本轮前 X 名晋级),或按成绩取(凡是好于某个阈值 X 的都晋级)。组委会赛前二选一并公布。',
                'By ranking (the best X competitors advance) or by result (everyone with a result better than a threshold X advances). Organizers pick one in advance and announce it.'
              )}{' '}
            ...
            <span key="cut">
              ...
              {t(
                ...
                'Between consecutive rounds at least 25% of competitors must be eliminated — the next round cannot be nearly the same field; it must genuinely shrink.'
              )}{' '}
              <span className="evt-tag">9p1</span>
            </span>,
            <span key="dnf">
              <strong>{t('全 DNF/DNS 不能晋级。', 'All-DNF/DNS cannot advance.')}</strong>{' '}
              {t(
                '本轮一把有效成绩都没有(只有 DNF 和/或 DNS)的选手,不得进入下一轮。',
                'A competitor with no successful result in the round (only DNFs and/or DNSs) may not proceed to the next round.'
              )}{' '}
        ...
        <h3 className="evt-block-title">
          <Crown size={18} />
          {t('能开几轮,看人数', 'How many rounds the field allows')}
        </h3>
        <p className="evt-block-lede">
          {t(
            '一个项目最多 4 轮。能再加几轮取决于这一轮的参赛人数 —— 人越少,后续轮次越少,避免“人比奖牌还少”还硬分轮。',
            'An event may have at most four rounds. How many more you can add depends on the number of competitors in a round — the fewer the entrants, the fewer the subsequent rounds, so you don’t split a tiny field into needless rounds.'
          )}
        </p>
        <div className="evt-table-wrap">
          <table className="reg-table evt-rounds-table">
            <thead>
              <tr>
                <th>{t('本轮人数', 'Competitors in the round')}</th>
                <th>{t('之后最多还能开', 'At most this many more rounds')}</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td><span className="reg-num">≤ 7</span></td>
                <td>{t('不得再开后续轮次', 'No subsequent rounds')} <span className="evt-tag">9m3</span></td>
              </tr>
              <tr>
                <td><span className="reg-num">≤ 15</span></td>
                <td>{t('最多再开 1 轮', 'At most 1 more round')} <span className="evt-tag">9m2</span></td>
              </tr>
              <tr>
                <td><span className="reg-num">≤ 99</span></td>
                <td>{t('最多再开 2 轮', 'At most 2 more rounds')} <span className="evt-tag">9m1</span></td>
              </tr>
              <tr>
                <td><span className="reg-num">≥ 100</span></td>
                <td>{t('合计最多 4 轮(决赛在内)', 'Up to 4 rounds total (including the final)')} <span className="evt-tag">9m</span></td>
              </tr>
            </tbody>
          </table>
        </div>
