...
/**
 * /math/sq1 — Square-1 上帝之数 / 最优解(最少步)。
 *
 * 结构:
 *   hero + 四张要点卡(13 已证 / 31 已证 / 12c4 未知 / 态空间)
 *   §1 魔方与态空间          → StateSpaceCalculator
 *   §2 三套计步口径(对照表)  → MoveCountCalculator
 *   §3 上帝之数:已证的两套    → DepthDistribution
 *   §4 未解之谜:WCA 12c4     → OpenProblemBracket
 *   §5 怎么求解 + 求解器入口
 *   参考资料
 *
 * 数据全部经穷举一手来源核查,见 _components/sq1_data.ts。
 */
...
import { TeXBlock } from './_components/Tex';
import { METRICS, STATE_SPACE } from './_components/sq1_data';
import './sq1.css';

const StateSpaceCalculator = lazy(() => import('./_components/StateSpaceCalculator'));
const MoveCountCalculator = lazy(() => import('./_components/MoveCountCalculator'));
const DepthDistribution = lazy(() => import('./_components/DepthDistribution'));
const OpenProblemBracket = lazy(() => import('./_components/OpenProblemBracket'));
...
export default function SQ1GodPage() {
  const { i18n } = useTranslation();
  const isZh = i18n.language.startsWith('zh');
  useDocumentTitle('SQ1 上帝之数', "Square-1's God Number");
  ...
  return (
    <div className="sq1-page">
      <header className="sq1-header">
        <Link href="/math" className="sq1-back">
          <ArrowLeft size={16} />
          <span>{t('返回数学', 'Back to math')}</span>
        </Link>
        <Link href="/math/god" className="sq1-back">
          <span>{t('上帝之数总览', "All God's numbers")}</span>
        </Link>
      </header>

      <main className="sq1-main">
        {/* ── HERO ───────────────────────────────────────────────── */}
        <section className="sq1-hero">
          <div className="sq1-hero-eyebrow">{t('魔方与计算:未解问题', 'Cubing & computation: open problem')}</div>
          <h1 className="sq1-title">
            {t('Square-1 上帝之数', "Square-1's God Number")}{' '}
            <span className="sq1-title-zh">{t('最少步之谜', 'the fewest-move mystery')}</span>
          </h1>
