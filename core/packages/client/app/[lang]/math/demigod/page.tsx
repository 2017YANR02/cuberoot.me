    ...
    <div className="demigod-page">
      ...
      <main className="dg-main">
        ...
        <section className="dg-hero">
          ...
          <p className="dg-lead">
            {<T zh={<>2010 年 Rokicki 等人用 35 CPU·年 在 Google 集群证明了三阶魔方的上帝之数恰为 <TeX src="20" /> HTM。论文不易复核,几乎没人独立跑过。2024 年底 Merino & Subercaseaux 提出一种新得多的方法:不去求精确直径,而是承认略弱的 <TeX src="\le 36" />,但只需 <strong>500,000 个随机样本 + 100 小时单机算力</strong>,并附完整可验证的证书 — 任何具备一台笔记本与 Kociemba 求解器的人都能在一晚跑完。本页详细拆解全篇,把证明、概率界、采样算法、与上帝之数的关系全摊开来。</>} en={<>In 2010 Rokicki et al. used 35 CPU-years on a Google cluster to prove 3×3 God's number is exactly <TeX src="20" /> HTM — a result almost no one has ever independently reproduced. In late 2024, Merino & Subercaseaux proposed a fundamentally different approach: settle for the weaker <TeX src="\le 36" /> bound, in exchange for needing only <strong>500,000 random samples and ~100 hours of single-machine compute</strong>, each one certified by a short Kociemba solution. Anyone with a laptop and a Kociemba solver can replicate it overnight. This page walks through the paper end-to-end — proof, concentration bound, sampling algorithm, and its place beside God's number.</>} />}
          </p>
          ...
          <div className="dg-callout is-ok">
            ...
            {<T zh={<>对任意顶点传递图 <TeX src="G" />,<TeX src="D < 2\mu" />。三阶魔方的 Cayley 图顶点传递,采 500k 样本 + Hoeffding 给出 <TeX src="\hat\mu \approx 18.32 \pm 0.1" />,故 <TeX src="D \le 2 \cdot 18.4804 < 37" />,且 <TeX src="D" /> 是整数 ⇒ <TeX src="D \le 36" />。</>} en={<>For any vertex-transitive graph <TeX src="G" />, <TeX src="D < 2\mu" />. The 3×3 Cayley graph is vertex-transitive; 500k samples + Hoeffding give <TeX src="\hat\mu \approx 18.32 \pm 0.1" />, so <TeX src="D \le 2 \cdot 18.4804 < 37" />, and since <TeX src="D \in \mathbb{Z}" /> ⇒ <TeX src="D \le 36" />.</>} />}
          </div>
        </section>
        ...
        <section className="dg-section">
          ...
          <ul>
            <li>
              {<T zh={<><strong>什么:</strong>一种"重新证 3×3 上帝之数"的方式 — 把精确的 <TeX src="20" /> 放宽到概率性的 <TeX src="\le 36" />,但代价小到一台笔电就能跑完。作者管这种"减半的算力换略弱结论"的范式叫 <em>demigod number</em>。</>} en={<><strong>What:</strong> a re-derivation of the 3×3 diameter, relaxed from the exact <TeX src="20" /> down to a probabilistic <TeX src="\le 36" />, but reproducible on a single laptop. The authors dub this "trade compute for a weaker conclusion" pattern a <em>demigod number</em>.</>} />}
            </li>
            <li>
              {<T zh={<><strong>怎么:</strong>(i) 在任意顶点传递图上有 <TeX src="D < 2\mu" />(基本三角不等式);(ii) 三阶魔方的 Cayley 图顶点传递;(iii) Hoeffding 不等式把 <TeX src="\mu" /> 收紧到 <TeX src="\hat\mu" /> 邻域。</>} en={<><strong>How:</strong> (i) on any vertex-transitive graph, <TeX src="D < 2\mu" /> (basic triangle inequality); (ii) the 3×3 Cayley graph is vertex-transitive; (iii) Hoeffding pins <TeX src="\mu" /> close to the empirical <TeX src="\hat\mu" />.</>} />}
            </li>
            <li>
              {<T zh={<><strong>关键技巧:</strong>朴素 Hoeffding 用 <TeX src="C = 205" />(凡人之数当 worst-case)需要 ~<TeX src="3 \times 10^{8}" /> 样本;通过证"几乎所有状态都 <TeX src="\le 20" /> 步"(<TeX src="\ge 99.97\%" />),实际取 <TeX src="C = 20" /> 把样本量降到 <TeX src="5 \times 10^{5}" /> 量级。</>} en={<><strong>Key trick:</strong> Plain Hoeffding with <TeX src="C = 205" /> (Human's number as worst case) demands ~<TeX src="3 \times 10^{8}" /> samples; proving "<TeX src="\ge 99.97\%" /> of states need <TeX src="\le 20" /> moves" lets <TeX src="C = 20" />, cutting samples to <TeX src="5 \times 10^{5}" />.</>} />}
            </li>
            <li>
              {<T zh={<><strong>实验:</strong>MacBook Pro M3 跑 500k samples <TeX src="\approx" /> 5 小时。所有 sample 都 <TeX src="\le 20" /> 步,均值 <TeX src="18.3189" />。误证概率 <TeX src="<10^{-10}" />。</>} en={<><strong>Experiment:</strong> MacBook Pro M3 ran 500k samples in <TeX src="\approx" /> 5 hours. Every sample solves in <TeX src="\le 20" />, mean <TeX src="18.3189" />, total error probability <TeX src="<10^{-10}" />.</>} />}
            </li>
            <li>
              {<T zh={<><strong>推广:</strong>对任何"顶点传递 + 可均匀采样"的群图(Pyraminx、Megaminx、5×5、torus 上的 15-puzzle)都适用,即便没有 God's number。</>} en={<><strong>Generalises:</strong> applies to any vertex-transitive graph that admits uniform sampling — Pyraminx, Megaminx, 5×5, the 15-puzzle on a torus — even where God's number is unknown.</>} />}
            </li>
          </ul>
        </section>
        ...
        <section className="dg-section">
          ...
          <p>
            {<T zh={<>把魔方建模成群:位置态对应 <TeX src="S_{54}" />(54 个色块)的某个置换;面转 <TeX src="\{R, L, U, D, F, B\}" /> 是生成元;六个固定中心使其同构地落入 <TeX src="S_{48}" /> 的一个子群。论文用的是经典记号:</>} en={<>Model the cube as a group: every position is a permutation of <TeX src="S_{54}" /> (54 stickers); face turns <TeX src="\{R, L, U, D, F, B\}" /> are generators; the six fixed centres let us embed it in <TeX src="S_{48}" />. Classical setup:</>} />}
          </p>

          <div className="dg-definition">
            <span className="dg-def-head">{t('定义 1 — Cayley 图', 'Definition 1 — Cayley graph')}</span>
            <p style={{ margin: 0 }}>
              {<T zh={<>给定群 <TeX src="G = (A, \star)" /> 和生成元集 <TeX src="S \subseteq A" />,Cayley 图 <TeX src="G_S" /> 的顶点集是 <TeX src="A" />,边 <TeX src="(u, v)" /> 当且仅当存在 <TeX src="s \in S" /> 使 <TeX src="u \star s = v" />。</>} en={<>Given group <TeX src="G = (A, \star)" /> and generators <TeX src="S \subseteq A" />, the Cayley graph <TeX src="G_S" /> has vertex set <TeX src="A" /> with edge <TeX src="(u, v)" /> iff some <TeX src="s \in S" /> gives <TeX src="u \star s = v" />.</>} />}
            </p>
          </div>

          <p>
            {<T zh={<>论文中,</>} en={<>For the cube, this gives</>} />}
          </p>
          <div className="dg-formula">
            <TeX src={`S_{\\mathcal{R}} := \\{R, R', R^{2}, L, L', L^{2}, U, U', U^{2}, D, D', D^{2}, F, F', F^{2}, B, B', B^{2}\\}`} />
          </div>
          <div className="dg-formula-cap">
            {<T zh={<>18 个生成元 —— 把 <TeX src="R^2" /> 算作"一步"就是 HTM(Half-Turn Metric);若把 <TeX src="R^2" /> 算作两步则是 QTM,直径变成 <TeX src="26" />。</>} en={<>18 generators — counting <TeX src="R^2" /> as a single move is HTM; counting it as two gives QTM, where God's number is <TeX src="26" />.</>} />}
          </div>
          ...
          <div className="dg-definition">
            ...
            <p style={{ margin: '0.5rem 0 0', fontSize: '0.9rem' }}>
              {<T zh={<>"上帝之数" = <TeX src="D" />;"半神之数"是关于 <TeX src="2\mu" /> 的概率上界。</>} en={<>"God's number" = <TeX src="D" />; the demigod's number is a high-probability upper bound on <TeX src="2\mu" />.</>} />}
            </p>
          </div>
        </section>
        ...
        <section className="dg-section">
          ...
          <p>
            {<T zh={<>一般图的 <TeX src="D / \mu" /> 可以无界 — 论文里给出"团 + 一根长尾"的反例,比值是 <TeX src="\Omega(n^{1/2})" />,且 Wu et al. 2011 证明该界紧。但只要图顶点传递,这个比例就被 2 锁死。</>} en={<>For general graphs <TeX src="D / \mu" /> can blow up — the paper gives the clique-plus-tail example reaching <TeX src="\Omega(n^{1/2})" />, which Wu et al. 2011 prove tight. Vertex transitivity, however, forces the ratio below 2.</>} />}
          </p>

          <div className="dg-theorem">
            <span className="dg-theorem-head">{t('Theorem 2 (论文)', 'Theorem 2 (paper)')}</span>
            <p style={{ margin: 0 }}>
              {<T zh={<>对任意顶点传递图 <TeX src="G" />,设其直径 <TeX src="D" />,平均距离 <TeX src="\mu" />,则 <TeX src="D < 2\mu" />。</>} en={<>For any vertex-transitive graph <TeX src="G" /> with diameter <TeX src="D" /> and mean distance <TeX src="\mu" />, we have <TeX src="D < 2\mu" />.</>} />}
            </p>
          </div>
          ...
          <div className="dg-proof">
            ...
            <p style={{ margin: '0.5rem 0' }}>
              {<T zh={<>先做一个引理:在顶点传递图中,平均距离等于"任意固定顶点出发的平均距离":</>} en={<>First a lemma: in a vertex-transitive graph, the mean distance equals the average distance from any fixed vertex:</>} />}
            </p>
            <div className="dg-formula">
              <TeX src={`\\mu \\;=\\; \\frac{1}{|V| - 1} \\sum_{v \\in V} d(x, v) \\quad \\text{for any fixed } x \\in V.`} />
            </div>
            <p style={{ margin: '0.5rem 0' }}>
              {<T zh={<>(因为每个 <TeX src="u" /> 都有 automorphism <TeX src="\phi_u : V \to V" /> 把 <TeX src="u" /> 送到 <TeX src="x" />,而 automorphism 保距;对 <TeX src="u, v" /> 双求和后用这个事实把 <TeX src="\sum_u" /> 消掉。)</>} en={<>(For each <TeX src="u" /> there is an automorphism <TeX src="\phi_u" /> sending <TeX src="u" /> to <TeX src="x" />, and automorphisms preserve distances; double-summing and using this collapses the <TeX src="\sum_u" />.)</>} />}
            </p>
            <p style={{ margin: '0.5rem 0' }}>
              {<T zh={<>现在取 <TeX src="u, v" /> 使 <TeX src="d(u, v) = D" />。应用 Lemma 4 两次:</>} en={<>Now take <TeX src="u, v" /> with <TeX src="d(u, v) = D" />. Apply Lemma 4 twice:</>} />}
            </p>
            <div className="dg-formula">
            ...
            <p style={{ margin: '0.5rem 0 0' }}>
              {<T zh={<>三角不等式给出 <TeX src="d(u, x) + d(v, x) \ge d(u, v) = D" />。<span className="dg-qed">∎</span></>} en={<>Triangle inequality: <TeX src="d(u, x) + d(v, x) \ge d(u, v) = D" />. <span className="dg-qed">∎</span></>} />}
            </p>
          </div>

          <div className="dg-callout">
            <span className="dg-callout-h">{t('紧吗?', 'Tight?')}</span>
            {<T zh={<>紧。在 <TeX src="C_{2n}" /> 上 <TeX src="D = n" /> 且 <TeX src="\mu = n/2 + o(1)" />,所以 <TeX src="D / 2\mu \to 1" />。超立方 <TeX src="Q_n" /> 同样:<TeX src="D = n" /> 而 <TeX src="\mu = n/2" />。</>} en={<>Yes. On <TeX src="C_{2n}" /> we have <TeX src="D = n" /> and <TeX src="\mu = n/2 + o(1)" />, so <TeX src="D / 2\mu \to 1" />. Same for the hypercube <TeX src="Q_n" />: <TeX src="D = n" />, <TeX src="\mu = n/2" />.</>} />}
          </div>
        </section>
        ...
        <section className="dg-section">
          ...
          <p>
            {<T zh={<>这一步把魔方挂上 Theorem 2。证明很短,且揭示出"群结构 = 对称性"的最朴素表达。</>} en={<>This is the step that hooks the cube onto Theorem 2. The proof is a one-liner and makes "group structure = symmetry" concrete.</>} />}
          </p>

          <div className="dg-lemma">
            <span className="dg-lemma-head">{t('Lemma 3', 'Lemma 3')}</span>
            <p style={{ margin: 0 }}>
              {<T zh={<>每个 Cayley 图都顶点传递。</>} en={<>Every Cayley graph is vertex-transitive.</>} />}
            </p>
          </div>
          ...
          <div className="dg-proof">
            <span className="dg-proof-head">{t('证明', 'Proof')}</span>
            <p style={{ margin: '0.5rem 0' }}>
              {<T zh={<>给定 <TeX src="u, v \in V" />,构造 <TeX src="\phi : V \to V" />,<TeX src="\phi(x) = v \star u^{-1} \star x" />。它把 <TeX src="u" /> 送到 <TeX src="v" />,有逆 <TeX src="x \mapsto u \star v^{-1} \star x" />,且保边:</>} en={<>Given <TeX src="u, v" />, set <TeX src="\phi(x) = v \star u^{-1} \star x" />. It sends <TeX src="u" /> to <TeX src="v" />, has inverse <TeX src="x \mapsto u \star v^{-1} \star x" />, and preserves edges:</>} />}
            </p>
            <div className="dg-formula">
              <TeX src={`\\phi(x)^{-1} \\star \\phi(y) \\;=\\; (v \\star u^{-1} \\star x)^{-1} \\star (v \\star u^{-1} \\star y) \\;=\\; x^{-1} \\star y,`} />
            </div>
            <p style={{ margin: '0.5rem 0' }}>
              {<T zh={<>所以 <TeX src="(x, y) \in E \iff x^{-1} \star y \in S \iff \phi(x)^{-1} \star \phi(y) \in S \iff (\phi(x), \phi(y)) \in E" />。<span className="dg-qed">∎</span></>} en={<>so <TeX src="(x, y) \in E \iff x^{-1} \star y \in S \iff \phi(x)^{-1} \star \phi(y) \in S \iff (\phi(x), \phi(y)) \in E" />. <span className="dg-qed">∎</span></>} />}
            </p>
          </div>

          <p>
            {<T zh={<>在简单的 <TeX src="C_{2n}" /> 上把 Theorem 2 看一遍 —— 拖动 <TeX src="x" /> 让三角不等式可视化:</>} en={<>Watch Theorem 2 act on the simplest non-trivial Cayley graph, <TeX src="C_{2n}" /> — drag <TeX src="x" /> to see the triangle inequality flexing:</>} />}
          </p>
        ...
        <section className="dg-section">
          ...
          <p>
            {<T zh={<>核心结果。把 <TeX src="D < 2\mu" /> 与 Hoeffding 不等式拼起来。</>} en={<>The headline. Combine <TeX src="D < 2\mu" /> with Hoeffding's inequality.</>} />}
          </p>

          <div className="dg-lemma">
            <span className="dg-lemma-head">{t('Lemma 5 — Hoeffding 不等式', "Lemma 5 — Hoeffding's inequality")}</span>
            <p style={{ margin: '0 0 0.4rem' }}>
              {<T zh={<>独立同分布 <TeX src="X_1, \ldots, X_s \in [0, C]" />,均值 <TeX src="\mu" />,经验均值 <TeX src="\hat\mu = \tfrac{1}{s} \sum X_i" />,则对任意 <TeX src="t > 0" />:</>} en={<>For i.i.d. <TeX src="X_1, \ldots, X_s \in [0, C]" /> with mean <TeX src="\mu" /> and empirical mean <TeX src="\hat\mu" />, for any <TeX src="t > 0" />:</>} />}
            </p>
          ...
          <div className="dg-theorem">
            ...
            <p style={{ margin: '0 0 0.4rem' }}>
              {<T zh={<>给定状态 <TeX src="s" />,令 <TeX src="d(s)" /> 为它到解的距离;<TeX src="S" /> 为均匀采得的状态集,<TeX src="\hat\mu_S = \tfrac{1}{|S|} \sum_{s \in S} d(s)" />,则:</>} en={<>For a state <TeX src="s" />, let <TeX src="d(s)" /> be its distance to the identity; let <TeX src="S" /> be a uniformly sampled set with empirical mean <TeX src="\hat\mu_S" />. Then:</>} />}
            </p>
            <div className="dg-formula">
          ...
          <div className="dg-proof">
            ...
            <p style={{ margin: '0.5rem 0' }}>
              {<T zh={<>用 Lemma 8(凡人之数)给出 <TeX src="d(s) \in [0, 205]" />。代 <TeX src="C = 205" />,<TeX src="t = 0.1" /> 进 Hoeffding:</>} en={<>Lemma 8 (Human's number) yields <TeX src="d(s) \in [0, 205]" />. Plug <TeX src="C = 205" />, <TeX src="t = 0.1" /> into Hoeffding:</>} />}
            </p>
            <div className="dg-formula">
              <TeX src={`\\Pr[\\mu \\ge \\hat\\mu + 0.18] \\;\\le\\; 2\\exp\\!\\left(\\frac{-2|S| \\cdot 0.1^{2}}{205^{2}}\\right) \\;<\\; 2\\exp\\!\\left(\\frac{-|S|}{1\\,541\\,939}\\right).`} />
            </div>
            <p style={{ margin: '0.5rem 0' }}>
              {<T zh={<>再用 Theorem 2 (<TeX src="D < 2\mu" />):</>} en={<>Then by Theorem 2 (<TeX src="D < 2\mu" />):</>} />}
            </p>
            <div className="dg-formula">
          ...
          <p>
            {<T zh={<>下面这个面板把三个旋钮放出来:样本量 <TeX src="|S|" />、容差 <TeX src="t" />、Hoeffding 用 <TeX src="C = 205" /> 还是 <TeX src="C = 20" />(下一节会推出 <TeX src="C = 20" /> 的合法性)。两条曲线展示朴素 vs 改进的衰减速度,圆点是当前选择。</>} en={<>The panel below exposes the three knobs: sample size <TeX src="|S|" />, tolerance <TeX src="t" />, and the Hoeffding constant <TeX src="C \in \{205, 20\}" />. Two curves show the naive vs refined decay; the dot marks your current selection.</>} />}
          </p>
          <Suspense fallback={<div style={{ padding: '2rem', textAlign: 'center', color: 'var(--dg-text-mute)' }}>…</div>}>
            <HoeffdingExplorer
              isZh={isZh}
        ...
        <section className="dg-section">
          ...
          <p>
            {<T zh={<>Theorem 1 在 <TeX src="C = 205" /> 下要 <TeX src="\sim 3 \times 10^{8}" /> 样本才能把误证概率压到 1% —— 单核 0.2 秒/样本要跑 ~2 年。怎么改进?观察:绝大多数状态实际只要 <TeX src="\le 20" /> 步,只有极小比例 "far apart"。</>} en={<>Theorem 1 with <TeX src="C = 205" /> demands <TeX src="\sim 3 \times 10^{8}" /> samples for a 1% error — about a year of single-core compute. The fix: most states actually solve in <TeX src="\le 20" /> moves; only a vanishing fraction are "far apart".</>} />}
          </p>

          <div className="dg-definition">
            <span className="dg-def-head">{t('定义 — "far apart"', 'Definition — "far apart"')}</span>
            <p style={{ margin: 0 }}>
              {<T zh={<>对状态对 <TeX src="(u, v)" />,若 <TeX src="d(u, v) \ge 21" /> 称它们 far apart,否则 close。</>} en={<>For a pair <TeX src="(u, v)" />, call it <em>far apart</em> if <TeX src="d(u, v) \ge 21" />, otherwise <em>close</em>.</>} />}
            </p>
          </div>

          <p>
            {<T zh={<>考虑命题 <TeX src="\varphi := " /> "至少 0.03% 的状态对 far apart"。Lemma 6:如果 <TeX src="\varphi" /> 真,那么 500,000 次采样里 <strong>零次</strong> 看到 far-apart pair 的概率是:</>} en={<>Let <TeX src="\varphi := " /> "at least 0.03% of pairs are far apart". Lemma 6: if <TeX src="\varphi" /> holds, the probability of 500,000 samples turning up <strong>zero</strong> far-apart pair is:</>} />}
          </p>
          <div className="dg-formula">
            <TeX src={`(1 - 0.0003)^{500{,}000} \\;<\\; 7.02 \\times 10^{-66}.`} />
          </div>
          <p>
            {<T zh={<>但作者就是观察到了 0 例!所以 <TeX src="\varphi" /> 几乎必然为假,即至多 0.03% 状态对 far apart。剩下 <TeX src="\ge 99.97\%" /> 的 close pair 上 <TeX src="d \le 20" />,即 <TeX src="C = 20" />,Hoeffding 系数从 <TeX src="205^2" /> 暴跌到 <TeX src="20^2" />,样本量降 105 倍。</>} en={<>Yet they observed exactly zero. So <TeX src="\varphi" /> is overwhelmingly false: <TeX src="\le 0.03\%" /> of pairs are far apart, and on the close pairs <TeX src="d \le 20" />, hence <TeX src="C = 20" />. Hoeffding's coefficient drops from <TeX src="205^{2}" /> to <TeX src="20^{2}" />, cutting sample size by <TeX src="\approx 105" /> times.</>} />}
          </p>

          <h3>{t("凡人之数的来源:7-step 入门法", "Where the Human's number comes from: 7-step beginner's method")}</h3>
          <p>
            {<T zh={<>论文 Appendix A 给出一个粗放但严格的"入门法"上界。逐步累加:</>} en={<>Appendix A presents a generous-but-rigorous beginner's-method bound. Step by step:</>} />}
          </p>
          ...
          <p className="dg-sampler-note">
            {<T zh={<>明显不紧 — 任何 LBL 玩家平均都在 <TeX src="< 100" /> 步内解,但作为 "trivially provable in one page" 的 worst-case 已经够用。</>} en={<>Comically loose — any beginner averages <TeX src="< 100" /> moves — but as a worst case that you can prove in a single page it's plenty.</>} />}
          </p>
        ...
        <section className="dg-section">
          ...
          <p>
            {<T zh={<>定理证完了,实际跑还得有一个 <em>真</em> uniform 的采样器。论文用经典的 Fundamental Theorem of Cubology:</>} en={<>The theorem is proven, but you still need a <em>true</em> uniform sampler. The paper invokes the classical Fundamental Theorem of Cubology:</>} />}
          </p>

          <div className="dg-theorem">
            <span className="dg-theorem-head">{t('Theorem 3 — Cubology', 'Theorem 3 — Cubology')}</span>
            <p style={{ margin: 0 }}>
              {<T zh={<>把 8 个角 + 12 个棱拆下来重装回一个 3×3:重组合法当且仅当 (a) 角的排列 sign = 棱的排列 sign(<em>parity</em>);(b) 顺时针扭转角数 <TeX src="\equiv" /> 逆时针扭转角数 <TeX src="\pmod 3" />(<em>twist sum 0</em>);(c) 翻转棱数为偶数(<em>flip sum 0</em>)。</>} en={<>If you take all corners + edges off and reattach them, the reassembly is reachable from solved iff (a) corner sign = edge sign (<em>parity</em>); (b) corner twists sum to <TeX src="0 \pmod 3" /> (<em>twist sum</em>); (c) edge flips sum to <TeX src="0 \pmod 2" /> (<em>flip sum</em>).</>} />}
            </p>
          </div>

          <p>
            {<T zh={<>三个 <TeX src="\mathbb{Z}_2 \times \mathbb{Z}_3 \times \mathbb{Z}_2 = 12" /> 个等价类里有且只有一个合法 ⇒ 朴素 "uniform 随机置换 + reject" 期望 12 次重组。论文用更聪明的 fix-it 版本:重组后,在 3 个不变量上分别取一个固定 invalid 操作把它扳成合法,只要 1 次重组。这对应:</>} en={<>The three constraints partition <TeX src="\mathbb{Z}_2 \times \mathbb{Z}_3 \times \mathbb{Z}_2 = 12" /> equivalence classes; exactly one is legal. Naive "uniform permute + reject" averages 12 retries. The paper picks the smarter route: after one random reassembly, apply at most three deterministic correctives:</>} />}
          </p>
          <ul>
          ...
          <p>
            {<T zh={<>所有 <TeX src="4.3 \times 10^{19}" /> 状态被这 12 个非法操作的纤维覆盖,恰好 1/12 落到合法子群上 ⇒ 单次 reassemble + 三个 fix = 1 次有效采样。</>} en={<>The 12 invalid operations fibre every state above the legal subgroup; reassemble once + apply the three correctives gives 1 effective sample per pass.</>} />}
          </p>

          <div className="dg-callout">
            <span className="dg-callout-h">{t('对照:WCA scrambler', 'Compare: WCA scramblers')}</span>
            {<T zh={<>这跟 cubing.js / TNoodle 在每场 WCA 比赛上做的事情完全一样 — 随机选一个状态,Kociemba 求解,反转输出当 scramble。下面的"现场重跑实验"节就直接调它。</>} en={<>This is identical to what cubing.js / TNoodle do at every WCA competition — pick a random state, solve via Kociemba, reverse the solution as the scramble. The live-sampler section below taps the same code path.</>} />}
          </div>
        </section>
        ...
        <section className="dg-section">
          ...
          <p>
            {<T zh={<>论文实验在 MacBook Pro M3 (36 GB RAM, 16 cores) 上跑,使用开源 Kociemba 实现 <a href="https://github.com/efrantar/rob-twophase" target="_blank" rel="noopener noreferrer">efrantar/rob-twophase</a>。结果:</>} en={<>The paper's experiment ran on a MacBook Pro M3 (36 GB RAM, 16 cores) using the open-source Kociemba implementation <a href="https://github.com/efrantar/rob-twophase" target="_blank" rel="noopener noreferrer">efrantar/rob-twophase</a>. Results:</>} />}
          </p>
          <ul>
          ...
          <p>
            {<T zh={<>原图(论文 Figure 5)的精确计数,我们重画并加可切对数轴 / 单 bin hover:</>} en={<>The exact counts of Figure 5, redrawn with a log-toggle and per-bin hover:</>} />}
          </p>
          <Suspense fallback={<div style={{ padding: '2rem', textAlign: 'center', color: 'var(--dg-text-mute)' }}>…</div>}>
            <DistanceHistogram
              isZh={isZh}
        ...
        <section className="dg-section">
          ...
          <p>
            {<T zh={<>下面按钮起一个 Web Worker,反复在你浏览器里 (i) 取均匀随机三阶态;(ii) Kociemba 求解;(iii) 把解长度计入直方图。看着 <TeX src="\hat\mu" /> 一步步向论文的 <TeX src="18.3189" /> 收敛。你的样本会以绿色叠加在上面那张论文图上。</>} en={<>The button below spins up a Web Worker that, in a loop, (i) draws a uniformly random 3×3 state, (ii) solves via Kociemba, (iii) histograms the solution length. Watch <TeX src="\hat\mu" /> creep toward the paper's <TeX src="18.3189" />. Your samples are overlaid in green on the histogram above.</>} />}
          </p>
          <Suspense fallback={<div style={{ padding: '2rem', textAlign: 'center', color: 'var(--dg-text-mute)' }}>…</div>}>
        ...
        <section className="dg-section">
          ...
          <p>
            {<T zh={<>从 1981 Thistlethwaite 的 52 步开始,30 年里上下界一步步合拢。点每个方格看年份:</>} en={<>From Thistlethwaite's 52-move bound in 1981 to the 20/20 closure in 2010 — three decades of gap-shrinking. Each cell is one historical (lower, upper) snapshot:</>} />}
          </p>
        ...
        <section className="dg-section">
          ...
          <p>
            {<T zh={<>条件只两个:(i) 顶点传递 (这样 <TeX src="D < 2\mu" /> 成立);(ii) 能 uniform sample 顶点(这样 <TeX src="\hat\mu \to \mu" />)。所有 Cayley 图自动满足 (i);(ii) 看群的可计算结构。论文点名几个候选:</>} en={<>Two ingredients: (i) vertex transitivity (so <TeX src="D < 2\mu" /> holds); (ii) efficient uniform sampling. Every Cayley graph gives (i) for free; (ii) depends on the group's computable structure. The paper names several candidates:</>} />}
          </p>
          <div className="dg-table-wrap">
            <table className="dg-table">
              <thead>
          ...
          <p className="dg-sampler-note">
            {<T zh={<>5×5 当前最佳上下界 gap 比因子 2 还大 — 这正是 demigod 收益最大的场景。值得一提的是, So Hirata 2024(论文 ref [12,13])用 girth + branching factor 攻同一题,跟本文方法互补。</>} en={<>The 5×5 lower/upper-bound gap is wider than a factor of 2 — exactly where demigod buys the most. So Hirata's 2024 papers (refs [12,13] in the paper) attack the same problem via girth and branching factor — complementary to this approach.</>} />}
          </p>
        </section>
        ...
        <section className="dg-section">
          ...
          <p>
            {<T zh={<>论文最后一段值得抄录:它把 demigod 类结果摆在 <em>plausibility</em> 谱上,跟素数概率测试、Kepler 猜想机器辅助证明放一起。一个数是不是素数是 deterministic fact,可现实世界我们只能用 Miller-Rabin 拿到 "我有 <TeX src="2^{-128}" /> 误证概率" 的回答 —— 而我们接受了它当作"事实"。</>} en={<>The paper's closing argument is worth recording: it places demigod-class results on the spectrum of <em>plausibility</em> alongside probabilistic primality tests and machine-assisted proofs (Kepler's conjecture). Whether <TeX src="N" /> is prime is a deterministic fact, but in practice we accept "Miller-Rabin gives error <TeX src="\le 2^{-128}" />" as if it were one.</>} />}
          </p>
          <p>
            {<T zh={<>论文的总结很妙:"如果三阶魔方的直径真大于 36,那观察到 500,000 个样本均值 18.3 且 0 个 <TeX src="> 20" /> 的概率 <TeX src="< 10^{-10}" />"。这个量级跟"反对手用一台 2024 笔电几小时内枚举出 SHA-256 碰撞"的概率比 —— 谁更值得相信?</>} en={<>The paper's closing summary: "if the cube's diameter were actually greater than 36, the probability of observing an empirical mean of ~18.3 and zero <TeX src="> 20" /> samples in 500,000 draws is <TeX src="< 10^{-10}" />". Compare this to the chance of someone finding a SHA-256 collision overnight on a 2024 laptop — which is more believable?</>} />}
          </p>
          <div className="dg-callout">
            <span className="dg-callout-h">{t('开放问题(论文提出)', 'Open question (paper poses)')}</span>
            {<T zh={<><em>"Are there properties of finite mathematical objects that can only be certified efficiently to a high degree of confidence by probabilistic algorithms, but that we never can be certain of through a short proof?"</em></>} en={<><em>"Are there properties of finite mathematical objects that can only be certified efficiently to a high degree of confidence by probabilistic algorithms, but that we never can be certain of through a short proof?"</em></>} />}
          </div>
