    ...
    <div className="unit-distance-page">
      ...
      <main className="ud-main">
        ...
        <section className="ud-section">
          ...
          <ul>
            <li>
              {<T zh={<><strong>什么:</strong>设 <TeX src="\nu(n)" /> 是平面 n 点集中单位距离对子数的最大值。Erdős 1946 猜想 <TeX src="\nu(n) \le n^{1 + C / \log \log n}" />。</>} en={<><strong>What:</strong> let <TeX src="\nu(n)" /> = max number of unit-distance pairs among n planar points. Erdős 1946 conjectured <TeX src="\nu(n) \le n^{1 + C / \log \log n}" />.</>} />}
            </li>
            <li>
              {<T zh={<><strong>反例:</strong>存在常数 <TeX src="\delta > 0" />,使得 <TeX src="\nu(n) \ge n^{1+\delta}" /> 对无穷多 n 成立。</>} en={<><strong>Counter:</strong> there is <TeX src="\delta > 0" /> such that <TeX src="\nu(n) \ge n^{1+\delta}" /> for infinitely many n.</>} />}
            </li>
            <li>
              {<T zh={<><strong>怎么做:</strong>把 Erdős 经典的 ℤ[i] 网格构造提升到高维。用一个全实数域 L 的无穷无支 pro-3 塔(度数 → ∞,根判别式保持有界),取 K = L(i) 这种 CM 域;Chebotarev 选定的素数在每层完全分裂,产生指数多 |σ(u)| = 1 的元素 u,Minkowski 嵌入到 ℂ^f 中再投影到第一坐标,得到 ℝ² 中的点集。</>} en={<><strong>How:</strong> lift Erdős's classical ℤ[i] grid trick to high dimensions. Use an infinite unramified pro-3 tower over a totally-real field L (degrees → ∞, root discriminant bounded), take the CM field K = L(i); Chebotarev gives primes that split completely in every layer, producing exponentially many u with |σ(u)| = 1. Minkowski-embed into ℂ^f and project to the first coordinate to get an ℝ² point set.</>} />}
            </li>
            <li>
              {<T zh={<><strong>关键技术:</strong>Golod–Shafarevich 不等式保证 pro-3 塔无穷;Frattini 子群论证使添加分裂条件时不降低生成元秩;CM 域 + Minkowski 把"代数 norm-1"翻译成"几何 |σ(u)| = 1"。</>} en={<><strong>Key tools:</strong> Golod–Shafarevich keeps the pro-3 tower infinite; a Frattini-subgroup argument adds splitting conditions without dropping the generator rank; CM + Minkowski translates "algebraic norm 1" to "geometric |σ(u)| = 1".</>} />}
            </li>
            <li>
              {<T zh={<><strong>AI 元亮点:</strong>问题由 AI 撰写,内部模型完全自主求解,AI 评分流水线打分后才进入人类审阅。原始模型输出与论文一同公开。</>} en={<><strong>AI angle:</strong> AI wrote the problem, the internal model solved it autonomously, an AI grading pipeline scored it, <em>then</em> humans reviewed. The raw model output is published alongside the paper.</>} />}
            </li>
          </ul>
        </section>
        ...
        <section className="ud-section">
          ...
          <p>
            {<T zh={<>给定平面上 n 个不同的点 <TeX src="P \subset \mathbb{R}^2" />,定义</>} en={<>Given n distinct points <TeX src="P \subset \mathbb{R}^2" /> in the plane, define</>} />}
          </p>
          <div className="ud-formula">
            <TeXBlock src={`\\nu(P) \\;=\\; \\#\\{\\{x, y\\} \\subset P : |x - y| = 1\\},\\qquad \\nu(n) \\;=\\; \\max_{|P| = n} \\nu(P).`} />
          </div>
          <p>
            {<T zh={<>下面拖点试试。等边三角形给最高 ν/n(每个三角形边都是单位距离);切到方格立刻能看出 <em>水平 + 垂直</em> 两族,但少了对角线那一族。</>} en={<>Drag any point below. Equilateral triangles maximise local ν/n (every triangle edge is a unit distance); switching to the square lattice loses the diagonal family entirely.</>} />}
          </p>
        ...
        <section className="ud-section">
          ...
          <p>
            {<T zh={<>由 <strong>Kővári–Sós–Turán 定理</strong>(1954),任何 K₂,₃-free 图边数 ≤ <TeX src="\tfrac{1}{2}(1 + \sqrt{4n-3})n^{1/2} \cdot n = O(n^{3/2})" />。Erdős 1946 原文给出的就是这条 <TeX src="O(n^{3/2})" /> 上界。</>} en={<>By <strong>Kővári–Sós–Turán</strong> (1954), any K₂,₃-free graph has ≤ <TeX src="\tfrac{1}{2}(1 + \sqrt{4n-3})n^{1/2} \cdot n = O(n^{3/2})" /> edges. Erdős's 1946 paper used exactly this to bound ν.</>} />}
          </p>
          <p>
            {<T zh={<>1984 年 Spencer–Szemerédi–Trotter 用<strong>点-曲线 incidence 定理</strong>(同年自己证的)把它压到 <TeX src="O(n^{4/3})" />。1997 年 Székely 给出<strong>用 crossing number 的一页证明</strong>:把单位距离图画在平面上,每对单位距离对应一段单位圆弧;Crossing Lemma <TeX src="\mathrm{cr}(G) \ge c \cdot e^3 / n^2" /> 直接推出 <TeX src="e = O(n^{4/3})" />。这是 40 年来最佳上界,只有常数被微调过(2022 Ágoston–Pálvölgyi)。</>} en={<>In 1984 Spencer–Szemerédi–Trotter sharpened this to <TeX src="O(n^{4/3})" /> via their point-curve incidence theorem (proved that same year). In 1997 Székely gave a <strong>one-page crossing-number proof</strong>: draw the unit-distance graph in the plane, every unit distance is an arc on some unit circle; the Crossing Lemma <TeX src="\mathrm{cr}(G) \ge c \cdot e^3 / n^2" /> directly forces <TeX src="e = O(n^{4/3})" />. This is the best upper bound to date — only the constant has been improved (Ágoston–Pálvölgyi 2022).</>} />}
          </p>
        </section>
        ...
        <section className="ud-section">
          ...
          <p className="ud-figure-caption">
            {<T zh={<>↑ 全分辨率精确复刻 — 与 OpenAI 博客头图同构造。每条线都是 <em>恰好</em> √k 长。切换 k 看 r₂(k) 怎么决定密度。</>} en={<>↑ Full-resolution exact reproduction — same construction as OpenAI's hero figure. Every segment is <em>exactly</em> √k long. Switch k to feel how r₂(k) drives density.</>} />
            }
          </p>

          <p>
            {<T zh={<>取 √n × √n 整数网格,直接用单位 1 给出的对子数只有 <TeX src="2s(s-1) \approx 2n" />,这是平凡的线性下界。
                                                                                                        Erdős 的关键观察:把"单位距离"<em>重定义</em>成 <TeX src="\sqrt{k}" />,其中 k 可以是任何整数。这时,两点 (x₁, y₁) (x₂, y₂) 之间是"单位距离" ⟺ <TeX src="(x_1 - x_2)^2 + (y_1 - y_2)^2 = k" /> ⟺ k 是两个整数的平方和。</>} en={<>Take the √n × √n integer grid. With "unit = 1", there are only <TeX src="2s(s-1) \approx 2n" /> unit-distance pairs — a trivial linear bound.
          ...
          <p>
            {<T zh={<>设 <TeX src="r_2(k)" /> 是 k 写成两整数平方和的方式数。 r₂(1) = 4 ((±1, 0), (0, ±1));但 r₂(5) = 8 ((±1, ±2), (±2, ±1))。在 s×s 网格里,squared-distance = k 的对子数 ≈ <TeX src="\frac{1}{2} \sum_{(a,b): a^2+b^2=k} (s - |a|)(s - |b|)" />,正比于 r₂(k)。</>} en={<>Let <TeX src="r_2(k)" /> count the ways to write k as a sum of two squares. r₂(1) = 4 ((±1, 0), (0, ±1)); r₂(5) = 8 ((±1, ±2), (±2, ±1)). In the s×s grid, the count of pairs at squared distance k is ≈ <TeX src="\frac{1}{2} \sum_{(a,b): a^2+b^2=k} (s - |a|)(s - |b|)" />, proportional to r₂(k).</>} />}
          </p>
          <p>
            {<T zh={<>挑使 r₂(k) 最大的 k ≤ n,Erdős 得到</>} en={<>Choosing k ≤ n with maximum r₂(k), Erdős got</>} />}
          </p>
          <div className="ud-formula">
            <TeXBlock src={`\\nu(n) \\;\\ge\\; n \\cdot n^{c / \\log \\log n} \\;=\\; n^{1 + c / \\log \\log n}.`} />
          </div>
          <p>
            {<T zh={<>正是猜想的形式!Erdős 猜的上界与他自己的下界<em>形式</em>一样,差的是常数 c → C。他相信这就是真实增长。下面拖滑块改 k 和 s 看看 r₂(k) 怎么影响 ν 的:</>} en={<>The same form as his conjectured upper! He believed his own lower bound was tight up to constants. Below, slide k and s to see how r₂(k) drives ν:</>} />}
          </p>
          <Suspense fallback={<div className="ud-loading">Loading…</div>}>
            <GridConstruction />
          </Suspense>
        ...
        <section className="ud-section">
          ...
          <p>
            {<T zh={<>Erdős 网格构造给的指数 1 + c/log log n 趋近 1,看起来"接近线性";自己的上界 O(n^(3/2)) 后来被 SST 压到 O(n^(4/3)),离 n^(1+o(1)) 还有距离。从 80 年代起一系列结果都<em>支持</em> n^(1+o(1)) 这边。</>} en={<>Erdős's grid gave 1 + c/log log n, which approaches 1 ("almost linear"); his upper bound O(n^(3/2)) was improved by SST to O(n^(4/3)), still leaving room for n^(1+o(1)). Decades of subsequent work all <em>pointed toward</em> n^(1+o(1)).</>} />}
          </p>
          ...
          <div className="ud-evidence-cards">
            <div className="ud-evidence-card">
              ...
              <p>
                {<T zh={<>对 ℝ² 上 <em>大多数</em>(Baire-generic)范数,ν ≤ <TeX src="O(n \log n \log\log n)" /> — <strong>几乎线性</strong>。</>} en={<>For <em>most</em> (Baire-generic) norms on ℝ², ν ≤ <TeX src="O(n \log n \log\log n)" /> — <strong>near-linear</strong>.</>} />}
              </p>
            </div>
            <div className="ud-evidence-card">
              ...
              <p>
                {<T zh={<>对 ℝ^d 中 <em>同源</em>(comeagre)范数集,ν ≤ <TeX src="\tfrac{d}{2} n \log^2 n" /> — 任意维度都几乎线性。</>} en={<>For <em>comeagre</em> norms on ℝ^d, ν ≤ <TeX src="\tfrac{d}{2} n \log^2 n" /> — near-linear in all dims.</>} />}
              </p>
            </div>
            <div className="ud-evidence-card">
              <div className="ud-evidence-year">2025</div>
              ...
              <p>
                {<T zh={<>对<em>所有</em>范数 (不只 comeagre),匹配下界 <TeX src="(d/2 - o(1)) n \log^2 n" />。证据更强了。</>} en={<>For <em>all</em> norms (not just comeagre), matching lower bound <TeX src="(d/2 - o(1)) n \log^2 n" />. Even stronger evidence.</>} />}
              </p>
            </div>
          </div>
          ...
          <p>
            {<T zh={<>Bloom 在 Remarks §4 直白说:"<em>大部分人在这个问题上花的时间,都在试图证明上界,而不是证伪。</em>" — 80 年的人类研究在错的方向用力。</>} en={<>Bloom in Remarks §4 puts it bluntly: "<em>Most of the human effort has been on trying to prove the upper bound, rather than disprove it.</em>" — 80 years of effort in the wrong direction.</>} />}
          </p>
        </section>
          ...
          <div className="ud-timeline">
            {TIMELINE.map((row, i) => (
              <div key={i} className={`ud-timeline-row ${row.final ? 'is-final' : ''}`}>
                ...
                <div className="ud-timeline-text">{tr(row)}</div>
              </div>
            ))}
          </div>
        ...
        <section className="ud-section">
          ...
          <p>
            {<T zh={<>Erdős 1946 那篇<em>同时</em>提出两个对偶的离散几何问题。一个是单位距离 ν(n)(本页主题);另一个是 <strong>不同距离 D(n)</strong>:n 个平面点能有<em>最少</em>多少种不同的两两距离?</>} en={<>Erdős's 1946 paper actually posed <em>two</em> dual discrete-geometry problems. One is unit distances ν(n) (this page's topic); the other is <strong>distinct distances D(n)</strong>: the <em>minimum</em> number of distinct pairwise distances among n planar points.</>} />}
          </p>
          ...
          <p>
            {<T zh={<>两个问题<em>对偶</em>:ν 问"重复多少次最大",D 问"不同种类多少最小"。直觉上 ν · D ≥ n²/2 (鸽笼:n² 个对子分配到 D 种距离里,最大那个至少占 n²/(2D))。</>} en={<>The two problems are <em>dual</em>: ν asks "max repetition", D asks "min variety". Intuitively ν · D ≥ n²/2 (pigeonhole: n² pairs across D distances, the most common one occurs ≥ n²/(2D) times).</>} />}
          </p>
          <p>
            {<T zh={<>历史上 D 比 ν 容易得多:Guth–Katz 2015 用 <strong>多项式方法 + 代数几何</strong>(Dvir 的 finite-field Kakeya 思路升级版),给出 <TeX src="D(n) \ge c \cdot n / \log n" /> — 离 Erdős 猜想 <TeX src="n / \sqrt{\log n}" /> 只差 <TeX src="\sqrt{\log n}" /> 因子。
                                                                                                        <strong>ν 反方向</strong>:这次反而是<em>下界</em>被刷新到 n^1.014,但 Erdős 的猜想方向(认为是上界)<em>错了</em>。Litt 在 Remarks §6 评论说:"D 的解决用了 polynomial method 这种<em>新工具</em>;ν 的反驳<em>没</em>引入新几何工具,只是把数论用得更深 — 这两题的难度气质完全不同。"</>} en={<>Historically D was the easier one: Guth–Katz 2015 used <strong>the polynomial method</strong> (an upgrade of Dvir's finite-field Kakeya argument) to prove <TeX src="D(n) \ge c \cdot n / \log n" /> — only a <TeX src="\sqrt{\log n}" /> factor from Erdős's conjecture <TeX src="n / \sqrt{\log n}" />.
        ...
        <section className="ud-section">
          ...
          <div className="ud-theorem">
            ...
            <p style={{ margin: '0.6rem 0 0' }}>
              {<T zh={<>存在绝对常数 <TeX src="\delta > 0" />,以及无穷多正整数 n,使得 <TeX src="\nu(n) \ge n^{1 + \delta}" />。</>} en={<>There exists an absolute constant <TeX src="\delta > 0" /> and infinitely many positive integers n such that <TeX src="\nu(n) \ge n^{1 + \delta}" />.</>} />}
            </p>
          </div>
          <p>
            {<T zh={<>这反驳了 Erdős 1946 的猜想 <TeX src="\nu(n) \le n^{1+C/\log\log n}" />:任何形如 <TeX src="n^{1+\delta}" /> 的下界,只要 δ &gt; 0 是常数,就最终 (即 n 足够大时) 超越 <TeX src="n^{1+C/\log\log n}" />,因为后者的指数 1 + C/log log n 收敛到 1。</>} en={<>This refutes Erdős's 1946 conjecture <TeX src="\nu(n) \le n^{1+C/\log\log n}" />: any constant-δ lower bound of the form <TeX src="n^{1+\delta}" /> eventually (for large enough n) exceeds <TeX src="n^{1+C/\log\log n}" />, because the latter's exponent 1 + C/log log n converges to 1.</>} />}
          </p>
          <p>
            {<T zh={<>它<em>还</em>反驳了 Erdős–Fishburn 1997 的更强猜想 (任意点都有 ≥ k 等距邻居 ⟹ k ≤ n^o(1)):构造里点集的单位距离图<em>平均度</em>是 n^Ω(1),平均度 ≥ 2k 的图有最小度 ≥ k 的子图。</>} en={<>It <em>also</em> refutes the stronger Erdős–Fishburn 1997 conjecture (every point has ≥ k equidistant neighbours ⟹ k ≤ n^o(1)): the construction's unit-distance graph has average degree n^Ω(1), and average-degree ≥ 2k always contains a subgraph of minimum degree ≥ k.</>} />}
          </p>
        </section>
        ...
        <section className="ud-section">
          ...
          <p>
            {<T zh={<>第 4 节那个 r₂(k) 的小聪明,几何上其实是在 Gauss 整数 ℤ[i] 上做"范数为 k"的分解。在 ℤ[i] 里,k = q 是 ≡ 1 mod 4 的素数 ⟹ q = π · π̄,π 是 Gauss 素数。许多这样的 q 乘起来,得到 z ∈ ℤ[i] 满足 z·z̄ = k,几何上 z 就是从原点出发长度为 √k 的格点。</>} en={<>The r₂(k) trick in §4 is, geometrically, a "norm = k" factorisation in the Gaussian integers ℤ[i]. In ℤ[i], if k = q is a prime ≡ 1 mod 4, then q = π · π̄ for a Gaussian prime π. Multiplying many such q's gives many z ∈ ℤ[i] with z·z̄ = k — each z is a lattice point at distance √k from origin.</>} />}
          </p>
          <p>
            {<T zh={<>新构造做的事:把 ℚ 换成更大的全实场 L,把 ℤ[i] 换成 <TeX src="\mathcal{O}_K" /> (其中 K = L(i))。L 的非平凡自同构 c (限制到 K 上的复共轭) 把"复模 = 1"翻译成"对所有复嵌入 σ 都有 |σ(u)| = 1"。L 的度数 [L:ℚ] → ∞,所以 ℂ^f 是真正的高维空间,可以塞下指数多 norm-1 元素。</>} en={<>The new construction: replace ℚ by a larger totally real field L, and ℤ[i] by <TeX src="\mathcal{O}_K" /> (K = L(i)). The non-trivial automorphism c (becoming complex conjugation under any embedding of K) translates "complex modulus 1" into "|σ(u)| = 1 for every complex embedding σ". Since [L:ℚ] → ∞, ℂ^f is genuinely high-dimensional and fits exponentially many norm-1 elements.</>} />}
          </p>
        ...
        <section className="ud-section">
          ...
          <p>
            {<T zh={<>下面 5 步构造里反复出现这几个词,先各给一句话直觉 + 一行公式。</>} en={<>The 5-stage construction below uses these terms repeatedly. One sentence of intuition + one formal line each.</>} />}
          </p>
          ...
          <div className="ud-glossary">
            <div className="ud-glossary-card">
              ...
              <div className="ud-glossary-desc">
                {<T zh={<>L 的所有共轭嵌入到 ℂ 后<em>全是实数</em>。例:<TeX src="\mathbb{Q}(\sqrt{2})" />,<TeX src="\mathbb{Q}(\zeta_7 + \zeta_7^{-1})" />。</>} en={<>All conjugate embeddings of L land in ℝ. E.g. <TeX src="\mathbb{Q}(\sqrt{2})" />, <TeX src="\mathbb{Q}(\zeta_7 + \zeta_7^{-1})" />.</>} />}
              </div>
            </div>
            ...
            <div className="ud-glossary-card">
              ...
              <div className="ud-glossary-desc">
                {<T zh={<>全实域上加 i 得到的全虚二次扩张。非平凡自同构 c 在<em>每个</em>复嵌入下都是普通复共轭。<strong>这是 |σ(u)|=1 ⟺ u·c(u)=1 这条关键等价的来源。</strong></>} en={<>Totally imaginary quadratic extension of a totally real field. The non-trivial automorphism c becomes ordinary complex conjugation under <em>every</em> complex embedding. <strong>This is what makes |σ(u)|=1 ⟺ u·c(u)=1.</strong></>} />}
              </div>
            </div>
            ...
            <div className="ud-glossary-card">
              ...
              <div className="ud-glossary-desc">
                {<T zh={<>分式理想 / 主理想 这个商群的大小。等于 1 时 OK 是 PID。<strong>论证里用作鸽笼分母:2^m 个理想分到 h(K) 个等价类,某类至少 2^m / h(K) 个 — 这就是 norm-1 元素的来源。</strong></>} en={<>Size of fractional-ideals / principal-ideals. If 1, OK is a PID. <strong>Used as a pigeonhole denominator: 2^m ideals into h(K) classes ⇒ some class has ≥ 2^m / h(K) — this is where norm-1 elements come from.</strong></>} />}
              </div>
            </div>
            ...
            <div className="ud-glossary-card">
              ...
              <div className="ud-glossary-desc">
                {<T zh={<>判别式开 n 次根 — 一种"度无关"的复杂度度量。<strong>无支扩张保持根判别式不变</strong>,这就是 Golod–Shafarevich 塔的核心好处:度数→∞ 但 rd 不爆,Minkowski 估计 h(K) ≤ rd(K)^[K:ℚ] 才有用。</>} en={<>The n-th root of the discriminant — a "degree-independent" complexity. <strong>Unramified extensions preserve rd</strong>, the entire reason the Golod–Shafarevich tower is useful: degree → ∞ but rd stays bounded, so Minkowski's h(K) ≤ rd(K)^[K:ℚ] remains usable.</>} />}
              </div>
            </div>
            ...
            <div className="ud-glossary-card">
              ...
              <div className="ud-glossary-desc">
                {<T zh={<>无支素 p 上的"提升"自同构,作用 <TeX src="x \mapsto x^{N\mathfrak{p}}" /> mod p。<strong>Frob 类平凡 ⟺ p 在扩张中完全分裂。</strong>Chebotarev 让我们能选定 Frob 类。</>} en={<>The "lift" automorphism above an unramified prime p, acting as <TeX src="x \mapsto x^{N\mathfrak{p}}" /> mod p. <strong>Frob class trivial ⟺ p splits completely.</strong> Chebotarev lets us prescribe these.</>} />}
              </div>
            </div>
            ...
            <div className="ud-glossary-card">
              ...
              <div className="ud-glossary-desc">
                {<T zh={<>所有极大开子群的交。元素属于 Φ(G) ⟺ "不当生成元用也无所谓"。<strong>关键:把 Φ(G) 里的元素当关系加进商群,不会降低生成元秩。</strong>这就是为啥可以杀掉选定 Frobenius 而不破坏 Golod–Shafarevich 估计。</>} en={<>Intersection of all maximal open subgroups. An element is in Φ(G) iff "removing it as a generator doesn't matter". <strong>Key: imposing Φ(G)-elements as relations doesn't drop d(G).</strong> That's why we can kill chosen Frobenius elements without breaking the Golod–Shafarevich count.</>} />}
              </div>
            </div>
            ...
            <div className="ud-glossary-card">
              ...
              <div className="ud-glossary-desc">
                {<T zh={<>有限 p-群的逆极限 — 拓扑群,所有有限商都是 p-群。<strong>无支 pro-p 扩张 F^{`{ur,p}`} 的 Gal 群是这种结构</strong>;Golod–Shafarevich 在这上面才生效(r ≤ d²/4 ⇒ 无穷)。</>} en={<>Inverse limit of finite p-groups — a topological group with all finite quotients p-groups. <strong>The Galois group of the maximal unramified pro-p extension F^{`{ur,p}`} is one</strong>; Golod–Shafarevich works in this category (r ≤ d²/4 ⇒ infinite).</>} />}
              </div>
            </div>
            ...
            <div className="ud-glossary-card">
              ...
              <div className="ud-glossary-desc">
                {<T zh={<>每个素 p 在 K 中分解时,所有素 P_i 的<em>分支指数</em>都是 1(没有重复因子)。结果:rd(K) 保持不变。<strong>OpenAI/Sawin 构造的塔每一层都无支</strong>,这是 rd 不爆的源头。</>} en={<>Every prime p factors in K with all ramification indices = 1 (no repeated factors). Consequence: rd(K) stays put. <strong>Every layer of the OpenAI/Sawin tower is unramified</strong>, the reason rd doesn't blow up.</>} />}
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="ud-section">
          ...
          <p>
            {<T zh={<>这一节是论文 §2 的可视化。论文里 f → ∞,这里我们退到 f = 2 (Gauss 整数 ℤ[i] in ℂ) 来直观地看。改 R 看 |X| 和单位距离对怎么同时指数增长 — 关键比是它们的指数差,正比于 1 + δ。</>} en={<>This visualises §2 of the paper. The real proof has f → ∞; we drop to f = 2 (Gaussian integers ℤ[i] in ℂ) for intuition. Slide R to see |X| and unit-pair counts both grow exponentially — the gap between their exponents is what 1 + δ is.</>} />}
          </p>
          <Suspense fallback={<div className="ud-loading">Loading…</div>}>
            <LatticeProjection />
          </Suspense>
          ...
          <p>
            {<T zh={<>论文里的<strong>关键引理 2.4</strong>:多圆盘 W 里某个余集 a + Λⱼ 至少有 <TeX src="e^{\gamma f_j / 2} |X|" /> 个 X 上的有序对差落在 Uⱼ 中。这就是为什么投影到第一坐标后的 ℝ² 点集 P 有 <TeX src="\nu(P) \ge \frac{1}{2} e^{\gamma f / 2} |P|" /> 个单位距离对 — 远超平凡的 |P| 量级。</>} en={<>The paper's <strong>key Lemma 2.4</strong>: some coset a + Λⱼ in W has ≥ <TeX src="e^{\gamma f_j / 2} |X|" /> ordered pairs whose difference lies in Uⱼ. That's why the projected planar set P has <TeX src="\nu(P) \ge \frac{1}{2} e^{\gamma f / 2} |P|" /> unit pairs — far exceeding the trivial linear count.</>} />}
          </p>
        </section>
        ...
        <section className="ud-section">
          ...
          <div className="ud-reactions">
            ...
            <div className="ud-reaction">
              ...
              <blockquote>
                {<T zh={<>"这是<em>第一个</em>我发现本身就令人兴奋(而非作为'下一步会怎样'的领先指标)的 AI 自主生成结果。类比:有限域 Kakeya 猜想被 Dvir 用一个简短而漂亮的论证解决;sensitivity conjecture 被黄昊用一个 elegant argument 解决。这次单位距离问题的解似乎是同一种气质。"</>} en={<>"This is the <em>first example</em> of a result produced autonomously by an AI that I find exciting in itself, as opposed to as a leading indicator. Analogues: Dvir's finite-field Kakeya proof; Huang's sensitivity conjecture proof. This unit-distance solution feels like the same flavor."</>} />}
              </blockquote>
            </div>
          </div>
          ...
          <div className="ud-open-grid">
            <div className="ud-open-card">
              ...
              <p>
                {<T zh={<>当前 <TeX src="n^{1.014} \le \nu(n) \le O(n^{4/3})" />。差 24 倍指数(<TeX src="(4/3 - 1)/0.014" />)。<strong>真值在哪没人知道。</strong>可能 4/3 就是答案,可能介于,可能更小。</>} en={<>Current: <TeX src="n^{1.014} \le \nu(n) \le O(n^{4/3})" />. Factor-24 gap in exponent. <strong>True value unknown.</strong> Could be 4/3, could be in between, could be smaller.</>} />}
              </p>
            </div>
            <div className="ud-open-card">
              <div className="ud-open-h">{t('SST 上界 40 年没动', 'SST upper bound stuck')}</div>
              <p>
                {<T zh={<>SST 1984 的 <TeX src="O(n^{4/3})" /> 自 1984 以来除常数微调外没有改进。<strong>能否压到 <TeX src="O(n^{4/3 - \varepsilon})" />?</strong>会是 40 年来最重大的进展。</>} en={<>SST 1984 hasn't budged beyond constant tweaks. <strong>Can the upper be sharpened to <TeX src="O(n^{4/3 - \varepsilon})" />?</strong> Would be the biggest advance in 40 years.</>} />}
              </p>
            </div>
            <div className="ud-open-card">
              <div className="ud-open-h">{t('显式构造?', 'Explicit construction?')}</div>
              <p>
                {<T zh={<>目前所有证明都依赖 Golod–Shafarevich 这种<em>存在性</em>论证。<strong>能否给出显式 P_j 的具体坐标?</strong>哪怕一层都好。</>} en={<>All proofs rely on Golod–Shafarevich <em>existence</em>. <strong>Can we exhibit explicit P_j coordinates?</strong> Even one layer would be a milestone.</>} />}
              </p>
            </div>
            <div className="ud-open-card">
              <div className="ud-open-h">{t('Erdős–Fishburn', 'Erdős–Fishburn')}</div>
              <p>
                {<T zh={<>更强变体:每个点都有 ≥ k 个等距邻居 ⟹ k ≤ n^o(1)。OpenAI 也<em>顺手反驳</em>了这条 — 新构造的平均度是 n^Ω(1)。</>} en={<>Stronger variant: every point with ≥ k equidistant neighbours ⟹ k ≤ n^o(1). OpenAI also <em>incidentally refuted</em> this — the new construction has avg degree n^Ω(1).</>} />}
              </p>
            </div>
            <div className="ud-open-card">
              <div className="ud-open-h">{t('Bloom 的"Top 10"还剩哪些?', 'Bloom\'s "Top 10" — which next?')}</div>
              <p>
                {<T zh={<>2026-04 Bloom 列了 10 个最 prominent 的 Erdős 问题。单位距离一个月内倒下。<strong>下一个会是哪个?</strong>区别在哪?这种"非显式构造性反例"的成功模式还能复用吗?</>} en={<>2026-04 Bloom listed his 10 most prominent Erdős problems. Unit distance fell in a month. <strong>Which next?</strong> What distinguishes them? Does the "non-constructive AI counterexample" pattern generalise?</>} />}
              </p>
            </div>
            <div className="ud-open-card">
              <div className="ud-open-h">{t('高维推广', 'Higher dimensions')}</div>
              <p>
                {<T zh={<>在 ℝ^d (d ≥ 3) 中相应问题怎样?Sawin 的方法应该可推广,但 d=3,4 时的上界 (Erdős-Hickerson) 也已经 Ω(n^{`{4/3}`}) — 现在 d ≥ 3 是不是同样可达 n^(1+δ) 下界?</>} en={<>How about ν in ℝ^d, d ≥ 3? Sawin's method should generalise, but upper bounds (Erdős–Hickerson) are already Ω(n^{`{4/3}`}). Is the n^(1+δ) lower bound now reachable for d ≥ 3?</>} />}
              </p>
            </div>
          </div>
