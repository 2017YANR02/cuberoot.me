/**
 * RegionPicker — World / 6 大洲 / 国家三层下拉,支持 single 和 multi 两种 mode。
 * 提取自 RecordsPage 内部的同名组件,供 records / calendar 等页面共用。
 *
 * mode=single (default):
 *   value: 'world' | continent slug ('africa' / 'asia' / 'europe' / 'northAmerica' / 'oceania' / 'southAmerica') | iso2 lower
 *   选中即关闭。Records 页用 URL ?region=xxx 跟它绑。
 * mode=multi:
 *   value: 数组,token 是 iso2 lower 或 continent code uppercase ('AF' / 'AS' / 'EU' / 'NA' / 'OC' / 'SA')
 *   点击 toggle、不关闭。展开大洲为下属国家由消费方调 expandCountrySelection 处理。
 *
 * restrictTo:可选,限定国家列表;传入顺序保留(消费方负责排序逻辑)。
 */
import { useEffect, useMemo, useState } from 'react';
import { Search } from 'lucide-react';
import { Flag } from '../../utils/flag';
import { countryName } from '../../utils/country_name';
import { isContinentCode, type ContinentCode } from '../../utils/continent';
import './region_picker.css';

interface ContinentInfo {
  slug: string;          // records URL token (africa / northAmerica ...)
  code: ContinentCode;   // calendar multi token (AF / NA ...)
  zh: string;
  en: string;
}
...
interface SharedProps {
  isZh: boolean;
  className?: string;
  /** 限定可选国家列表(任意大小写 iso2),按传入顺序保留 */
  restrictTo?: string[];
  /** "全部区域" 文案,默认 World/All regions */
  allLabel?: string;
