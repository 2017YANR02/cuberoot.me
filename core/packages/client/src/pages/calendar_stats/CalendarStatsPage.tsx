/**
 * /calendar/stats — WCA 比赛随时间分布的可视化。
 * 数据源：utils/comp_search.loadComps()（all_past + all_upcoming，按 id 去重）。
 * 视图：年×月热力图（主） + Top 国家条形（次） + Top 项目条形。
 * 过滤器：国家、项目。
 */
...
// NOTE: 单色蓝调色阶——0 ~ max 切 6 段；与 timer/PracticeHeatmap 同色家族
...
export default function CalendarStatsPage() {
  ...
  // NOTE: 折线图缩放窗口（独立于 bars——三个面板互不联动，各跑各的）
  const [view, setView] = useState({ start: 0, end: 1 });

  useEffect(() => { loadComps().then(setComps); }, []);

  // NOTE: filter / 粒度变化或 series 首次就绪时——视图默认从 2003 年起步（早期 WCA 数据稀疏，2003 之前一年也就一两场）。
  //       series 是 useMemo，依赖在 country/event/granularity/yearMonthGrid 变化时变化，单一 effect 同时覆盖三种触发场景。
  ...
  // ── 折线图数据点（按粒度切换） ───────────────────────────
  // NOTE: 时间锚点从 2003-08（月）/ 2003（年）起步——遵循 CLAUDE.md "时间序列展示" 默认从 2003-08 起.
  //       1982 那场 WC1982 仍计入 bars 的"全部时间"聚合（filtered 不被截）。
  ...
  // ── bars 跑全 series 范围（不受折线图 zoom 影响，互相独立） ──
  ...
  // ── scope 计算：播放头 null → 全时段（含 1982，符合 CLAUDE.md "统计含第 1 场"）；
  //    播放头有值 → 累计窗口 [series[0]=2003-08, series[idx]] ──
  const computeScope = useCallback((animIdx: number | null) => {
    if (series.length === 0) return null;
    if (animIdx === null) return null; // 全数据
    const a = series[0];
    ...
    <div className="cs-page">
      <header className="cs-header">
        <div className="cs-header-left">
          <Link to={`/calendar${getLangQuery()}`} className="cs-back" aria-label={isZh ? '返回日历' : 'Back to calendar'}>
            <ArrowLeft size={18} />
          </Link>
...
// ── 播放头 hook：统一管理 idx + playing + auto-advance ─────────────
interface Animator {
  idx: number | null;
  playing: boolean;
  toggle: () => void;
  ...
  return (
    <div className="cs-scrubber">
      <input
        ...
        style={{ '--cs-scrubber-pct': `${pct}%` } as React.CSSProperties}
        aria-label="seek"
      />
...
function LineChart({ series, granularity, isZh, view, onViewChange, onPointClick }: LineChartProps) {
  ...
  // 缩放/平移状态由父组件控制（bars 跟着这个窗口走）
  const setView = onViewChange;
  const svgRef = useRef<SVGSVGElement>(null);
  // 拖动状态——存进 ref 避免 mousemove 重渲染抖动
  const dragRef = useRef<{ pointerId: number; clientX: number; vStart: number; vEnd: number } | null>(null);
  // pinch（双指）状态
  const pinchRef = useRef<Map<number, number>>(new Map()); // pointerId → clientX
  ...
  const clampView = (s: number, e: number): { start: number; end: number } => {
    const minW = series.length > 1 ? 2 / series.length : 0.05; // 至少 2 个点
    let width = Math.max(minW, Math.min(1, e - s));
    let start = Math.max(0, Math.min(1 - width, s));
    return { start, end: start + width };
  };

  // ── 滚轮缩放（passive:false 才能阻止页面滚动；React 默认 passive，所以手动 addEventListener） ──
  useEffect(() => {
  ...
  // Y 轴刻度——选合理整数步长（1, 2, 5, 10, 20, 50, 100, ...）
