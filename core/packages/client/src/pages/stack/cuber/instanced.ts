...
// Cubelet rounded corner 凹陷最深 = BORDER = 3。Core box 面要比这更深才不会从外
// 面 cubelet 的圆角缝里漏出来。我们用 4 个单位 (BORDER + 1 安全 margin) →
// box size = N*SIZE - 8。 Slice 旋转的洞里最外 4 单位看到 slice cubelet,内部全
// 是 box。对 N ≥ ~10 那 4 单位在屏幕上可忽略;N=3/4 会有一点"外缘"残留 sticker,
// 接受。
const CORE_INSET = 4;
...
export default class InstancedRenderer extends THREE.Group {
  ...
  /** "实心" cube interior:全局 dark box,size = N*SIZE - 2*CORE_INSET。 */
  ...
  constructor(cube: Cube) {
    ...
    // 全局 core box: cube interior 实心填充。Cubelet rounded corner 凹陷最深 = BORDER = 3,
    // 我们 inset = 4 → box 面比凹陷再深 1,从外面 100% 看不见;旋转洞里仅最外 4 单位
    // 是空(看到 slice cubelet),内部全是灰 box。
    if (cube.order >= 2) {
      const coreSize = cube.order * Cubelet.SIZE - 2 * CORE_INSET;
  ...
  set hollow(value: boolean) {
    ...
    if (this.coreMesh) this.coreMesh.material = mat;
    this.cube.dirty = true;
  }
