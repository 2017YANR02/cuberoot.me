 ...
 *
 * 入口:
 *  - AlgCategoryPage 顶部 admin 按钮 "Validate set" → scope { kind: 'set' }
 *  - AlgIndexPage admin 按钮 "Validate all" → scope { kind: 'all' }
 *
...
export default function ValidationReportModal({
  scope, isZh, onClose, onPickCase, refreshKey = 0,
}: Props) {
  ...
  const title = scope.kind === 'set'
    ? (isZh ? `校验 ${scope.puzzle} / ${scope.set}` : `Validate ${scope.puzzle} / ${scope.set}`)
    : (isZh ? '校验全库' : 'Validate all sets');
        ...
        <div className="alg-admin-modal-body alg-validation-body">
          ...
          {running && (
            <div className="alg-validation-progress">
              {isZh ? '校验中' : 'Validating'} {progress.done} / {progress.total}
            </div>
          )}
          {!running && !error && (
            <div className="alg-validation-summary">
              {failures.length === 0
                ...
                : (isZh ? `共 ${failures.length} 条不通过 (点击跳到对应 case 修):`
                        : `${failures.length} failures (click row to fix):`)}
            </div>
          )}
          {failures.length > 0 && (
