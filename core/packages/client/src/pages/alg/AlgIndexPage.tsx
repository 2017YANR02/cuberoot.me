...
export default function AlgIndexPage() {
  ...
  return (
    <div className="alg-root alg-landing">
      ...
      {!loading && !error && searchResults === null && (
        <div className="alg-bento">
          {cards.map(c => (
            <Link
              key={c.cat}
              to={`/alg/c/${encodeURIComponent(c.cat)}`}
              className={`alg-bento-card tier-${c.tier}`}
            >
              <div className="alg-bento-icon">
                <c.Icon size={c.tier === 'hero' ? 40 : c.tier === 'hero-side' ? 32 : c.tier === 'medium' ? 28 : c.tier === 'utility' ? 18 : 24} strokeWidth={1.5} />
              </div>
              <div className="alg-bento-name">{c.label[pageLang]}</div>
            </Link>
          ))}
        </div>
      )}
