...
import { k, s, n, f, p, c } from '../stack_tool_types';
...
export const TAILWIND: StackTool = {
  ...
  zh: {
    ...
    cuberootDesc: <>cuberoot.me 装了 Tailwind v4, 但坦白说<strong>几乎没在 JSX 里写 utility class</strong>。主样式是每页一份手写语义化 CSS, 类名页面前缀 (<code>.compare-card</code> / <code>.stack-card-glyph</code>), 不是 <code>flex p-4 rounded-lg</code> 这种 chain。</>,
    outlookTitle: '当下与前景',
  ...
  en: {
    ...
    cuberootDesc: <>cuberoot.me has Tailwind v4 installed, but to be straight: <strong>almost no utility classes are written in the JSX</strong>. Primary styling is hand-rolled semantic CSS per page, with page-prefixed class names (<code>.compare-card</code> / <code>.stack-card-glyph</code>), not <code>flex p-4 rounded-lg</code> chains.</>,
