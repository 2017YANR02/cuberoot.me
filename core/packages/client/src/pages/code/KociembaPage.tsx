  ...
  return (false, minOver)`}</code></pre>
          ...
          <section className="algo-section">
            ...
            <ul>
              ...
              <li>
                <L
                  zh={<><strong>min2phase</strong> — Shuang Chen (cs0x7f) 的 Java 实现。对称压缩 + huge prune (CornUDSliceFlip / CornEdg) + 交错搜索,把平均步数压到 20、平均耗时压到 1ms。csTimer、TNoodle 用的就是它。详见 <Link to="/code/algorithms/min2phase">/code/algorithms/min2phase</Link>。</>}
                  en={<><strong>min2phase</strong> — Shuang Chen (cs0x7f)'s Java implementation. Symmetry compression + huge prune (CornUDSliceFlip / CornEdg) + interleaved search, pushing the average to 20 moves and 1 ms per solve. csTimer and TNoodle ship it. See <Link to="/code/algorithms/min2phase">/code/algorithms/min2phase</Link>.</>}
                />
              </li>
              <li>
                <L
          ...
          <footer className="algo-page-foot">
            <Link to="/">CubeRoot</Link> · <Link to="/code/algorithms">/code/algorithms</Link> · <Link to="/code/algorithms/min2phase">min2phase →</Link>
          </footer>
        </div>
      </div>
    </LangCtx.Provider>
