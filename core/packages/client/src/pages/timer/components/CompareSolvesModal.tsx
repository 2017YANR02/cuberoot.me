...
function getOrCompute(s: Solve): StageSegments | null {
  ...
  // Lazily compute from move stream when not yet populated. This matches
  // ReconstructModal's behavior, so old solves still light up after a single
  // open without requiring "Reanalyze".
...
/** Format a delta `b - a` (ms) — green if B faster (negative), red if slower. */
...
/** Higher-is-better delta (TPS): positive d means B is faster. */
...
/** Higher-is-better delta (HTM count): negative d means B used fewer moves. */
...
/** Helper: stage TPS as a raw number (not a formatted string) for delta calc. */
