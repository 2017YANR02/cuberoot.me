...
export default function PlaybackPanel({ event, scramble, moves, totalMs, isZh }: Props) {
  ...
  // Cap idx if moves array changes underneath us.
  useEffect(() => {
    if (idx > total) setIdx(total);
  }, [total, idx]);

  // Composed scramble: base scramble + first idx recorded moves.
  const composed = idx > 0
    ? `${scramble} ${moves.slice(0, idx).map(m => m.m).join(' ')}`
    : scramble;

  // Elapsed timestamp at current idx (0 before first move).
  const elapsedMs = idx === 0 ? 0 : (moves[idx - 1]?.ts ?? 0);

  // Auto-advance loop while playing. Schedules next-move timeout based on
  // delta between current move's ts and next move's ts, scaled by speedMult.
  useEffect(() => {
    if (!playing) return;
    if (idx >= total) {
      setPlaying(false);
