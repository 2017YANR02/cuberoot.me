...
import { FORMAT_LABEL, type WcaFormat } from './wca_round';
...
function roundDetailString(sheet: RoundSheetInput, isZh: boolean): string {
  const evName = eventDisplayName(sheet.event, false); // tnoodle uses English event titles
  void isZh;
  const round = `Round ${sheet.roundIdx + 1}`;
  const fmt = FORMAT_LABEL[sheet.format];
  const grp = sheet.groupIdx > 0 ? ` Group ${String.fromCharCode(65 + sheet.groupIdx)}` : '';
  return `${tnoodleEventTitle(sheet.event) ?? evName} ${round}${grp} · ${fmt}`;
}
...
// ─── SVG aspect helper ───────────────────────────────────────────────────
function svgWidthToHeight(svgStr: string): number {
...
async function renderPage(
  ...
  // ── header / footer (tnoodle: 3/4 of margin from top) ─────────
  const headerY = (3 * PAGE_MARGIN_V) / 4;
  doc.setFont(FONT_SANS, 'normal');
  doc.setFontSize(12);
  doc.text(hdr.todayIso, 2 * PAGE_MARGIN_H, A4_HEIGHT - headerY + 12, { align: 'center', baseline: 'middle' });
  // header center: top line = compTitle, second line = roundDetail
  doc.text(hdr.compTitle, A4_WIDTH / 2, A4_HEIGHT - (2 * headerY) / 3, { align: 'center', baseline: 'middle' });
  doc.text(hdr.roundDetail, A4_WIDTH / 2, A4_HEIGHT - headerY + 4, { align: 'center', baseline: 'middle' });
  if (hdr.totalPages > 1) {
    doc.text(`${hdr.pageNum}/${hdr.totalPages}`, A4_WIDTH - 2 * PAGE_MARGIN_H, A4_HEIGHT - headerY + 12, { align: 'center', baseline: 'middle' });
  }
  // footer
  const footerY = (3 * PAGE_MARGIN_V) / 4;
  doc.text(`Generated by ${hdr.generatorTag}`, A4_WIDTH / 2, footerY, { align: 'center', baseline: 'middle' });
  ...
  // Render one preview SVG up-front to know puzzle aspect
  const firstAttempt = chunk[0];
  const svgFirst = await getScramble2DSvg(sheet.event, lineFor(firstAttempt));
  const widthToHeight = svgFirst ? svgWidthToHeight(svgFirst) : 4 / 3;
  ...
  const renderRows = async (attempts: AttemptInput[], isExtra: boolean) => {
    for (let i = 0; i < attempts.length; i++) {
      ...
      // Image
      if (svgFirst != null && i === 0 && !isExtra) {
        // already have first svg, embed it
        const svgEl = svgStringToElement(svgFirst);
        ...
        const svgStr = await getScramble2DSvg(sheet.event, lineFor(a));
        if (svgStr) {
