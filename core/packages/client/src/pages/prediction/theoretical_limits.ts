    ...
    extended_sections: [
      {
        ...
        body_zh: `二阶有 3,674,160 个可达态,God's number = 11 HTM (Rokicki 2006 完整证明 — 二阶是第一个被完整 optimal 求解的 WCA 项目)。平均 optimal HTM = 4.0,整张状态图压成 ~4 MB,**手机本地都能放一个 optimal solver**。这是 WCA 里唯一一个能「穷举式解」的项目。\n\n后果就是 2x2 的速拧问题不再是「用什么方法」,而是「抽到什么 scramble」。EG-2 (全 126 算法,CLL + EG-1 + EG-2) 把每个最后一层 case 都收敛到 1 个算法;cubeshape skip + 4 步 skip 占据 WR 尾部。低于 0.5 秒之后,主导下一个 0.1 秒的是 scramble 分布,不是技术。`,
      },
      {
        title_en: `The 2024 Frame-Count Incident and Regulation 11f1`,
        title_zh: `2024 帧分析事件与 11f1 新规`,
        body_en: `Yiheng Wang briefly held a 0.78 s 2x2 average in March 2024, but the result was annulled after frame-by-frame review of the timer revealed a **timer-sliding pattern** (the hand depresses the StackMat pad with enough horizontal motion that the trigger fires before the cube settles). WCA Regulation 11f1, in force from 2025-01-01, requires frame-count review for any WR-class 2x2 result below ~1.0 s. Several 2024-2025 results were retroactively voided.\n\nThe regulation has a long-term effect: it puts a soft floor on 2x2 WRs at approximately the optical-tracking confidence threshold of consumer cameras (~30 fps = 33 ms per frame). Ye Ziyu's 0.39 s WR has been frame-verified and stands; future sub-0.4 averages will need verifiable triggering on every solve.`,
        body_zh: `王艺衡 2024 年 3 月一度持有 0.78 秒 2x2 平均,但帧分析复盘揭示**滑计时器**模式 — 手压 StackMat 时带横向位移,trigger 在 cube 真正放稳之前先触发 — 成绩取消。WCA Regulation 11f1 自 2025-01-01 起,对任何 WR 级 2x2 < ~1.0 秒结果强制帧分析复核。多个 2024-2025 成绩被回溯撤销。\n\n规则的长期效应是给 2x2 WR 设了一道「软地板」:消费级摄像机光学跟踪置信阈值(~30 fps = 33 ms/帧)。叶子瑜 0.39 秒 WR 已帧验证,将来 sub-0.4 平均都需要每把可验证触发。`,
      },
      {
        ...
        body_zh: `**Sub-3 平均**: 学 Ortega (EG-0, 12 算法),买个磁铁二阶 (YJ MGC 即可),练 cubeshape 识别。墙在 5 OLL × 5 PLL 识别压到 <0.5 秒。\n\n**Sub-2 平均**: 学全 CLL (42 算法)。核心变化是 **inspection 全规划第一层** — 任何 WR 级二阶,第一层都在 inspection 内规划完,常常 0.2 秒就敲完,LL 识别才开始。\n\n**Sub-1 平均**: 加 EG-1 + EG-2 (再 84 算法)。工作不再是背算法,而是**手指动作优化** — 5 步 LL 上 0.4 秒和 0.7 秒的差,来自 RU 换握开销不是步数。\n\n**WR 区域 (sub-0.6 单 / sub-0.9 平均)**: scramble 运气主导。顶级 cuber 每天练 ~6 小时,单 session 50-100 次尝试,WR 级 scramble 出现率 ~0.1%。`,
      },
    ],
  },
    ...
    extended_sections: [
      {
        ...
        body_en: `The 4x4 single record has cycled between a handful of solvers over two decades. **Erik Akkersdijk** held it in 2007 with 47.36 s on a Rubik's brand cube (no magnets, no MoYu). **Sebastian Weyer** brought it under 23 s in 2017. **Feliks Zemdegs**'s 18.42 s in 2019 was the first sub-19. **Max Park** took it to 17.42 (2021), 16.79 (2022), 16.11 (2024), and finally 15.71 s at Colorado Mountain Tour 2024. **Tymon Kolasiński** broke that to 15.18 s at the Spanish Championships in 2025 — his first 4x4 single WR despite holding the 4x4 Ao5 record since 2022.\n\nThe pattern is informative: 4x4 WRs cluster around magnet-flagship releases. The 16-second band held for two years until ball-core hardware (MoYu AoSu V7) reset the floor in 2024. The next WR will need either a sub-9.5 TPS sustained run on existing hardware or another hardware generation.`,
        body_zh: `二十年来 4x4 单次 WR 在少数 solver 之间轮转。**Erik Akkersdijk** 2007 年用初代 Rubik's brand (无磁铁、无 MoYu) 短暂持有 47.36 秒。**Sebastian Weyer** 2017 年带入 23 秒以内。**Feliks Zemdegs** 2019 年 18.42 秒首破 19。**Max Park** 一路 17.42 (2021) → 16.79 (2022) → 16.11 (2024) → 15.71 秒 (Colorado Mountain Tour 2024)。**Tymon Kolasiński** 2025 西班牙锦标 15.18 秒打破 Park,这是他首个 4x4 单次 WR — 尽管 Ao5 WR 自 2022 起就一直是他。\n\n模式有信息量: 4x4 WR 都集中在磁铁旗舰发布期。16 秒带卡了两年,直到 ball-core 硬件 (MoYu AoSu V7) 2024 年重置地板。下一个 WR 要么需要现役硬件上持续 sub-9.5 TPS,要么再等一代硬件。`,
      },
      {
        ...
        body_zh: `每个 4x4 scramble 都独立面对两个 parity:\n\n- **OLL parity** (50% 概率, ~15 步算法) — 最后 OLL 时单个 dedge 翻转\n- **PLL parity** (50% 概率, ~12 步算法) — PLL 时相邻两个 dedge 互换\n\n两者独立,四种等概率场景: 无 parity (25%)、单 parity (50%)、双 parity (25%)。**期望值**上每把多吃 50% × 1.5 + 50% × 1.0 = ~1.25 秒,与技术无关。顶级 cuber 直接感受方差: WR 级双 parity 比无 parity 慢 2-3 秒。\n\n进阶 cuber (Tymon、Park) 在**棱配对阶段就跟踪 parity** — 知道 parity 要来,就把 parity 修正折进算法步骤里,每个 parity 省 ~0.5 秒。100 年下界 9.9 秒预设了近完美的 parity 预判。`,
      },
      {
        ...
        body_zh: `Reduction (中心 → 棱 → 三阶) 是直觉首选方法。**Yau** (Robert Yau 2009 提出) 调整阶段顺序: 1 中心 → 3 cross 棱 → 后 3 中心 + 最后 cross 棱 → 棱配对 → 三阶阶段。重排步数零增,但**棱配对开始前就能规划 cross**,且 cross 棱在整个配对阶段都摆在那让你跟踪。\n\nReduction 学起来更快 (少记一个阶段顺序),全球 sub-30 平均多数靠它。但 sub-22 平均以上的 WR 级 cuber 全用 Yau,因为 cross 跟踪优势会复利: 配对结束时 Yau 玩家已经把 cross 规划好直接插入,Reduction 玩家还要在刚露出的状态从零规划 cross。\n\nYau5 (5x5) 的少数派同理;Yau6/7 收益递减,因为 cross 棱跟踪在 12-17 层尺度上被稀释。`,
      },
      {
    ...
    extended_sections: [
      {
        ...
        body_en: `Max Park held the 5x5 single WR from October 2018 (39.65 s) continuously until October 2024 — six years and one month of dominance, the longest active streak of any non-BLD WCA event in the modern era. He progressively dropped the WR through 36.06 (2019), 34.92 (2020), 33.95 (2021), 32.04 (2022), 31.78 (2023). **Tymon Kolasiński** ended the streak with 31.60 s at WCA Asian Championship 2024, then 30.45 s later that same year, then **29.49 s** in 2025 — the first official sub-30 single. He took the Ao5 from Park at WCA World Championship 2025 with 34.31, since improved to 33.73.\n\nThe streak's end was hardware-coincident. Park used MoYu AoChuang WR M and refined Reduction; Tymon switched to **Yau5** plus a GAN 562 M flagship released in late 2023. The hardware made Yau5's specific cross-edge tracking advantage actionable for the first time at the WR scale.`,
        body_zh: `Max Park 自 2018 年 10 月 (39.65 秒) 起连续持有 5x5 单次 WR 直到 2024 年 10 月 — 六年零一个月,WCA 现代史上非 BLD 项目的最长有效垄断。一路 36.06 (2019) → 34.92 (2020) → 33.95 (2021) → 32.04 (2022) → 31.78 (2023)。**Tymon Kolasiński** 2024 亚锦 31.60 秒终结垄断,同年再砍到 30.45,2025 年 **29.49 秒**首次官方破 30。Ao5 在 2025 世锦 34.31 秒接管 Park,之后改写到 33.73。\n\n垄断终结和硬件同步。Park 用 MoYu AoChuang WR M + 精修 Reduction;Tymon 切到 **Yau5** + 2023 末发布的 GAN 562 M 旗舰。新硬件第一次让 Yau5 的 cross 跟踪优势在 WR 尺度上变现。`,
      },
      {
        ...
        body_zh: `奇数层 cube (3x3 / 5x5 / 7x7) 每个面有固定中心朝向,因此棱配对的手性是确定的。让偶数层 cube 头疼的 dedge-flip / dedge-swap 状态在 5x5 上**数学上不可能** — 单个 midge 不可能相对配对反向。\n\n后果是 5x5 的成绩分布**显著比 4x4 集中**。4x4 平均跑 5 把,每把独立投 parity 骰子 (25% 双 / 50% 单 / 25% 无);5x5 平均没有这种方差。这就是历史数据中 5x5 平均改进比单次更快的原因 — 方差降低让"稳定 35 秒"可实现,即使单次下界受棱跟踪准确率限制。\n\n对物理下界的意义: 5x5 的 Ao5 - single 差距 (现 ~4.5 秒) 几乎是纯执行噪声,而 4x4 的差距 (~3.5 秒) 是执行噪声**减去**幸运 parity 逃脱。`,
      },
      {
        ...
        body_zh: `35 秒级 5x5 中,**棱配对消耗 10-12 秒 — 约总时长的三分之一**。对比 4x4 棱配对 4-5 秒 / 16 秒总时 (也 ~30%),但底层难度非线性: 4x4 配 12 dedge (每槽 2 个块),5x5 配 24 wing edge 围绕 12 midge,且 wing 必须匹配 midge 朝向。\n\n顶级技术转折点是 **freeslice 配对**: 一次连续动作配每个 slice 3 棱,只在 slice 满时旋转 cube。把 5x5 棱配对从 ~140 STM 砍到 ~100 STM,且因换握减少 TPS 提升。Yu Da-Hyung (韩国 2018) 和 Bálint (匈牙利) 的 S2L 变种都把 freeslice 扩展到第二批 2 层,有效但要 ~6 个月专项练习才稳。\n\n再上一层需要 **中心阶段就 look-ahead 棱配对** — Tymon 等 cuber 在最后中心阶段就规划前 2-3 个棱配对,消除 ~1.5 秒识别停顿。这是 100 年下界的边界。`,
      },
    ],
  },
    ...
    extended_sections: [
      {
        ...
        body_zh: `6x6 在尴尬区间: 层数太多,偶数层的磁铁密度不足以充分稳定 (4x4 每面 ~8 磁铁 vs 6x6 每面 ~12 磁铁但摊薄);直径太大 (~67 mm),棱配对阶段单手握持困难。内层打滑 — wide turn 扫到上一步未稳定的内层 — 在每个牌子 (MoYu AoShi WRM / GAN 660 M / YJ MGC) 都未解决。\n\n后果是 6x6 WR 进展比任何其他项目都明显跟硬件发布走。Park 57.69 用 MoYu AoShi WRM (2023 发布)。2008-2014 V-Cube 6 时代曲线长期持平 — 拼图在速拧下勉强可用;QiYi WuHua (2016) 和 YJ MGC (2019) 恢复机械可行性后曲线才大幅下降。`,
      },
      {
        title_en: `The Park Era and Beyond`,
        title_zh: `Park 时代及之后`,
        body_en: `**Max Park** has held the 6x6 single WR continuously since 2020. His progression: 1:14.86 (2020), 1:09.51 (2022), 1:04.52 (2023, first sub-1:05), 0:59.74 (2024, first sub-1:00), 0:57.69 (2025, Burbank). He also holds Mo3 1:05.04. **Feliks Zemdegs** held the WR previously (2017-2020) but moved focus away from big cubes after his 2019 retirement-comeback.\n\nThe competitive landscape outside Park is thin: only Tymon Kolasiński, Bence Barát, and Sebastian Weyer have produced top-3 finishes since 2022. The Tymon-Park rivalry that dominates 4x4 and 5x5 has not extended to 6x6 because Tymon's training focus on Yau5 does not extend efficiently to Yau6 (where cross-edge tracking attenuates over 12 visible layers per face). Until a Yau6 specialist emerges or hardware closes the slipping gap, Park's monopoly is likely to continue.`,
        body_zh: `**Max Park** 自 2020 年起连续持有 6x6 单次 WR。一路: 1:14.86 (2020) → 1:09.51 (2022) → 1:04.52 (2023, 首破 1:05) → 0:59.74 (2024, 首破 1 分) → 0:57.69 (2025, Burbank)。Mo3 也是 1:05.04 在手。**Feliks Zemdegs** 之前持有 (2017-2020),2019 年宣布"退役-回归"后逐渐离开大魔方。\n\nPark 之外赛道空旷: 2022 年以来只有 Tymon Kolasiński / Bence Barát / Sebastian Weyer 进过 top-3。Tymon 与 Park 在 4x4 / 5x5 上的对抗未延伸到 6x6,因为 Tymon 的 Yau5 训练不能高效扩到 Yau6 (每面 12 层可见,cross 跟踪稀释)。除非出现 Yau6 专精选手或硬件解决打滑,Park 的垄断短期内难破。`,
      },
    ],
  },
    ...
    extended_sections: [
      {
        ...
        body_zh: `**Dhruva Sai Meruva** 2024 瑞士锦标的 OH 单次 5.66 秒恰好等于 **Feliks Zemdegs** 2011 年的双手 3x3 单次 WR 5.66。这一刻 — OH 追上 13 年前的双手 WR — 比任何预测都更能反映项目轨迹。2011 Feliks 5.66 是非磁铁 cube 上 9 TPS 持续 CFOP;2024 Meruva 5.66 是 GAN 12 / 13 UltraM 磁铁旗舰上 9.7 TPS 持续 CFOP。\n\n对称不是巧合 — 是 OH 生物力学下界追上了"磁铁革命之前"的双手 CFOP 水平。OH 下界还会随磁铁 OH 训练的一代成长 (12-13 TPS 单手突发) 继续下降,但永远到不了 Wang 的 16+ TPS 双手持续,因为 4 指约束是解剖学限制。`,
      },
      {
      ...
      },
      {
        ...
        body_zh: `**Sub-30 Ao5**: 先用双手把 CFOP 练熟,再 OH 适应 OLL / PLL。别早学 OH 专属算法 — 单手手指动作肌肉要 6 个月发育,算法迟早重学。\n\n**Sub-15 Ao5**: 选 OH 友好 OLL (~10 个 case 改 RU-only)、PLL (~5 个 case 用 2-gen 替代)。单手 F2L 练熟,cross 只用左手。硬件开始重要 — 从 3x3 主用 cube 换到 OH 专属磁铁旗舰 (54-55mm)。\n\n**Sub-10 Ao5**: 学全 RU-heavy OLL 子集,选学局部 ZBLL。槽 3-4 练全左手 F2L。增量在消除换握 — sub-10 OH 意味着每把 <2 次换握。\n\n**WR 区 (sub-7 Ao5)**: 需要持续 9+ TPS 单手、近零换握、OLL → PLL 切换 look-ahead。全球只有 ~15 人在这里。`,
      },
    ],
  },
  ...
  '333bf': {
    ...
    extended_sections: [
      ...
      },
      {
        ...
        body_zh: `3BLD 记忆把 **22 色块 (12 棱 + 8 角 + 2 buffer)** 编码成字母对存进选定的音频码 (常用 Speffz: 每色块固定一个字母, ABC...XYZ 系统)。编码速度下界受 **短期听觉复述速度** 约束 — 心理语言学研究 (Baddeley 1986+) 把压缩语音内复述定在 ~1.5-2 音节/秒。一对字母 = 2 音节 = 1 秒/对最低。\n\n3BLD memo 大约 ~11 对 (棱 5-6 + 角 4-5),纯编码硬下界 ~11-12 秒。幸运 scramble (字母对已被习惯性 chunked 成名词或图像) 砍到 ~3 秒;全新且不顺的字母对序列可能推到 8-10 秒。\n\n真正幸运 scramble 上的 sub-2 秒 memo (Cherry 11.84 单次据传 memo ~2 秒) 是渐近下界。**低于 2 秒需要完全不同的记忆系统** — 空间 mind-palace 映射到固定位置、或不经字母对中介的直接视觉编码。两者都尚未在速拧产线成熟,但在长格式 5BLD 社区有支持者。`,
      },
      {
  ...
  '333fm': {
    ...
    extended_sections: [
      ...
      },
      {
        ...
        body_zh: `**Domino Reduction** (DR) 把 3x3 降到子群 <U, D, R2, L2, F2, B2> — 等价于 Square-1 cubeshape,只允许 U/D 1/4 转和 R2/L2/F2/B2 半转。一旦进入 DR,DR → solved optimal 平均 **13.58 STM**,最大 18 STM (穷举证明)。\n\nFMC 的诀窍是: scramble → DR 用任意 HTM (通常 7-11 STM) 搜索,DR → solved optimal (12-14 STM),根据 DR 进入的幸运程度合计 16-20 HTM。NISS (正逆 scramble 互换搜) 把搜索空间翻倍 — 同一 DR 搜索在逆 scramble 上跑一遍,任一侧找到 16 HTM 都算。**Block-building + EO** 在 DR 之前再加一层: 干净 EO 状态能把 DR 阶段缩到 7-8 STM。\n\n现代 FMC 选手 1 小时 session 看起来是: 10 分钟 scramble 解读 + EO 方向 + 初始 blockbuilding;30 分钟带 NISS 找 DR;20 分钟优化插入。16 步整数下界从这个流水线自然涌现,低于它需要 ≤15 HTM scramble (1/20000) + 时间压力下找 optimal。`,
      },
    ],
  },
    ...
    extended_sections: [
      {
        ...
        body_zh: `**Stanley Chapel** (美,2002 年生) 同时持 4BLD 单次 (51.96 秒, 2023) 和 4BLD Mo3 (59.39 秒, NY Multimate PBQ II 2025 — 史上首破 1 分钟 4BLD 平均)。5BLD 记录也是他 — 单 1:58.59 (2026) 和 Mo3 2:27.63 (2019,至今未破)。是历史上唯一一个同时持有 4BLD + 5BLD 单 + Mo3 全部 WR 的 solver。\n\n解释是结构性的。4BLD 与 5BLD 共享执行技术 (五种 piece orbit 全浮动 buffer 3-Style) 和记忆系统 (Speffz 扩展到 wing / center / midge)。掌握一项的 solver,~80% 技术准备可迁移到另一项,剩下的是项目专属工作记忆训练 — 让 28 字母对 (4BLD) 或 46 对 (5BLD) 在执行期间保持有序可调用。Chapel 自 2018 同时训练两项,双垄断反映了当前无人匹敌的训练量护城河。`,
      },
      {
    ...
    extended_sections: [
      {
        ...
        body_zh: `Chapel **5BLD Mo3 2:27.63** (Michigan Cubing Club Epsilon 2019) 7 年未破 — WCA 非 FMC 项目最长有效停滞。同期他**单次**从 ~2:30 改进到 1:58.59 (2026 史上首破 5BLD 2 分钟),压缩 22%。差距有信息量。\n\n解释: **5BLD 记忆 92 色块超出可靠工作记忆容量**。单次尝试 Chapel 速度下 ~85% 准确率: cube 92 字母对全编码正确、执行期回忆准确、280 STM commutator 链不出错。Mo3 要 3 把连续都做到,联合准确率 ~85%³ = 61%。memo 加速 → 准确率更降;为安全减速 → 时间膨胀。\n\n天花板要破开,要么 (a) **双 cube 训练** 配植入干扰 memo (心理语言学研究证明能扩展工作记忆 ~20%),要么 (b) **方法改变** 通过直接空间编码减少字母对数量。两条路都还没出过具竞争力的 solver。`,
      },
      {
    ...
    extended_sections: [
      {
        ...
        body_zh: `金字塔是 **WCA 中唯一一个人类可以常规在 15 秒 inspection 内规划全 optimal 解**的项目。状态空间仅 933,120 个非平凡态 (比 2x2 367 万小几个数量级),God's number 11 HTM,平均 optimal 8.36 步 — 视觉可枚举的量级。\n\n顶级金字塔 cuber (Kellum / Drew Brads / Dominik Górny / Lingkun Jiang) 训练 inspection 视觉: 扫一遍 Pyraminx,~10 秒内产出 ≤9 步解,留 5 秒缓冲准备执行。这种技术更像国际象棋战术题求解,而非典型 scramble — 需要模式识别库 (建"V" + 剩 4 棱) solver 主动调用。\n\n这种结构特性意味着 **scramble 运气就是全部**。6 步 optimal ~1/200 出现;7 步 ~1/30。低于 0.7 秒 WR,方差来自"inspection 产出了 6-7 步方案 AND 执行无失误"复合运气。`,
      },
      {
        ...
        body_zh: `金字塔硬件收敛在少数磁铁旗舰: MoYu Magnetic / X-Man Bell V2 / QiYi MS。MoYu **威龙磁铁金字塔** (Kellum 0.73 WR 用) 和 **X-Man Bell V2** 是当前量产顶级。2018 以来每代未带簧质量降 ~5-8%,磁铁位置更精细。\n\n下一个硬件问题是 **刚性 tip 变种** (锁死目前被视为"免费"的 3 个角 tip 旋转) 是否能进 WCA 合法形态。现有金字塔允许 tip 自由旋转,贡献了 933,120 状态;tip 锁死后有效状态空间砍 81 倍,平均 optimal HTM 可能降到 ~7.5 — WR 尺度上 10% 时间减少。WCA 合法性是瓶颈: 这种变种要规则修订。\n\n没有规则改变,前进路径是硬件渐进调优把持续 TPS 从 14 推到 18,加持续 scramble 运气狩猎。Sub-0.5 秒单次数学上可能 (5 步 scramble × 22 TPS + 0.1 = 0.33 秒) 但需 ~1/1000 scramble。`,
      },
    ],
  },
  ...
  'skewb': {
    ...
    extended_sections: [
      ...
      },
      {
        ...
        body_zh: `Skewb 和金字塔都是 3D 扭角拼图,状态空间相近 (315 万 vs 93 万),God's number 都是 11 HTM,都奖励 inspection 优先解法。但 **第一层规划上两者大不同**:\n\n- **金字塔** 有"V"结构 (一个面附近 3 棱 + 3 中心),允许直觉式 ≤4 步第一层。Inspection-optimal 可行,因为前 4 步视觉显而易见。\n- **Skewb** 是扭角语义 — 每步同时转 4 个角的耦合循环。没有直觉的"前 4 步",每步都影响 8 角中的 4 个。Inspection 规划更像 3x3 cross 规划: ~6 步第一面要靠算法识别,不是纯视觉扫描。\n\n后果是 Skewb sub-1 秒区由 **TCLL 识别速度** 主导,不是 inspection-optimal 规划。solver 即便第一面规划完美,1080 个 LL 状态识别慢,也会到 1.5 秒。这就是为什么 TCLL 没像金字塔的 V-first 那样到 2022 年达到 100% 普及。`,
      },
    ],
  },
    ...
    extended_sections: [
      {
        ...
        body_zh: `Square-1 (官方名 Cube 21, Karel Hršel + Vojtěch Kopský 1992 设计) 是 WCA 中唯一一个动作原语不是 **面转** 而是 **切片** 的拼图。Cube 分三层 — 上 / 中 / 下,中层是水平切片,只有两半对齐 (赤道处 cubeshape 为"扁平") 时才能转。U 和 D 独立转上下,/ 沿赤道翻转左右半。\n\n后果有两面。**Cubeshape** 在不同态间变化 — cube 可以扁平 / 风筝 / 扇贝形,到下一次切片必须先回到扁平态。Cubeshape 识别是每把 SQ1 的第一阶段,Brandon Lin 2015 年的 **Cubeshape Parity (CSP)** 技术 — 预测 cubeshape 阶段结束时 parity 是否已经被解决 — 是现代 SQ1 最显著的方法进步。\n\nSlice-bound 原语让 TPS 上限 ~10,因为每次切都需要亚毫米对齐。面转拼图能到 15-18 TPS,因为面旋转有容差 — 面可以旋转即使下一个 grip 准备错;SQ1 切片不行。`,
      },
      {
        ...
        body_zh: `**Hassan Khanani** (美, 当前单 WR 3.40 秒, Steel City Sprint 2026) 是 1LLL 最高知名度实践者。Square-1 全 1LLL 覆盖所有 ~400+ 后层 permutation case 用 1 个算法 (vs Vandenbergh 的 CO → CP / EO → EP 两步)。Khanani 没全背完,他用 ~150-200 算法覆盖最常见 LL 状态,剩下 Vandenbergh 兜底。\n\n**Sameer Aggarwal** (美, 当前 Ao5 WR 4.63 秒, Cubing in Southern Oregon 2025) 用类似的局部 1LLL 思路。两人一起代表当前 SQ1 前沿。\n\n全 1LLL 普及是长期问题。400+ 算法 + SQ1 实操中 scramble 吞吐相对低 (切片机械慢,手指 trick 重复慢),全 1LLL 可能要 18-24 个月学完 vs CFOP ZBLL 的 12 个月。这种经济摩擦是为什么 SQ1 是 WCA 中 **最低 T_phys/WR 比** — 方法已知有空间 (sub-3 单 / sub-4 Ao5) 但没人投入。`,
      },
    ],
  },
  ...
  'clock': {
    ...
    extended_sections: [
      ...
      },
      {
        ...
        body_zh: `魔表硬件只有 3 个主要世代:\n\n1. **LingAo Clock (2013)** — 第一款速拧友好量产;~7-8 秒平均可行。\n2. **Modded Mr. M (2017)** — 社区给每 pin 和 dial 装磁铁;~5-6 秒平均。\n3. **QiYi 磁铁魔表 (2020)** — 第一款出厂磁铁,**共 208 个磁铁** (QiYi 营销数据)。基本消除 dial / pin 卡顿。当前 WR 时代硬件。\n\nQiYi 设计在每个 dial-frame 接合处用小双极磁铁,消除了 LingAo 时代的"dial 拖滞" — 让 1 秒 WR 区不可能的不可预测阻力。QiYi 磁铁让 **pin 翻和 dial 转有确定惯量**,cube 响应与设计一致。这是 7-SIMUL 的工程前提: 正背同步执行要求两面对相同力响应可预测。\n\n后续没有魔表在 WR 尺度上替代 QiYi 磁铁。下一代可能集成 **快速翻面 core** 把 0.3 秒翻面包络缩小;在那之前,下界分析以 QiYi 级硬件为前提。`,
      },
    ],
  },
    ...
    extended_sections: [
      {
        ...
        body_zh: `五魔方有 12 个五边形面,但任何手位下只有 **6 个可通过手指动作触达** — 通常 R / U / F / BL / BR / L。其余 6 面 (D、4 个背面、L/BL 之一视握法) 需要解题中 **cube rotation**,每次旋转含 grip 回复花 0.3-0.6 秒。\n\n21 秒级顶级解通常含 **3-6 次 cube rotation**,占 1-3 秒纯非生产时间。Yu Da-Hyung S2L 变种 (韩, ~2018) 和 Bálint S2L (匈) 通过重新组织 S2L 阶段、让更多块留在可触达半球,减少 rotation 数。即便如此,五魔方是 WCA 中 **旋转成本最显眼** 的项目 — 比家族里任何其他拼图都大。\n\n长期缓解可能是 **大手 solver** 或 **更小的 Megaminx (~62mm vs 当前 ~67mm)**,两者都能扩展可触达面数。硬件正向小型化走 (X-Man Galaxy V2 Sculpted 是 67mm,有 64mm 原型流言),但 60mm 以下会撞人体工学极限 — 手指无法区分相邻面。`,
      },
      {
