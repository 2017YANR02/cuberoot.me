...
export function MethodCompareSection({ isZh }: Props) {
  return (
    <section className="pred-section" id="methods-compare">
      ...
      <p>
        {isZh ? (
          <>速拧 3x3 不是只有 CFOP。 每个方法把"打乱→还原"切成不同阶段, 阶段数 × 阶段长度 共同决定 STM. 元数据来自 cubeskills (Zemdegs), speedsolving.com wiki, ZBLL 统计页 (Chris Hardwick), Devagio 的 Mehta 模拟. 经验值 = 现役顶级 cuber 复盘平均, 不是新手 / 中阶值.</>
        ) : (
      ...
      <p className="pred-note">
        {isZh ? (
          <><strong>解读.</strong> CFOP 的"现役顶级 55-58 STM"和"理论下界 45 STM"之间约有 13 步空间;
          ZB / Roux / Petrus 的理论下界都落在 40-43 范围, 但越短的方法对识别和 lookahead 的要求越高,
          实际 WR 还是 CFOP 居多 (Zajder 2.76 / Wang 3.08 均 CFOP). Roux 至今未破 4 秒单次 (Tsvetkov 3.95 是 Roux 唯一一次).
          1LLL 全 3915 算法只有 Damasceno 一人学完, 几乎不可能在 sub-3 速度下识别.
          <strong>历史规律: TPS 连续演化, 方法阶跃式演化</strong> — 大跳几年才一次 (CFOP 2007, 磁铁 2017, ZB 2026).</>
        ) : (
