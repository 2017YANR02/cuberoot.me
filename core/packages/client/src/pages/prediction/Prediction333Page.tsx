...
const SECTIONS = [
  ...
  { id: 'skips',         labelZh: '幸运 scramble + skip 概率',    labelEn: 'Lucky Scrambles + Skip Probability' },
      ...
      <div className="pred-layout">
        ...
        <article className="pred-article">
          ...
          <Section id="tldr" titleZh="一句话结论" titleEn="Top Line" isZh={isZh}>
            <div className="pred-tldr pred-tldr-333">
              <p className="pred-tldr-lede">
                {isZh ? (
                  <>三阶魔方单次 WR 在 <strong>2026 年 2 月 8 日</strong> 由 9 岁波兰小将 <strong>Teodor Zajder</strong> 以 <strong>2.76 秒</strong> 拿下,
                  人类首次跌破 3 秒;Ao5 WR 在两个月后由 8 岁中国小将 <strong>Xuanyi Geng</strong> 用全 ZB 方法压到 <strong>3.71 秒</strong>.
              ...
              <p className="pred-tldr-note">
                {isZh
                  ? '所有预测带 80% 置信区间, 见各章节细节. 文末有完整建模说明.'
          ...
          <Section id="history" titleZh="23 年 WR 编年史" titleEn="23-Year WR Chronicle" isZh={isZh}>
            <p>
              {isZh ? (
                <>WCA 在 2003 年 8 月 23-24 日的多伦多 (Ontario Science Centre) 世锦赛上正式成立, 同时改写了静止 21 年的"单次 WR" — 美国选手 Dan Knights 以 <strong>16.71 秒</strong> 一举打破 Minh Thai 1982 年的 22.95 秒. 此后 23 年, 单次 WR 从 16.71 一路压到 Zajder 2.76, <strong>压缩 6.06 倍</strong>, 平均每年下降 ~7%. 注意 1982 那场不在本文趋势模型中 — 中间 21 年的"无数据空窗"对拟合无意义.</>
              ) : (
            ...
            <p>
              {isZh
                ? <>下方表格按时间排列, 共 <strong>{WR_SINGLE_HISTORY.length} 次单次 WR 改写</strong>. STM/TPS 为社区 reconstruction 数据.</>
            ...
            <p>
              {isZh
                ? <>WCA 于 2007 年起以 Ao5 为正式排名指标 (之前是 Mo3). 本表 Ao5 时代完整 WR, 共 <strong>{WR_AO5_HISTORY.length} 次</strong>. Feliks Zemdegs 单人霸榜 9 年 (2010-2019).</>
          ...
          <Section id="reconstructions" titleZh="著名复盘 (STM / TPS)" titleEn="Famous Reconstructions" isZh={isZh}>
            <p>
              {isZh
                ? <>每次 WR 单次都对应一个 (scramble, 解, STM, TPS) 四元组. 摆在一起最直观: <strong>TPS 连续提升, STM 因 lucky scramble + 方法跃迁非线性下移</strong>. Wang 45 STM × 14.61 TPS (高 TPS 路径); Geng 33 STM × 10.81 TPS (高效路径); Zajder 29 STM × 10.50 TPS (combo: 高效 + ZBLL).</>
...
素因子分解: |G| = 2^27 · 3^14 · 5^3 · 7^2 · 11`}</pre>
            <ul>
              <li>{isZh ? <><strong>角朝向和 ≡ 0 mod 3.</strong> 每个角有 3 种扭转, 第 8 个被前 7 决定.</> : <><strong>Corner orientation sum ≡ 0 mod 3.</strong> Each has 3 twist states; the 8th is forced.</>}</li>
              <li>{isZh ? <><strong>棱朝向和 ≡ 0 mod 2.</strong> F/B quarter-turn 翻 4 个棱, 第 12 个被强制.</> : <><strong>Edge orientation sum ≡ 0 mod 2.</strong> F/B quarter turns flip 4 edges; 12th is forced.</>}</li>
              <li>{isZh ? <><strong>角棱奇偶性耦合.</strong> 一次 face quarter turn 同时是角和棱的 4-cycle (奇置换), 两个奇偶性永远同步 — 这就是 /2.</> : <><strong>Corner-edge parity coupling.</strong> Every face quarter turn is a 4-cycle on both corner and edge ring (both odd) — they stay locked. This is the /2.</>}</li>
            </ul>
            <p>
              {isZh
                ? <><strong>宇宙年龄对比.</strong> 宇宙 ~4.36 × 10¹⁷ 秒老. 每秒数 1 个魔方状态, 需要 ~1.37 万亿年 ≈ <strong>宇宙年龄的 100 倍</strong>.</>
          ...
          <Section id="gods-number" titleZh="God's number 演化 (1981-2010)" titleEn="God's Number Evolution (1981-2010)" isZh={isZh}>
            <p>
              {isZh
                ? <>"God's number"是 cube group 在指定 metric 下的 Cayley graph 直径, 即最坏 optimal 解长度. <strong>1981 年 Thistlethwaite 给出第一个上界 52 HTM</strong>, 29 年后 Rokicki / Kociemba / Davidson / Dethridge (2010) 用 ~35 CPU-年 Google 算力把 HTM 上界证至 <strong>20</strong>, 且至少有一个状态 (superflip composite) 恰好需要 20.</>
            ...
            <ul>
              ...
              <li><strong>STM</strong> {isZh ? '未定 (16 ≤ ? ≤ 20)' : 'unsettled (16 ≤ ? ≤ 20)'}; superflip {isZh ? '在 STM 下 16 步可解' : 'is 16-STM-solvable'}</li>
            ...
            <p>
              {isZh
                ? <><strong>Superflip</strong> (8 角正确, 12 棱原位翻转) 是著名的 20-HTM-难态, 位于 cube group 的 center. STM 下只要 16 步可解 — 这是"STM God's number 不可能小于 16"的下界.</>
            ...
            <p>
              {isZh
                ? <><strong>Two-phase 算法 (Kociemba 1992).</strong> Phase 1 把状态约简到子群 H = ⟨U, D, R², L², F², B²⟩, Phase 2 在 H 内求解. Phase 1 余类数 = |G|/|H| = 2,217,093,120. 现代 Cube Explorer + nissy 是 optimal solver 标杆.</>
          ...
          <Section id="optimal-dist" titleZh="随机 scramble 的 optimal HTM 分布" titleEn="Random Scramble Optimal HTM Distribution" isZh={isZh}>
            <p>
              {isZh
                ? <>cube20.org 全状态枚举给出分布. <strong>67% 的随机打乱需要正好 18 HTM optimal</strong>, 平均 17.92 HTM, 中位 18. 仅 ~4.9 × 10⁸ 个状态需要 20 HTM (~10⁻¹¹).</>
            ...
            <div className="pred-method-table-wrap">
              <table className="pred-fit-table">
                ...
                <tbody>
                  ...
                  <tr><td><strong>ATM</strong></td><td>1</td><td>1</td><td>1</td><td>1</td><td><strong>1</strong></td><td>{isZh ? '双手轴向' : 'axial; R+L = 1'}</td></tr>
                </tbody>
              </table>
            </div>
            <p>
              {isZh
                ? <><strong>STM 是速拧的自然度量.</strong> M / M' / M2 是 thumb-ring 单指动, 物理上一动作. HTM 把它当 2 步不符合手部生物力学. 本文 TPS / 步数全用 STM. HTM 用于学术, QTM 用于群论.</>
          ...
          <Section id="method-cfop" titleZh="CFOP 解剖学: Cross → F2L → OLL → PLL" titleEn="CFOP Anatomy: Cross → F2L → OLL → PLL" isZh={isZh}>
            <p>
              {isZh
                ? <>CFOP 由 Jessica Fridrich 1997 年公开, 与 Hans Dockhorn / Anneke Treep 等 1980 年代独立工作汇流, 共 <strong>119 个核心算法</strong> (41 F2L + 57 OLL + 21 PLL), 顶级 ~<strong>57.5 HTM</strong>. 23 年 WR 历程的绝对主流方法.</>
          ...
          <Section id="method-zb" titleZh="ZB / ZBLS / ZBLL — 顶级方法栈" titleEn="ZB / ZBLS / ZBLL — The Elite Stack" isZh={isZh}>
            ...
            <p>
              {isZh
                ? <><strong>合计 ZBLS + ZBLL = ~795-798 algs</strong>. 现役全 ZB 用户: <strong>Xuanyi Geng</strong> (Ao5 WR 3.71), <strong>Qixian Cao</strong> (Worlds 2025 5.07), <strong>Tymon Kolasiński</strong>. Wang 3.08 / Park 3.13 是 CFOP + 部分 ZBLL, 非全 ZB.</>
            ...
            <p>
              {isZh
                ? <><strong>1LLL</strong> — 全 OLL+PLL 在一个 alg, <strong>3,915 case</strong>. <strong>Eduardo Silva Damasceno 2022 首次完整学完</strong>. <em>没有人在 WR 水平用</em> — 识别时间 ~0.8-1.4s, 比 OLL+PLL 分两步 (0.4+0.4) 还慢. 1LLL 是"算法学习极限"而非"速拧极限".</>
          ...
          <Section id="method-roux" titleZh="Roux / ZZ / Petrus / Mehta — 非 CFOP 派" titleEn="Roux / ZZ / Petrus / Mehta — Non-CFOP" isZh={isZh}>
            <p>
              {isZh
                ? <><strong>Roux (Gilles Roux 2003).</strong> 4 步: 左 1x2x3 → 右 1x2x3 → CMLL (42 alg) → LSE (M, U 解最后 6 棱). 平均 <strong>~48 STM</strong>. <strong>从未破 sub-4 单次</strong>; M-slice 持续 TPS &lt; 7. 顶级 Roux: Kian Mansour, Sean Patrick Villanueva, Alexey Tsvetkov (3.95 — 唯一 sub-4 Roux 单次).</>
            ...
            <p>
              {isZh
                ? <><strong>ZZ (2006).</strong> 起手 <strong>EOLine</strong> 把 12 棱朝向 + DF/DB pair 一次性解掉, F2L 只用 R/U/L. 算法量 28 (ZZ-A) 到 493 (ZZ+ZBLL). <strong>从未拿过 WR</strong> — EOLine 15s inspection 规划极难.</>
            ...
            <p>
              {isZh
                ? <><strong>Petrus (Lars Petrus 1981).</strong> 7 步: 2x2x2 → 2x2x3 → EO → F2L → COLL → CPLL → EPLL. ~45 STM. CFOP 兴起前世界第二大方法, 2004 后式微.</>
                : <><strong>Petrus (Lars Petrus 1981).</strong> 7 steps; ~45 STM. #2 method pre-CFOP era; declined after 2004.</>}
            </p>
            <p>
              {isZh
                ? <><strong>Mehta (Yash Mehta 2020).</strong> FB → 3QB → EOLE → 4 种结束. Mehta-TDR 843 alg 是有提议过的最高算法量速拧方法. 社区小, top-50 无使用者.</>
          ...
          <Section id="lookahead" titleZh="F2L lookahead — 顶级速度的真正瓶颈" titleEn="F2L Lookahead — The Real Bottleneck" isZh={isZh}>
            <p>
              {isZh
                ? <><strong>Lookahead</strong> = 执行 pair N 的同时眼睛追 pair N+1. <strong>故意把执行放慢, 给眼睛认知空间, 总时间反而变快</strong>. Zemdegs CubeSkills 分三阶段:</>
          ...
          <Section id="skips" titleZh="幸运 scramble + skip 概率" titleEn="Lucky Scrambles + Skip Probability" isZh={isZh}>
            <div className="pred-method-table-wrap">
              <table className="pred-fit-table pred-method-table">
            ...
            <p>
              {isZh
                ? <><strong>P(LL skip) = 1/216 × 1/72 = 1/15,552 ≈ 0.00643%</strong>. 顶级 cuber 一生 ~10⁶ 解次中, 期望出现 60 次. Du 3.47 / Park 3.13 都有 PLL skip. <strong>Zajder 2.76 没用 skip</strong> — 用 XX-cross + ZBLL 把 STM 压到 29.</>
          ...
          <Section id="hardware" titleZh="硬件 1980-2026" titleEn="Hardware 1980-2026" isZh={isZh}>
            <p>
              {isZh
                ? <>46 年硬件演化分 4 段: <strong>1980-2010 原始</strong> (原版 Rubik / Type-A) → <strong>2010-2014 现代速拧诞生</strong> (DaYan GuHong/ZhanChi/MoYu AoLong) → <strong>2016-2020 磁铁革命</strong> (Cubicle Mod / GAN 356 Air UM / GAN 11 M Pro 核心磁) → <strong>2021-2026 磁场网络 + 智能化</strong> (GAN 12-16 MagLev / MoYu Super RS3M).</>
          ...
          <Section id="smart-cube" titleZh="智能魔方革命 (2019-2026)" titleEn="Smart Cube Revolution (2019-2026)" isZh={isZh}>
            <p>
              {isZh
                ? <>智能魔方 = BLE + 陀螺仪 + 电池 + 应用. <strong>GAN 356 i (2019)</strong> 是首款主流商业 BLE 速拧. 之后 i2 (2021), i3 (2022), i Carry 2 (2024 — 无 dock, 700h 电池). 配套 App: GAN Cube Station, Cubeast (3rd party 专业级), csTimer (开源, 2020+ 支持 BLE).</>
            ...
            <p>
              {isZh
                ? <><strong>2022 后中国速拧儿童 cohort.</strong> Wang (生 2013), Geng (生 2017), Zajder (~2016) — 学习过程几乎完全在智能魔方时代. 预计 2030+ 进入 sub-3 的将主要是这一代.</>
          ...
          <Section id="biomech" titleZh="生物力学: TPS 的硬天花板" titleEn="Biomech: The TPS Ceiling" isZh={isZh}>
            <p>{isZh ? '三个独立生物力学来源:' : 'Three independent biomech benchmarks:'}</p>
            <ul>
              ...
              <li>{isZh ? <><strong>双手交替击鼓</strong>: Keita Hattori 2024 Guinness — 22.2 strokes/s (1334/min). 双手 alternating 上限.</> : <><strong>Dual-hand drum stroke</strong>: Keita Hattori 2024 Guinness — 22.2 strokes/s (1334/min). Two-hand alternating ceiling.</>}</li>
              <li>{isZh ? <><strong>速拧持续 TPS</strong>: Wang 3.08 全程 14.61 — 现役 WR 最高. Feliks 2012 估 11-12 上限, 已被 Wang 一代打破.</> : <><strong>Sustained cubing TPS</strong>: Wang 3.08 = 14.61 sustained. Feliks 2012 estimated 11-12 ceiling, broken by Wang generation.</>}</li>
            </ul>
            <p>
              {isZh
          ...
          <Section id="cubers" titleZh="顶级选手画像" titleEn="Top Cuber Profiles" isZh={isZh}>
            ...
            <CuberCard isZh={isZh} name="Yiheng Wang (王艺衡)" nation="CN" born="2013-12-16"
              ...
              training_zh="北京 cuber 圈. 2024 年 2x2 滑计时事件."
            ...
            <CuberCard isZh={isZh} name="Tymon Kolasiński" nation="PL" born="2005-06-21"
              ...
              training_zh="日均 6 小时 (2h 算法 + 4h 解)"
              hardware_en="GAN"
              hardware_zh="GAN"
            ...
            <CuberCard isZh={isZh} name="Yusheng Du (杜宇生)" nation="CN" born="~1998"
              ...
              accolades_zh="3.47 单次 (2018-11) — 持 WR 4 年 7 月"
          ...
          <Section id="stats" titleZh="统计建模: 4 个独立模型" titleEn="Statistical Modeling: 4 Independent Models" isZh={isZh}>
            ...
            <ol>
              <li>{isZh ? <><strong>Exp + floor.</strong> T(t) = L + A · exp(−k(t−t₀)). 网格搜索 L. 优势: floor 可解释. 劣势: 单一 floor 假设.</> : <><strong>Exp + floor.</strong> Grid search L. Pro: floor interpretable. Con: single-floor assumption.</>}</li>
              <li>{isZh ? <><strong>Gompertz decay.</strong> S 形, 比 exp+floor 多 inflection point.</> : <><strong>Gompertz decay.</strong> S-shaped; adds an inflection vs exp+floor.</>}</li>
            ...
            <p>{isZh ? <><strong>关键结论.</strong> 磁铁时代加速 (post-2017) 让 GEV (重尾) 最准. BMA: GEV 0.55, Exp-floor 0.30, Gompertz 0.15.</> : <><strong>Key finding.</strong> Magnet-era acceleration (post-2017) makes GEV most accurate. BMA: GEV 0.55, Exp-floor 0.30, Gompertz 0.15.</>}</p>
          </Section>

          <Section id="gev" titleZh="GEV 极值理论" titleEn="GEV Extreme Value Theory" isZh={isZh}>
            <p>{isZh ? <>WR 单次本质 <strong>N 次独立尝试的 sample minimum</strong>. Gumbel 渐近:</> : <>WR single = <strong>sample minimum of N attempts</strong>. Gumbel asymptotic:</>}</p>
...
T_min ≈ 4.5 × exp(−0.12 × √(2 × 16.12))
      = 4.5 × exp(−0.68) ≈ 2.27 s`}</pre>
            <p>{isZh ? <><strong>3 个失效模式</strong>: (1) log-normal 下尾被物理 floor 截断, 公式没这约束; (2) Solves 不独立 (hardware era / cohort non-stationary); (3) N → ∞ 时 Gumbel 不停外推, 真实极值受 16-STM 下界硬截. 严格建模用 <strong>reverse-Weibull (ξ &lt; 0)</strong> 给 finite endpoint τ ≈ 2.0 s, 与物理 floor 一致.</> : <><strong>Three failure modes</strong>: (1) lower tail truncated by physical floor; (2) non-iid (hardware era / cohort non-stationary); (3) Gumbel extrapolates to 0 as N→∞ vs cube20.org 16-STM floor. Rigorous modeling uses <strong>reverse-Weibull (ξ &lt; 0)</strong> giving finite endpoint τ ≈ 2.0 s, matching physical floor.</>}</p>
          </Section>
          ...
          <Section id="forecast" titleZh="综合预测 — Single + Ao5 (BMA Ensemble)" titleEn="Final Forecast — Single + Ao5 (BMA Ensemble)" isZh={isZh}>
            ...
            <p className="pred-note">
              {isZh
                ? <><strong>建模.</strong> BMA 集成 (GEV 0.55 + Exp-floor 0.30 + Gompertz 0.15), 80% CI 来自残差 bootstrap. Ao5/single 比例由 σ_log = 0.10-0.12 + Ao5 trimmed mean √5 shrinkage 推导, 顶级同 round Ao5/min-single ≈ 1.25-1.35. <strong>下界不超过物理 floor 0.99s 和 Ao5 1.9s</strong>.</>
            ...
            <div className="pred-method-table-wrap">
              <table className="pred-fit-table pred-method-table">
                ...
                <tbody>
                  ...
                  <tr><td>{isZh ? '🐌 减速' : '🐌 Stagnation'}</td><td className="pred-num-small">{isZh ? '智能魔方代际无效 / 新血断层' : 'smart-cube plateaus / drought'}</td><td className="pred-num">~2.60</td><td className="pred-num">~3.40</td><td className="pred-num-small">{isZh ? '类 mile run 27 年零进展' : 'Mile-run-style stall'}</td></tr>
                  <tr><td>{isZh ? '⚖️ WCA 调整' : '⚖️ WCA Adjust'}</td><td className="pred-num-small">{isZh ? '触发器 / 帧分析进一步收紧' : 'Timer / FBF tightened'}</td><td className="pred-num">~2.40</td><td className="pred-num">~3.10</td><td className="pred-num-small">{isZh ? '类 2024 滑计时后' : 'post-2024 sliding'}</td></tr>
                </tbody>
              </table>
            </div>
          </Section>
          ...
          <Section id="caveats" titleZh="局限与陷阱" titleEn="Caveats & Pitfalls" isZh={isZh}>
            <ol>
              <li>{isZh ? <><strong>未来"方法革命"是离散的.</strong> ZB→1LLL 或全新方法不在曲线趋势里. 历史每 8-10 年一次方法革命.</> : <><strong>Future "method revolutions" are discrete.</strong> ZB→1LLL or new methods aren't in the trend. Cadence ~8-10 years.</>}</li>
              <li>{isZh ? <><strong>极值统计的样本依赖.</strong> N 增长 → 期望 min 下移 1-2% (N = 10⁷).</> : <><strong>Extreme-value sample dependence.</strong> More attempts → expected min drops 1-2% (N = 10⁷).</>}</li>
              <li>{isZh ? <><strong>WCA 规则未来不可预测.</strong> 历史每 5-10 年 1-3% 规则不连续.</> : <><strong>WCA rule unpredictability.</strong> 1-3% historical discontinuities every 5-10 yrs.</>}</li>
