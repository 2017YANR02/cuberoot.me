...
ORDER BY subset_total ASC;`,
  ...
  steps: [
    ...
    {
      titleZh: '标 has_podium / events_done',
      titleEn: 'Flag has_podium / events_done',
      bodyZh: '17 项里任一 `wr <= 3` → `has_podium = TRUE`;`events_done` 计 `wr > 0` 的项目数。`/v1/wca/sum-of-ranks?hidePodium=1` 用 `has_podium = FALSE` 隐藏顶尖以看次梯队。',
      bodyEn: 'Any `wr <= 3` across the 17 → `has_podium = TRUE`; `events_done` counts slots with `wr > 0`. `/v1/wca/sum-of-ranks?hidePodium=1` filters to `has_podium = FALSE` to surface the tier just below the elite.',
    },
    {
