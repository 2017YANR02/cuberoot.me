/**
 * Generic trainer session store — keyed by (puzzle, set).
 *
 * Replaces the per-set zbll/zbls stores. State is loaded from localStorage when
 * `loadSession(puzzle, set, cases)` is called and persisted on every mutation.
 * Switching to a different (puzzle, set) wipes in-memory state and reloads.
 *
 * Timer state machine matches useSpaceHoldTimer (5 states, NOT_RUNNING=0 …
 * STOPPING=4). Selected case names + recorded solves persist across reloads.
 */
import { create } from 'zustand';
import type { AlgCase, AlgPuzzle } from '@cuberoot/shared';
import { generateScramble } from '../utils/trainerScramble';
import { caseKey, findCaseByKey } from '../utils/trainerCaseKey';
...
export interface TrainerSolve {
  i: number;
  /** Composite key: `${subgroup}|${name}`. Survives across subgroups with
   *  shared names (e.g. F2L Adv "EO" exists in A+, A-, B+, B-). */
  caseKey: string;
  /** Display name only — for sidebar/badge text. Not used for lookup. */
  caseName: string;
  scramble: string;
  ms: number;
}
...
interface PersistedSession {
  /** Composite case keys — see TrainerSolve.caseKey. */
  selected: string[];
  solves: TrainerSolve[];
}
...
interface TrainerState {
  ...
  set: string | null;
  cases: AlgCase[];
  selected: string[];        // composite keys
  solves: TrainerSolve[];
  /** Composite key of the current scramble's case (null if nothing selected). */
  currentKey: string | null;
  /** Display name of currentKey, derived; cached so views don't need lookup. */
...
export const useTrainerStore = create<TrainerState>((set, get) => ({
  ...
  loadSession: (puzzle, setSlug, cases) => {
    ...
    // Drop selected keys no longer in the loaded case list
