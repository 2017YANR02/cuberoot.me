...
//   CI          → 独立 analyzer-worker job 显式置 RUN_SLOW_TESTS=1,避免和快测争 CPU
//   本地 / AI   → 默认跳过这个约 4 分钟的单文件,改了 analyzer / worker 才 `pnpm test:slow`
// 这是业界通行的分层:本地只跑够用的,全集交给 CI。
const SLOW_TESTS = ['tests/analyzer_worker.test.ts'];
const runSlow = !!process.env.RUN_SLOW_TESTS;
if (!runSlow) {
