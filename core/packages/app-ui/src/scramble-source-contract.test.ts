...
describe('mobile scramble-source parity contract', () => {
  ...
  it('uses the shared controlled WCA config and persists every source-key field', () => {
    ...
    expect(app).toContain("? `${drillIdentity}|optimal:${randomOptimalKey}`");
    expect(app).toContain("url: mobileApiUrl('/v1/scramble/optimal-solve')");
    expect(app).toContain('Authorization: `Bearer ${token}`');
    expect(app).toContain('error instanceof CloudOptimalScrambleHttpError');
    expect(app).toContain('&& !signal.aborted');
    expect(app).toContain('&& authSessionRef.current?.token === token');
    expect(app).toContain('void logoutEverywhereRef.current();');
    expect(app).toContain('logoutEverywhereRef.current = logoutEverywhere;');
    expect(app).toContain('&& !auth.loading');
    expect(app).toContain('&& !auth.busy');
    expect(app).toContain("? 'loading-optimal'");
    expect(app).toContain("? 'error-optimal'");
    expect(app).toContain("failure: { kind: 'optimal' }");
  ...
  it('reuses shared occurrence progress and marks only after a persisted solve succeeds', () => {
    ...
    expect(app).toContain('const activeAuthSession = wcaAutoMarkLiveSession(auth.session, auth.busy);');
    expect(app).toContain('authSessionRef.current = activeAuthSession;');
