...
/**
 * GAN stores the MAC in reverse byte order at the end of the canonical
 * nine-byte manufacturer payload. Newer firmware may append fields after that
 * window, so those bytes must not participate in address or key derivation.
 * Callers must still distinguish ManufacturerData from a complete AD packet so
 * an unrelated short packet cannot be mistaken for a MAC.
 */
export function extractGanV4MacFromAdvertisement(
  advertisement: ArrayBuffer | Uint8Array | undefined,
  format: 'manufacturer-data' | 'full-ad' = 'manufacturer-data',
): Uint8Array | null {
  ...
  const decode = (payload: Uint8Array): Uint8Array | null => {
    ...
    const canonical = payload.subarray(0, Math.min(payload.length, 9));
    const mac = new Uint8Array(6);
    for (let index = 0; index < mac.length; index++) {
      mac[index] = canonical[canonical.length - 1 - index];
    }
  ...
  if (format === 'full-ad') {
    ...
    // uses type 0xff and starts with GAN's two-byte company identifier.
    for (let offset = 0; offset < bytes.length;) {
      const length = bytes[offset];
      if (length === 0) break;
      const end = offset + length + 1;
      if (end > bytes.length) break;
      if (bytes[offset + 1] === 0xff && length >= 9 && bytes[offset + 2] === 0x01) {
  ...
  // WeChat's `advertisData` is the ManufacturerData segment. On iOS this can
  // retain GAN's little-endian company identifier (0x??01). Newer firmware can
  // append fields after the canonical nine-byte payload; decode() deliberately
  // ignores those suffix bytes while still accepting the longer segment.
  if (bytes.length >= 8 && bytes[0] === 0x01) {
    return decode(bytes.subarray(2));
  }
