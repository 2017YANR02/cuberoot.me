 ...
 * 直接对「要执行的公式串」正序做一遍 3 握位状态 DP(等价,且去掉了上游因读越界位引入的
...
// 公式名 → computeCost 的 case 序号。上游 case 体按「被执行的公式」写(= inverseMoveToString[i]),
// 故这里直接把要执行的公式映到对应 case,而不是 moveToString 序(否则会把每招当成它的逆招算)。
...
/**
 * 二阶公式串的转动代价(越低越顺手)。
 * @param sequence   空格分隔的公式串,只认 U/R/F 及其 2/′ 变体
