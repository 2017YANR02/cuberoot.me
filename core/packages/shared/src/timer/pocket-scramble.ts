 ...
 *
 * 状态模型与 @cuberoot/puzzle-solvers/cube222 一致(固定 DBL 角,U/R/F 九招);代价与 ./pocket-cost 同源
 * (这里对解序用上游递归 computeCost,与 pocketCost 差一个恒定常数,不影响 argmin)。
 */
import { POCKET_COSTS } from './pocket-cost';
import type { Scramble222Mode } from './scramble-222';
...
// 公式模型(moveToString 序 0..8);与 cube222-metric 的 HTM 一致。打乱 = 解序取逆,故只用 INV_NAME。
...
/** 公式名(code 0..8),与 ELEMS / INV_NAME 同序:U U2 U' R R2 R' F F2 F'。 */
export const POCKET_MOVE_NAMES: readonly string[] = ['U', 'U2', "U'", 'R', 'R2', "R'", 'F', 'F2', "F'"];

/**
 * 归一化态(DBL 角在位且朝向 0)的 HTM 最优解 —— 返回公式 code 序列(依次施加即还原)。
...
/** Exact runtime-neutral validator for the WCA 2x2 mode contract. */
export function isWcaPocketScramble(scramble: string): boolean {
  const moves = scramble.trim().split(/\s+/).filter(Boolean);
  if (moves.length !== SCRAMBLE_LEN) return false;
  return moves.every((move, index) => (
    /^[URF](?:2|')?$/.test(move)
    && (index === 0 || move[0] !== moves[index - 1][0])
  ));
}

/**
 * Re-express an existing U/R/F 2x2 state using the selected timer style.
 * `wca` enumerates exactly-11 solutions and applies TNoodle's grip-cost
 * tie-break. `optimal` enumerates the shortest-HTM solutions, then minimizes
 * QTM (Q|H) and grip cost. The returned algorithm reaches exactly the same
 * state as `scramble`; malformed tokens fail closed.
 */
export function pocketScrambleForState(
  scramble: string,
  ...
  for (const token of scramble.trim().split(/\s+/).filter(Boolean)) {
    const move = POCKET_MOVE_NAMES.indexOf(token);
    if (move < 0) return null;
    perm = permMove[perm * N_MOVES + move];
    orient = oriMove[orient * N_MOVES + move];
  }
  const length = mode === 'wca' ? SCRAMBLE_LEN : optimalLen(perm, orient);
  bestCost = Infinity;
  bestQtm = Infinity;
  tieByQtm = mode === 'optimal';
  search(perm, orient, 0, length, 42);
  return solutionToScramble(bestSol, length);
}
