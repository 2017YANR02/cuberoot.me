 ...
 */
export interface RangeMoveItem {
  token: string;
  ts: number;
  endTs: number;
  startIdx: number;
...
export function moveItemsForRange(
  moves: SolveMove[],
  counted: HtmMove[],
  from: number,
  to: number,
): RangeMoveItem[] {
  const startsAt = new Map<number, HtmMove>();
  const covered = new Set<number>();
  for (const h of counted) {
    if (h.startIdx >= from && h.startIdx <= to) startsAt.set(h.startIdx, h);
    for (let i = h.startIdx; i <= h.endIdx; i++) covered.add(i);
  }
  const out: RangeMoveItem[] = [];
  ...
  for (let i = lo; i <= hi; i++) {
    ...
    if (merged) {
      out.push({
        token: merged.m, ts: merged.ts, endTs: merged.endTs,
        startIdx: merged.startIdx, endIdx: merged.endIdx,
      });
      continue;
    }
    ...
    if (raw) out.push({ token: raw, ts: moves[i].ts, endTs: moves[i].ts, startIdx: i, endIdx: i });
  }
  return out;
}
...
export function tokensForRange(
  ...
  return moveItemsForRange(moves, counted, from, to).map(item => item.token);
}
