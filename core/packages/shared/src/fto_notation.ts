/**
 * Canonical FTO notation and sticker-state engine shared by the simulator,
 * reconstruction UI and reconstruction API.
 *
 * The canonical spelling follows LowCubes EIF (`Bl` / `Br`). Common
 * WCA/csTimer spellings (`BL` / `BR`) and Unicode prime marks are accepted at
 * the boundary, but every parsed token is returned in canonical form.
 */
...
const ROOT_ALIASES: Readonly<Record<string, string>> = {
  ...
  BL: 'Bl', BR: 'Br', bl: 'Bl', br: 'Br',
  rw: 'Rw', uw: 'Uw', fw: 'Fw', lw: 'Lw', dw: 'Dw', bw: 'Bw',
  RW: 'Rw', UW: 'Uw', FW: 'Fw', LW: 'Lw', DW: 'Dw', BW: 'Bw',
  BLw: 'Blw', BRw: 'Brw', blw: 'Blw', brw: 'Brw', BLW: 'Blw', BRW: 'Brw',
  rs: 'Rs', us: 'Us', fs: 'Fs', ls: 'Ls', RS: 'Rs', US: 'Us', FS: 'Fs', LS: 'Ls',
  ro: 'Ro', uo: 'Uo', fo: 'Fo', lo: 'Lo', RO: 'Ro', UO: 'Uo', FO: 'Fo', LO: 'Lo',
  rt: 'Rt', ft: 'Ft', lt: 'Lt', RT: 'Rt', FT: 'Ft', LT: 'Lt',
  t: 'T',
};
...
export function parseFtoEifToken(rawToken: string): FtoEifTokenParts | null {
  let normalized = rawToken.trim().replace(/[’′]/g, "'");
  if (!normalized) return null;
  if (normalized.endsWith("'2")) normalized = `${normalized.slice(0, -2)}2'`;
  if (normalized.endsWith("2'")) {
    const rawRoot = normalized.slice(0, -2);
    const root = canonicalRoot(rawRoot);
    normalized = root === 'Rt' || root === 'Lt' || root === 'Ft' || root === 'T' ? `${root}2` : root;
  }
  let suffix: '' | "'" | '2' = normalized.endsWith("'") ? "'" : normalized.endsWith('2') ? '2' : '';
  let root = canonicalRoot(suffix ? normalized.slice(0, -1) : normalized);
  // cubing.js `T` is the same physical vertex rotation as LowCubes EIF `Ft'`.
  if (root === 'T') {
    root = 'Ft';
    suffix = suffix === "'" ? '' : suffix === '2' ? '2' : "'";
  }
  if (!VALID_ROOTS.has(root)) return null;
  // EIF macros and whole-puzzle 120 degree rotations only have forward/prime forms.
  ...
  return {
    ...
    suffix,
    token,
    turns: suffix === "'" ? (orderFour ? 3 : 2) : suffix === '2' ? 2 : 1,
  };
}
...
export function parseFtoEifAlgorithm(algorithm: string): { tokens: string[]; invalid: string[] } {
  ...
  for (const rawToken of raw) {
    const parsed = parseFtoEifToken(rawToken);
    if (parsed) tokens.push(parsed.token);
    else invalid.push(rawToken);
  }
  return { tokens, invalid };
}

export function canonicalFtoEifAlgorithm(algorithm: string): string | null {
  const parsed = parseFtoEifAlgorithm(algorithm);
  return parsed.invalid.length === 0 ? parsed.tokens.join(' ') : null;
}

/** Canonicalize and cancel adjacent turns on the same primitive/rotation root. */
export function reduceFtoEifAlgorithm(algorithm: string): string | null {
  const parsed = parseFtoEifAlgorithm(algorithm);
  if (parsed.invalid.length > 0) return null;
  const reduced: Array<{ root: string; turns: number; order: 3 | 4 }> = [];
  for (const token of parsed.tokens) {
    const parts = parseFtoEifToken(token)!;
    // S/H are compound algorithms, so only canonicalize them here.
    if (parts.root === 'S' || parts.root === 'H') {
      reduced.push({ root: parts.token, turns: 1, order: 3 });
      continue;
    }
    const order = parts.root === 'Rt' || parts.root === 'Lt' || parts.root === 'Ft' ? 4 : 3;
    const previous = reduced.at(-1);
    if (!previous || previous.root !== parts.root || previous.root === 'S' || previous.root === "S'" || previous.root === 'H' || previous.root === "H'") {
      reduced.push({ root: parts.root, turns: parts.turns, order });
      continue;
    }
    previous.turns = (previous.turns + parts.turns) % previous.order;
    if (previous.turns === 0) reduced.pop();
  }
  return reduced.map(({ root, turns, order }) => {
    if (root === 'S' || root === "S'" || root === 'H' || root === "H'") return root;
    if (turns === 1) return root;
    if (turns === 2) return order === 3 ? `${root}'` : `${root}2`;
    return `${root}'`;
  }).join(' ');
}

export function invertFtoEifAlgorithm(algorithm: string): string {
  return algorithm.trim().split(/\s+/).filter(Boolean).reverse().map((rawToken) => {
    const parsed = parseFtoEifToken(rawToken);
    // Preserve unsupported input so inversion cannot disguise it as a valid move.
    if (!parsed) return rawToken;
    if (parsed.suffix === "'") return parsed.root;
    if (parsed.suffix === '2') return /^(Rt|Lt|Ft)$/.test(parsed.root) ? parsed.token : parsed.root;
    return `${parsed.root}'`;
  }).join(' ');
}

export function ftoEifStickerState(
  algorithm: string,
  faceValues: Readonly<Record<FtoEifFaceKey, string>> = {
    u: 'u', f: 'f', r: 'r', l: 'l', d: 'd', e: 'e', i: 'i', b: 'b',
  },
): FtoEifStickerState {
  const state = {} as FtoEifStickerState;
  for (const face of FTO_EIF_FACE_KEYS) {
    for (let index = 1; index <= 9; index += 1) state[`${face}${index}` as StickerKey] = faceValues[face];
  }
  for (const token of parseFtoEifAlgorithm(algorithm).tokens) {
    ...
    for (let turn = 0; turn < parts.turns; turn += 1) {
      if (parts.root === 'Rt' || parts.root === 'Lt' || parts.root === 'Ft') {
        ...
        if (parts.turns === 2) applySequence(state, sequence);
        break;
      } else {
...
/** A solved FTO may be globally rotated; every logical face remains uniform. */
export function isFtoEifSolved(algorithm: string): boolean {
  const parsed = parseFtoEifAlgorithm(algorithm);
  if (parsed.invalid.length > 0) return false;
  const state = ftoEifStickerState(parsed.tokens.join(' '));
