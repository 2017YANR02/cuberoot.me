 ...
 * (`// OLL-S+` / `// PLL-Gd`)用。
 */
import { ALG_CATALOG, type AlgCase, type AlgPuzzle } from '../alg';
import { displaySq1ShapeName } from '../sq1_shapes';
export { SQ1_SHAPE_NAMES } from '../sq1_shapes';
...
export function displayCollName(name: string): string {
  const m = /^(?:(?:CLL|EG1|EG2|LEG-?1)\s+)?(AS|Anti[ -]Sune|Sune|S|L|U|T|Pi|H)(?:\s+(\d+))?$/.exec(name.trim());
  if (!m) return name;
  const group = m[1] === 'Sune' ? 'S' : /^Anti[ -]Sune$/.test(m[1]) ? 'AS' : m[1];
