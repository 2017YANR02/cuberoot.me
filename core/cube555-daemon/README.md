...
| `CUBE555_BIDIR` | `1` | 双向求解:除 S 外也解 S⁻¹,挑短的,正反向两条 thread 并行(各 reducer/solver 实例独立,Worker 自带两套)。**不破坏 uniformity**(L\*(S)=L\*(S⁻¹) 群论恒等,两个 heuristic 估计同一目标)。实测 -0.7~1.0 步 / latency +29% (par=1) ~ +63% (par=3,CPU contention)。详 BENCHMARKS v3。设 `0` 关。 |
