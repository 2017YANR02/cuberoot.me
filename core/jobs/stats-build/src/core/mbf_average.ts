// 333mbf / 333mbo 非官方 Mo3 平均(WCA 不追踪)。MBLD 两套编码按数值大小分流(旧 ≥ 1e9、新 < 1e9),
// 不是按 event id —— 333mbf 永远新编码,333mbo 历史里两套都有,甚至同一轮 attempts 混用。
// 先把每把解到统一三段(DD/TTTTT/MM),各取 3 次均值(四舍五入)再拼回(均值 DD ≤ 99 用新编码,
// 否则用旧编码)。与 client/lib/mbf-average.ts 保持一致。
...
//   seconds = TTTTT(99999 = 未知);missed = MM
