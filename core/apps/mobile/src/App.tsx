...
function smartCubeTargetFor(scramble: string): string | null {
  try {
...
export function App() {
  ...
  const selectPrimaryView = useCallback((next: PrimaryView) => {
    if (timingRunningRef.current || timerContextMutationBusyRef.current) {
                ...
                phase={timer.machine.phase}
                 scrambleSlot={(
                   <TimerScrambleStrip
                     ...
                     onActivate={scrambleReady && scramble.length > 0 ? copyCurrentScramble : undefined}
                     scramble={scrambleReady && scramble.length > 0 ? scrambleText : ''}
                     title={copy.scrambleClickCopy}
                     verificationLabels={{
