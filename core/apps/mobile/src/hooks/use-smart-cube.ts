...
import { GanV4CubeConnection } from '../bluetooth/gan-v4-cube';
import { NativeBleTransport } from '../bluetooth/native-ble-transport';
import type { SupportedLanguage } from '../copy';

type SmartCubePhase = 'idle' | 'requesting' | 'connecting' | 'connected' | 'error';
...
export interface SmartCubeHandle {
  connect(): Promise<string>;
  deviceName: string;
  disconnect(): Promise<void>;
  facelets: string;
  lastMove: string;
  phase: SmartCubePhase;
}

export function useSmartCube({ language, onMove, onSolved }: UseSmartCubeOptions): SmartCubeHandle {
  const transportRef = useRef<NativeBleTransport | null>(null);
  if (!transportRef.current) transportRef.current = new NativeBleTransport();
  ...
  const [phase, setPhase] = useState<SmartCubePhase>('idle');
