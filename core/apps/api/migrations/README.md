...
账号管理员角色增量 `0200_app_user_admin_role.sql` 为注册账号增加可授予、可撤销的管理员角色；站主账号仍由根管理员配置保护，不能通过管理接口撤销。

二维码卡片设计增量 `0202_qr_card_designs.sql` 按二维码只追加保存严格校验后的可打印卡片设计版本；公共 SVG 由现役 API 按最新版本确定性生成。

二维码落地页增量 `0203_qr_landing_content.sql` 为二维码补回管理员内部备注、独立的跳转/落地页类型与严格校验的链接列表；落地页简介和术语仍以最新卡片设计为唯一事实源。
...
页面启动诊断增量 `0208_app_boot_diagnostics.sql` 保存可按诊断编号检索的脱敏错误摘要和粗粒度运行环境；不保存账号、IP、完整 UA 或 URL 参数，并由应用保留最近 90 天。

赞助认领增量 `0188_sponsor_claims.sql` 为公开赞助记录增加可解除的账号归属，并保存私密资料快照、付款线索、审核结论与撤销历史；WCA ID 精确匹配可自动通过，其他申请进入管理员审核。

会员编号增量 `0212_membership_vip_id.sql` 为新旧会员分配唯一、稳定的数字编号；对外由 API 统一格式化为 `VIP` 前缀。撤销只结束权益，不再删除会员记录和编号。
...
课时视频封面增量 `0244_platform_lesson_covers.sql` 让每个课时修订关联独立封面媒体；上传图片与视频截帧共用同一受保护媒体链路。
