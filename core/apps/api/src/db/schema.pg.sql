...
CREATE TABLE app_users (
  ...
  avatar_url          TEXT,
  avatar_source       VARCHAR(16) NOT NULL DEFAULT 'auto'
...
-- Membership identity and current entitlement (0046, 0212).
CREATE TABLE memberships (
  wca_id        VARCHAR(20)  PRIMARY KEY,
  vip_number    BIGINT       GENERATED BY DEFAULT AS IDENTITY UNIQUE,
  name          VARCHAR(200) NOT NULL DEFAULT '',
  avatar_url    TEXT,
  plan_slug     VARCHAR(40)  NOT NULL,
  started_at    TIMESTAMPTZ  NOT NULL DEFAULT NOW(),
  expires_at    TIMESTAMPTZ,
  source        VARCHAR(20)  NOT NULL DEFAULT 'xunhupay',
  last_order_no VARCHAR(64),
  contact       VARCHAR(200),
  contact_kind  VARCHAR(12),
  note          TEXT,
  created_at    TIMESTAMPTZ  NOT NULL DEFAULT NOW(),
  updated_at    TIMESTAMPTZ  NOT NULL DEFAULT NOW()
);
...
-- Member-uploaded music, stored on disk under MUSIC_STORAGE_DIR and reviewed by admins.
...
CREATE TRIGGER vault_user_keys_updated_at BEFORE UPDATE ON vault_user_keys
  FOR EACH ROW EXECUTE FUNCTION trg_set_updated_at();
...
CREATE INDEX idx_vault_items_owner_updated ON vault_items(owner_user_id, updated_at DESC);
...
-- Canonical schema additions from migration 0167.
...
CREATE TABLE platform_media_assets (
  ...
  owner_tombstone_key VARCHAR(160)
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  CHECK (owner_user_id IS NOT NULL OR owner_instructor_id IS NOT NULL OR owner_tombstone_key IS NOT NULL)
);
...
CREATE TABLE platform_instructor_payouts (
  ...
  amount_minor BIGINT NOT NULL CHECK (amount_minor BETWEEN 1 AND 9007199254740991),
  currency VARCHAR(3) NOT NULL CHECK (currency ~ '^[A-Z]{3}$'),
  payout_profile_snapshot_encrypted BYTEA CHECK (octet_length(payout_profile_snapshot_encrypted) > 0),
  payout_key_version SMALLINT CHECK (payout_key_version > 0),
...
-- Platform account-deletion safeguards (final schema state from migration 0168).
...
CREATE TRIGGER home_card_positions_updated_at BEFORE UPDATE ON home_card_positions
  FOR EACH ROW EXECUTE FUNCTION trg_set_updated_at();
...
-- ── 22b. page_notices (顶部运维通知 + 首页焦点新闻) ──
-- migrations 0073 + 0169。admin 管理,public GET 短缓存并按生效时间窗过滤。
-- path 为匹配模式:精确 `/scramble/stats`、前缀 `/recon/*`、全站 `/*`(已 strip lang)。
CREATE TABLE page_notices (
  id           BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  path         TEXT         NOT NULL,
  placement    TEXT         NOT NULL DEFAULT 'page_top' CHECK (placement IN ('page_top','home_featured')),
  ...
  href         TEXT         NOT NULL DEFAULT '' CHECK (href = '' OR href = '/' OR href ~ '^/[^/]' OR href ~ '^https?://'),
  enabled      BOOLEAN      NOT NULL DEFAULT TRUE,
...
CREATE TABLE sponsors (
  ...
  message            TEXT,
  claimed_by_user_id BIGINT REFERENCES app_users(id) ON DELETE SET NULL,
  claimed_at         TIMESTAMPTZ,
  created_at         TIMESTAMPTZ   NOT NULL DEFAULT NOW(),
  updated_at         TIMESTAMPTZ   NOT NULL DEFAULT NOW()
);
CREATE INDEX idx_sponsors_amount ON sponsors(amount DESC, created_at);
CREATE INDEX idx_sponsors_claimed_by_user ON sponsors(claimed_by_user_id)
  WHERE claimed_by_user_id IS NOT NULL;
CREATE TRIGGER sponsors_updated_at BEFORE UPDATE ON sponsors
  FOR EACH ROW EXECUTE FUNCTION trg_set_updated_at();

-- ── 26a. sponsor_claims (/support 赞助认领) ──
-- migration 0188。WCA ID 精确匹配自动通过;其余申请由管理员审核。
-- profile_snapshot 仅供审核,公开 sponsors API 只暴露 claimed 布尔值。
...
CREATE UNIQUE INDEX uq_sponsor_claims_active_sponsor ON sponsor_claims(sponsor_id)
...
-- 148,384 行不进 migration:本地 solver/lsll 跑完 → export_cases.mjs → update_lsll.mts 增量灌。
...
-- 全体选手最长连续取得个人纪录的参赛场数(0211;每周由 stats extra 管道全量重灌)。
...
CREATE INDEX pr_streak_world ON wca_pr_streaks (streak DESC, wca_id);
CREATE INDEX pr_streak_continent ON wca_pr_streaks (continent_id, streak DESC, wca_id);
CREATE INDEX pr_streak_country ON wca_pr_streaks (country_id, streak DESC, wca_id);

-- WCA 选手按项目登记学习来源(0114，0185)。每位学生每个项目最多一位老师，也可明确登记为自学；
-- 普通写入者只能登记自己且须为有效会员，管理员可指定任意 WCA 选手。WCA ID 是跨数据源自然键，不建外键。
CREATE TABLE wca_teachers (
  student_wca_id VARCHAR(20)  NOT NULL,
  event_id       VARCHAR(20)  NOT NULL,
  teacher_wca_id VARCHAR(20),
...
-- 0238: record subscriptions and observation ledger.
