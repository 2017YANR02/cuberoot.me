...
friendRoutes.post('/friends/requests', async (c) => {
  ...
  const result = await sql.begin(async (tx) => {
    ...
    if (!users) return { error: 'user not found' as const };
    const blocks = await tx`
      SELECT 1 FROM user_blocks
       WHERE (blocker_user_id = ${userId} AND blocked_user_id = ${targetUserId})
          OR (blocker_user_id = ${targetUserId} AND blocked_user_id = ${userId})
       LIMIT 1`;
    if (blocks.length) return { error: 'friend request unavailable' as const };
    ...
    if (existing?.status === 'accepted') return { state: 'friends' as const, users };
    if (existing && Number(existing.requested_by_user_id) === userId) {
      return { state: 'outgoing' as const, users };
    }
    if (existing) {
      await tx`
      ...
      return { state: 'friends' as const, users, notifyAccepted: true };
    }
    await tx`
      INSERT INTO user_friendships (user_low_id, user_high_id, requested_by_user_id)
      VALUES (${low}, ${high}, ${userId})`;
    return { state: 'outgoing' as const, users, notifyRequest: true };
  });
