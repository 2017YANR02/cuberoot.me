...
- The worker has two CPU cores, 4 GB memory and a four-hour limit per native command. Interrupted jobs become failed and can be retried explicitly. Account/file deletion before publication prevents publication. No extra queue service is needed.
