...
--> statement-breakpoint
INSERT INTO `prompt_templates` (`name`, `category`, `body`, `sort_order`, `created_at`)
SELECT '伦勃朗暗调速度感', '大片', '伦勃朗光影布光,仅一束硬光打亮魔方的一面与棱角,其余面没入深邃阴影;整体暗调、明暗对比强烈;魔方身后拉出横向动态模糊拖影,营造疾速旋拧、飞速复原的速度感;纯黑背景,电影级质感,细节丰富,写实摄影质感。
EN: Rembrandt lighting, a single hard key light on one face and edges of a Rubik''s cube, rest in deep shadow, low-key high-contrast, horizontal motion-blur streaks behind it for speed, pure black background, cinematic, photorealistic --ar 1:2', 330, 1781164700
WHERE NOT EXISTS (SELECT 1 FROM `prompt_templates` WHERE `name` = '伦勃朗暗调速度感');
--> statement-breakpoint
INSERT INTO `prompt_templates` (`name`, `category`, `body`, `sort_order`, `created_at`)
SELECT '高速水花定格', '大片', '一颗 WCA 魔方坠入彩色液体激起皇冠状水花四溅,高速摄影瞬间定格,深色背景,水珠晶莹剔透、动感凝固,商业广告级超清。
EN: high-speed splash photography, a Rubik''s cube hitting colorful liquid, crown-shaped splash frozen mid-air, dark backdrop, glossy droplets, commercial grade --ar 1:2', 340, 1781164700
WHERE NOT EXISTS (SELECT 1 FROM `prompt_templates` WHERE `name` = '高速水花定格');
--> statement-breakpoint
INSERT INTO `prompt_templates` (`name`, `category`, `body`, `sort_order`, `created_at`)
SELECT '逆光黄昏剪影', '大片', '黄昏暖金逆光,一颗魔方逆光剪影、边缘镶一圈金边,空气中浮尘与丁达尔光束,温暖电影感、氛围浓郁、浅景深。
EN: golden hour backlight, a Rubik''s cube rim-lit silhouette with glowing edge, floating dust and god rays, warm cinematic atmosphere --ar 1:2', 350, 1781164700
WHERE NOT EXISTS (SELECT 1 FROM `prompt_templates` WHERE `name` = '逆光黄昏剪影');
--> statement-breakpoint
INSERT INTO `prompt_templates` (`name`, `category`, `body`, `sort_order`, `created_at`)
SELECT '长曝光光绘', '大片', '暗背景下用魔方六色光线长曝光绘出环绕魔方的流动光轨与笔触,光绘摄影质感,炫彩流动、动感拖尾。
EN: long-exposure light painting, six-color luminous trails swirling around a cube, dark background, glowing motion streaks --ar 1:2', 360, 1781164700
WHERE NOT EXISTS (SELECT 1 FROM `prompt_templates` WHERE `name` = '长曝光光绘');
--> statement-breakpoint
INSERT INTO `prompt_templates` (`name`, `category`, `body`, `sort_order`, `created_at`)
SELECT '双重曝光', '大片', '双重曝光艺术,一颗魔方的轮廓里叠映城市天际线或浩瀚星空,黑白到品牌蓝过渡,文艺高级、留白讲究。
EN: double exposure art, a Rubik''s cube silhouette filled with a city skyline or starfield, monochrome to brand blue, editorial --ar 1:2', 370, 1781164700
WHERE NOT EXISTS (SELECT 1 FROM `prompt_templates` WHERE `name` = '双重曝光');
--> statement-breakpoint
INSERT INTO `prompt_templates` (`name`, `category`, `body`, `sort_order`, `created_at`)
SELECT '移轴微缩摄影', '质感', '移轴镜头微缩效果,俯拍像玩具模型般的速拧桌面/赛场,魔方与小人前后景深虚化成「迷你世界」,清新可爱、明快布光。
EN: tilt-shift miniature photography, toy-like cubing desk from above, shallow blur into a tiny world, cute bright --ar 1:2', 380, 1781164700
WHERE NOT EXISTS (SELECT 1 FROM `prompt_templates` WHERE `name` = '移轴微缩摄影');
--> statement-breakpoint
INSERT INTO `prompt_templates` (`name`, `category`, `body`, `sort_order`, `created_at`)
SELECT '黑金奢华', '质感', '纯黑背景配金色描边与流光,一颗魔方点缀金箔与镜面反光,低调奢华、高端克制、强反射质感。
EN: black and gold luxury, pure black background, gold accents, gold-foil and mirror reflections on a cube, premium --ar 1:2', 390, 1781164700
WHERE NOT EXISTS (SELECT 1 FROM `prompt_templates` WHERE `name` = '黑金奢华');
--> statement-breakpoint
INSERT INTO `prompt_templates` (`name`, `category`, `body`, `sort_order`, `created_at`)
SELECT '复古胶片颗粒', '质感', '35mm 胶片质感,暖旧色调、轻微颗粒与漏光,一颗魔方静物,怀旧文艺、复古摄影氛围。
EN: 35mm film grain photography, warm vintage tones, subtle grain and light leaks, a cube still life, nostalgic --ar 1:2', 400, 1781164700
WHERE NOT EXISTS (SELECT 1 FROM `prompt_templates` WHERE `name` = '复古胶片颗粒');
--> statement-breakpoint
INSERT INTO `prompt_templates` (`name`, `category`, `body`, `sort_order`, `created_at`)
SELECT '极简纯色产品摄影', '质感', '极简产品摄影,纯品牌蓝或柔色背景,一颗魔方居中,柔和投影与高光,干净留白、电商主图级。
EN: minimal product photography, solid pastel or brand-blue backdrop, centered Rubik''s cube, soft shadow and highlight, clean ecommerce --ar 1:2', 410, 1781164700
WHERE NOT EXISTS (SELECT 1 FROM `prompt_templates` WHERE `name` = '极简纯色产品摄影');
--> statement-breakpoint
INSERT INTO `prompt_templates` (`name`, `category`, `body`, `sort_order`, `created_at`)
SELECT '扁平吉祥物卡通', '插画', '扁平矢量卡通,一颗拟人魔方吉祥物有手脚和俏皮表情,活泼友好、品牌 IP 感,撞色简洁、干净描边。
EN: flat vector cartoon mascot, cute anthropomorphic Rubik''s cube character with limbs and face, brand IP, vibrant --ar 1:2', 420, 1781164700
WHERE NOT EXISTS (SELECT 1 FROM `prompt_templates` WHERE `name` = '扁平吉祥物卡通');
--> statement-breakpoint
INSERT INTO `prompt_templates` (`name`, `category`, `body`, `sort_order`, `created_at`)
SELECT '包豪斯几何', '插画', '包豪斯 / 构成主义平面设计,红黄蓝基本几何形与粗线条网格,魔方融入构成,理性现代、克制有秩序。
EN: Bauhaus constructivist graphic design, primary geometric shapes and bold grid lines, cube integrated, modern --ar 1:2', 430, 1781164700
WHERE NOT EXISTS (SELECT 1 FROM `prompt_templates` WHERE `name` = '包豪斯几何');
--> statement-breakpoint
INSERT INTO `prompt_templates` (`name`, `category`, `body`, `sort_order`, `created_at`)
SELECT '等距桌面场景', '插画', '等距 2.5D 插画,一张速拧玩家的桌面:魔方、计时器、键盘、奖牌、绿植与台灯,干净描边、柔和阴影,温馨整洁。
EN: isometric 2.5D illustration of a cuber''s desk: cube, timer, keyboard, medal, plant, lamp, clean lines, cozy --ar 1:2', 440, 1781164700
WHERE NOT EXISTS (SELECT 1 FROM `prompt_templates` WHERE `name` = '等距桌面场景');
--> statement-breakpoint
INSERT INTO `prompt_templates` (`name`, `category`, `body`, `sort_order`, `created_at`)
SELECT '新中式金箔', '插画', '新中式设计,水墨留白配金箔线条与几何窗棂纹样,一颗魔方典雅居中,东方高级、克制大气。', 450, 1781164700
WHERE NOT EXISTS (SELECT 1 FROM `prompt_templates` WHERE `name` = '新中式金箔');
--> statement-breakpoint
INSERT INTO `prompt_templates` (`name`, `category`, `body`, `sort_order`, `created_at`)
SELECT '街头涂鸦嘻哈', '插画', '街头涂鸦 / 喷漆风,粗犷字符纹理与六色喷溅,魔方带潮流贴纸炸街感,叛逆活力、嘻哈街头。
EN: street graffiti spray-paint style, gritty textures and color splatter, sticker-bomb Rubik''s cube, hip-hop street --ar 1:2', 460, 1781164700
WHERE NOT EXISTS (SELECT 1 FROM `prompt_templates` WHERE `name` = '街头涂鸦嘻哈');
--> statement-breakpoint
INSERT INTO `prompt_templates` (`name`, `category`, `body`, `sort_order`, `created_at`)
SELECT '蒸汽朋克齿轮', '插画', '蒸汽朋克,黄铜齿轮、管道与铆钉机械构造环绕一颗机械感魔方,暖棕金属色、复古工业、精密细节。
EN: steampunk brass gears, pipes and rivets around a mechanical Rubik''s cube, warm copper, vintage industrial, intricate --ar 1:2', 470, 1781164700
WHERE NOT EXISTS (SELECT 1 FROM `prompt_templates` WHERE `name` = '蒸汽朋克齿轮');
