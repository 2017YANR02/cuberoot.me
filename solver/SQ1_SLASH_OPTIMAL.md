...
**已接进一条龙(2026-06-18,增量自动)**:TypeScript puzzle 统计入口的「SQ1 块」增量抽 sq1 语料 → 有新打乱且 13GB `sq1_wca_jsqfull.bin` 在场时,依次跑 SQ1 WCA 精确和 slash 最优，只解 delta；无新打乱跳过,表缺失告警跳过。日常在 `core/` 运行 `pnpm stats:scramble:local --jobs puzzles`，发布另走授权入口。**全量历史 backfill 是一次性后台**(下面的手动跑),完成后日常只走增量。

`scripts/stats/sq1.ts slash`（对应 `scripts/stats/sq1.ts wca`）：
...
发布：经授权在 `core/` 运行 `pnpm stats:scramble:publish --publish --jobs puzzles`。client `lib/puzzle-distribution.ts` / `puzzle-examples.ts` 的 `V` 已 bump 到 `20260618sq1slashopt`。
...
```bash
# 0. 前置：在 solver/ 编译 analyzer，SQ1 WCA 精确数据已生成
cargo build --release --bin sq1_analyzer

# 1. 跑歧义态 + 合并(默认 30s 超时,~3h 上限,可续)
./core/node_modules/.bin/tsx scripts/stats/sq1.ts slash --timeout-secs 30

# 2. 想消更多歧义怪物:加大超时续跑(只解未决)
./core/node_modules/.bin/tsx scripts/stats/sq1.ts slash --timeout-secs 120

# 3. 重生统计 JSON + 发布
cd core/jobs/scramble-stats-build
pnpm exec tsx src/build_puzzle_dist.ts
pnpm exec tsx src/build_puzzle_examples.ts
# 发布须授权：在 core/ 运行 pnpm stats:scramble:publish --publish --jobs puzzles
```
...
实战(`scripts/stats/sq1.ts slash`:Tier1 lite 单线程 + Tier2 `-Split` 根分裂并行):**5,392 歧义态 → 5,382 条穷尽证明 t=s,10 条残留怪物,全程 0 条 t=s−1**(`meta.less=0`)。残留 10 = 5×(W23,s=12)+ 5×(W25,s=13):
