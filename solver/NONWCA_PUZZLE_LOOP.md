...
- [⛔] **A5** `dino` 恐龙 — `redi.js:119`。move F L B R f l b r(2 power)。**⛔ 卡在档位边界,等用户拍板(见 §3,2026-06-20)**:实测 cstimer 模型可达态 = **A12 = 239,500,800**(= 12!/2,**非**条目原估 ~35,640、**非** 6 色版 19,958,400)—— cstimer 把 12 条棱全当不同件,8 个 move 都是偶 3-循环,生成群在 12 点上**传递且本原**,由 Jordan 定理(传递本原 + 含 3-循环 = A_n/S_n,全偶故 = A12)。**远超 TIER A 的 2×10⁶ 上限**:无浏览器全 BFS dist 表、无全空间直方图、无「下载全部状态」,A4 范式整套不可套;cstimer 自带可采纳启发式(3 个 slice 坐标各 11,880)最大只到距离 4 vs 上帝之数 ~13 → 最优 IDA* 不现实。纯 TS 最优不可行。**未写代码、未 commit**。
