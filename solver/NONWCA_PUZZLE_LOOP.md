...
- **A4(2026-06-20,commit `d29dea86b`)**:Playwright 实时 UI 门跳过 —— `127.0.0.1:3000` 无 dev server,按硬约束(禁 `pnpm dev`)不自启,故未开 Chromium。非 UI 的硬核全部独立验过:可达态 3,041,280(独立旋转推导参照 BFS,对上 jaapsch.net 权威值)/ God 13 / 直方图 `toEqual` 真锁;cstimer 记号用 vm 真跑 sfl 生成器 × 2000 条真打乱 → 0 解析失败 / 0 非法 move / 0 round-trip 失败。**待补**:dev server 起来后 Playwright 验 `/scramble/solver?event=sfl`(平板网图预览 solved 自证)+ `/scramble/stats?event=sfl`(直方图 + 均值 9.00/God 13 + 示例 + 「全 3,041,280 态」+ 下载)桌面 + 390px、「更多魔方」下拉(长方体家族)切到 sfl、零 console error,清 `[untested-ui]` 债。
