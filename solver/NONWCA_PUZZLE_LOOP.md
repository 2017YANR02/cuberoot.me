...
- **A2(2026-06-20,commit `7c30974d7`)**:Playwright 实时 UI 门跳过 —— 本机 RAM 紧(~2.4GB 空闲,用户在跑大型求解器),按 MEMORY CONSTRAINT 不另起 Chromium。已做的轻量验证:运行中的 dev server `127.0.0.1:3000` 实测 `/scramble/solver?event=223`(200,46.7KB)与 `/scramble/stats?event=223`(200,87.9KB)正常返回(CSR 壳无路由/编译崩溃),但未做浏览器内 hydration / 网图渲染 / 0-console-error / 390px 实测。**欠**:下次内存宽松时 Playwright 开两路由 + 桌面/390px,经 NonWcaPuzzlePicker「更多魔方」下拉(长方体组)切到 2×2×3,确认 32 贴纸网图 solved 自证纯色、随机打乱后非均匀、零 console error。求解器逻辑 + 网图正确性已由 vitest 11/11(含 SVG solved 6 色计数 + R2 R2=solved 一致性)硬验,UI 欠账仅限交互层。
