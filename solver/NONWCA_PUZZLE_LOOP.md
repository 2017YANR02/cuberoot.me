...
- **A3(2026-06-20,commit `fa5d301f2`)**:Playwright 实时 UI 门跳过 —— 本机 RAM 紧(~2.4GB 空闲)+ 大求解器在跑,按内存约束不开 Chromium。三路由经运行中 dev server 实测返回 HTTP 200:`/scramble/solver?event=8p`、`/scramble/stats?event=8p`、`/scramble/solver?event=8p&scramble=D R2 U L`。**待补**:RAM 转宽松后 Playwright 验数字格预览(solved = 1..8 canonical,空格右下)、桌面 + 390px、「更多魔方」下拉(slide 家族)切到 8p、零 console error,清 `[untested-ui]` 债。
