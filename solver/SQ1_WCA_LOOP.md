...
- [ ] **A1 内存安全的大表 builder**:把 `build_proj_wca_par` 的 frontier 从「存全态 Sq1State(24B)」改成「存 u32 索引(4B)」或 scan-based,**6× 省内存** —— 这是建 >1GB 表不 OOM 的前提(§0.4)。需 `corn_idx` 的逆(unrank → 代表态)。门:与现 builder 逐字节相等(扩 `pdb_par_matches_serial`)+ 峰值内存实测下降。
...
- **A1 设计提示(交接给下一轮)**:`build_proj_wca_par` 的 frontier 现存全态 `Sq1State`(24B),峰值 ~10×表大小。改存 u32 投影索引(4B)需 **unrank**(index→代表态:sid→(pt,pb) 逆 shape 表 + rank8 逆 + 按 tiling 摆件,filler 任意,因投影是 move 同态)。**仅大表(corn/edge/未来 joint)需要**,5 小表保持现 builder 即可。门:c4 上字节相等(扩 `pdb_par_matches_serial`,需 c4 unrank 或改测 corn round-trip)+ 296M corn 新建 byte-diff 现有 `tables/sq1_wca_cornp.bin` + 峰值 WS << 3.6GB(建到 temp 别覆盖好表)。**曾试 fork 跑 A1 失败**(fork 继承我的编排上下文、错把自己当 orchestrator 空转 ⇒ 别再用 fork 跑独立 worker 任务;要么自己 inline、要么 fresh general-purpose agent 给完全自包含 prompt)。
