//! e2e 集成测试:跑 block223_analyzer 二进制,验证 CSV 形状 + 与 lib 直查一致。
...
#[test]
fn block223_analyzer_matches_lib() {
    ...
    let work_dir = root.join("target").join("test-tables").join("e2e-223-work");
    ...
    let mut child = Command::new(&bin)
        ...
        .stderr(Stdio::piped())
        .spawn()
        .expect("spawn block223_analyzer");
    {
        use std::io::Write;
    ...
    let output = child.wait_with_output().expect("wait block223_analyzer");
    assert!(
    ...
    let out_path = work_dir.join("scramble_5_223.csv");
    let got = std::fs::read_to_string(&out_path).expect("read output");
    let lines: Vec<&str> = got.lines().collect();
    assert_eq!(
        lines[0],
        "id,block223_z0,block223_z2,block223_z3,block223_z1,block223_x3,block223_x1"
    );
    assert_eq!(lines.len(), 6, "header + 5 rows");

    std::env::set_var("CUBE_TABLE_DIR", &table_dir);
    let solver = Block223Solver::new();
