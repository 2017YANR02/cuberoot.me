...
- **下界 13(松,旧)**:任意态 `WCA_dist ≥ twist_dist`;twist=13 的态 ⇒ 其 `WCA_dist ≥ 13`。故 `D_WCA ≥ 13`。
- **下界 25(紧,交替论证,2026-06-18 用户问出来的)**:最优解必然 **turn/slash 严格交替**(两 slash 相邻抵消=slash 是对合;两 turn 相邻合并)。一条 `t` 刀的最优解形如 `T₀ / T₁ / … / T_t`:夹在两刀之间的 **`t−1` 个中间转槽被逼成非恒等**(否则两刀贴一起就抵消),首尾两槽可恒等(白送)。故 **`WCA(s) ≥ t + (t−1) = 2·twist(s) − 1`**。对 twist=13 antipode ⇒ `WCA ≥ 25` ⇒ **`D_WCA ≥ 25`(纯理论,免搜索;比旧的 13 紧)**。
  - 副产物:用更多刀会超上界 `2t+1`(矛盾)⇒ **WCA 最优解的 slash 数恰 = twist 距离**;且每态 `WCA(s) ∈ {2t−1, 2t, 2t+1}`(三值窗)。
  - **够不到 26**:只差「首尾两个 turn 能否也白送」,对所有态泛证不了;要 26 得拿具体见证(twist=13 且首/尾强制再转的态)—— 即 §5 全量扫真题找到的 9 个 `WCA=26` 真态。故**实测下界 26 > 理论下界 25**,两者都 ≤ 借来的上界 27。
...
- **⚠️ 来源 + 强度不对称(2026-06-18 核实,别在 UI/文档抹平成对称的「可证」)**:
  - **上界 27 不是本项目独立证明的** = twist 上帝之数 13(**Mike Masonjones 2005**,经 https://www.jaapsch.net/puzzles/square1.htm 公开;2026-06-18 已 fetch 原页逐字核实:"In 2005 Mike Masonjones calculated God's Algorithm ... at most 13 twists")+ 上面那步初等 `2k+1` 换算。**借来的已知结果**,我们没重算 twist=13。
  - **下界已从 13 收紧到 26(本项目实证)**:`Sq1WcaSolver` 对全 125605 真实打乱可证最优,9 条 = 26 ⇒ `D_WCA ≥ 26`(2026-06-18)。
  - 故现状 **`D_WCA ∈ [26,27]`**,但**两端来源不同强度**:下界=本项目实证、上界=外部借用。精确值(26 还是 27)仍未知 ⇒ 要么找到真 27 态(→27)、要么证全空间无 27 态(→26,= EPIC D 大计算)。
...
### 诊断 A2 —— profile 深态瓶颈(bounded profiler,✅ 结论清晰)
> 2026-06-16,走本 loop。新增运行时门控 profiler(`wca_profile` 模块 + `Sq1WcaSolver::solve_profile`,默认关 ⇒ 求解热路径零成本只一条 relaxed load;node/time cap **保证不挂**)。profile 前 5 条真深态(id 774-778,`wca_profile_deep` #[ignore] 测,cap 20M 节点 / 15s)。h=phase-1 启发,near=cstimer 近最优上界,firstSq=首个方形态的 phase-1 深度。

| id | h | near_twist | near_wca | gap≈ | firstSq | 慢模式 |
|----|---|-----------|----------|------|---------|--------|
| 774 | 14 | 11 | 23 | 9 | 9 | 节点爆炸(bound20=650K,abort) |
| 775 | 14 | 11 | 23 | 9 | 9 | **SOLVED=19**(235K 节点,1.4s) |
| 776 | 15 | 11 | 23 | 8 | 6 | phase-2 时间槽(bound18 仅 9.7K 节点却 7.5s) |
| 777 | 13 | 12 | 25 | 12 | 8 | 节点爆炸(bound18=258K) |
| 778 | 15 | 11 | 22 | 7 | 8 | phase-2 时间槽(bound19 仅 24K 节点却 15s) |

**关键发现**:
1. **深态 h 只有 13-15(非 20)**:尝试 C 报的 max h=20 是样本极值;这些真深态拿到 h=13-15,**gap=7-12**(比之前估的「~5-7」更大)。h 在深态严重饱和。
2. **首个方形态深度浅(6-12)**:所有态 phase-1 都很早够到方形 ⇒ **§6.6 #1「更早转 phase-2」是伪命题(已经很早)**,不是瓶颈。
3. **两种慢模式**:(a) **phase-1 节点爆炸** ×10-30/bound(774/777,大 h-gap 直接炸);(b) **phase-2 时间槽**(776/778:phase-1 才 1-2.4 万节点却烧 7-15s)—— 现 phase-2 = csq/esq(角 8!/棱 8! 各取 max)+ `p2_dfs_wca` IDA*,**每个方形态都现搜一次,很贵**。
4. 易态(775)秒级 solved;难态真最优 ~20-25,cap 内不收敛。

**结论(定杠杆)**:瓶颈 = 「弱 h(gap 大)+ phase-2 是昂贵的现搜」。**正确杠杆 = A3:角×棱联合精确 phase-2 表(`8!·8!·2`≈3.25GB)**,把 phase-2 从「现搜 IDA*」变成「一次查表到精确」⇒ 直接消灭时间槽模式(776/778 的 15s → 瞬时),且 phase-1 可在**首个方形态(深度 6-12)一步收尾** ⇒ 有效深度砍半、节点爆炸受控。**不是**更早入口(已早)、**也不是**单纯再堆 phase-1 h(子空间直径封顶,尝试 C 已证递减)。下一步 A1(内存安全 builder,3.25GB 表建时按 §0.4 必 OOM,先改 u32-index frontier)→ A3。

### 工程 A1 —— 内存安全 index-frontier builder(✅,2026-06-16,313ed1e28)
> 走本 loop。目的:为 A3 的 3.25GB 联合 phase-2 表铺路 —— 现全态-frontier builder 建 3.25GB 表瞬时 frontier ~15-30GB 必 OOM(§0.4)。
- **做法**:新增 `build_pdb_idx_par`,BFS frontier 存 **u32 投影索引(4B)** 而非全态 `Sq1State`(24B,实占 24 因 align 8)。出队时 `unrank_pdb(ix)` 重建**一个**代表态再扩展。能这么做的根据 = **投影是 move 同态**:`corn_idx(move(z))` 只依赖 `corn_idx(z)`(move 是由 shape 决定的固定槽置换,角落位只取决于角排列,与棱无关)⇒ 任一代表态给出与全态 builder 逐字节相同的邻居索引。
- **unrank 两块**:(1) `unrank8`(rank8 的逆,Lehmer/阶乘进制,固定数组无堆分配,builder 热路径);(2) `unrank_pdb(idx, carry_corner)`——拆 `ml`/`rank`/`sid`,**`base.shapes[sid]=(pt<<12)|pb` 直接是 shape 逆映射(免建逆表,省事关键)**,承载件(角或棱)按 `unrank8(rank)` 摆进 tiling 扫描序、另一类填 filler(filler 角 = id 1 占相邻 2 槽同 id 保 LSB/切缝合法;filler 棱 = id 0)。round-trip:`corn_idx(unrank_pdb(i,true))==i` 全域成立(shape 由占位 LSB 重建、角按序读回、ml 对齐)。
- **门全过**:`wca_pdb_unrank_roundtrip`(corn+edge 各 0..296593920 全扫,`idx(unrank(i))==i`)绿;`wca_pdb_idx_builder_matches_file`(#[ignore],on-demand)新 builder 重建 corn+edge **逐字节等于**磁盘旧全态 builder 表(attempt C 建的 `tables/sq1_wca_{corn,edge}p.bin`);**峰值 WS 1.24GB**(旧全态 builder ~3.6GB,降 2.9×;且测里峰值含磁盘副本 296MB+dist 296MB+built 296MB,生产 load_or_build 不持磁盘副本会更低);oracle/baseline/par 绿;wasm32 check 编过。
- **坑/教训**:(1) wasm 下 `unrank_pdb` 是 dead-code warning —— 与既有 `corn_idx`/`edge_idx`/`shapes`(本就只 native 用)同组,非新错,无 deny 不阻塞。(2) **filler 角必须同 id 占 2 槽**:若两槽填不同值,`slash_legal`/`layer_pattern` 会在切缝处误判(角跨缝检测靠 nib(i)==nib(i+1))。(3) 这是 builder 内存技术,**A3 不能直接复用 `build_pdb_idx_par`**(它走全 move 集;方形子群表要方形保形 move + 两-rank8 无 shape 的 unrank),复用的是 u32-frontier 模式 + unrank8,见 LOOP §3 A3 设计提示。

### 工程 A3 —— jsq 角×棱联合 phase-2 表(⚠️ 半成:正确+有效,但没达 <30s-all,2026-06-16,9d2e7c4ed)
> 走本 loop。目的:消灭 A2 诊断的 phase-2 时间槽。**结果打脸交接预期**(诚实记录)。
- **做法**:`jsq[(rank8(cp)*40320+rank8(ep))*2+ml]` = 方形子群内到 solved **陪集**的 WCA 距离,边 = **配对**的方形→方形 slash + 方形保形旋转(`derive_sq_actions_paired`,角/棱 action 同物理动作绑定;笛卡尔积会造伪动作)。`sq_h_wca` 从 `max(csq,esq)` 升级为 jsq 查表。建表用新 `scan_bfs_par`(scan-based:每轮扫全表找前沿,**峰值仅表本身 3.25GB 无 frontier 累积**;结尾 `Vec<AtomicU8>→Vec<u8>` 原地复用 buffer 免 2× 瞬时 —— A3-full 13GB 表更非此不可)。8 线程 ~65min 建成。
- **交接预期 vs 现实**:LOOP §3 原想「方形态一次查表到精确、替 p2 现搜」。**做不到** —— `sq_proj_arrays` 的 (cp,ep,ml) 投影**只精确到陪集**(忘了每层 SQ_PAT_A/B shape 偏移,一个陪集 ≤4 个精确态)⇒ jsq 只能当**更强可采纳启发**(`max(csq,esq) ≤ jsq ≤ 真距离`),phase-2 仍是 `p2_dfs_wca` 搜索。
- **正确性门全过**:`scan_bfs_matches_build_sq_wca`(scan 驱动重建单类 csq/esq **逐字节**==frontier 版 `build_sq_wca`)+ `sq1_jsq_actions_sound`(BFS 枚举方形态,19.76 万物理转移全被配对 action 复现)+ `wca_a3_jsq_admissible_dominant`(#[ignore],3104 态:`max(csq,esq) ≤ jsq ≤ 独立精确 p2`,独立精确用 csq/esq 不碰 jsq 避循环)+ `wca_matches_oracle`(5568 态**逐字节**,最优性保持)+ wasm32。
- **❌ 性能没达标**:profile(jsq 已载,5 真深态)——
  | id | h | 最优/near | 模式 | 关键数 |
  |----|---|-----------|------|--------|
  | 775 | 14 | 19/23 | 易 | SOLVED=19 @1.3s |
  | 777 | 13 | 20/25 | phase-1 爆炸 | ~23µs/节点,bound18=258K nodes;@35.6s |
  | 774 | 14 | ~22/23 | phase-1 爆炸 | ~23µs/节点 ×9/bound,>90s(估 ~20min) |
  | 776 | 15 | ?/23 | **phase-2 时间槽** | bound18 仅 9.7K 节点却 7.3s = **753µs/节点** |
  | 778 | 15 | ?/22 | **phase-2 时间槽** | bound19 仅 32K 节点却 15s = **461µs/节点** |
- **定论(两种慢模式都没根治)**:(a) **phase-1 爆炸**(774/777):节点 ~23µs(纯 phase-1 速率)、~9-17×/bound,jsq 在 phase-2 不参与 phase-1 的 `h_le_wca` ⇒ **碰不到**(777 仍从 5min→35s 是因 phase-2 收尾更快更准,非 h 变强)。(b) **phase-2 时间槽**(776/778):节点少但 461-753µs/节点 = 每个方形态的 `p2_dfs_wca` 现搜(分支~145)仍贵。**jsq 是更强剪枝 ≠ 消灭搜索** ⇒ 要根治 (b) 必须 phase-2 变**精确 O(1) 查表无搜索**,即 A3-full(+per-layer shape 位,4×3.25=13GB)。
- **坑/教训**:(1) **「联合表」≠「精确 phase-2 查表」**:投影忘 shape 偏移 ⇒ 只到陪集 ⇒ 仍需搜索补对齐。想要纯查表必须把 shape 编进索引(13GB)。交接时没看穿 sq_proj 的陪集性,白期待了一轮。(2) **更强启发 ≠ 杀时间槽**:时间槽的根是「每方形态现搜 + 高分支」,启发再紧也得搜;杀它要换成查表。(3) scan-based 实测比 index-frontier 更稳(峰值锁死表大小),3.25GB 8 线程 ~65min 可接受;**原地 Vec 转换是 13GB 级表的前置**(2× 会 26GB 爆)。(4) 求解是**单线程**(只建表并行),线程数不影响解速。
- **下一步**:LOOP §3「⛔ 取舍」三选一(接受部分最优 / A3-full 13GB / A4 phase-1),等用户。

### 工程 A3-full —— jsq_full 精确 phase-2 查表(✅ 成,2026-06-16,aa93699b7 码 + b91475707 文档,本轮建表+实测收尾)
> 用户选了 ⛔ 取舍路 2。目的:把 phase-2 从「现搜」换成「O(1) 精确查表」根治 776/778 时间槽。**实测远超预期**(见下「惊喜」)。
- **做法**:补回 A3-half 丢的 per-layer shape 位 ⇒ 精确态双射 `full_idx = (((tb*2+bb)*40320+rank8(cp))*40320+rank8(ep))*2+ml`(`tb/bb` = 顶/底层 `SQ_PAT_B?1:0`),`4·8!·8!·2 = 13,005,619,200 字节 = 13GB`。`build_jsq_full` 用同一 `scan_bfs_par`(峰值仅表本身 13GB,**原地 `Vec<AtomicU8>→Vec<u8>` 免 2×**——26GB 必爆,非此不可),seed=`full_idx(SOLVED)`,边=真实方形 move(slash + 方形保形旋转,WCA cost)。`sq_h_wca` 与 `p2_dist_le_wca` 在 `jsq_full` 在位时直接查表;`p2_reconstruct` 梯度下降(选 `jsq_full==d-1` 的 move:先 slash 后旋转)重建解序列,**无搜索**。`jsq_full` 在位则跳过载 jsq。
- **建表**:`build_jsq_full_only`(精简,12 线程,BelowNormal)**3118.36s(~52min)**,写出 `tables/sq1_wca_jsqfull.bin` 13,005,619,200 字节。建时 ~17GB 空闲,峰值驻留 ~13GB,**无 OOM**(scan-based + 原地转换守住)。
- **正确性门全过**:`full_idx_unrank_roundtrip`(2M 采样 + 3104 BFS 态,`full_idx(full_unrank(i))==i` 双射)+ `wca_a3_jsqfull_exact`(**3104 方形态 `jsq_full` 逐态 == 独立精确 `p2_exact_indep`**,即真精确非陪集近似 ✓)+ `wca_matches_oracle`(5568 态逐字节,最优性保持)+ wasm32(`JSQ_FULL_SIZE` cfg-gate native:13e9 超 wasm 32-bit usize 的 const-eval)。
- **✅ 深态实测(`wca_a3_deep_timing`,5 真深态 id 774-778,jsq_full 已载,每条 replay→精确 SOLVED + token 数=cost 校验全过)**:

  | id | WCA(最优) | 耗时 | A3-half 旧预期(jsq-lite profile) |
  |----|-----------|------|------|
  | 774 | 22 | **6.70s** | phase-1 爆炸,估 ~20min |
  | 775 | 19 | **0.26s** | 易,@1.3s |
  | 776 | 22 | **37.86s** | phase-2 时间槽,A3-full 目标 |
  | 777 | 20 | **0.52s** | phase-1 爆炸,@35.6s |
  | 778 | 22 | **5.00s** | phase-2 时间槽 |

  **零超时(timeouts(>90s)=[]),最慢 37.86s**(旧版多条 >5min)。
- **🎯 惊喜(实测打脸 A3-half 的悲观预测)**:A3-full 的精确查表**不只杀了 phase-2 时间槽(776/778),还顺带驯服了 phase-1 爆炸(774/777)**——774 从估 ~20min→6.70s、777 从 35.6s→0.52s。**根因**:`sq_h_wca` 现返回**精确**距离 `jsq_full[full_idx]`,是比 jsq-lite 的 coset 下界(`max(csq,esq)≤jsq≤真`)紧得多的可采纳启发式 ⇒ phase-1 IDA* 剪枝大幅收紧。A3-half「774/777 需 A4」的判断是基于 jsq-lite profile 的**悲观外推**,实测推翻——**这正是 §0.5 要跑干净实测而非信预测的价值**。
- **残留**:776 @ 37.86s 是唯一较慢态(phase-2 现已 O(1) ⇒ 这 37.86s 是**纯 phase-1 成本**),属 A4 territory(更强 phase-1 表/剪枝)。但它在 cap 内完成、解正确、且是最难的对抗深态;真实随机打乱普遍更浅(`wca_bracket` 的 300 随机 + 81 真打乱常规跑内全过)。
- **坑/教训**:(1) **精确启发能跨阶段受益**:把 phase-2 表做精确,phase-1 IDA* 也跟着快(更紧的 h),不是只修 phase-2——别用单阶段 profile 外推多阶段收益。(2) **精确查表按构造消灭搜索**:phase-2 不再有「每方形态现搜 + 高分支」,O(1) + 梯度重建。(3) **13GB 原地转换是硬前置**(2× 瞬时=26GB 必爆);scan-based 峰值=表本身。(4) **载 13GB 需 ~15GB 物理空闲**否则狂刷 pagefile;驻留 13GB **对服务器部署太重**(EPIC C 须走云 daemon / 部分最优,见 LOOP §3 C2 + 取舍路 1)。(5) 之前那次失败跑(旧 assert `!w.jsq.is_empty()` 在 jsq_full 在位时 jsq 为空 ⇒ 误 panic)提醒:换表后配套断言要同步放宽(已修为 `jsq 或 jsq_full 任一非空`)。
