...
fn dfs(
    ...
    if face_idx == 6 {
        ...
        let count = count_legal_states(fixed_e.count_ones() as usize, fixed_c.count_ones() as usize)
            as i128;
        *total += -weight * count;
        return;
    }
    for &m in &VALID_MASKS {
        ...
        if next_e.count_ones() > 12 || next_c.count_ones() > 8 {
            continue;
        }
        dfs(
            face_idx + 1,
