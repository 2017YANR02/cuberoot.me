...
- [x] **H3** WASM 类 + 重建仪式(`solver/scripts/build_wasm.mts` names → copy pkg-web 产物到 `tools/solver/rust-cross/` → 手维护 worker 加 `need==='htr'` 分支 → `lib/rust-cross-client.ts` bump `V`+TABLE_BYTES/TABLE_SETS+接口 → `rust-cross-pool.ts` PoolNeed)。门:typecheck 干净。✅ 2026-06-11 typecheck tsgo+tsc 双绿(主 loop 复核);零盘表 `TABLE_SETS.htr=[]` 零下载,wasm 首查惰性 BFS ~335ms;非 DR 哨兵 `HTR_NOT_DR`(u32::MAX);node 冒烟 native↔wasm 逐格相等。
...
- [x] **FL4 CLIENT PREBUILD** ✅ 2026-08-11 `dd4249fa0c`。浏览器不再现场生成角4/棱4移动表或跑 44,906,400 态 BFS；构建机产 `opt_first_layer` bundle（gzip 27,584,701B / 解压 57,383,828B，PDB 4-bit），WASM 解压+装载实测约 228ms、随后六色 Face 3ms / Layer 9ms。native 统计端保留 u8 表与现场建表（稳定约 107MiB、峰值约 258MiB），Rust 5+1、WASM 对拍、前端45、typecheck 全绿。playbook 已锁“能预构建就不留 client 算”。
...
> 2026-08-12 语义纠正：输入也必须已经还原第一层。目标虽然等于 `xxxxcross`，条件状态域并不等于普通 WCA / XXXXCross；旧薄别名已撤销。

- [x] **SL0 CONDITIONAL COORDINATE** ✅ 固定第一层后只追踪 4 条有标号中层棱：`P(8,4) × 2^4 = 26,880` 态；U 层其余块全部商掉。搜索可临时拆第一层，终点必须还原完整 F2L。
- [x] **SL1 EXACT CORE** ✅ `XCrossSolver::second_layer_distance` 复用现有 54,743,056B 单槽精确 PDB 做可采纳 max 启发式；独立物理目标、六色浅层 IDDFS、条件坐标↔普通招式路径与无重枚举共 4 项 release 审计全绿。无 22.9GiB 大表、无新持久化表。
- [x] **SL2 OFFLINE DISTRIBUTION** ✅ 14 线程 release 全枚举 26,880 态用时 57.75s；直方图 `[1,0,0,0,0,22,24,283,682,2590,7006,12400,3854,18]`，严格证明 God=13 HTM。
- [x] **SL3 INTEGRATION** ✅ 删除 WCA/recent/gen/timer/live StageSolver/完整状态空间矩阵中的伪 Second Layer 别名；`/scramble/stats` 新增独立 `第一层已还原 / First layer solved` 数据集，客户端只读 14 档离线常量，不运行 BFS/IDA*、不下载求解表。
...
- 2026-06-11 — **P2c** 非 3x3 统计管线注册,`550f71c0d`。puzzle_distribution.json 新形态 + scripts/stats/puzzles-cli.ts 增量管道 + 数据契约 lib/puzzle-distribution.ts + 范式入 playbook §8;350 条小样本两跑验形。(harness 报 build_puzzle_dist.ts node 类型诊断 = LSP 误报,import 与既有 build.ts 同款且 tsx 实跑两遍绿。)
...
- 2026-08-11 — **SL3 Second Layer 独立审查** `1f16dfd176`。修复 analyzer 真题筛选未归一别名；native↔WASM 六色与独立 replay、前端135项、typecheck、Second Layer/Daisy/First Face/First Layer Rust 回归全绿。确认统计与资产完全复用，MANUAL 无需另跑；仅剩浏览器实页欠账。
- 2026-08-12 — **Second Layer 语义纠正**。前述 SL0-SL3 只证明目标谓词等价，错误忽略了“输入第一层已还原”的条件域；已撤销 WCA/XXXXCross 薄别名，改为 26,880 态条件全枚举，God=13 HTM，并以独立数据集发布离线直方图。
...
- **📦 MANUAL(2x2x2 pocket) 交接**(2026-06-11,P2a–P2e 代码侧全绿落地,等用户在场手动):
  1. 全量灌注:`cd core && pnpm exec tsx ../scripts/stats/puzzles-cli.ts --puzzles 222`(增量、可续跑;analyzer 查表 ~百万/s,瓶颈在 IO/CSV,全量 WCA 222 语料预计分钟级)。
...
- **📦 MANUAL(Pyraminx) 交接**(2026-06-11,P3a–P3e 代码侧全绿落地,等用户在场手动):
  1. 灌注:`scripts/stats/puzzles-cli.ts`(默认 --puzzles 空=全部注册含 pyraminx;语料=Scrambles.tsv event `pyram`,id 差集增量,--max-new 可分批)。
...
- **📦 MANUAL(Skewb) 交接**(2026-06-11,P4a–P4e 代码侧全绿落地,等用户在场手动):
  1. 灌注:`scripts/stats/puzzles-cli.ts`(默认 --puzzles 空=全部注册含 skewb;语料=Scrambles.tsv event `skewb`,id 差集增量,--max-new 可分批)。
...
- **📦 MANUAL(HTR) 交接**(2026-06-11,H1–H5 代码侧已全绿落地,等用户在场手动):
  ...
  2. 若选 (b):统计管道注册(scripts/stats/update-local.ts / build 流程加 `_htr` 列)→ 灌 master/xcross → `stats/scramble/distribution.json` 进 htr 分桶。
...
- **SL3** — 旧伪 Second Layer 入口已撤销；当前只欠 `/scramble/stats` 新条件数据集的桌面 + 390px Playwright、0 console error/无横溢出。此前 Browser skill 确认实例列表为空。
