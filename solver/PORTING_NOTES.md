...
### 2026-08-04:先问「这几块真的分得清吗」,再去估内存

E3(F2LEO 十字)按上面那套算是 51 亿态、11.24 GB nibble,当时的结论是「等机时」。
错在把那四条中层棱当成可区分的棋子追。它们在这一档的目标里只需要「朝向好」——
**谁在哪个位不参与判定**;而一条转动对翻转的作用只看位和转动、与是谁无关。于是
交换标签是状态图的自同构、目标集在它下面封闭,距离在每条轨道上是常数。

`Comp::EdgeSet(m)` 就是把这 m! 商掉:P(12,4)·2⁴ = 190,080 → C(12,4)·2⁴ = 7,920,
整问题 51 亿 → 2.13 亿,乘积 225 亿 → 15 亿(nibble 753 MB),11 GB 的活跑 **2.6 秒**。
逐档 ×m! 还原回站内分母。证据不是推理:`f2leo2` vs `f2leo2_set` 是同一个问题的两种写法,
verify 断言逐档正好 2 倍。

**它不是万能的**:只在目标是 `OrientedAnywhere` 时成立,真要归位的块彼此分得清,一个
都省不掉。反例也现成 —— `f2leo_xcross` 的真实态数确实降了 6 倍(1,226 亿 → 204 亿),
但三个分量各铺满 12 个位,乘积空间反而是 1.93×10¹¹(96 GB)。**省的是真实态数,不是
乘积空间**,这两件事在这台引擎里不是一回事。

### 同一次:目标集换一换,能顶一个新坐标

另外两个开关都只动 `starts`:

* `d_offset` —— 目标集按 D/D2/D' 闭包 = 伪口径。直接查两个因子各自的转动表,不必知道
  「底层是哪几块」,也就没有再推一遍公式的机会。金标是现成的:站内 `pseudo_cross`
  那格(12!·2¹¹ 全空间口径)整除 5,160,960 就是追踪坐标下的九个数,d=0 那个 4 正是
  四个 D 偏移。
* `fold_y` —— 并上 y 共轭那份目标 = 两条 EO 轴取最短。依据是 y 共轭为状态图自同构:
  `d(x, 轴2目标) = d(y x y⁻¹, 轴1目标)`,所以换目标集就等于换轴,一次 BFS 拿下,
  不必跑两遍再逐格取 min。

σ **不手推**(棱朝向约定是最容易悄悄写反的东西):由恒等式 `σ(x·m) = σ(x)·rot(m)`
从归位态传播,每条边都对一次,再验 σ⁴ = 恒等。它还自带适用性检查 —— 因子盯的那组块
在 y 下不整体不变时,传播当场自相矛盾。EOCross 那种「12 条棱的翻转字」就会撞这条断言:
翻转字在 y 下怎么变要看类排列,坐标里没这个信息。**断言炸 = 这条捷径在这里不成立**,
不是 bug。

口径这件事本身也是这次的教训:`f2leo_solver::get_stats` 折叠的是 `min(rot, rot·y)`,
即每个底色**两个帧**。台账原先写「一个底色只有一个帧」,照那个填就会把固定轴那条
上界当成站内曲线发出去(均值差 0.27 步)。现在两条都算、分列两格,站内那格与 131.8 万
条真题逐档对过,最大偏差 0.027 个百分点。
