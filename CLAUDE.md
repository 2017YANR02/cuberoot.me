...
- 卡面元素布局 `qr_codes.layout`(`CardLayout` = 各元素 mm 位移 + 背景图/二维码缩放 s + 背景图 fit):正反两面都可设背景图(`front_art` / `back_art`,无压亮罩,文字靠描边保可读),DOM 卡 `QrCard.tsx` 的 `ArtImage` + 矢量卡 `cardSvg.ts` 的 `artLayer` 同源几何。二维码可缩放(`layout.qr.s`,无下限到 1%)且自由拖动跨折线移到正面(背面 panel `overflow:visible` + 背景层各自 clip + 编辑器把二维码拖动边界放到整卡)。`_CardEditor.tsx` 所见即所得:正反背景图共用 `ArtPanel`,拖动 / 缩放走统一手势引擎 `startGesture`(单指拖 + 双指捏合,桌面另有滚轮),背景图(front/back)与二维码同一套路径,别再各写一份
- 文字样式 `qr_codes.text_styles`(`CardTextStyles` = 各文字元素 `{font,color,size,stroke,strokeW,hidden}`):字体走 `CARD_FONTS`(cardText,DOM/SVG 共用 font-family 栈),DOM 卡 `txtCss` + 矢量卡 `styled()`/`text()` 同源应用(描边走 `paint-order:stroke`)。`hidden` = 删除该内置文字(仅卡面,不影响 title/intro 喂落地页);编辑器各文字面板挂 `TextStyleControls`,删掉的文字靠顶部「已删文字」恢复条找回
- 自建文本框 `qr_codes.custom_texts`(`CardCustomText[]` = `{id,side,text,x,y,style}`):用户「加文字」往正/背面加自由文本框,`x,y` 是相对面板中心 mm 偏移,DOM 卡 `CustomTextLayer` + 矢量卡 `customTextsSvg` 渲染。编辑器拖动 / 捏合走同一手势引擎(`startGesture` 的 `kind:"ct"`,`boxes` 键泛化成 string,`makeBoxDragger` 通用化),`activeCt` 选中后改内容 / 面 / 样式 / 删除
