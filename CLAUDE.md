...
- 卡面元素布局 `qr_codes.layout`(`CardLayout` = 各元素 mm 位移 + 背景图/二维码缩放 s + 背景图 fit):正反两面都可设背景图(`front_art` / `back_art`,背面自动加 `backShade` 浅色压亮罩保文字/码可读),DOM 卡 `QrCard.tsx` 的 `ArtImage` + 矢量卡 `cardSvg.ts` 的 `artLayer` 同源几何。二维码可缩放(`layout.qr.s`)且自由拖动跨折线移到正面(背面 panel `overflow:visible` + 背景层各自 clip + 编辑器把二维码拖动边界放到整卡)。`_CardEditor.tsx` 所见即所得:正反背景图共用 `ArtPanel`,拖动 / 缩放走统一手势引擎 `startGesture`(单指拖 + 双指捏合,桌面另有滚轮),背景图(front/back)与二维码同一套路径,别再各写一份
