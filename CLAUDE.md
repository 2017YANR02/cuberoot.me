...
2. **`core/`** — pnpm + Turbo monorepo，所有新开发都在这里：
   ...
   - `packages/server` — Hono + **PostgreSQL 13**（WCA OAuth + recon + alg 公式库 + 训练数据，部署到云服务器;2026-05-06 从 MariaDB 迁过来,旧 MariaDB 已下架观察期)
   - `packages/shared` — 共享类型(`shared/src/alg.ts` 等);**公式数据从 JSON 迁到 PG `alg_sets/alg_cases` 两张表** (2026-05-06,见 `core/docs/ALG_DB_MIGRATION.md`),`loadAlg(puzzle, set)` 走 `/api/alg/sets/:p/:s` fetch
