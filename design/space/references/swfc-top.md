...
本档案记录 SWFC 顶部的已读原始资料、可用尺度和建模约束。**2026-09-13 已将修订后的顶部保存到正式 `shanghai.blend` 并导出网页 GLB**；97F 透射玻璃及浅白色屋架已合入，100F 透明步道与夜景仍未达标。精确方位及构件尺寸仍有估算，不能把结构检查通过记为测绘 1:1 或电影级复原完成。
...
6. 当前顶部已经保存、正式导出并复看日夜画面；后续改动继续从当前规范工程生成新候选，不重跑旧 `--apply`。优先核定 100F 三条玻璃地板及桥底透视构造，再补 97F 内部和有实拍依据的灯具；每次独立记录候选、保存、导出与实际网页检查结果。
