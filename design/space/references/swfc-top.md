...
本轮实际查看 Carsten Ullrich 的 [2008 年 100F 端部照片](https://commons.wikimedia.org/wiki/File:The_top_observatory_in_the_Shanghai_World_Financial_Center,_called_Skywalk.jpg)，并与下文 Gerhard Huber 的 2012 年夜景交叉核对。Ullrich 原片拍摄于 **2008-09-11**，Flickr 上传于 2008-09-15，Commons 上传于 2012-11-30，许可 **CC BY-SA 2.0**；查阅于 **2026-09-13**。这些日期不能证明当前布置。参考照片留在 `.tmp/png/space-swfc-ends-20260913/references/`，未作为运行纹理发布，来源已加入网页中英文列表。

- 实拍可以辨认一端的偏置开口、浅色面板、窄金属收边、深色嵌板和绿色疏散灯；Huber 照片还可辨认柱脚低位灯盒。模型补充上述固定构件，不复刻不同年份照片中内容不一致的广告。
- 门洞暂按宽 **1.9 m**、净高 **2.32 m**，端部宽 **5.4 m**；低位灯盒暂按 **0.46 × 0.18 m**、中心高 **0.36 m**。这些均是照片比例估算，灯具数量与间距也未经实测。低位 10 处、门头 1 处和吊挂 1 处是当前模型数量，不是现场清点结果。
- 照片未确定端部的地理朝向，放在桥局部 `+u` 端仍是估计；不将此立面复制到另一端。业主 100F 租赁平面标出的后方电梯厅尚未补齐，开口后目前仍为既有端部玻璃空间。
- 疏散图形是简化的人形和门框，没有照抄无法辨认的细字或猜造箭头。第二轮将首轮过方的人形改为圆头、斜向肢体，并补深色嵌板细框；仍不能称作原牌图案的精确复刻。发光强度是视觉近似，未经现场标定。

候选 A、B 的网页日夜与近景均已实际查看；B 已保存到正式源工程。正式导出、检查与待办以[本轮跟踪](../../../docs/space-blender-tracker.md#2026-09-13-100f-端部与疏散灯具)为准。端部背后、电梯厅、另一端、最新标识文字和夜间金属反射仍需继续核定。以下保留此前迭代记录。

## 2026-09-13 金属饰面细化

再次实际查看下节两张夜间实拍，依据可辨认的折顶反光、板面不平整和细接缝，细化 100F 镜顶及宽柱饰。没有将照片采样成运行贴图：`refine_swfc_metal_finish.py` 生成原创 1024 × 1024 法线图，包含 16 种板面变化并打包进 `.blend`，网页导出为本地共享纹理。

首轮纹理重复且原有 50 mm 缝过宽，第二轮调整图块变化并收细到 **估算 6 mm**。镜顶法线强度 0.065、柱饰 0.039，粗糙度分别 0.09、0.14；这些数值、板缝宽度及波纹形状均未经实测，不能视为真实表面扫描。楼面使用平直法线区域，保留三条透明玻璃带。最终保存候选与第二轮视觉资产相同，仅同步脚本注释。

网页继续复用 [Three.js Reflector](https://threejs.org/docs/pages/Reflector.html)，按 [MeshStandardMaterial 的 normalMap / normalScale 契约](https://threejs.org/docs/pages/MeshStandardMaterial.html)传递 Blender 材质参数，查阅于 2026-09-13。共面构件共享反射时保留各面 UV、颜色、粗糙度和法线强度；波纹倒影采用估算 2 m 光学距离的一阶平面偏移，不等于准确曲面反射或多次反射。

日夜及多角度正式网页截图、保存和导出证据见[本轮跟踪](../../../docs/space-blender-tracker.md#2026-09-13-100f-金属板面与细接缝)。夜间柱饰仍比实拍暗；两张照片的端部展板图案不同，也不能证明当前标识内容，暂不虚构不可辨认文字。端部与低位疏散标识、97F 结构和精确光学参数仍待补证。下文保留此前迭代记录。
...
本次实际打开[业主观光厅租赁页](https://www.swfc-shanghai.com/space_leasing_ob.php)、[100F 平面图](https://www.swfc-shanghai.com/images/common/8-lease/map_100f.png)和[室内实拍 pic_140](https://www.swfc-shanghai.com/images/common/8-lease/pic_140.jpg)，访问、下载及审图日期为 **2026-09-13**；网页未给出单张图的摄影者、拍摄日期或开放许可，不能用访问日期代表建成现状。仅本地研究参考，未作为网页纹理发布。

- 平面图明确标注 **L 50 m、W 6.2 m、H 3–3.85 m**；两端为电梯厅。这是租赁厅的尺寸范围，与业主项目册印刷页 11–12 所述 **55 m 观光桥、三条透明玻璃地板、100F 地上 474 m** 分别建模，不能把 50 m 和 55 m 当成同一长度。
- 室内照片可辨认连续的浅灰宽框架、倾斜镜面顶棚、反射地面与侧窗分格。本轮实现 50 m 厅体、3–3.85 m 净高与三条 55 m 玻璃带；**0.8 m 带宽、2.5 m 分格、0.72 m 框架宽度、玻璃厚度、窗台、桥底开孔及光学参数都是估算**，没有测绘依据。
- 三轮候选中，round-02 将凹形框架截面按扇形三角化，错误三角面伸入走道；网页与 Cycles 均实际发现。round-03 拆成两个凸截面，补 15 条走道通畅射线；修正后网页和 Cycles 逐图复查，未见这些突出面。另有 15 处玻璃楼面及下方无遮挡检查、3 处净高检查通过。
- round-03 已保存并正常导出。Blender Cycles 能反射厅内结构；网页原生金属材质目前只使用环境反射，白天镜顶偏蓝、夜间偏黑，**室内镜面和夜景视觉验收仍未通过**。灯位、灯具和光度没有可靠新资料，本轮未虚构增灯。实际玻璃向下可见 97F 屋架，但层间结构与实物对应仍需核定。

本地来源位于 `.tmp/png/space-swfc-skywalk-20260913/references/`。`map_100f.png` SHA-256 为 `fbc3b0617cc27fe7a8c1fc0f1af00a944344e882dc999a28b1842b5b4d652588`；`pic_140.jpg` 为 `b85c13d4fb89cfe8808297c7c46ba9699516e8f0464ed4f31a33f853f13e9c7c`。候选、保存、正式资源与网页证据见[本轮跟踪](../../../docs/space-blender-tracker.md#2026-09-13-100f-观光厅三轮细化)。下节为本次室内细化前的顶部合入快照。

## 2026-09-13 正式保存与网页复验

沿用 candidate11 的浅白屋架方案，在最新园区工程上重建 `integration-20260913-01`，避免用旧候选覆盖后续建筑。隔离 GLB 为 **3,791,508 字节、10 nodes、9 meshes**，SHA-256 `738b78926fd03b3f4fb57abf103c112d59ea98b1602a0c7f2c544c40a8482467`。候选及正式导出的日夜截图均已查看；`--apply` 比对源文件、脚本、审计文件和候选哈希，备份后保存，原有城市对象身份与五组立面灯保持。

规范工程为 **199,904,029 字节**；正式网页 GLB 为 **399,736,608 字节**，SHA-256 `40e10f928063176557302ba20c0c57942e050f8f8fe250700196e6b2136540d5`。实际网页加载此哈希，顶部 60 项表面探针、完整城市检查通过；正常刷新后控制台无错误或警告。保存报告、资源响应体哈希、图像和检查日志统一见[最新跟踪记录](../../../docs/space-blender-tracker.md#2026-09-13-环球金融中心顶部候选合入正式工程)。

白色屋架对照 [IMA 竣工实拍](https://www.imae.co.jp/en/works/shanghai-world-financial-center)校色，采用非金属、无自发光材质；97F 顶部与侧窗使用透射玻璃。颜色与光学参数仍是照片估算。100F 地板依旧不透明，连续桥底阻挡向下视线，尚不能呈现业主资料中的三条玻璃步道。夜间室内偏暗、幕墙窗光重复、裙楼及内部设施待细化，整体视觉验收仍未通过。下节为 2026-09-10 历史记录，其中“未保存”仅指当时状态。
...
- 100F 桥底绝对标高、楼板厚度及其上结构/设备层的准确边界；观光厅净高范围 3–3.85 m 已由业主平面图标注，各处顶棚剖面及构件厚度仍未核定。
