...
当前上海版本（2026-09-13）：环球金融中心 100F 新增中央灯槽、20 盏筒灯外形、窗边扶手与镜面柱饰，已保存进 `scenes/shanghai.blend` 并正式导出，保留园区、外滩和三条透明步道。网页 GLB 为 **400,900,300 字节**；厅内七个镜面保留玻璃带开孔，六盏共享近景灯在原地转身时保持稳定，局部环境反射在进入或天气、时间变化后更新。最新一轮修正网页镜面深度偏移，解决斜看时镜面错误遮住筒灯的问题，20 组 GPU 对照及日夜、反向、俯视、窄屏实查通过。夜间柱饰仍暗，板面波纹、端部标识和层间结构仍待细化，尚未达到 1:1 或电影级。当前状态、来源与证据以[最新跟踪记录](../../docs/space-blender-tracker.md#2026-09-13-100f-镜面与筒灯遮挡修正)为准，下方日期段落保留历史状态。重资产继续本地保存，Git LFS 配置与上传暂缓。
...
本地开发默认加载 Blender 产物。生产默认仍走已有场景生成逻辑；需要先在构建/部署环境提供完整模型、纹理和对应 JSON 清单，核对哈希和资源可达，再在构建时设置 `NEXT_PUBLIC_SPACE_BLENDER=1`。代码提交和 push 不包含被忽略的重资产。上海模型当前为 400,900,300 字节（约 401 MB，2026-09-13 环球 100F 筒灯扶手导出），正式发布前仍需分区加载、压缩与移动设备验证。
...
环球 100F 随后保存 `swfc-hall-lighting-20260913`，现共六组 Blender 作者灯。`AUTHORING | SWFC hall light proxies` 的六盏灯继续共用网页灯池，每盏 120 cd 为视觉估计，不代表 20 个实物筒灯的测量光度。根对象 `facadeLighting.interiorBounds` 使用局部 Y-up 坐标，让厅内相机原地转向时保持选中该灯组。`refine_swfc_hall_lighting.py` 已首次应用，禁止重跑覆盖；后续编辑现有工程、正常导出，并复查日夜与转身效果。[灯具实拍及估算边界](references/swfc-top.md#2026-09-13-筒灯扶手与夜间实拍)。
...
其后已保存庭院修订 `alibaba-courtyard-20260912`：Y 公共中庭补细石材分格、线性排水格栅、四处加长树池与低绿篱。`refine_alibaba_courtyard.py --round 2 --apply` 已执行，不得重复应用；源指纹和已审候选脚本哈希必须同时匹配。尺寸及植物为照片对照估算，参见[庭院档案](references/alibaba-xuhui.md#庭院铺地与种植池增量)。后续从现有工程增量编辑，不回用旧候选覆盖。
...
以下为历史记录；后续 2026-09-13 已保存修订后的候选并正式导出，见[顶部档案当前状态](references/swfc-top.md#2026-09-13-正式保存与网页复验)。本节“未保存”和旧文件大小均不代表当前版本。
