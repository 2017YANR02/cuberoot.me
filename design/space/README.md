...
当前上海版本（2026-09-13）：环球金融中心 100F 已按实拍补出一端门洞、面板、金属细框及 12 处疏散灯具，源工程已保存并导出 **401,235,500 字节**的 GLB。本轮将扶手、灯具收边、端部门框与面板接入既有室内反射，夜间增加金属高光和明暗层次；日夜、多方向、离开与重新进入及窄屏已实查，类型检查和 58 项定向测试通过。没有增加采样次数或重新导出。宽柱面仍暗、镜顶波纹需校准，端部方位和尺寸仍为估算，另一端、电梯厅与层间结构待补证，尚未达到 1:1 或电影级。当前状态、来源与证据以[最新跟踪记录](../../docs/space-blender-tracker.md#2026-09-13-100f-门框扶手的室内反射)为准，下方日期段落保留历史状态。重资产继续本地保存，Git LFS 配置与上传暂缓。
...
本地开发默认加载 Blender 产物。生产默认仍走已有场景生成逻辑；需要先在构建/部署环境提供完整模型、纹理和对应 JSON 清单，核对哈希和资源可达，再在构建时设置 `NEXT_PUBLIC_SPACE_BLENDER=1`。代码提交和 push 不包含被忽略的重资产。上海模型当前为 400,900,980 字节（约 401 MB，2026-09-13 环球 100F 金属饰面导出），正式发布前仍需分区加载、压缩与移动设备验证。
