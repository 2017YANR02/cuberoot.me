...
当前上海版本（2026-09-13）：金茂塔冠之后继续细化幕墙，补成对竖肋、三道横杆、金属层间板、连接板和螺栓，三轮候选审图后保存。正式 GLB 为 **488,003,840 字节**、16 张共享纹理；金茂 51 个网格与候选的几何、材质和变换一致，Three.js 加载器与完整 HTTP 下载校验见[最新跟踪记录](../../docs/space-blender-tracker.md#2026-09-13-金茂幕墙横竖构件与连接件)。最终网页日夜画面尚未复查；塔身收分、塔冠交接、玻璃反射和照明仍需校准，尚未达到 1:1 或电影级。下方日期段落保留历史状态。重资产继续本地保存，Git LFS 配置与上传暂缓。
...
本地开发默认加载 Blender 产物。生产默认仍走已有场景生成逻辑；需要先在构建/部署环境提供完整模型、纹理和对应 JSON 清单，核对哈希和资源可达，再在构建时设置 `NEXT_PUBLIC_SPACE_BLENDER=1`。代码提交和 push 不包含被忽略的重资产。上海模型当前为 488,003,840 字节（约 488 MB，2026-09-13 金茂幕墙导出），正式发布前仍需分区加载、压缩与移动设备验证。
