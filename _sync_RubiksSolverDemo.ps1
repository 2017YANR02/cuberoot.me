...
$localSrc = Join-Path $LocalDir "tools" "src"
...
foreach ($file in $config.rootFiles)
{
    ...
    $dest = Join-Path $LocalDir "tools" $file
...
if ($config.rootDirs)
{
    foreach ($dir in $config.rootDirs)
    {
        ...
        $destDir = Join-Path $LocalDir "tools" $dir
...
$swPath = Join-Path $LocalDir "tools" "sw.js"
if (Test-Path $swPath)
{
...
$manifestPath = Join-Path $LocalDir "tools" "manifest.json"
...
foreach ($page in $config.pages)
{
    ...
    $destDir2 = Join-Path $LocalDir "tools" $page.subdir
