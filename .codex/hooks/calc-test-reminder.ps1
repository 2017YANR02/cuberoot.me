...
# 由 .codex/hooks.json 调用;Codex apply_patch 的补丁在 tool_input.command。
...
$inputText = if ($json.tool_input -is [string]) { "$($json.tool_input)" } else { "$($json.tool_input.command)$($json.tool_input.file_path)" }

if ($inputText -match 'app[/\\][^/\\]+[/\\]calc[/\\]') {
