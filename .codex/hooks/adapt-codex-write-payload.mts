...
type ProposedWrite = { filePath: string; sourcePath?: string; content: string };

export function parseApplyPatch(patch: unknown): ProposedWrite[] {
...
function parseEmbeddedApplyPatch(source: unknown): ProposedWrite[] {
...
export function writesFromPayload(payload: any): ProposedWrite[] {
...
function targetCommand(target: string) {
  const args = [target];
  if (isArchitectureTarget(target)) args.push('--hook');
  return { command: process.execPath, args };
}

function isArchitectureTarget(target: string) {
  return normalizePath(target).toLowerCase().endsWith('/check-architecture-boundaries.mjs');
}

function runTarget(target: string, payload: unknown, cwd: string): string {
...
function mightMatch(target: string, content: string): boolean {
