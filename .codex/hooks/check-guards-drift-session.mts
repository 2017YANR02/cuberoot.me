...
for (const group of settings.hooks?.PreToolUse ?? []) {
  for (const hook of group.hooks ?? []) {
    for (const match of String(hook.command ?? '').matchAll(/[\w-]+\.(?:mjs|cjs|mts)/g)) registered.add(match[0]);
  }
}
registered.delete('adapt-codex-write-payload.mts');
registered.delete('adapt-codex-command-payload.mts');
...
for (const match of guardsSource.matchAll(/\{\s*id:\s*'[^']+',\s*scope:\s*'project',\s*hook:\s*'([^']+)'/gs)) {
  ...
  for (const file of registeredPart.matchAll(/[\w-]+\.(?:mjs|cjs|mts)/g)) documented.add(file[0]);
}
