...
# Cheap gate for the current registry: close/clear buttons, password inputs, project selectors,
# BackHome placement, and the canonical algorithm case-detail layout.
if ($payload -notmatch '<button|<select|<input|password|<X\b|<BackHome\b|<CaseThumb\b|<AlgPlayer\b|inlinePlayer|multiOri|multi-ori|is-paired-player|grid-template-columns|is-without-thumb|×|✕|关闭|close|clear|dismiss|EventIcon|CubingIcon|EventPicker|PuzzlePicker|PuzzleTypeSelect|eventPickerOpen|puzzlePickerOpen') { exit 0 }
